...
def api(base_url: str) -> CreatorApiClient:
    """直连前端代理的 REST 客户端（/api/qwenpaw-creator/...），用于前置数据与断言。"""
...
def pending_review_target(api: CreatorApiClient) -> dict:
    ...
    if explicit:
        ...
        candidates = [
            item["projectId"] for item in reversed(response.json()["items"])
        ]
    ...
    for project_id in candidates:
        ...
        if session_response.status_code != 200:
            rejection_reasons.append(
                f"{project_id}: session {session_response.status_code}",
            )
            continue
        ...
        if session["status"] != "PENDING_REVIEW":
            ...
            continue
        transaction_id = e2e_config.PENDING_TRANSACTION_ID or session.get(
            "activeTransactionId",
        )
        if not transaction_id:
            rejection_reasons.append(
                f"{project_id}: missing activeTransactionId",
            )
            continue
        ...
        pending_groups = [
            item
            for item in manifest["decisionGroups"]
