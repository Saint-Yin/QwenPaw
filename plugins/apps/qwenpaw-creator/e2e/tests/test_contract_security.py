...
def test_old_http_routes_and_old_payloads_have_no_fallback(api, cutover_ids):
    ...
    try:
        ...
        section = next(
            item
            for item in cutover_plan_before["sections"]
        ...
        assert (
            legacy_value_response.status_code == 422
        ), legacy_value_response.text
        ...
        assert (
            api.get(f"/projects/{cutover_project}/session").json()
            == session_before
        )
        assert (
            api.get(
...
def test_path_cross_project_ref_forged_artifact_and_role_write_are_rejected(
    api,
):
    ...
    try:
        ...
        drift = api.post_file(
            f"/projects/{source_id}/assets",
            files={
                "file": ("cross-project.txt", b"changed bytes", "text/plain"),
            },
            data=data,
            headers={"Idempotency-Key": ingest_key},
        )
        ...
        version_id = (
            accepted.get("assetVersionId")
            or task["resultRefs"][0].split(":", 1)[1]
        )
        ...
        cross_attach = _command(
            ...
            target_version=_presentation(api.view(target_id, "assets"))[
                "targetVersion"
            ],
        )
        ...
        forged_artifact = _command(
            ...
            target_version=_presentation(api.view(target_id, "plan"))[
                "targetVersion"
            ],
        )
