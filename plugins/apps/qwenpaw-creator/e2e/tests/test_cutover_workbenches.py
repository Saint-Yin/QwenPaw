...
def test_r2v_cutover_view_and_origin_media_surface(page, api, cutover_ids):
    ...
    for action in ("生成分镜 Prompt", "生成分镜图", "生成视频 Prompt", "生成视频"):
        expect(
            page.get_by_role("button", name=action, exact=True),
        ).to_be_visible()
    for panel in ("分镜文本", "分镜Prompt与分镜图", "视频结果", "资产绑定"):
        expect(
            page.get_by_role("heading", name=panel, exact=True),
        ).to_be_visible()

    image = page.get_by_role("img", name="分镜图", exact=True)
    expect(image).to_be_visible()
    page.wait_for_function(
        "node => node.naturalWidth > 0",
...
def test_edit_cutover_nested_plan_in_origin_vlm_and_timeline_panels(
    page,
    api,
    cutover_ids,
):
    ...
    expect(
        page.get_by_role("button", name="生成剪辑方案", exact=True),
    ).to_be_visible()
    expect(page.get_by_role("button", name="执行剪辑", exact=True)).to_be_visible()
    expect(
        page.get_by_role("heading", name="剪辑目标", exact=True),
    ).to_be_visible()
    storyboard_panel = surface.panel("VLM 关键帧分镜")
    expect(
        storyboard_panel.get_by_text("7 panels", exact=True),
    ).to_be_visible()
    ...
    expect(
        page.get_by_role("heading", name="剪辑时间线（7）", exact=True),
    ).to_be_visible()
...
def test_plan_section_and_final_compose_keep_origin_surfaces_and_truthful_blockers(
    ...
    expect(
        page.get_by_role("heading", name=re.compile(r"^单元视频清单（\d+ 个）$")),
    ).to_be_visible()
    ...
    if final["candidates"]:
        source_label = (
            "单元成片"
            if final["candidates"][0].get("sourceKind") == "unit"
            else "整段成片"
        )
        expect(
            page.get_by_text(source_label, exact=True).first,
        ).to_be_visible()
    else:
        expect(
            page.get_by_text(
        ...
        expect(
            page.get_by_role("button", name=re.compile(r"执行合成$")),
        ).to_be_disabled()
