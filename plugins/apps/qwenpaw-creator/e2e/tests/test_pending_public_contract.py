...
def test_pending_action_commands_are_durable_deferred_with_zero_task_or_run_delta(
    ...
    runs_before = api.get(f"/projects/{project_id}/specialist-runs").json()[
        "items"
    ]
    ...
    tasks_after_commands = api.get(f"/projects/{project_id}/tasks").json()[
        "items"
    ]
    ...
    assert (
        follow_up["targetRef"] == f"asset:{accepted_upload.json()['assetId']}"
    )

    session_after = api.get(f"/projects/{project_id}/session").json()[
        "session"
    ]
    ...
    assert (
        assets_after["reviewRevisionId"] == review_before["reviewRevisionId"]
    )
    review_after = api.get(review_url).json()
    assert review_after["id"] == review_before["id"]
    assert (
        review_after["reviewRevisionId"] == review_before["reviewRevisionId"]
    )
    ...
    assert [
        (item["id"], item["kind"], item["transactionId"]) for item in new_tasks
    ] == [
...
def test_two_browser_contexts_only_one_decision_wins_and_stale_token_is_409(
    ...
    project_id = pending_review_target["projectId"]
    transaction_id = pending_review_target["transactionId"]
    api_path = f"/api/qwenpaw-creator/projects/{project_id}/transactions/{transaction_id}/review"
    ...
    try:
        ...
        stale = _browser_fetch(
            ...
            method="PUT",
            body={
                "decisionToken": copy_b["decisionToken"],
                "decision": "REJECT",
            },
