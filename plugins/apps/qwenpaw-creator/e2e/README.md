...
这套测试只面向重构后的唯一 Creator 契约：`/api/qwenpaw-creator/projects/**`、semantic Command、page-specific View、受控媒体路由和 HashRouter UI。浏览器定位器严格对应 `origin/main@24e505e03ba54b0f916267c10673cc28b65f7eed` 保留的可见文案、DOM 层级与交互，不再保留任何旧 `/ai/**`、`/agent/**`、PUT Project 或 Next 文件路由用例。

## 前置

1. 后端以待验收的 `CREATOR_DATA_ROOT` 启动在 `127.0.0.1:18110`。
2. Vite UI 启动在 `127.0.0.1:5173`，并把 `/api/qwenpaw-creator` 代理到该后端。
3. 安装测试依赖及 Chromium：
