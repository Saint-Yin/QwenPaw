...
def _move(order: list[str], item_id: str, args: dict[str, Any]) -> None:
    ...
    before = (
        args.get("beforeId")
        or args.get("beforeSectionId")
        or args.get("beforeUnitId")
    )
    after = (
        args.get("afterId")
        or args.get("afterSectionId")
        or args.get("afterUnitId")
    )
    if before in order:
        ...
        index = (
            ...
            else max(0, min(int(raw_index), len(order)))
        )
...
def _exact_media_ref(candidate: dict[str, Any], value: Any) -> tuple[str, str]:
    ...
    if ref.startswith("artifact://"):
        ...
        version = candidate["assets"]["artifact_versions_by_id"].get(
            version_id,
        )
...
def _visual_media(
    ...
    routes = (
        (
            "/media/assets/",
            "asset",
            candidate["assets"]["source_versions_by_id"],
        ),
        (
            "/media/artifacts/",
            "artifact",
            ...
            for record in records:
                if kind == "character":
                    record["character_refs"] = [
                        value
                        for value in record["character_refs"]
                        if value != entity_id
                    ]
...
def _apply_supplement_asset(
    ...
    *,
    ...
    if operation == "delete":
        ...
        if (
            raw_version_ref is not None
            or target_id in candidate["assets"]["source_versions_by_id"]
        ):
            version_id = target_id
            if raw_version_ref is not None:
                kind, referenced_id = _exact_media_ref(
                    candidate,
                    raw_version_ref,
                )
                if kind != "asset":
                    raise ValidationError("assetVersionRef 必须指向 AssetVersion")
                referenced = candidate["assets"]["source_versions_by_id"][
                    referenced_id
                ]
                if target_id not in {
                    referenced_id,
                    referenced["logical_asset_id"],
                }:
                    raise ConflictError(
                        "assetVersionRef 与 target asset id 不一致",
                    )
    ...
    if field == "appearance":
        ...
        variant["prompt"] = _text(args.get("prompt", ""), "prompt")
        if any(
            args.get(key) for key in ("imageRef", "assetVersionId", "imageUrl")
        ):
            _add_visual_media(
                entity,
                variant,
...
def _apply_deterministic(
    ...
    *,
    ...
    if command is CreatorCommandType.SET_SECTION_TEXT:
        section = _section(
            candidate,
            _target_id(request.target_ref, "section"),
        )
        mapping = {
            "title": "title",
            "summary": "summary",
    ...
    if command is CreatorCommandType.SET_UNIT_TEXT:
        _parent, unit = _unit_location(
            candidate,
            _target_id(request.target_ref, "unit"),
        )
    ...
    if command is CreatorCommandType.UPSERT_SHOT:
        ...
        shot_id = str(
            raw.get("id")
            or _stable_id("shot", project_id, request.client_command_id),
        )
        shot = {
            "shot_id": shot_id,
    ...
    if command in {
        ...
        if field == "sources":
            ...
            target = unit[
                "character_refs" if field == "characters" else "prop_refs"
            ]
        elif field == "scene":
            value = _visual_id_for_ref(source_ref)
    ...
    if command in {
        ...
        if command is CreatorCommandType.SET_EDIT_AUDIO_PLAN:
            ...
            plan["audio_plan"] = {
                ...
                "music_prompt": _text(
                    audio_plan.get("music_prompt", ""),
                    "audio_plan.music_prompt",
                ),
                "voiceover": _text(
                    audio_plan.get("voiceover", ""),
                    "audio_plan.voiceover",
                ),
                "notes": _text(
                    audio_plan.get("notes", ""),
                    "audio_plan.notes",
                ),
            }
        ...
        if command is CreatorCommandType.SET_EDIT_CLIP_RANGE:
            ...
            if anchor:
                ...
                timeline["order"].insert(
                    anchor_index if before else anchor_index + 1,
                    clip_id,
                )
            else:
    ...
    if command in {
        ...
        for raw_ref in refs:
            raw = str(raw_ref)
            version_id = (
                raw.rsplit("@", 1)[-1] if "@" in raw else raw.split(":", 1)[-1]
            )
            version = candidate["assets"]["source_versions_by_id"].get(
                version_id,
            )
            if version is None:
                raise NotFoundError("AssetVersion 不存在")
            ...
            if (
                command is CreatorCommandType.ATTACH_SOURCE_ASSETS
                ...
                command is CreatorCommandType.DETACH_SOURCE_ASSETS
        ...
        composition["transitions"].append(
            {
                ...
                "duration_ms": max(
                    0,
                    round(float(args.get("durationSeconds") or 0) * 1000),
                ),
            },
        )
        return
