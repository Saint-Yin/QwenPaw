...
class WorkspaceToolContext:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            self.trigger_message_seq is not None
            and self.trigger_message_seq <= 0
        ):
    ...
    def role_value(self) -> str:
        return (
            self.role.value
            if isinstance(self.role, SpecialistRole)
            else self.role
        )
...
class WorkspaceFileTools:
    ...
    def read_file(
        ...
        if start_line is not None or end_line is not None:
            ...
            bounded_end = min(
                end if end is not None else len(lines),
                len(lines),
            )
            selected = lines[start - 1 : bounded_end]
            result_content = "\n".join(
                f"{start + index:6d}\u2192{line}"
    ...
    def _candidate_entries(
        ...
        *,
        ...
        matcher = (
            compile_workspace_glob(include_pattern)
            if include_pattern
            else None
        )
        ...
        return tuple(
            ...
            and (
                matcher is None
                or matcher.fullmatch(PurePosixPath(entry.path).name)
            )
        )
        ...
        for entry in self._candidate_entries(
            ...
            for index, line in enumerate(lines):
                if matcher.search(line):
                    ...
                    if context_count:
                        item["context"] = lines[
                            max(0, index - context_count) : index
                            + context_count
                            + 1
                        ]
                    matches.append(item)
    ...
    def _runtime_write_state(
        ...
        if mismatched:
            raise PhaseConflictError(
                "Writer context 与 Runtime lease 归属不匹配",
                details=mismatched,
            )
        try:
        ...
        if (
            self.context.observed_working_head
            != state.lease_observed_working_head
        ):
            raise ConflictError(
    ...
    def _commit_text(
        ...
        *,
        ...
        if result.working_tree.id != self.context.working_branch_id:
            raise StorageIntegrityError(
                "Mutation gateway 返回了错误的 Working Branch",
            )
