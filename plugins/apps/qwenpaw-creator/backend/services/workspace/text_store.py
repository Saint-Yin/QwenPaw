...
class FileWorkingTreeRepository:
        ...
        with self._lock:
            if target.exists():
                ...
                if existing == payload:
                    return
                raise ConflictError(
                    "working branch id 已存在",
                    details={"branchId": tree.id},
                )
    ...
    def get(self, project_id: str, branch_id: str) -> WorkingTree:
        ...
        try:
            ...
            raise StorageIntegrityError(
                f"working tree metadata 无法读取: {target}",
            ) from exc
...
class TextStore:
    ...
    def __init__(
        ...
        *,
        ...
        self.repository = repository or FileWorkingTreeRepository(
            content_store.root,
        )
    ...
    def list_entries(
        ...
        *,
        ...
        return tuple(
            ...
            or entry.path.startswith(prefix_with_slash)
        )
    ...
    def read_bytes(
        ...
        *,
        ...
        entry = next(
            (item for item in entries if item.path == normalized),
            None,
        )
        if entry is None:
            raise NotFoundError(
    ...
    def move(
        ...
        *,
        ...
        expected_head: str | None = None,
        expected_source_blob_hash: str
        | None
        | _UnsetExpectation = UNSET_EXPECTATION,
    ) -> WorkspaceMutationResult:
        ...
        kind = (
            MutationKind.RENAME
            if PurePosixPath(source).parent
            == PurePosixPath(destination).parent
