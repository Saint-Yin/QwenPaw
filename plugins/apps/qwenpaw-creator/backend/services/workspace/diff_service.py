...
class TreeDiff:
    ...
    def operations_of_kind(
        ...
        return tuple(
            item for item in self.operations if item.kind is normalized
        )
...
class DiffService:
    ...
    def _read_ref(self, entry: RevisionEntry | None) -> str | None:
        if entry is None or not entry.path.endswith(".ref"):
            return None
        try:
            return self.content_store.read_text(
                entry.blob_hash,
                namespace="blob",
            )
        ...
        *,
        ...
        for path in common_paths:
            ...
            if (
                ...
                and before.content_type == after.content_type
            ):
                continue
            before_ref = self._read_ref(before)
            after_ref = self._read_ref(after)
        ...
        for path in sorted(
            set(base_entries).difference(target_entries) - consumed_base,
        ):
            before = base_entries[path]
            operations.append(
                DiffOperation(
        ...
        for path in sorted(
            set(target_entries).difference(base_entries) - consumed_target,
        ):
            after = target_entries[path]
            operations.append(
                DiffOperation(
