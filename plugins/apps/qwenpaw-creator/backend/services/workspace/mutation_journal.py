...
class FileMutationJournalRepository:
    ...
    def load(
        ...
        try:
            ...
            records = tuple(
                mutation_from_dict(item) for item in payload.get("records", [])
            )
        except (
            OSError,
            UnicodeDecodeError,
            ...
            raise StorageIntegrityError(
                f"Mutation Journal 无法读取: {target}",
            ) from exc
        if (
        ...
        if payload.get("journalHash") != actual_hash:
            raise StorageIntegrityError(
                ...
                details={
                    "expected": payload.get("journalHash"),
                    "actual": actual_hash,
                },
            )
        return records
        ...
        *,
        ...
        with self._lock:
            ...
            expected_seqs = list(
                range(
                    expected_last_seq + 1,
                    expected_last_seq + len(records) + 1,
                ),
            )
    ...
    def append_many(
        ...
        *,
        ...
        if any(
            ...
            or item.transaction_id != transaction_id
            for item in pending
        ):
            raise ValidationError(
                "append_many 只能包含同一 Project/Transaction 的 records",
            )
        ...
        current_last_seq = current[-1].seq if current else 0
        if (
            expected_last_seq is not None
            and current_last_seq != expected_last_seq
        ):
            raise ConflictError(
