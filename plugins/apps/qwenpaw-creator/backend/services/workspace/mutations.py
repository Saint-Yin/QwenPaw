...
class MutationRecord:
    ...
    def __post_init__(self) -> None:
        ...
        if self.kind is MutationKind.CREATE:
            if (
                self.before_blob_hash is not None
                or self.after_blob_hash is None
            ):
                raise ValidationError("create mutation 必须只有 after_blob_hash")
        elif self.kind is MutationKind.DELETE:
            if (
                self.before_blob_hash is None
                or self.after_blob_hash is not None
            ):
            ...
            if self.before_blob_hash is None or self.after_blob_hash is None:
                raise ValidationError(
                    f"{self.kind.value} mutation 必须包含 before/after blob hash",
                )
...
def database_mutation_kind(record: MutationRecord) -> str:
    ...
    reference_change = any(
        path.endswith(".ref") for path in record.affected_paths
    )
    if reference_change:
