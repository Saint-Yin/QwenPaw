...
WHISPER_BIN_CANDIDATES = ("whisper",)
...
def _write_cache(key: str, segments: list[dict[str, Any]]) -> None:
    ...
    try:
        ...
        logger.warning(
            "audio_segments cache write failed: %s (%s)",
            path,
            error,
        )
...
def _run_whisper(binary: str, media_path: Path) -> list[dict[str, Any]]:
    ...
    try:
        ...
        logger.warning(
            "Whisper timed out after %ss on %s",
...
def analyze_speech_boundaries(media_path: str) -> list[dict[str, Any]]:
    ...
    if not path.exists():
        logger.warning(
            "analyze_speech_boundaries: missing file %s",
            media_path,
        )
        return []
    ...
    if not binary:
        logger.warning(
            "Whisper CLI not found; skipping speech boundary analysis for %s",
            media_path,
        )
        return []
