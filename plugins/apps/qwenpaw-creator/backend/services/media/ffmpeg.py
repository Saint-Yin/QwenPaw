...
# pylint: disable=subprocess-run-check,too-many-branches
...
def download_video_to_local(
    ...
    try:
        ...
        directory = ensure_task_work_subdir(
            "downloads",
            project_id,
            task_id=task_id,
        )
        target = (
            directory
            / f"{_sanitize_filename(display_name or video_id)}_{version}{extension}"
        )
        ...
        logger.warning(
            "Failed to materialize remote video %s: %s",
            video_id,
            exc,
        )
        return remote_url
...
def compute_total_duration(
    ...
    if not durations:
        return 0.0
    crossfade_pairs = sum(
        1 for t in transitions if _normalize_transition(t) != "cut"
    )
    return max(0.0, sum(durations) - crossfade_pairs * transition_duration)


def _clip_durations_sanity(
    durations: list[float],
    transitions: list[str],
) -> None:
...
def build_xfade_filter_chain(
    ...
    for i in range(n):
        ...
        video_prep_filters.append(
            f"[{i}:v]" + ",".join(filters_for_clip) + f"[v{i}]",
        )
    ...
    for i in range(n):
        if has_audio is None or has_audio[i]:
            audio_filters.append(
                f"[{i}:a]aresample=async=1,asetpts=PTS-STARTPTS[ai{i}]",
            )
        else:
...
def build_xfade_command(
    ...
    if len(clip_paths) != len(clip_durations) or len(clip_paths) != len(
        transitions,
    ):
        raise ValueError(
            "clip_paths / clip_durations / transitions length mismatch",
        )
    if not clip_paths:
...
def _local_media_path(video_url: str) -> Path | None:
    ...
    return (
        candidate.expanduser().resolve() if candidate.is_absolute() else None
    )
...
def _has_audio_stream(ffmpeg_path: str, media_path: str) -> bool:
    try:
        ...
        logger.warning(
            "Media probe unavailable; skipping audio stream verification",
        )
        return True
...
def _runtime_concat_command(
    ...
    *,
    ...
    localized = [
        manifest_path if arg == "concat.txt" else arg for arg in command
    ]
    localized[-1] = output_path
    return localized


def build_agent_review_notes(
    ordered_clips: list[dict[str, Any]],
    uses_remote_sources: bool,
) -> list[dict[str, str]]:
    notes = [
        {
...
def execute_stitch(
    ...
    if not plan["can_stitch"]:
        return {
            "success": False,
            "error": "Cannot stitch: " + "; ".join(plan["blockers"]),
            "output_url": None,
        }

    try:
        output_filename = _safe_output_filename(output_path)
        work_dir = ensure_task_work_subdir(
            "stitches",
            uuid.uuid4().hex,
            task_id=task_id,
        )
        ...
        for index, clip in enumerate(
            sorted(clips, key=lambda item: item.get("order", 0)),
            start=1,
        ):
            video_url = str(clip.get("video_url") or "")
        ...
        if not plan["can_stitch"]:
            return {
                "success": False,
                "error": "Cannot stitch cached clips: "
                + "; ".join(plan["blockers"]),
                "output_url": None,
            }
        ...
        transitions = [
            str(clip.get("transition") or "fade").lower() or "fade"
            for clip in ordered_local
        ]
        ...
        if use_xfade:
            ...
            clip_durations = [
                _probe_clip_duration_seconds(ffmpeg_path, p) or 1.0
                for p in clip_paths
            ]
            ...
            if xfade_result.returncode == 0:
                ...
                if result.returncode != 0:
                    logger.warning(
                        f"FFmpeg stream-copy stitch failed, retrying with re-encode: {_ffmpeg_error_summary(result.stderr, 300)}",
                    )
                    reencoded_out_path = str(
                        work_dir / f"reencoded-{output_filename}",
                    )
                    result = _run_reencode_concat(
                        ffmpeg_path,
                        manifest_path,
                        reencoded_out_path,
                    )
                    if result.returncode != 0:
                        return {
                            "success": False,
                            "error": f"FFmpeg failed: {_ffmpeg_error_summary(result.stderr)}",
                            "output_url": None,
                        }
        ...
        if source_has_audio and not output_has_audio:
            logger.warning(
                "Stitched output has no audio stream while source clips do; retrying with explicit audio map",
            )
            reencoded_out_path = str(
                work_dir / f"audio-fixed-{output_filename}",
            )
            result = _run_reencode_concat(
                ffmpeg_path,
                manifest_path,
            ...
            if result.returncode != 0:
                return {
                    "success": False,
                    "error": f"FFmpeg audio-preserving retry failed: {_ffmpeg_error_summary(result.stderr)}",
                    "output_url": None,
                }
            out_path = reencoded_out_path
        ...
        return {
            "success": False,
            "error": "FFmpeg execution timed out (300s)",
            "output_url": None,
        }
...
def build_stitch_plan(
    ...
    blockers: list[str] = []
    ordered_clip_ids = [
        str(clip.get("id") or f"clip-{index}")
    ...
    duplicate_orders = sorted(
        {order for order in orders if orders.count(order) > 1},
    )
    if duplicate_orders:
        blockers.append(
            f"Clip顺序重复: {', '.join(str(order) for order in duplicate_orders)}。",
        )
    ...
    uses_remote_sources = any(
        _is_remote_video_url(str(clip.get("video_url") or ""))
    ...
    transition_strategy = (
        "ffmpeg_xfade_acrossfade"
        if (can_stitch and len(ordered_clips) > 1 and any_xfade)
        else "ffmpeg_concat_demuxer_copy"
    )
    ...
    return {
        ...
        "protocol_whitelist": REMOTE_PROTOCOL_WHITELIST
        if can_stitch and uses_remote_sources
        else "",
        "quality_checks": QUALITY_CHECKS if can_stitch else [],
        "agent_review_notes": build_agent_review_notes(
            ordered_clips,
            uses_remote_sources,
        )
        if can_stitch
