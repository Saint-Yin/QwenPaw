...
_OPTIONAL_WORKING_PHASES = frozenset(
    {"executing", "interrupting", "waiting_input"},
)
...
def derive_ui_phase(
    ...
    if session_status == "RUNNING":
        ...
        raise ProjectionInputError(
            "RUNNING session requires ACTIVE, REVISING, COMPLETION_CHECK, or SEALING",
        )
    if session_status == "WAITING_RUNTIME":
        if transaction_status not in {"ACTIVE", "REVISING"}:
            raise ProjectionInputError(
                "WAITING_RUNTIME requires an active or revising transaction",
            )
        return "executing"
    ...
    if session_status == "PENDING_REVIEW":
        if transaction_status != "PENDING_REVIEW":
            raise ProjectionInputError(
                "PENDING_REVIEW session and transaction must agree",
            )
    ...
    if session_status == "ERROR":
        return "error"
    raise ProjectionInputError(
        f"unknown Creator Session status: {session_status}",
    )
...
def _assert_snapshot_project(
    ...
    *,
    ...
    if snapshot.project_id != project_id:
        raise ProjectionInputError(
            f"{label} snapshot belongs to a different project",
        )
...
def build_view_envelope(
    ...
    *,
    ...
    if snapshots.approved.revision_id != runtime.approved_revision_id:
        raise ProjectionInputError(
            "approved snapshot and Runtime pointer do not match",
        )
    phase = derive_ui_phase(runtime.session_status, runtime.transaction_status)

    if phase == "waiting_review":
        if (
            review_presentation is None
            or snapshots.review_presentation is None
        ):
            raise ProjectionInputError(
                "review phase requires presentation metadata and an effective snapshot",
            )
        _assert_snapshot_project(
            snapshots.review_presentation,
            runtime.project_id,
            label="review presentation",
        )
        if (
            review_presentation.approved_revision_id
            != runtime.approved_revision_id
        ):
            raise ProjectionInputError(
                "review presentation has a stale approved revision",
            )
        if (
            review_presentation.review_revision_id
            != runtime.review_revision_id
        ):
            raise ProjectionInputError(
                "review presentation and Runtime review pointer do not match",
            )
        raw_view = builder(snapshots.review_presentation)
        view: Mapping[str, Any] = build_review_presentation_view(
            raw_view,
            review_presentation,
        )
    ...
    envelope: dict[str, Any] = {
        ...
        "uiPhase": phase,
        "manualEditOverlay": runtime.manual_edit_overlay.to_view()
        if runtime.manual_edit_overlay
        else None,
