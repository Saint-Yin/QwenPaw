...
def _validate_version_refs(command: CreatorCommandRequest) -> None:
    ...
    if command.type == CreatorCommandType.SUPPLEMENT_ASSET:
        ...
        if operation == "create":
            ...
            raise ValidationError(
                "SUPPLEMENT_ASSET operation 只能是 create 或 delete",
            )
...
def translate_command(
    command: CreatorCommandRequest,
    *,
    ui_phase: UiPhase | str,
) -> CommandDisposition:
    ...
    lane: CommandLane = (
        ...
        if command.type in DETERMINISTIC_COMMANDS
        else "creator_action"
    )

    if (
        lane == "deterministic_mutation"
        and not command.expected_target_versions
    ):
        raise ValidationError(
            "deterministic command 必须携带 expectedTargetVersions CAS",
        )
    ...
    if phase == UiPhase.FINALIZING:
        ...
        if lane == "creator_action":
            ...
            if not (has_first_write_token or has_overlay_token):
                raise ValidationError(
                    "Pending 手动编辑必须携带 presentationVersion 或 overlayHead",
                )
            phase_state = "APPLY"
...
if set(_DESCRIPTIONS) != set(CreatorCommandType):
    raise RuntimeError(
        "Every Creator command must have one product-semantic description",
    )
