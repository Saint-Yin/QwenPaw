...
def _selected_artifact(
    ...
    *,
    ...
    records = [
        ...
        if item.owner_ref == owner_ref and item.kind == kind
    ]
    ...
    if selected is None:
        return (
            None,
            slot_id,
            [f"SELECTED_ARTIFACT_VERSION_NOT_FOUND:{selected_id}"],
        )
    if selected.stale:
        return (
            selected,
            slot_id,
            [
                f"{kind.upper()}_STALE:{selected.stale_reason or 'INPUT_CHANGED'}",
            ],
        )
...
def _artifact_views(
    ...
    *,
    ...
    return [
        _artifact_view(
            item,
            selected=selections.artifact_versions.get(item.slot_id) == item.id,
        )
        for item in records
    ]
...
def _production_reference_bindings(
    ...
    *,
    ...
    for reference_set, prefix in (
        ("storyboard", storyboard_prefix),
        ("video", video_prefix),
    ):
        for path in snapshot.paths(prefix):
            ...
            if category not in {"scene", "characters", "props", "sources"}:
                raise ProjectionInputError(
                    f"unknown production reference category: {path}",
                )
            source_ref = snapshot.text(path, required=True)
            grouped.setdefault((category, source_ref), set()).add(
                reference_set,
            )
    return [
        {
...
def build_r2v_workbench_view(
    ...
    *,
    ...
    if (
        unit.duration < provider.min_duration
        or unit.duration > provider.max_duration
    ):
        blockers.append("R2V_DURATION_OUTSIDE_PROVIDER_BOUNDS")
    if (
        provider.allowed_durations
        and unit.duration not in provider.allowed_durations
    ):
    ...
    video_input_refs = ([storyboard.source_ref] if storyboard else []) + [
        ...
        if storyboard is None or ref != storyboard.source_ref
    ]
    if len(video_input_refs) > provider.max_reference_images:
        blockers.append("R2V_REFERENCE_IMAGE_LIMIT_EXCEEDED")
    resolved, ref_blockers = resolve_many(
        index,
        [*storyboard_inputs, *video_input_refs],
    )
    ...
    return {
        ...
        "selectionSource": {
            "revisionId": selections.revision_id,
            "storyboardSlotId": storyboard_slot,
        },
    }
