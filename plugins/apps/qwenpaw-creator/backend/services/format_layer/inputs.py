...
class WorkspaceFile:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(
            self,
            "object_version",
            _non_empty(self.object_version, "object_version"),
        )
...
class TextWorkspaceSnapshot:
        ...
        for raw_path, entry in self.files.items():
            ...
            if (
                ...
                raise ProjectionInputError(
                    f"invalid Text Workspace path: {raw_path!r}",
                )
            if not isinstance(entry, WorkspaceFile):
                raise ProjectionInputError(
                    f"file entry must be WorkspaceFile: {raw_path!r}",
                )
            checked[path.as_posix()] = entry
        versions = {
            str(ref): _non_empty(version, f"target version for {ref}")
            for ref, version in self.target_versions.items()
        }
        object.__setattr__(
            self,
            "files",
    ...
    def text(self, path: str, *, required: bool = False) -> str:
        ...
        if entry is None:
            if required:
                raise ProjectionInputError(
                    f"required Workspace file is missing: {path}",
                )
            return ""
    ...
    def target_version(self, target_ref: str) -> str:
        try:
            ...
            raise ProjectionInputError(
                f"target version is missing: {target_ref}",
            ) from exc
...
class ProjectionCatalogs:
    ...
    def asset_version(self, version_id: str) -> AssetVersionRecord | None:
        return next(
            (item for item in self.assets if item.id == version_id),
            None,
        )
    ...
    def artifact_version(
        ...
        return next(
            (item for item in self.artifacts if item.id == version_id),
            None,
        )

    def artifact_versions_for_slot(
        self,
        slot_id: str,
    ) -> tuple[ArtifactVersionRecord, ...]:
        return tuple(
            item for item in self.artifacts if item.slot_id == slot_id
        )
...
class RevisionSelections:
    ...
    def __post_init__(self) -> None:
        ...
        values = {
            str(slot): _non_empty(version, f"selection for {slot}")
            for slot, version in self.artifact_versions.items()
        }
        object.__setattr__(
            self,
            "artifact_versions",
...
class AiEditPlanVersion:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "workbench_envelope",
            _immutable_map(deepcopy(dict(self.workbench_envelope))),
        )
...
class ManualEditOverlayRecord:
    ...
    def __post_init__(self) -> None:
        ...
        if self.mutation_count < 0:
            raise ProjectionInputError(
                "overlay mutation_count cannot be negative",
            )
...
class RuntimeViewState:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            ...
            _non_empty(self.approved_revision_id, "approved_revision_id"),
        )
        object.__setattr__(
            self,
            "session_status",
            _non_empty(self.session_status, "session_status"),
        )
        object.__setattr__(
            self,
            "agent_status_bar",
            _immutable_map(deepcopy(dict(self.agent_status_bar))),
        )
...
class ReviewTargetMetadata:
    ...
    def __post_init__(self) -> None:
        ...
        if (self.decision_group_id is None) != (self.decision_token is None):
            raise ProjectionInputError(
                "decision group id and token must be supplied together",
            )
...
class ReviewPresentationState:
    ...
    def __post_init__(self) -> None:
        ...
        if (self.overlay_id is None) != (self.overlay_head is None):
            raise ProjectionInputError(
                "overlay id and head must be supplied together",
            )
        origin_map = dict(self.origins)
        target_map = dict(self.target_versions)
        if set(origin_map) != set(target_map):
            raise ProjectionInputError(
                "review origins and target versions must address the same targets",
            )
        for target_ref, origin in origin_map.items():
            if origin not in {"approved", "review_candidate", "user_overlay"}:
                raise ProjectionInputError(
                    f"invalid review origin for {target_ref}",
                )
            metadata = target_map[target_ref]
            if (
                origin == "review_candidate"
                and metadata.decision_group_id is None
            ):
                raise ProjectionInputError(
                    f"review candidate is missing decision CAS metadata: {target_ref}",
                )
            if origin == "user_overlay" and self.overlay_id is None:
                raise ProjectionInputError(
                    f"overlay origin is missing overlay metadata: {target_ref}",
                )
        object.__setattr__(
            self,
            "origins",
            MappingProxyType(dict(sorted(origin_map.items()))),
        )
        object.__setattr__(
            self,
            "target_versions",
            MappingProxyType(dict(sorted(target_map.items()))),
        )
