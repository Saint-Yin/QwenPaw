...
def parse_duration(raw: str, *, label: str) -> float:
    ...
    try:
        ...
        raise ProjectionInputError(
            f"invalid duration for {label}: {raw!r}",
        ) from exc
    if value < 0:
...
def _read_refs(
    ...
    for path in snapshot.paths(root):
        ...
        if "\n" in value or not value:
            raise ProjectionInputError(
                f"reference file must contain one value: {path}",
            )
    ...
    *,
    ...
    for segment in _child_directories(snapshot, prefix):
        ...
        if (
            not description
            or not raw_duration
            or (route == "r2v" and not raw_camera)
        ):
            continue
        camera = ""
        framing = ""
...
def _parse_units(
    ...
    for segment in _child_directories(snapshot, prefix):
        ...
        if route not in {"r2v", "edit"}:
            raise ProjectionInputError(
                f"unit route must be r2v or edit: {unit_id}",
            )
        units.append(
            ParsedUnit(
                id=unit_id,
        ...
        sections.append(
            ParsedSection(
                ...
                constraints=parse_list(
                    snapshot.text(f"{root}/constraints.md"),
                ),
...
def find_unit(
    ...
    raise ProjectionResourceNotFoundError(
        f"unit not found in snapshot: {unit_id}",
    )
