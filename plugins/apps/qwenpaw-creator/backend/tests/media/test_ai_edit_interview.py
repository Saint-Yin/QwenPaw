...
from services.media.ai_edit.audio_segments import (
    find_enclosing_sentence_boundaries,
)
...
def test_interview_snaps_file_asset_boundaries_and_caps_whole_clips(
    monkeypatch,
) -> None:
    monkeypatch.setattr(
        ai_edit_core,
...
def test_interview_remote_source_keeps_original_timestamps_without_asr() -> (
    None
):
...
def test_interview_panel_keeps_original_clip_identity_after_reorder(
    monkeypatch,
) -> None:
    plan = _interview_plan()
    late, early = plan["timeline"][:2]
    late.update(
        {"clip_id": "clip-late", "start": 10.0, "end": 14.0, "duration": 4.0},
    )
    early.update(
        {"clip_id": "clip-early", "start": 0.0, "end": 4.0, "duration": 4.0},
    )
...
def test_normalize_synthesizes_panels_from_the_normalized_timeline(
    monkeypatch,
) -> None:
    fallback = {
        "summary": "fallback",
    ...
    monkeypatch.setattr(
        ai_edit_core,
        "_heuristic_plan",
        lambda *_args: deepcopy(fallback),
    )
    monkeypatch.setattr(
        ai_edit_core,
        "_media_duration_seconds",
    ...
    assert (
        result["storyboard"][0]["clip_id"] == result["timeline"][0]["clip_id"]
    )
    assert result["storyboard"][0]["clip_id"] != "heuristic-only"
    assert result["storyboard"][0]["timestamp"] == 4.0


def test_interview_director_generates_overlay_during_semantic_selection(
    monkeypatch,
) -> None:
    project = {
...
def test_execute_interview_adds_summary_title_card(
    monkeypatch,
    tmp_path,
) -> None:
    ...
    project = {
        ...
        "plan": {
            "sections": [
                {
                    "units": [
                        {"id": "edit-1", "taskType": "edit", "duration": 60.0},
                    ],
                },
            ],
        },
    ...
    result = asyncio.run(
        ai_edit_core.execute_ai_edit_plan(project, "edit-1", plan),
    )
...
def test_find_enclosing_sentence_boundaries_returns_outermost_sentence() -> (
    None
):
    segments = _segments()
    snap_start, snap_end = find_enclosing_sentence_boundaries(
        4.0,
        8.0,
        segments,
    )
...
def test_interview_snap_falls_back_to_enclosing_boundaries(
    monkeypatch,
) -> None:
    segments = [
...
def test_snap_to_asr_boundary_snaps_round_to_nearest_sentence() -> None:
    ...
    assert (
        _snap_to_asr_boundary(4.0, segments, tolerance=2.0, prefer_end=False)
        == 3.2
    )
    # 9.0 → nearest start >= 7.0 → 7.8 (distance 1.2)
    assert (
        _snap_to_asr_boundary(9.0, segments, tolerance=2.0, prefer_end=False)
        == 7.8
    )
    # 11.0 → no start >= 9.0 → fallback: current sentence start (largest <= 11.0) → 7.8
    assert (
        _snap_to_asr_boundary(11.0, segments, tolerance=2.0, prefer_end=False)
        == 7.8
    )
    ...
    assert (
        _snap_to_asr_boundary(4.0, segments, tolerance=2.0, prefer_end=True)
        == 7.8
    )
    # 9.0 → nearest end >= 9.0 within 2.0 → none (12.4 is 3.4 away) → fallback: earliest end >= 9.0 → 12.4
    assert (
        _snap_to_asr_boundary(9.0, segments, tolerance=2.0, prefer_end=True)
        == 12.4
    )
    # 11.0 → nearest end >= 11.0 within 2.0 → 12.4 (distance 1.4)
    assert (
        _snap_to_asr_boundary(11.0, segments, tolerance=2.0, prefer_end=True)
        == 12.4
    )
...
def test_snap_to_asr_boundary_returns_original_when_no_match() -> None:
    ...
    assert (
        _snap_to_asr_boundary(20.0, segments, tolerance=2.0, prefer_end=False)
        == 0.0
    )
    # Empty segments → always returns original
    assert (
        _snap_to_asr_boundary(20.0, [], tolerance=2.0, prefer_end=True) == 20.0
    )
...
def test_find_nearest_boundary_prefers_later_end_for_clip_end() -> None:
    ...
    result = find_nearest_sentence_boundary(
        52.1,
        segments,
        tolerance=2.0,
        prefer_end=True,
...
def test_interview_storyboard_timestamp_snapped_to_asr_boundary(
    monkeypatch,
) -> None:
...
def test_interview_duration_extension_respects_source_limit(
    monkeypatch,
) -> None:
