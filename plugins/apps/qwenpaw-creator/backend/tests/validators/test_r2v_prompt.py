...
_REQUIRED_FINAL_CONSTRAINT = (
    "保持无字幕，避免生成任何文字或字幕，不要生成Logo，不要生成水印，" "人物面部和身体比例稳定不变形，动作自然流畅，无卡顿无闪烁。"
)
...
def test_video_prompt_evaluator_accepts_complete_seedance_prompt() -> None:
    ...
    prompt = (
        ...
        "<保留自然环境声和细微动作声>。"
        "整体画面保持高清电影质感，动作自然流畅、人物结构稳定。" + _REQUIRED_FINAL_CONSTRAINT
    )
...
def test_video_prompt_evaluator_rejects_neighbor_context_exact_time_and_missing_rules() -> (
    ...
    assert "使用了精确秒段，Seedance建议使用镜头顺序而非强行限制0-3秒。" in evaluation.failures
    assert "台词未按Seedance建议使用中文花括号{}标注。" in evaluation.failures
    assert (
        "缺少稳定性约束：保持无字幕, 避免生成任何文字或字幕, " "不要生成Logo, 不要生成水印。"
