...
def test_ai_edit_core_uses_basic_plan_without_source_intelligence(monkeypatch):
    project = _project()
    monkeypatch.setattr(
        ai_editing.model_config,
        "get_text_api_key",
        lambda: "fake-key",
    )
    envelope = asyncio.run(
        ai_editing.build_ai_edit_plan(project, "edit-1", goal=GOAL),
    )
...
def test_execute_allows_pet_clip_without_director_overlay_copy():
    assert (
        ai_editing._validate_execution_overlay_copy(
            {"reason": "小猫在院子角落低头闻嗅"},
...
def test_execute_readiness_failure_return_is_frozen(monkeypatch, tmp_path):
    ...
    result = asyncio.run(
        ai_editing.execute_ai_edit_plan(_project(), "edit-1", _execute_plan()),
    )
    assert result == {
        "success": False,
        "output_url": None,
...
def test_execute_concat_failure_return_is_frozen(monkeypatch, tmp_path):
    ...
    result = asyncio.run(
        ai_editing.execute_ai_edit_plan(_project(), "edit-1", plan),
    )
    expected_work_dir = tmp_path / "edit-1-12345678"
    assert len(calls) == 2
    assert result == {
...
def test_execute_success_return_commands_and_timeline_are_frozen(
    monkeypatch,
    tmp_path,
):
    readiness = {"status": "ok", "path": "/usr/bin/ffmpeg", "blockers": []}
    ...
    assert [
        calls[0][calls[0].index("-ss") + 1],
        calls[1][calls[1].index("-ss") + 1],
    ] == ["1.0", "8.0"]
    assert [
        calls[0][calls[0].index("-t") + 1],
        calls[1][calls[1].index("-t") + 1],
    ] == ["4.0", "3.0"]
    assert calls[-1][2:8] == [
        "-f",
        "concat",
        ...
        "0",
        "-i",
        str(expected_work_dir / "concat.txt"),
    ]
