...
def test_agentscope_matches_qwenpaw_and_creator_receives_only_attachment_manifest() -> (
    ...
    assert version("agentscope") == "2.0.4.post1"
    ...
    assert (
        '"projectContentType":"highlight_reel"' in messages[1].content[0].text
    )
    assert (
...
def test_direct_creator_adapter_uses_native_agentscope_tool_call() -> None:
    ...
    assert {item["function"]["name"] for item in fake.tools} >= {
        "plan",
        "final",
        "delegate_to_agent",
    }
...
def test_direct_creator_adapter_does_not_emit_empty_text_delta_for_tool_only_turn() -> (
    None
):
...
def test_direct_creator_adapter_streams_raw_text_and_provider_thinking_deltas() -> (
    None
):
