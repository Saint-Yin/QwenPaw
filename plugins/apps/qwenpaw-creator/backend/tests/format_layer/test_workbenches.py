...
def test_r2v_projection_is_storyboard_first_and_version_explicit(
    projection_fixture,
):
    ...
    assert (
        view["videoInputRefs"][0] == fixture.artifacts["storyboard"].source_ref
    )
...
def test_r2v_without_selected_storyboard_is_visible_as_blocked(
    projection_fixture,
):
    fixture = projection_fixture
...
def test_edit_projection_preserves_frozen_envelope_and_seconds_losslessly(
    projection_fixture,
):
    fixture = projection_fixture
    ...
    assert (clip["start"], clip["end"], clip["duration"], clip["OS"]) == (
        25.3,
        31.8,
        6.5,
        "保留原声",
    ...
    assert (
        fixture.ai_edit_plan.workbench_envelope["plan"]["timeline"][0]["start"]
        == 25.3
    )
