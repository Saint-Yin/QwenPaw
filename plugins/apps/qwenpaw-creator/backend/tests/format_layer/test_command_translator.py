...
def test_all_42_commands_have_exactly_one_lane() -> None:
    ...
    for kind in CreatorCommandType:
        ...
        if kind == CreatorCommandType.CHANGE_UNIT_ROUTE:
            ...
            command = _command(
                kind,
                arguments={"assetVersionRefs": ["asset://source@v1"]},
            )
        ...
        assert (disposition.lane == "deterministic_mutation") is (
            kind in DETERMINISTIC_COMMANDS
        )
        assert disposition.requires_manual_edit_outbox is (
            kind in DETERMINISTIC_COMMANDS
        )
        assert (disposition.coalesce_key is not None) is (kind in coalesced)

    audio = translate_command(
        _command(CreatorCommandType.SET_EDIT_AUDIO_PLAN),
...
def test_pending_generation_is_durable_deferred_not_executed() -> None:
    disposition = translate_command(
        _command(CreatorCommandType.GENERATE_R2V_VIDEO),
        ui_phase=UiPhase.WAITING_REVIEW,
    )
...
def test_pending_manual_edit_requires_presentation_or_overlay_cas() -> None:
    ...
    accepted = command.model_copy(
        update={"expected_presentation_version": "presentation-1"},
    )
...
def test_attach_uses_exact_immutable_asset_versions_and_route_has_no_alias() -> (
    None
):
    with pytest.raises(ValidationError):
        translate_command(
            _command(
                CreatorCommandType.ATTACH_SOURCE_ASSETS,
                arguments={"assetVersionRefs": ["asset:a1"]},
            ),
            ui_phase=UiPhase.IDLE,
        )


def test_asset_create_delete_use_the_fixed_deterministic_supplement_lane() -> (
    None
):
...
def test_finalizing_queues_while_stopped_phases_reject_new_commands() -> None:
    assert (
        translate_command(
            _command(CreatorCommandType.GENERATE_SCRIPT),
            ui_phase=UiPhase.FINALIZING,
        ).phase_state
        == "QUEUE_FOR_BOUNDARY"
    )
