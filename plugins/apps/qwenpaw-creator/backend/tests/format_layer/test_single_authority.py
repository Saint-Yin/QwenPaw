...
def test_format_layer_has_no_old_authority_or_global_join_implementation():
    source = "\n".join(
        ...
        for path in sorted(FORMAT_ROOT.glob("*.py"))
    )
    forbidden = (
...
def test_missing_target_version_is_rejected_instead_of_invented(
    projection_fixture,
):
    ...
    with pytest.raises(
        ProjectionInputError,
        match="target version is missing: unit:u-edit",
    ):
...
def test_workspace_snapshot_only_accepts_documented_text_formats():
    with pytest.raises(
        ProjectionInputError,
        match="invalid Text Workspace path",
    ):
        TextWorkspaceSnapshot(
            project_id="p1",
            revision_id="r1",
