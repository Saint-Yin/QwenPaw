...
def test_header_plan_assets_and_refs_are_page_specific_envelopes(app):
    async def scenario():
        async with AsyncClient(
            ...
            header_idle = await client.get(
                f"/projects/{project['projectId']}/header",
            )
            plan_idle = await client.get(
                f"/projects/{project['projectId']}/plan",
            )
            ...
            header_working = await client.get(
                f"/projects/{project['projectId']}/header",
            )
            ...
            refs = await client.get(
                f"/projects/{project['projectId']}/refs",
                params={"types": "asset"},
            )
            return header_idle, plan_idle, header_working, assets, refs

    header_idle, plan_idle, header_working, assets, refs = asyncio.run(
        scenario(),
    )
