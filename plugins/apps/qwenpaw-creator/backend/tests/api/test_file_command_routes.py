...
def test_model_command_is_queued_as_one_runtime_message_and_replays(
    tmp_path,
) -> None:
    app, services = _app(tmp_path)
    payload = {
...
def test_image_command_dispatches_file_worker_without_agent_message(
    ...
    assert (
        services.sessions.list_messages("project-1", session.session_id) == []
    )
...
def test_ai_edit_commands_mutate_embedded_plan_directly(tmp_path) -> None:
    ...
    assert all(
        response.json()["status"] == "APPLIED" for response in responses
    )
...
def test_move_and_delete_edit_clip_keep_timeline_and_storyboard_closed(
    tmp_path,
) -> None:
    app, services = _app(tmp_path, _rich_project())
    ...
    production = services.projects.read(
        "project-1",
...
def test_supplement_asset_crud_updates_variants_and_reference_closure(
    tmp_path,
) -> None:
    app, services = _app(tmp_path, _rich_project())
    ...
    responses = asyncio.run(scenario())
    assert [response.status_code for response in responses] == [202] * len(
        responses,
    )
    assert all(
        response.json()["status"] == "APPLIED" for response in responses
    ...
    new_variant = project.visual.entities.items[
        "visual-new"
...
def test_supplement_asset_refuses_to_delete_referenced_source_version(
    tmp_path,
) -> None:
    app, services = _app(tmp_path, _rich_project())
