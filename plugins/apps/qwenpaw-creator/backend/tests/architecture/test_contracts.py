# -*- coding: utf-8 -*-
# flake8: noqa: E501
# pylint: disable=implicit-str-concat,use-implicit-booleaness-not-comparison
...
def test_reference_contracts_reject_aliases_and_path_escape() -> None:
    ...
    assert safe_relative_path(
        "story/sections/001000--s/units/001000--u/title.txt",
    )
...
def test_plugin_entrypoint_imports_only_the_file_native_runtime() -> None:
    ...
    entrypoint = PLUGIN_ROOT / "backend" / "main.py"
    tree = ast.parse(
...
def test_creator_declares_only_dependencies_missing_from_qwenpaw() -> None:
    ...
    qwenpaw_packages = {
        ...
        for requirement in (
            ...
            + [
                item
                for group in qwenpaw_project.get(
...
def test_removed_paths_construction_files_and_runtime_outputs_are_absent() -> (
    None
):
    forbidden = (
        BACKEND / "agents",
...
def test_backend_has_no_forbidden_business_tokens_and_ui_has_no_retired_authorities() -> (
    None
):
    second = re.compile(
        r"taskType[^\n]*(?:t2v|i2v)|['\"]task_type['\"]"
