...
def test_filter_chain_emits_xfade_and_acrossfade_with_running_offsets():
    ...
    assert (
        "xfade=transition=fade:duration=0.400000:offset=4.600000" in chain
    )
    # 第二对的 offset 基于已消耗 blend 的累计链时长：4.6 + 4 - 0.5 = 8.1。
    assert (
        "xfade=transition=fadeblack:duration=0.500000:offset=8.100000"
        in chain
    )
...
def test_runner_maps_transitions_to_segment_tail_trim_and_joins():
    ...
    tail_trims, joins_by_pair = (
        FfmpegLocalMediaRunner._transition_directives(spec)
    )
