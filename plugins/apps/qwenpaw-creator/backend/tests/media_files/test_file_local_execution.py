...
class FakeLocalMediaRunner:
    def __init__(
        self,
        hook: Callable[[LocalMediaExecutionSpec], Awaitable[None] | None]
        | None = None,
    ) -> None:
    ...
    raw["assets"] = {
        "files_by_id": {
            "file-source": _indexed_file(
                "file-source",
                "assets/sources/source.mp4",
                source_content,
            ),
            "file-unit": _indexed_file(
                "file-unit",
                "assets/artifacts/unit.mp4",
                unit_content,
            ),
            "file-section": _indexed_file(
                "file-section",
                "assets/artifacts/section.mp4",
                section_content,
            ),
        },
...
def test_execute_edit_persists_task_attempt_output_and_replays(
    tmp_path: Path,
) -> None:
    ...
    assert (
        spec.work_dir == project_root / "runtime" / "task-work" / first.task_id
    )
    ...
    assert [
        item.status.value
        for item in runtime.list_attempts("project-1", task.task_id)
    ] == [
        "RUNNING",
        "SUCCEEDED",
    ]
...
def test_execute_edit_can_materialize_url_source_without_ingest_cache(
    ...
    assert (
        runner.calls[0].inputs[0].path.read_bytes() == b"remote-source-video"
    )
...
def test_execute_edit_prefers_existing_remote_runtime_cache(
    ...
    project_root = services.projects.project_root("project-1")
    cache_path = (
        project_root / "runtime" / "asset-cache" / "source-version-1.mp4"
    )
    ...
    monkeypatch.setattr(
        ...
        lambda *_args: pytest.fail(
            "existing cache must avoid a second download",
        ),
    )
    runner = FakeLocalMediaRunner()
...
def test_section_and_final_compose_select_explicit_output_versions(
    ...
    final_artifact = project.assets.artifact_versions_by_id[
        final.artifact_version_id
    ]
...
def test_cancelled_task_quarantines_late_local_output(tmp_path: Path) -> None:
    ...
    worker = FileLocalMediaExecutionService(
        services,
        runner=FakeLocalMediaRunner(cancel),
    )
...
def test_restart_recovery_converges_running_task_with_durable_result(
    ...
    runtime = ProjectExecutionStore(services.root)
    assert (
        runtime.get_task("project-1", ids["task_id"]).status
...
def test_terminal_task_replay_preserves_original_runtime_error(
    tmp_path: Path,
) -> None:
    services = _services(tmp_path)
...
def test_ffmpeg_timeout_terminates_kills_and_fails_task(
    ...
    assert (
        task.metadata["artifactVersionId"]
        not in project.assets.artifact_versions_by_id
    )
...
def test_default_ffmpeg_runner_smoke(tmp_path: Path) -> None:
    ...
    project = services.projects.read("project-1").project
    artifact = project.assets.artifact_versions_by_id[
