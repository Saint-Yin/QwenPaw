...
def test_generate_asset_updates_variant_history_and_selected_pointer(
    ...
    assert variant.generated_artifact_version_ids == [
...
def test_generate_asset_resolves_exact_artifact_refs_and_variant_refs(
    ...
    variant = candidate["visual"]["entities"]["items"]["character-1"][
        "variants"
    ]["items"]["variant-1"]
    variant["reference_artifact_version_ids"] = [
        storyboard.artifact_version_id,
    ]
    ...
    assert asset_provider.calls[0]["reference_image_urls"][0].startswith(
        "file://",
    )
...
def test_cancelled_task_quarantines_late_provider_output(
    tmp_path: Path,
) -> None:
    services = _services(tmp_path)
    runtime = ProjectExecutionStore(services.root)
...
def test_provider_generated_file_must_belong_to_current_task_work(
    ...
    assert (
        project.assets.files_by_id[artifact.file_id].media_type == "image/png"
    )
    assert project.assets.files_by_id[artifact.file_id].relative_uri.endswith(
        ".png",
    )
...
def test_provider_scratch_is_project_scoped_and_url_round_trips(
    ...
    assert url.startswith(
        "/generated/projects/project-1/task-work/task-1/images/",
    )
