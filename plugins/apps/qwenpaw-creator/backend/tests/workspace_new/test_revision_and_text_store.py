...
def test_initial_approved_revision_is_idempotent_immutable_and_persistent(
    workspace_stack,
):
    ...
    second = workspace_stack.revisions.create_initial_approved_revision(
        project_id="project-1",
        files=dict(reversed(list(INITIAL_FILES.items()))),
    )
    ...
    with pytest.raises(ConflictError, match="different|不同"):
        workspace_stack.revisions.create_initial_approved_revision(
            project_id="project-1",
            files={**INITIAL_FILES, "title.txt": "另一个初始值"},
        )


def test_same_entry_tree_has_same_hash_across_distinct_revisions(
    workspace_stack,
):
...
def test_immutable_revision_id_cannot_be_overwritten(workspace_stack):
    ...
    changed_entries = workspace_stack.revisions.entries_from_files(
        {"title.txt": "changed"},
    )
...
def test_working_tree_reads_and_writes_only_through_blob_mapping(
    workspace_stack,
):
    initial = workspace_stack.revisions.create_initial_approved_revision(
        project_id="project-1",
    ...
    current_branch = workspace_stack.text.get_working_tree(
        "project-1",
        branch.id,
    )
    ...
    branch_metadata = next(
        (workspace_stack.content.root / "working-tree-store").rglob(
            "branch-1.json",
        ),
    )
    ...
    assert all(
        "blobHash" in entry and "text" not in entry
        for entry in payload["entries"]
    )
    assert not (workspace_stack.content.root / "project-workspaces").exists()


def test_stale_head_or_target_version_cannot_overwrite_new_value(
    workspace_stack,
):
    initial = workspace_stack.revisions.create_initial_approved_revision(
        project_id="project-1",
...
def test_failed_branch_metadata_publish_keeps_previous_authoritative_head(
    ...
    monkeypatch.setattr(
        "services.workspace.text_store.atomic_replace_bytes",
        fail_before_replace,
    )
...
def test_text_workspace_format_rules_are_enforced(
    workspace_stack,
    path,
    text,
    message,
