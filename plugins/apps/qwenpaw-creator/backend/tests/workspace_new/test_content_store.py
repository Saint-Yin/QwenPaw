...
def test_blob_and_artifact_are_content_addressed_and_deduplicated(tmp_path):
    ...
    assert (
        artifact.path
        == store.root / "artifacts" / "sha256" / digest[:2] / digest
    )
...
def test_content_publish_order_is_temp_fsync_hash_rename_directory_fsync(
    tmp_path,
):
...
def test_failure_before_atomic_rename_never_publishes_target(tmp_path):
    ...
    store = ContentStore(
        tmp_path / "creator-data",
        stage_hook=crash_after_hash,
    )
