...
class RecordingGateway:
    ...
    def commit_text_write(
        ...
        self._seq += 1
        return CommittedWorkspaceMutation(
            result,
            f"mutation-{self._seq}",
            self._seq,
        )
...
def test_reads_current_working_branch_or_immutable_revision_and_accumulates_versions(
    ...
    assert (
        working_context.read_set.snapshot()[0].object_version
...
def test_truncated_glob_still_records_every_enumerated_file_version(
    workspace_stack,
    branch,
):
    _, working = branch
...
def test_read_scope_filters_search_and_direct_reads_are_denied(
    workspace_stack,
    branch,
):
    _, working = branch
...
def test_public_tool_map_is_role_filtered(workspace_stack, branch):
    ...
    assert set(
        WorkspaceFileTools(workspace_stack.text, creator).public_tools(),
    ) == {
        "read_file",
        "write_file",
        "edit_file",
    ...
    assert set(
        WorkspaceFileTools(workspace_stack.text, review).public_tools(),
    ) == {
        "read_file",
        "grep_search",
        "glob_search",
    ...
    assert (
        WorkspaceFileTools(workspace_stack.text, ai_edit).public_tools() == {}
    )


def test_write_edit_append_use_cas_readset_and_atomic_gateway_evidence(
    workspace_stack,
    branch,
):
    _, working = branch
    ...
    assert (
        context.read_set.snapshot()[0].object_version
        == second["objectVersion"]
    )
...
def test_write_file_creates_sibling_leaf_when_virtual_parent_exists(
    ...
    assert (
        workspace_stack.text.read_text(
            "project-1",
            path,
            branch_id=working.id,
        )
...
def test_unrelated_working_head_change_does_not_invalidate_scoped_writer(
    workspace_stack,
    branch,
):
    _, working = branch
...
def test_changed_readset_file_rejects_write_even_when_target_is_unchanged(
    workspace_stack,
    branch,
):
    _, working = branch
...
def test_target_cas_blocks_stale_writer_and_gateway_race(
    workspace_stack,
    branch,
):
    _, working = branch
    ...
    race_gateway = RecordingGateway(
        workspace_stack.text,
        race_text="external winner",
    )
    race_tools, _ = _tools(
        workspace_stack,
        context,
...
def test_non_writable_transaction_phases_reject_immediately(
    workspace_stack,
    branch,
    status,
):
...
def test_owner_review_ai_edit_and_creator_write_boundaries_are_hard(
    workspace_stack,
    branch,
):
    _, working = branch
...
def test_unsafe_path_is_rejected_before_store_or_gateway_access(
    workspace_stack,
    branch,
):
    _, working = branch
...
def test_source_run_may_write_an_older_analysis_version_when_run_isolation_is_disabled(
    workspace_stack,
    branch,
):
    _, working = branch
