...
from services.workspace.dependency_graph import (
    DependencyEdge,
    DependencyGraph,
    DependencyStrength,
)
...
def test_hard_components_form_independent_review_groups() -> None:
    ...
    components = graph.hard_components(
        (
            "unit:u1",
            "section:s1/sequence",
            "post:final",
    ...
    assert (
        frozenset({"unit:u1", "section:s1/sequence", "post:final"})
        in components
    )
...
def test_impact_closure_distinguishes_hard_and_soft_edges() -> None:
    ...
    assert graph.impact_closure(
        ("story:b1",),
        include_soft=False,
    ) == frozenset(
        {"story:b1", "unit:u1", "artifact:v1"},
    )
    assert "visual:hero" in graph.impact_closure(
        ("story:b1",),
        include_soft=True,
    )
