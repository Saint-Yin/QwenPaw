...
export const useReviewManifestStore = create<ReviewManifestState>(
  (set, get) => {
    let requestEpoch = 0;
    let nextRequestId = 0;
    ...
    return {
      ...
      load: async (projectId, transactionId) => {
        ...
        const manifestRequest = beginRequest(
          "manifest",
          projectId,
          transactionId,
        );
        const authorizationRequest = beginRequest(
          "authorizations",
          projectId,
          transactionId,
        );
        ...
        try {
          ...
          set((state) => ({
            ...(isCurrent(manifestRequest, state)
              ? { manifest, loading: false }
              : {}),
            ...(isCurrent(authorizationRequest, state)
      ...
      decide: async (groupId, request) => {
        ...
        if (!projectId || !transactionId)
          throw new Error("当前没有可审阅的 Transaction");
        ...
        const clientRequestId =
          actionRetryIds.get(signature) ?? newClientId("review-decision");
        actionRetryIds.set(signature, clientRequestId);
        const response = await decideReviewGroup(
          projectId,
          transactionId,
          groupId,
          request,
      ...
      comment: async (groupId, text, selection) => {
        ...
        const clientCommentId =
          commentRetryIds.get(signature) ?? newClientId("review-comment");
      ...
      loadAuthorizations: async () => {
        ...
        const response = await listExecutionAuthorizations(
          projectId,
          transactionId,
        );
        set((state) =>
          isCurrent(token, state) ? { authorizations: response.items } : {},
        );
      },
      ...
      loadOperationContent: async (operationId) => {
        ...
        if (
          ...
          !transactionId ||
          !manifest ||
          operationContents[operationId] ||
          operationContentLoading[operationId]
        )
          return;
        ...
        set((state) => ({
          operationContentLoading: {
            ...state.operationContentLoading,
            [operationId]: true,
          },
          operationContentErrors: {
            ...state.operationContentErrors,
            [operationId]: "",
          },
        }));
        try {
          const content = await getReviewOperationContent(
            projectId,
            manifest.reviewRevisionId,
            operationId,
          );
          set((state) =>
            isCurrent(token, state)
          ...
          set((state) =>
            isCurrent(token, state)
              ? {
                  ...
                  operationContentErrors: {
                    ...state.operationContentErrors,
                    [operationId]: (error as Error).message,
                  },
                }
              : {},
          );
      ...
      approveAuthorization: async (authorizationId, request) => {
        ...
        const updated = transactionId
          ? await approveExecutionAuthorization(
              projectId,
              transactionId,
              authorizationId,
              request,
          ...
          : await approveFileExecutionAuthorization(
              projectId,
              authorizationId,
              request,
              clientRequestId,
      ...
      declineAuthorization: async (authorizationId, authorizationToken) => {
        ...
        actionRetryIds.set(signature, clientRequestId);
        const updated = transactionId
          ? await declineExecutionAuthorization(
              projectId,
              transactionId,
              authorizationId,
              request,
          ...
          : await declineFileExecutionAuthorization(
              projectId,
              authorizationId,
              request,
              clientRequestId,
