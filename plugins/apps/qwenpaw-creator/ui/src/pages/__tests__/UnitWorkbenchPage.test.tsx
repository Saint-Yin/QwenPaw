...
describe("UnitWorkbenchPage origin presentation routing", () => {
  ...
  it("derives the original section/unit breadcrumb from canonical Header + Plan Views", async () => {
    ...
    const { calls } = installMockFetch([
      {
        ...
        response: { json: envelope(assetsWithPlannedVisual) },
      },
      {
        match: "/projects/p1/units/u1/workbench",
        response: { json: envelope(r2vView) },
      },
    ]);
    ...
    await waitFor(() => {
      expect(
        screen.getByText("视频方案 / 01 开场 / 01 Unit 1 / 制作工作台"),
      ).toBeInTheDocument();
    });
    expect(calls.filter((call) => call.url.endsWith("/header"))).toHaveLength(
      0,
    );
    expect(calls.filter((call) => call.url.endsWith("/plan"))).toHaveLength(0);
    expect(calls.filter((call) => call.url.endsWith("/assets"))).toHaveLength(
      1,
    );
    expect(
      calls.filter((call) => call.url.endsWith("/units/u1/workbench")),
    ).toHaveLength(1);
  ...
  it("settles the formal Workbench route without remounting or repeating bootstrap requests", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "Shot 列表（1）" }),
    ).toBeInTheDocument();
    expect(
    ...
    await waitFor(() => {
      ...
      expect(calls.filter((call) => call.url.endsWith("/plan"))).toHaveLength(
        1,
      );
      expect(calls.filter((call) => call.url.endsWith("/assets"))).toHaveLength(
        1,
