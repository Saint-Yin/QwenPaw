...
describe("AssetsPage", () => {
  ...
  it("preserves the origin/main category-grid-inspector information architecture", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByRole("button", { name: "用户上传 1" }),
      ).toBeInTheDocument(),
    );
    expect(screen.getByRole("complementary", { name: "资产分类" })).toHaveClass(
      "flex",
      "w-52",
      "shrink-0",
      "flex-col",
    );
    expect(screen.getByTestId("asset-grid-column")).toHaveClass(
      "flex",
      "min-w-0",
      "flex-1",
      "flex-col",
    );
    expect(screen.getByRole("button", { name: "全部资产 1" })).toHaveClass(
    ...
    expect(screen.getByText("来源").closest("aside")).toHaveClass(
      "w-80",
      "shrink-0",
    );
  });
  ...
  it("requires the server presentation DTO and never reconstructs cards from availableAssets", async () => {
    type PresentationAssetsIsRequired = {} extends Pick<
      AssetLibraryView,
      "presentationAssets"
    >
      ? false
    ...
    const explicitEmptyPresentation: AssetLibraryView = {
      ...assetView,
      presentationAssets: [],
    };
    installMockFetch([
      {
        ...
        response: { json: envelope(explicitEmptyPresentation) },
      },
    ]);
    renderPage();
    await waitFor(() =>
      expect(
        screen.getByRole("button", { name: "全部资产 0" }),
      ).toBeInTheDocument(),
    );
  ...
  it("serves selected visual artifacts through the Creator media endpoint instead of file URLs", async () => {
      ...
      presentationAssets: [
        {
          ...
          sourceRef:
            "artifact://slot-orange-cat@artifact-version-orange-cat-v1",
          referenceCount: 0,
          ...
          detail: {
            ...
            images: [
              {
                id: "artifact-version-orange-cat-v1",
                name: "正面锚点",
                url: "file:///private/runtime/orange-cat.png",
              },
            ],
    ...
    installMockFetch([
      {
        match: "/projects/p1/assets",
        response: { json: envelope(visualView) },
      },
    ]);
    renderPage();
    ...
    expect(image).toHaveAttribute(
      "src",
      "/api/qwenpaw-creator/media/artifacts/artifact-version-orange-cat-v1",
    );
  ...
  it("resolves a visual selected from an uploaded version through the logical source Asset", async () => {
    ...
    expect(image).toHaveAttribute(
      "src",
      "/api/qwenpaw-creator/projects/p1/assets/hero-source/content?versionId=av-hero",
    );
  });
  ...
  it("does not mount uploaded media before its durable ingest succeeds", async () => {
    ...
    installMockFetch([
      {
        match: "/projects/p1/assets",
        response: { json: envelope(runningView) },
      },
    ]);
    renderPage();

    expect(
      (await screen.findAllByText("入库中")).length,
    ).toBeGreaterThanOrEqual(1);
    expect(screen.queryByLabelText("clip.mp4 视频")).not.toBeInTheDocument();
    expect(
      screen.queryByRole("img", { name: "clip.mp4" }),
  ...
  it("renders successful uploaded and generated videos as real video elements and hides load failures", async () => {
    ...
    installMockFetch([
      {
        match: "/projects/p1/assets",
        response: { json: envelope(successfulView) },
      },
    ]);
    renderPage();
    ...
    expect(uploadedVideo).toHaveAttribute(
      "src",
      "/api/qwenpaw-creator/projects/p1/assets/a-video/content?versionId=av-video",
    );
    expect(generatedVideo).toHaveAttribute(
      "src",
      "/api/qwenpaw-creator/media/artifacts/artifact-video",
    );
  ...
  it("detaches only the exact immutable source ref through the origin delete interaction", async () => {
      ...
      attachedSources: [
        {
          assetId: "a1",
          assetVersionId: "av1",
          sourceRef: "asset://a1@av1",
          name: "素材一",
          sourceLabel: "用户上传",
          mediaType: "video",
          referenceCount: 0,
          targetVersion: "ov-a1",
          uiLocator: { page: "assets", assetId: "a1", versionId: "av1" },
        },
      ],
      availableAssets: [
        {
          ...
          attached: true,
          presentationStatus: "accepted" as const,
        },
      ],
      presentationAssets: [
        {
    ...
    expect(confirm).toHaveBeenCalledWith(
      expect.objectContaining({ title: "删除「素材一」？", okText: "删除" }),
    );
    await waitFor(() =>
    ...
    expect(command.body).toMatchObject({
      type: "DETACH_SOURCE_ASSETS",
      targetRef: "project:assets",
      arguments: { assetVersionRefs: ["asset://a1@av1"] },
    });
  });
  ...
  it("honors the exact immutable version locator for the selected logical Asset", async () => {
    const view = {
      ...
      availableAssets: [
        assetView.availableAssets[0],
        {
          ...assetView.availableAssets[0],
          assetVersionId: "av2",
          sourceRef: "asset://a1@av2",
          objectVersion: "ov-a2",
          checksum: "sha2",
          createdAt: "later",
          uiLocator: { page: "assets", assetId: "a1", versionId: "av2" },
        },
      ],
      presentationAssets: [
        {
    ...
    installMockFetch([
      { match: "/projects/p1/assets", response: { json: envelope(view) } },
    ]);
    renderPage(
      "/project/p1/assets?asset=a1&version=av2&review=1&reviewPulse=2",
    );
    await waitFor(() =>
      expect(
        document.querySelector('[data-asset-version="av2"]'),
      ).toBeInTheDocument(),
    );
    expect(document.querySelector('[data-asset-version="av2"]')).toHaveClass(
      "border-[var(--color-accent)]",
    );
    expect(
      document.querySelector('[data-asset-version="av1"]'),
    ).not.toBeInTheDocument();
  });
  ...
  it("replays the asset-card review flash when View targets the same deep link again", async () => {
    ...
    await waitFor(() =>
      expect(
        document.querySelector('[data-asset-id="a1"]'),
      ).toBeInTheDocument(),
    );
  ...
  it("keeps the supplement close behavior but reports a remote 202 as processing", async () => {
    ...
    installMockFetch([
      ...
      },
      {
        match: "/projects/p1/assets",
        method: "GET",
        response: { json: envelope(assetView) },
      },
    ]);
    renderPage();
    ...
    await waitFor(() =>
      expect(info).toHaveBeenCalledWith("链接已接收，正在入库"),
    );
  ...
  it("reuses the ingest banner for the latest durable structured failure", async () => {
    ...
    installMockFetch([
      {
        match: "/projects/p1/assets",
        response: { json: envelope(failedView) },
      },
    ]);
    renderPage();
    expect(
      await screen.findByText("入库失败「large.mp4」 · 远程素材下载连接超时"),
    ).toBeInTheDocument();
  });
  ...
  it("keeps active ingest progress in the local asset status bar", async () => {
    ...
    expect(
      await screen.findByText("正在入库「large.mp4」 · 42%"),
    ).toBeInTheDocument();
  });
  ...
  it("reports optional cache failure without presenting the public asset as unusable", async () => {
    ...
    installMockFetch([
      {
        match: "/projects/p1/assets",
        response: { json: envelope(cacheFailedView) },
      },
    ]);
    renderPage();
    expect(
      await screen.findByText(
        "本地缓存失败，公网素材仍可用于模型 · 「large.mp4」：远程缓存连接超时",
      ),
    ).toBeInTheDocument();
    expect(
      screen.queryByText(/入库失败「large\.mp4」/),
    ).not.toBeInTheDocument();
  });
});
