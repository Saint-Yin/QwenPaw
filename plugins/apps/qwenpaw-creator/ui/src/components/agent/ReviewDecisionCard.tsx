...
export default function ReviewDecisionCard({
  ...
  const contentLoading = useReviewManifestStore(
    (state) => state.operationContentLoading,
  );
  const contentErrors = useReviewManifestStore(
    (state) => state.operationContentErrors,
  );
  ...
  const requestReviewRevision = useAgentDockUiStore(
    (state) => state.requestReviewRevision,
  );
  ...
  const reasons = useMemo(
    () => [
      ...new Set([
        ...
        ...operations.flatMap((item) => item.dependencyReasons),
      ]),
    ],
  ...
  const groupSelection =
    ...
    operations.some(
      (operation) =>
        operation.path === selectedText.field ||
        operation.targetRef === selectedText.ref,
    )
      ? selectedText
      : null;
  ...
  const textOperationKey = textOperations
  ...
  const detail =
    reasons.length > 0
      ? reasons.join("；")
      : `${operations.length} 项变更${
          first?.targetRef ? ` · ${first.targetRef}` : ""
        }`;

  return (
    <article
      data-review-decision-card={group.id}
    ...
    >
            ...
            <div className="mt-1.5 max-h-40 space-y-1 overflow-y-auto">
              {textOperations.map((operation) => {
                ...
                return (
                  <div
                    key={operation.id}
                    className="rounded bg-[var(--color-bg-secondary)] px-1.5 py-1"
                  >
                    <div className="mb-0.5 text-[10px] font-semibold text-[var(--color-text-tertiary)]">
                    ...
                    {contentLoading[operation.id] && (
                      <p className="text-[10px] text-[var(--color-text-tertiary)]">
                        加载 immutable text diff…
                      </p>
                    )}
                    {contentErrors[operation.id] && (
      ...
      <div className="mt-2 flex flex-wrap items-center gap-1.5">
        ...
        {(first?.uiLocator || integrationPreviews[0]?.uiLocator) && (
          <button type="button" onClick={view} className={BUTTON_GHOST}>
            查看
          </button>
        )}
      </div>
