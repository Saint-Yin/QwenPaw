...
function Harness({
  ...
  initial?: SelectionAttachment | null;
}) {
  ...
  return (
    <>
      <MentionInput
        ...
        onSubmit={vi.fn()}
      />
      <output data-testid="selection">{selected?.text || ""}</output>
    </>
  );
}
...
describe("MentionInput inline selection structure", () => {
  it("renders the selected review text inside the editor and serializes it for copy", () => {
    ...
    expect(
      decodeURIComponent(
        editor.querySelector<HTMLElement>("[data-selection-ref]")!.dataset
          .selectionRef!,
      ),
    ).toContain(selection.path!);
  });
