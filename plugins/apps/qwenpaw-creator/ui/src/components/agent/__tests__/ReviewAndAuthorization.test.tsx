...
const group: ReviewDecisionGroup = {
  id: "g1",
  title: "Unit 文案",
  operationIds: ["op1"],
...
function mediaVersion(
  overrides: Partial<ReviewMediaVersion> = {},
): ReviewMediaVersion {
  return {
    ...
    provenanceRefs: [
      "artifact://unit-storyboard@storyboard-v2",
      "asset://source-video@source-v1",
    ],
...
describe("review decisions and execution authorization", () => {
    ...
    const { calls } = installMockFetch([
      ...
      },
      {
        ...
        response: {
          json: {
            group: rejected,
            manifest: { ...manifest, decisionGroups: [rejected] },
          },
        },
      },
    ]);
  ...
  it("sends one group decision with its CAS token and replays repeated view highlighting", async () => {
    ...
    const { calls } = installMockFetch([
      {
        ...
        response: {
          json: {
            view: {
              operationId: "op1",
              beforeVersionRef: operation.beforeVersionRef,
              afterVersionRef: operation.afterVersionRef,
              before: "旧文案",
              after: "新文案",
              contentType: "markdown",
            },
      ...
      },
      {
        ...
        response: {
          json: {
            group: { ...group, decision: "ACCEPTED_APPLIED" },
            manifest: {
              ...manifest,
              decisionGroups: [{ ...group, decision: "ACCEPTED_APPLIED" }],
            },
          },
        },
      },
    ...
    const firstPulse =
      useNavigationStore.getState().reviewFocus?.query.reviewPulse;
    ...
    expect(decisionCall.headers["idempotency-key"]).toMatch(
      /^review-decision-/,
    );
  });
  ...
  it("loads sealed add/update/delete text content and renders content rather than opaque refs", async () => {
    const operations: ReviewOperation[] = [
      {
        ...
        id: "op-create",
        kind: "create",
        beforeVersionRef: undefined,
        afterVersionRef: "workspace-content://create@ov1",
      },
      {
        ...operation,
        id: "op-update",
      ...
      },
      {
        ...
        id: "op-delete",
        kind: "delete",
        beforeVersionRef: "workspace-content://delete@ov1",
        afterVersionRef: undefined,
      },
    ];
    installMockFetch([
      {
        match: "/review-operation-op-create",
        response: {
          json: {
            view: {
              operationId: "op-create",
              afterVersionRef: operations[0].afterVersionRef,
              after: "新增全文",
              contentType: "markdown",
            },
          },
      ...
      {
        match: "/review-operation-op-update",
        response: {
          json: {
            view: {
              operationId: "op-update",
              beforeVersionRef: operations[1].beforeVersionRef,
              afterVersionRef: operations[1].afterVersionRef,
              before: "alpha",
              after: "beta",
              contentType: "markdown",
            },
      ...
      {
        match: "/review-operation-op-delete",
        response: {
          json: {
            view: {
              operationId: "op-delete",
              beforeVersionRef: operations[2].beforeVersionRef,
              before: "删除全文",
              contentType: "markdown",
            },
          },
    ...
    useReviewManifestStore.setState({
      manifest: {
        ...
        manifestToken: "mt",
        summary: "",
        journalSeqRange: { fromExclusive: 0, toInclusive: 3 },
        decisionGroups: [reviewGroup],
        operations,
        createdArtifactVersionRefs: [],
        mediaComparisons: [],
        integrationPreviews: [],
    ...
    await waitFor(() =>
      expect(screen.getByText("新增全文")).toBeInTheDocument(),
    );
    ...
    expect(
      [...container.querySelectorAll(".agent-diff-del")].map(
        (item) => item.textContent,
      ),
    ).toContain("alph");
    expect(
      [...container.querySelectorAll(".agent-diff-add")].map(
        (item) => item.textContent,
      ),
    ).toContain("bet");
  ...
  it("keeps media evidence out of hidden card DOM and deep-links to the exact visible version chips", () => {
    const mediaOperations: ReviewOperation[] = [
      {
        ...
        path: "production/video/selected.ref",
      },
      {
        ...operation,
        id: "op-image",
        kind: "create",
        artifactKind: "reference",
        path: "production/storyboard/selected.ref",
      },
      {
        ...operation,
        id: "op-audio",
        kind: "replace_media",
        artifactKind: "reference",
        path: "post/audio/selected.ref",
      },
    ];
    const oldVideo = mediaVersion();
    const newVideo = mediaVersion({
      versionRef: "artifact://unit-video@video-v2",
      versionId: "video-v2",
      version: 2,
      selectedInBaseRevision: false,
      selectedInReviewRevision: true,
      candidateState: "SELECTED",
    });
    const image = mediaVersion({
      versionRef: "artifact://unit-storyboard@storyboard-v2",
      versionId: "storyboard-v2",
      ownerId: "unit-storyboard",
      mediaType: "image",
      mimeType: "image/png",
      artifactKind: "unit_storyboard",
      durationSeconds: null,
      selectedInBaseRevision: false,
      selectedInReviewRevision: true,
      candidateState: "SELECTED",
    });
    const oldAudio = mediaVersion({
      versionRef: "artifact://section-audio@audio-v1",
      versionId: "audio-v1",
      ownerId: "section-audio",
      mediaType: "audio",
      mimeType: "audio/wav",
      artifactKind: "section_audio",
      durationSeconds: 12,
    });
    const newAudio = mediaVersion({
      ...oldAudio,
      versionRef: "artifact://section-audio@audio-v2",
      versionId: "audio-v2",
      version: 2,
      selectedInBaseRevision: false,
      selectedInReviewRevision: true,
    ...
    const comparisons: MediaComparison[] = [
      {
        id: "cmp-video",
        operationIds: ["op-video"],
        targetRef: "unit:u1",
        path: mediaOperations[0].path,
        changeKind: "replace_media",
        before: oldVideo,
        after: newVideo,
        candidates: [newVideo],
        inputStoryboardRefs: ["artifact://unit-storyboard@storyboard-v2"],
        sourceRefs: ["asset://source-video@source-v1"],
      },
      {
        id: "cmp-image",
        operationIds: ["op-image"],
        targetRef: "unit:u1",
        path: mediaOperations[1].path,
        changeKind: "create",
        after: image,
        candidates: [image],
        inputStoryboardRefs: [],
        sourceRefs: ["asset://source-image@source-v2"],
      },
      {
        id: "cmp-audio",
        operationIds: ["op-audio"],
        targetRef: "post:s1",
        path: mediaOperations[2].path,
        changeKind: "replace_media",
        before: oldAudio,
        after: newAudio,
        candidates: [newAudio],
        inputStoryboardRefs: [],
        sourceRefs: ["asset://voice@voice-v1"],
      },
    ];
    const integration: IntegrationPreview = {
      id: "preview-s1",
      scope: "section",
      targetRef: "post:s1",
      title: "Section s1 集成预览",
      operationIds: ["op-video"],
      before: oldVideo,
      after: newVideo,
      affectedRefs: ["unit:u1", "post:s1"],
      uiLocator: { page: "section-compose", sectionId: "s1" },
    };
    const reviewGroup = {
      ...group,
      operationIds: mediaOperations.map((item) => item.id),
    };
    ...
    const { container } = render(
      <ReviewDecisionCard
        ...
        mediaComparisons={comparisons}
        integrationPreviews={[integration]}
      />,
    );

    expect(
      container.querySelector("[data-review-media-version]"),
    ).not.toBeInTheDocument();
    expect(
      container.querySelector("video, audio, [hidden]"),
    ).not.toBeInTheDocument();
    expect(screen.queryByText("Section s1 集成预览")).not.toBeInTheDocument();
    fireEvent.click(screen.getByText("查看"));
    expect(useNavigationStore.getState().reviewFocus?.path).toBe(
      "/project/p1/plan/section/s1",
    );
    expect(useNavigationStore.getState().reviewFocus?.query.version).toBe(
      "video-v2",
    );
    expect(useNavigationStore.getState().reviewFocus?.query.focus).toBe(
      "video",
    );
  });
  ...
  it("approves the exact authorization token, cost and original run", async () => {
    ...
    const { calls } = installMockFetch([
      {
        match: "/approve",
        response: { json: { ...auth, status: "APPROVED" } },
      },
    ]);
    ...
    expect(calls[0].url).toContain(
      "/transactions/tx1/execution-authorizations/auth1/approve",
    );
  });
  ...
  it("declines with the exact CAS token and a stable mutation key", async () => {
    const auth = {
      id: "auth2",
      transactionId: "tx1",
      specialistRunId: "run2",
      executionRequestId: "er2",
      targetRef: "unit:u2",
      scope: {},
      status: "PENDING" as const,
      authorizationToken: "decline-token",
      provider: "dashscope",
      model: "wan2.7-r2v",
      maxCandidates: 1,
      createdAt: "now",
    ...
    const { calls } = installMockFetch([
      {
        match: "/decline",
        response: { json: { ...auth, status: "DECLINED" } },
      },
    ]);
    render(<ExecutionAuthorizationCard authorization={auth} />);
  ...
  it("renders the main-style per-item review list with decisions but without low-level refs", () => {
    ...
    const recheck = {
      ...
      id: "recheck",
      title: "上游变化复核",
      operationIds: ["recheck-op"],
      groupingReasons: ["需复核"],
      decisionToken: "recheck-token",
    };
    const storyboard = {
      ...group,
      id: "storyboard",
      title: "镜头分镜",
      operationIds: ["storyboard-op"],
      decisionToken: "storyboard-token",
    };
    const processed = {
      ...group,
      id: "processed",
      title: "已处理视频",
      operationIds: ["processed-op"],
      decisionToken: "processed-token",
      decision: "ACCEPTED_APPLIED" as const,
    };
    const operations: ReviewOperation[] = [
      {
        ...operation,
      ...
      },
      {
        ...operation,
        id: "recheck-op",
        decisionGroupId: "recheck",
        artifactKind: "reference",
        path: "production/video.ref",
      },
      {
        ...operation,
        id: "storyboard-op",
        decisionGroupId: "storyboard",
        artifactKind: "storyboard_image",
        path: "production/storyboard.ref",
      },
      {
        ...operation,
        id: "processed-op",
        decisionGroupId: "processed",
        artifactKind: "video",
        path: "production/video.ref",
      },
    ];
    useReviewManifestStore.setState({
      manifest: {
        ...
        decisionGroups: [change, recheck, storyboard, processed],
        operations,
        createdArtifactVersionRefs: [],
        mediaComparisons: [],
        integrationPreviews: [],
    ...
    expect(
      document.querySelector("[data-review-thumbnail]"),
    ).not.toBeInTheDocument();
    expect(screen.queryByText("story/outline.md")).not.toBeInTheDocument();
    expect(
      screen.queryByText("production/storyboard.ref"),
    ).not.toBeInTheDocument();
  ...
  it("presents text review titles as a complete Plan hierarchy down to shot and field", () => {
    ...
    expect(presentation.locationSegments).toEqual([
      "视频方案",
      "01 开场",
      "01 相遇",
      "镜头 02",
      ...
      runs: [
        {
          ...
          displayName: "故事规划 Agent",
          status: "STALE",
          targetRefs: ["project:p1"],
          taskRefs: [],
          metadata: { reviewPending: true },
        },
      ],
    });
  ...
  it("projects the origin run review surface from run and decision-group authorities", async () => {
    ...
    const { calls } = installMockFetch([
      {
        ...
        response: {
          json: {
            group: { ...changeGroup, decision: "ACCEPTED_APPLIED" },
            manifest: {
              ...manifest,
              decisionGroups: [
                { ...changeGroup, decision: "ACCEPTED_APPLIED" },
              ],
            },
          },
        },
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "应用选中（其余保留待审） · 1" }),
    );
