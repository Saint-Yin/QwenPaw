...
function locatorForSemanticField(
  field: string,
...
function focusForMedia(
  version?: ReviewMediaVersion | null,
): string | undefined {
  const kind = version?.artifactKind?.toLowerCase() ?? "";
  if (kind.includes("storyboard") || version?.mediaType === "image")
    return "storyboard";
...
export function resolveReviewNavigationTarget({
  ...
  const groupOperations = operations.filter((operation) =>
    groupOperationIds.has(operation.id),
  );
  const integration = integrationPreviews.find((item) =>
    intersects(groupOperationIds, item.operationIds),
  );
  const comparison = mediaComparisons.find((item) =>
    intersects(groupOperationIds, item.operationIds),
  );
  const version =
    integration?.after ??
  ...
  const semanticOperation = groupOperations.find(
    (operation) =>
      Boolean(operation.uiLocator) &&
      Boolean(reviewFieldForOperation(operation)),
  );
  ...
  const mediaOperation = groupOperations.find(
    (operation) =>
      Boolean(operation.uiLocator) &&
