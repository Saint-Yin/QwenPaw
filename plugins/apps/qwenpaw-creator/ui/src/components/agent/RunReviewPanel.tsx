...
export default function RunReviewPanel({
  ...
  const loadContent = useReviewManifestStore(
  ...
  const pendingOperationIds = new Set(
    pendingGroups.flatMap((group) => group.operationIds),
  );
  const operationsByRun = useMemo(
    () =>
      new Map(
        runs.map((run) => [
          run.id,
          manifest?.operations.filter((operation) =>
            operation.actorRunIds.includes(run.id),
          ) ?? [],
        ]),
      ),
    [manifest, runs],
  ...
  const visibleRuns = runs.filter((run) => {
    ...
    return (
      (operationsByRun.get(run.id) ?? []).some((operation) =>
      ...
      for (const groupId of groupIds) {
        const current = useReviewManifestStore
          ...
          .manifest?.decisionGroups.find(
            (group) => group.id === groupId && group.decision === "PENDING",
          );
        if (current)
          await decide(groupId, {
  ...
  const jumpRef = (runId: string, ref: string) => {
    const operation = (operationsByRun.get(runId) ?? []).find(
      (item) => item.targetRef === ref,
    );
    if (!operation?.uiLocator) return;
    useAgentDockUiStore.getState().setOpen(false);
    navigateToLocator(projectId, operation.uiLocator, {
      description: "AgentDock Review",
    });
  };
  ...
  return (
    <div
      className="agent-run-review-panel"
      style={{ display: "flex", flexDirection: "column", gap: 12 }}
    >
      <section>
        <h4 style={{ margin: "4px 0" }}>
          {variant === "review" ? "待审提案" : "Sub-Agent 运行"}
        </h4>
        {visibleRuns.map((run) => {
          const operations = operationsByRun.get(run.id) ?? [];
          const groups = groupsForRun(run.id);
          const hasReviewGroups = groups.length > 0;
          const status = hasReviewGroups
            ? "pending_review"
            : run.status.toLowerCase();
          const selectedForRun = groups.filter((group) =>
          ...
          return (
            ...
            >
              <div
                ...
                tabIndex={0}
                onClick={() =>
                  setExpandedRunId((value) =>
                    value === run.id ? null : run.id,
                  )
                }
                onKeyDown={(event) => {
                  if (event.key === "Enter")
              ...
              >
                ...
                <span style={{ fontSize: 12, opacity: 0.7 }}>
                  {status} · {operations.length} 文件 · rev{" "}
                  {manifest?.baseRevisionId ?? "-"}
                </span>
              </div>
              ...
              {expandedRunId === run.id && (
                <div style={{ marginTop: 8 }}>
                  {run.finalSummaryText && (
                    <div
                      style={{ fontSize: 12, opacity: 0.85, marginBottom: 4 }}
                    >
                      {run.finalSummaryText}
                    </div>
                  )}
                  {run.finalMarker && (
                    <div
                      style={{
                        fontSize: 12,
                        color:
                          run.finalMarker === "SUCCESS" ? "#2e7d32" : "#c62828",
                      }}
                    >
                  ...
                  {groups.some((group) => group.groupingReasons.length > 0) && (
                    ...
                    >
                      影响下游 {groups.length} 项：
                      {groups
                        .flatMap((group) => group.groupingReasons)
                        .join("、")}
                    </div>
                  )}
                  {groups.map((group) => {
                    const checked = selectedGroupIds.includes(group.id);
                    const groupOperations = operations.filter((operation) =>
                      group.operationIds.includes(operation.id),
                    );
                    return (
                      <div key={group.id} style={{ marginTop: 6 }}>
                        ...
                        {groupOperations.map((operation) => {
                          ...
                          return content ? (
                            ...
                            >
                              {operation.path ?? operation.targetRef}
                            </div>
                          );
                        })}
                      </div>
                  ...
                  {hasReviewGroups && (
                    ...
                    >
                      <button
                        disabled={busy || selectedForRun.length === 0}
                        onClick={() =>
                          void decideGroups(
                            selectedForRun.map((group) => group.id),
                            "ACCEPT",
                          )
                        }
                      >
