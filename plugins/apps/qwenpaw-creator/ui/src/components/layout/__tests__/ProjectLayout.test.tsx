    ...
    operations: [
      {
        kind: "update",
        json_pointer:
          "/production/units_by_id/u1/plan/timeline/items/clip-01/source_in_seconds",
        file_id: null,
        target_ref: null,
        before_hash: "before",
        after_hash: "after",
...
describe("ProjectLayout visible shell", () => {
  ...
  it("preserves the 58/42 shell and default-open 440px AgentDock sidebar on a formal route", async () => {
    const { calls } = installMockFetch([
      ...
      {
        match: "/projects/p1/header",
        response: { json: envelope(headerView) },
      },
    ...
    expect(
      calls.filter((call) =>
        call.url.endsWith("/projects/p1/runtime/reviews/active"),
      ),
    ).toHaveLength(1);
    expect(
      calls.filter((call) => call.url.includes("/transactions/")),
    ).toHaveLength(0);
    ...
    act(() =>
      useCreatorSessionStore.getState().ingestEvents([
        ...
        {
          eventId: "event-review-legacy",
          seq: 3,
          type: "transaction.pending_review",
          projectId: "p1",
          creatorSessionId: "s1",
          transactionId: "tx1",
          at: "now",
        ...
        {
          eventId: "event-file-run",
          seq: 4,
          type: "agent.run.completed",
          projectId: "p1",
          creatorSessionId: "s1",
          at: "now",
          data: { runId: "run-file-1", reviewIds: [] },
        },
      ]),
    );
    await waitFor(() =>
  ...
  it("keeps the active Outlet mounted while an existing Header is revalidated", async () => {
    ...
    await waitFor(() =>
      expect(
        calls.filter((call) => call.url.includes("/models/config")),
      ).toHaveLength(1),
    );
    ...
    expect(
      calls.filter((call) => call.url.endsWith("/projects/p1/plan")),
    ).toHaveLength(1);
    act(() =>
    ...
    expect(
      calls.filter((call) => call.url.endsWith("/projects/p1/assets")),
    ).toHaveLength(0);
  });
  ...
  it("does not close AgentDock when the same project shell mounts again", async () => {
    ...
    await waitFor(() =>
      expect(
        calls.filter((call) => call.url.includes("/projects/p1/tasks")),
      ).toHaveLength(1),
    );
    expect(
      calls.filter((call) => call.url.includes("/projects/p1/header")),
    ).toHaveLength(1);
    expect(
      calls.filter((call) => call.url.endsWith("/projects/p1/session")),
    ).toHaveLength(1);
    ...
    await act(async () => {
      useCreatorSessionStore.getState().ingestEvents([
        ...
        {
          eventId: "nested-message-completed",
          seq: 8,
  ...
  it("navigates to and queues the changed clip only for a completed run with a Review", async () => {
    ...
    const { calls } = installMockFetch([
      ...
      { match: "/projects/p1/plan", response: { json: envelope(planView) } },
      {
        match: "/projects/p1/units/u1/workbench",
        response: { json: envelope(editView) },
      },
      { match: "/projects/p1/tasks", response: { json: { items: [] } } },
      {
        match: "/models/config",
      ...
      [
        {
          ...
          children: [
            ...
            },
            {
              ...
              element: (
                <div data-testid="review-workbench-route">Workbench</div>
              ),
            },
          ],
        },
    ...
    expect(
      calls.filter((call) => call.url.includes("/units/u1/workbench")),
    ).toHaveLength(0);
    ...
    expect(
      await screen.findByTestId("review-workbench-route"),
    ).toBeInTheDocument();
