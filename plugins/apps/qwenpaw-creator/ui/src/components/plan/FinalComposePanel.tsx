...
export default function FinalComposePanel({
  projectId,
  visible,
  onClose,
  focusVersion,
  ...
  const candidates = useMemo<ComposeCandidate[]>(
    () =>
      (view?.candidates || []).map((artifact) => {
        const sourceId = identifierOf(artifact.ownerRef);
        const sourceType =
          artifact.sourceKind ||
          (artifact.ownerRef.includes("://unit/") ? "unit" : "section");
        const sectionId =
          artifact.sectionId || (sourceType === "section" ? sourceId : "");
        const section = view?.sections?.find((item) => item.id === sourceId);
        const unit = plan?.sections
          .flatMap((item) => item.units)
          .find((item) => item.id === sourceId);
        const resolved = view?.resolvedRefs.find(
          (item) => identifierOf(item.ref) === sourceId,
        );
        return {
  ...
  useEffect(() => {
    ...
    const key = `${view.targetVersion}:${
      envelope?.reviewRevisionId || envelope?.approvedRevisionId || ""
    }`;
    if (initializedSelectionKey.current === key) return;
    initializedSelectionKey.current = key;
    if (view.selections.length > 0) {
      setSelection(
        [...view.selections].sort((left, right) => left.order - right.order),
      );
      return;
    }
    const sectionCandidates = candidates.filter(
      (candidate) => candidate.sourceType === "section",
    );
    const fallback =
      sectionCandidates.length > 0 ? sectionCandidates : candidates;
    setSelection(
      fallback.map((candidate, order) => ({
  ...
  const candidateById = useMemo(
    () =>
      new Map(
        candidates.map((candidate) => [candidate.artifactVersionId, candidate]),
      ),
    [candidates],
  );
  ...
  const totalDuration = selection.reduce(
    (sum, item) =>
      sum + (candidateById.get(item.artifactVersionId)?.duration || 0),
    0,
  );
  const rendered = view?.resolvedRefs.find(
    (item) => item.ref === view.renderedVideoRef,
  );
  ...
  const selectedCandidatesValid = selection.every((item) =>
    candidateById.has(item.artifactVersionId),
  );
  const nonSelectionBlockers =
    view?.blockers.filter(
      (blocker) => !blocker.startsWith("FINAL_COMPOSE_SOURCE_MISSING:"),
    ) ?? [];
  const canCompose = Boolean(
    selection.length > 0 &&
      selectedCandidatesValid &&
      nonSelectionBlockers.length === 0 &&
      !submitting &&
      !composing,
  );

  const persistSelection = (next: ComposeSelectionView[]) =>
    submit(
  ...
  const toggleCandidate = (candidate: ComposeCandidate) => {
    const exists = selection.some(
      (item) => item.artifactVersionId === candidate.artifactVersionId,
    );
    const next = exists
      ? selection.filter(
          (item) => item.artifactVersionId !== candidate.artifactVersionId,
  ...
  const decideRendered = async (decision: "ACCEPT" | "REJECT") => {
    const current = useReviewManifestStore
      ...
      .manifest?.decisionGroups.find(
        (group) =>
          group.id === renderedReviewGroup?.id && group.decision === "PENDING",
      );
    if (!current) return;
    ...
    message.success(
      decision === "ACCEPT" ? "已接受最终成片" : "已撤销最终成片",
    );
    await reload();
  };
      ...
      footer={
        <div className="flex items-center justify-between">
          <span className="text-xs text-[var(--color-text-tertiary)]">
            已选 {selection.length} 段
            {totalDuration > 0 ? ` · 预计 ${totalDuration}s` : ""}
          </span>
          <div className="flex gap-2">
            <Button onClick={onClose} disabled={submitting || composing}>
              关闭
            </Button>
            <Button
              type="primary"
      ...
      {rendered?.artifactVersionId && (
        <div className="mb-4 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-4">
          ...
          <div className="mt-2 text-xs text-[var(--color-text-tertiary)]">
            输出路径：{view?.renderedVideoUrl || view?.renderedVideoRef}
          </div>
            ...
            <div className="mt-2 flex items-center justify-between rounded-lg border border-[var(--color-warning)]/25 bg-[var(--color-warning-soft)] px-2.5 py-1.5">
              ...
              <div className="flex items-center gap-1">
                <Button
                  size="small"
                  type="primary"
                  onClick={() => void decideRendered("ACCEPT")}
                  className="!text-[11px]"
                >
                  接受
                </Button>
                <Button
                  size="small"
                  onClick={() => void decideRendered("REJECT")}
                  className="!text-[11px]"
                >
                  撤销
                </Button>
          ...
          <div className="min-h-0 overflow-y-auto rounded-lg border border-[var(--color-border)] p-3">
            ...
            <div className="space-y-3">
              {groupedCandidates.map((group) => (
                <div key={group.id}>
                  <div className="mb-1 text-xs font-bold text-[var(--color-text-primary)]">
                    {group.number}. {group.title}
                  </div>
                  {group.items.length === 0 ? (
                    <div className="pl-6 text-[11px] text-[var(--color-text-tertiary)]">
                      本段无已接受成片
                    </div>
                  ) : (
      ...
      <div className="mt-4 flex items-center gap-6 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-card)] px-4 py-3">
        ...
        <div className="flex min-w-[200px] flex-1 items-center gap-3">
          ...
          <Slider
            ...
            tooltip={{
              formatter: (value) =>
                value === 0 ? "硬切" : `${value?.toFixed(1)}s`,
            }}
          />
          <span className="w-12 shrink-0 text-right text-xs font-medium text-[var(--color-text-primary)]">
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="text-[11px] text-[var(--color-text-tertiary)]">
          {candidate.sourceType === "section" ? "整段成片" : "单元成片"}
...
function SelectedRow({
  ...
  return (
    ...
    >
      <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[var(--color-primary)] text-xs font-semibold text-white">
        {index + 1}
      </div>
      <div className="min-w-0 flex-1">
        <div className="truncate text-xs font-medium text-[var(--color-text-primary)]">
      ...
      <div className="flex shrink-0 items-center gap-0.5">
        ...
        <Button size="small" type="text" danger onClick={onRemove} title="移除">
          <CloseOutlined />
        </Button>
      </div>
    </div>
