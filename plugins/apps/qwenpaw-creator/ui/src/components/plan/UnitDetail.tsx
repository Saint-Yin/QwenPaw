...
function progressLabel(
  ...
  if (
    ...
    ["executing", "finalizing", "resuming"].includes(uiPhase || "")
  )
    return "生成中";
  if (unit.blockers.length > 0 || unit.readiness.blockers.length > 0)
    return "有阻塞";
  if (unit.videoUrl || unit.storyboardImageUrl)
    return uiPhase === "waiting_review" ? "待审" : "已接受";
  return "未生成";
}
...
function formatTimecode(value?: number): string {
  ...
  return `${String(minutes).padStart(2, "0")}:${secondText.padStart(
    secondText.includes(".") ? 4 : 2,
    "0",
  )}`;
}
      ...
      {storyboard.length > 0 ? (
        <div className="mt-3 grid gap-2 md:grid-cols-2">
          {storyboard.map((panel) => (
            <article
              key={panel.panel_id}
              className="rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-secondary)]/50 p-3"
            >
              ...
              {panel.timeline_start != null && panel.timeline_end != null && (
                <p
                  data-creator-field={`unit:${unitId}/editPlan/timeline/clip:${
                    panel.clip_id ?? panel.panel_id
                  }/sourceRange`}
                  data-creator-path={
                    panel.clip_id
                      ? projectJsonPointer(
                ...
                >
                  成片 {formatTimecode(panel.timeline_start)}–
                  {formatTimecode(panel.timeline_end)}
                </p>
              )}
            </article>
          ))}
...
export default function UnitDetail({
  ...
  const missingShotOperations = useMemo(() => {
    ...
    return manifest.operations.filter((operation) => {
      if (
        !operation.path?.includes(`--${unit.id}--`) ||
        !operation.path.includes("/shots/")
      )
        return false;
      const shotId = operation.targetRef.startsWith("shot:")
        ? operation.targetRef.slice(5).split("/", 1)[0]
        : "";
      return (
        operation.kind === "create" && Boolean(shotId) && !existing.has(shotId)
      );
    });
    ...
    <div className="flex h-full flex-col">
      <div className="flex items-start justify-between gap-3 border-b border-[var(--color-border)] px-4 py-3">
        <div className="min-w-0">
          ...
          >
            <ReviewFieldText field={`unit:${unit.id}/title`}>
              {unit.title || unitDisplayName(unit)}
            </ReviewFieldText>
          </h3>
          <p className="mt-0.5 truncate text-xs text-[var(--color-text-secondary)]">
            ...
            >
              <ReviewFieldText field={`unit:${unit.id}/duration`}>
                {unit.duration}
              </ReviewFieldText>
            </span>
            s · {isEditUnit ? "AI 剪辑工作台" : "生成制作台"} ·{" "}
            {progressLabel(unit, uiPhase, activeUnitId)}
          </p>
          {unit.sceneRef && (
            <p className="mt-0.5 truncate text-[11px] text-[var(--color-text-tertiary)]">
              ...
              >
                <ReviewFieldText field={`unit:${unit.id}/sceneRef`}>
                  {unit.sceneRef}
                </ReviewFieldText>
              </span>
            </p>
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-4">
          ...
          <section className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-4">
            ...
            {unit.shots.length === 0 ? (
              ...
              <div className="space-y-2">
                {unit.shots.map((shot) => (
                  <div
                    key={shot.id}
                    className="flex items-start gap-2.5 rounded-lg bg-[var(--color-bg-secondary)]/60 px-2.5 py-2"
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      >
                        <ReviewFieldText
                          field={`unit:${unit.id}/shot:${shot.id}/description`}
                        >
                          {shot.description}
                        </ReviewFieldText>
                      </p>
                      {shot.cameraDescription ? (
                        <p className="mt-0.5 text-[11px] text-[var(--color-text-tertiary)]">
                          ...
                          >
                            <ReviewFieldText
                              field={`unit:${unit.id}/shot:${shot.id}/cameraDescription`}
                            >
                              {shot.cameraDescription}
                            </ReviewFieldText>
                          </span>
                      ...
                      {shot.dialogue ? (
                        <p className="mt-0.5 text-[11px] text-[var(--color-text-tertiary)]">
                          ...
                          >
                            <ReviewFieldText
                              field={`unit:${unit.id}/shot:${shot.id}/dialogue`}
                            >
                              {shot.dialogue}
                            </ReviewFieldText>
                          </span>
                    ...
                    >
                      <ReviewFieldText
                        field={`unit:${unit.id}/shot:${shot.id}/duration`}
                      >
                        {shot.duration}
                      </ReviewFieldText>
                      s
            ...
            {missingShotOperations.length > 0 && (
              <div className="mt-2 rounded-lg border border-dashed border-[var(--color-accent)]/35 bg-[var(--color-accent-soft)]/20 p-2">
                ...
                {missingShotOperations.map((operation) => (
                  <ReviewDiffPreview
                    key={operation.id}
                    field={operation.path || operation.targetRef}
                    label="新增镜头"
                    focusTarget
                  />
                ))}
        ...
        {unit.storyboardPrompt ? (
          <section className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-4">
            ...
            >
              <ReviewFieldText field={`unit:${unit.id}/storyboardPrompt`}>
                {unit.storyboardPrompt}
              </ReviewFieldText>
            </p>
          </section>
          ...
          <ReviewDiffPreview
            field={`unit:${unit.id}/storyboardPrompt`}
            label="分镜 Prompt"
            focusTarget
          />
        )}
        ...
        {unit.videoPrompt ? (
          <section className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-4">
            ...
            <p
              ...
              className="whitespace-pre-wrap text-xs leading-5 text-[var(--color-text-primary)]"
            >
              <ReviewFieldText field={`unit:${unit.id}/videoPrompt`}>
                {unit.videoPrompt}
              </ReviewFieldText>
            </p>
          </section>
          ...
          <ReviewDiffPreview
            field={`unit:${unit.id}/videoPrompt`}
            label="视频 Prompt"
            focusTarget
          />
        )}
      ...
      <div className="border-t border-[var(--color-border)] p-4">
        <Button
          ...
          block
          onClick={() =>
            navigate(`/project/${projectId}/plan/unit/${unit.id}/workbench`)
          }
