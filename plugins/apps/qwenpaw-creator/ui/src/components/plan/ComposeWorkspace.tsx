...
function resolvedName(view: ComposeView, ref: string): string {
  ...
  return (
    view.resolvedRefs.find(
      (item) => item.ref === ref || identifierOf(item.ref) === identifier,
    )?.name || ref
  );
}
...
function clipsOf(
  ...
  return selections.map((selection) => {
    const candidate = view.candidates.find(
      (item) => item.artifactVersionId === selection.artifactVersionId,
    );
...
export default function ComposeWorkspace({
  ...
  const sectionName =
    view.sectionTitle || resolvedName(view, `section:${sectionId}`);
  const selectedClip =
    clips.find((clip) => clip.unitId === selectedUnitId) || null;
  ...
  const decideRendered = async (decision: "ACCEPT" | "REJECT") => {
    ...
    message.success(
      decision === "ACCEPT" ? "已接受该拼接成片" : "已撤销该拼接成片",
    );
    await reload();
  };
  ...
  const applySelections = (next: ComposeSelectionView[]) =>
    submit(
        ...
        <div className="mb-2 flex items-center gap-2">
          <Button
            ...
            onClick={() =>
              onClose
                ? onClose()
                : navigate(`/project/${projectId}/plan?section=${sectionId}`)
            }
          >
            返回
          </Button>
      ...
      <div className="flex-1 overflow-y-auto p-6">
                ...
                <ClipCard
                  ...
                  onSelect={() =>
                    setSelectedUnitId((previous) =>
                      previous === clip.unitId ? null : clip.unitId,
                    )
                  }
                />
              ))}
        ...
        <div className="mb-6 flex items-center gap-4">
          <Button
            ...
            onClick={() =>
              void submit("STITCH_SECTION", targetRef, {}, view.targetVersion)
            }
          >
            {submitting ? "拼接中..." : "执行拼接"}
          </Button>
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 text-xs text-[var(--color-text-tertiary)]">
          {clip.duration ? `${clip.duration}s` : "时长未知"}
        </div>
      </div>
      <div
      ...
      >
        ...
        <Button
          ...
          size="small"
          onClick={() =>
            navigate(`/project/${projectId}/plan/unit/${clip.unitId}/workbench`)
          }
        >
          编辑
        </Button>
