...
export default function PlanCard({
  ...
  return (
    <article
      ...
      data-creator-module-id={
        isSingleUnit ? section.units[0]?.id ?? section.id : section.id
      }
      data-creator-module-ref={
        isSingleUnit
          ? `unit:${section.units[0]?.id ?? section.id}`
          : `section:${section.id}`
      }
    ...
    >
      <div className="flex items-center justify-between text-[11px] font-bold uppercase tracking-wide text-[var(--color-text-tertiary)]">
        <span>
          {String(section.number).padStart(2, "0")} {kickerType}
        </span>
              ...
              >
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`shrink-0 rounded border px-1.5 py-px text-[10px] ${PROGRESS_TONES[progress]}`}
                    >
