...
function useDebouncedField(
  ...
  const timers = useRef(
    new Map<EditableSectionField, ReturnType<typeof setTimeout>>(),
  );
  const pending = useRef(
    new Map<EditableSectionField, string | number | string[]>(),
  );
  ...
  useEffect(
    () => () => {
      ...
      for (const [field, value] of pending.current)
        void callbackRef.current(field, value);
...
export default function SectionDetail({
  ...
  const actualDuration = section.units.reduce(
    (sum, unit) => sum + unit.duration,
    0,
  );
  const overBudget = Boolean(
    section.durationBudget && actualDuration > section.durationBudget,
  );
  const pendingOperationIds = useMemo(
    () =>
      new Set(
        manifest?.decisionGroups
          .filter((group) => group.decision === "PENDING")
          .flatMap((group) => group.operationIds) ?? [],
      ),
    [manifest],
  );
  const removedUnitIds = useMemo(
    () =>
      new Set(
        manifest?.operations
          .filter(
  ...
  const addedUnitOperations = useMemo(() => {
    ...
    return [
      ...new Map(
        operations.map((operation) => [operation.targetRef, operation]),
      ).values(),
    ];
  }, [manifest, pendingOperationIds, section.id]);

  const updateConstraint = (index: number, value: string) => {
    const next = constraintDrafts.map((constraint, itemIndex) =>
      itemIndex === index ? value : constraint,
    );
    ...
    <div className="flex h-full flex-col">
      <div className="flex items-start justify-between gap-3 border-b border-[var(--color-border)] px-4 py-3">
        <div className="min-w-0">
          <h3 className="truncate text-sm font-semibold text-[var(--color-text-primary)]">
            ...
            >
              <ReviewFieldText field={`section:${section.id}/title`}>
                {section.title}
              </ReviewFieldText>
            </span>
          </h3>
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-4">
          ...
          >
            {constraintDrafts.length === 0 ? (
              <p className="text-xs text-[var(--color-text-tertiary)]">
                暂无约束。段落级约束会被其下所有{terms.unit}继承。
              </p>
            ) : (
              <div>
                <div
                ...
                >
                  {constraintDrafts.map((constraint, index) => (
                    <div
                      key={index}
                      className="flex items-center gap-1 px-2 py-1 transition-colors hover:bg-[var(--color-bg-secondary)]"
                    >
                      ...
                      <Button
                        ...
                        onClick={() => {
                          const next = constraintDrafts.filter(
          ...
          {section.units.length === 0 && addedUnitOperations.length === 0 ? (
            ...
            <div className="space-y-1.5">
              {section.units.map((unit) => {
                ...
                return (
                  <button
                    ...
                    {...(removed
                      ? {
                          "data-review-field": `unit:${unit.id}`,
                          "data-review-field-label": `删除${terms.unit}`,
                        }
                      : {})}
              ...
              {addedUnitOperations.map((operation) => (
                <div
                  key={operation.id}
                  data-review-field={operation.path || operation.targetRef}
                  data-review-field-label={`新增${terms.unit}`}
                  className="flex w-full items-center gap-2 rounded-lg border border-dashed border-[var(--color-border)] bg-[var(--color-bg-secondary)]/40 px-2.5 py-2 text-left"
                >
                  <span className="agent-diff-add min-w-0 flex-1 truncate text-xs font-medium">
                    <ReviewFieldText
        ...
        >
          {scriptExpanded ? (
            ...
            >
              <ReviewFieldText field={`section:${section.id}/script`}>
                {scriptDraft.trim() || "暂无剧本原文。"}
              </ReviewFieldText>
            </p>
          )}
          {scriptExpanded && (
            <ReviewDiffPreview field={`section:${section.id}/script`} />
          )}
        </Block>
      </div>
      ...
      <div className="border-t border-[var(--color-border)] p-4">
        <Button
          ...
          block
          onClick={() =>
            navigate(`/project/${projectId}/plan/section/${section.id}`)
          }
