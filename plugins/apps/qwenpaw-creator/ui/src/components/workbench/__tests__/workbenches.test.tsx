...
describe("origin workbench UI backed by canonical Creator contracts", () => {
  ...
  it("restores the origin R2V toolbar, panel names, and 320px two-column shell", () => {
    ...
    expect(
      screen.getByText("视频方案 / 02 追逐 / 01 Unit 1 / 制作工作台"),
    ).toBeInTheDocument();
    ["生成分镜 Prompt", "生成分镜图", "生成视频 Prompt", "生成视频"].forEach(
      (name) => {
    ...
    expect(
      screen.queryByRole("heading", { name: "分镜文本" }),
  ...
  it("persists both origin prompt editors through SET_UNIT_TEXT semantic commands", async () => {
    const { calls } = installMockFetch([
      {
        match: "/commands",
        response: {
          json: { commandId: "c-prompt", status: "APPLIED", eventSeq: 1 },
        },
      },
    ]);
    render(
    ...
    const storyboardPrompt = screen.getByPlaceholderText(
      "生成分镜 Prompt 后可在此编辑…",
    );
    ...
    const { calls } = installMockFetch([
      {
        match: "/commands",
        response: {
          json: { commandId: "c-shot", status: "APPLIED", eventSeq: 1 },
        },
      },
    ]);
    render(
    ...
    const { calls } = installMockFetch([
      {
        match: "/commands",
        response: {
          json: { commandId: "c-select", status: "APPLIED", eventSeq: 1 },
        },
      },
    ]);
    render(
  ...
  it("restores the origin AI Edit toolbar, VLM/timeline panels, and 340px right rail", () => {
    ...
    expect(
      screen.getByText("视频方案 / 03 高潮 / 01 Unit 1 / AI 剪辑工作台"),
    ).toBeInTheDocument();
    expect(
    ...
    expect(
      screen.getByRole("heading", { name: "剪辑目标" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: "VLM 关键帧分镜" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: "剪辑时间线（1）" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: "剪辑成片" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("heading", { name: "剪辑素材" }),
    ).toBeInTheDocument();
    expect(
      container.querySelector(
        '[data-creator-field="unit:u1/editPlan/storyboard/panel:panel-1/description"]',
      ),
    ).toHaveAttribute(
      "data-creator-path",
    ...
    expect(
      container.querySelector(
        '[data-creator-field="unit:u1/editPlan/timeline/clip:clip-01/reason"]',
      ),
    ).toHaveAttribute(
      "data-creator-path",
    ...
    expect(
      container.querySelector(
        '[data-creator-field="unit:u1/editPlan/timeline/clip:clip-01/overlay/text"]',
      ),
    ).toHaveAttribute(
      "data-creator-path",
  ...
  it("scrolls to and highlights the clip selected by a pending Review navigation", async () => {
    ...
    await waitFor(() =>
      expect(clipPanel).toHaveClass("border-[var(--color-accent)]"),
    );
    await waitFor(() =>
  ...
  it("submits exact second values for AI Edit clip range without ms conversion", async () => {
    const { calls } = installMockFetch([
      {
        match: "/commands",
        response: {
          json: { commandId: "c-range", status: "APPLIED", eventSeq: 1 },
        },
      },
    ]);
    const { container } = render(
    ...
    await waitFor(() =>
      expect(
        calls.some(
          (call) =>
            (call.body as { type?: string })?.type === "SET_EDIT_CLIP_RANGE",
        ),
      ).toBe(true),
    );
    ...
    expect(
      calls.filter(
        (call) => (call.body as { type?: string })?.type === "EXECUTE_EDIT",
      ),
    ).toHaveLength(0);
  });
    ...
    const { calls } = installMockFetch([
      {
        match: "/commands",
        response: {
          json: { commandId: "c-os", status: "APPLIED", eventSeq: 1 },
        },
      },
    ]);
    render(
  ...
  it("keeps the origin AI Edit material multi-select and binds an exact canonical source ref", async () => {
    ...
    const { calls } = installMockFetch([
      {
        match: "/commands",
        response: {
          json: { commandId: "c-bind", status: "APPLIED", eventSeq: 1 },
        },
      },
    ...
    await waitFor(() =>
      expect(
        calls.some(
          (call) => (call.body as { type?: string })?.type === "BIND_REFERENCE",
        ),
      ).toBe(true),
    );
  ...
  it("routes the origin AI Edit storyboard fallback through the controlled media endpoint", () => {
    ...
    expect(screen.getByAltText("AI 剪辑关键帧分镜")).toHaveAttribute(
      "src",
      "/api/qwenpaw-creator/generated/storyboard.png",
    );
  });
  ...
  it("renders a richer canonical migrated AI Edit envelope in the origin surface", () => {
    const migratedView: EditWorkbenchView = {
        ...
        timeline: [
          {
            ...
            asset_id: "source-01",
            asset_name: "十分钟足球素材",
            source_url: "/generated/source.mp4",
            asset_version_id: "asset-version-01",
            start: 0,
            end: 8,
            duration: 8,
            order: 1,
      ...
      uiLocator: {
        page: "workbench",
        unitId: "edit-e2e-long-highlight",
        route: "edit",
      },
    };
    ...
    const { container } = render(
      <AiEditWorkbench
        projectId="p1"
        envelope={envelope(migratedView)}
        view={migratedView}
        reload={vi.fn()}
      />,
    );
    ...
    expect(screen.getByAltText("#1 真实关键帧 关键帧")).toHaveAttribute(
      "src",
      "/api/qwenpaw-creator/media/assets/asset-version-01/frame?timestamp=4.000&width=640",
    );
    expect(
      container.querySelector("video[data-clip-start]"),
    ).not.toBeInTheDocument();
    fireEvent.click(
      screen.getByRole("button", { name: "播放分镜片段 #1 真实关键帧" }),
    );
    const clipPreview = container.querySelector<HTMLVideoElement>(
      'video[src="/api/qwenpaw-creator/media/assets/asset-version-01#t=0,8"]',
    );
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "显示关键帧 #1 真实关键帧" }),
    );
    expect(
      container.querySelector("video[data-clip-start]"),
    ).not.toBeInTheDocument();
  ...
  it("matches storyboard panels to timeline clips by the latest origin/main clip_id contract", () => {
    ...
    const laterCard =
      screen.getByText("#1 后段画面").parentElement!.parentElement!;
    const earlierCard =
      screen.getByText("#2 前段画面").parentElement!.parentElement!;
    const laterInputs = laterCard.querySelectorAll<HTMLInputElement>(
      'input[type="number"]',
    );
    const earlierInputs = earlierCard.querySelectorAll<HTMLInputElement>(
      'input[type="number"]',
    );
  ...
  it("renders the exact origin empty states for a canonical workbench before its plan exists", () => {
    ...
    expect(
      screen.getByText("生成剪辑方案后会显示素材取舍、时间段和转场建议。"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("暂无视频素材。请在新建项目处选择文件夹或上传视频。"),
    ).toBeInTheDocument();
