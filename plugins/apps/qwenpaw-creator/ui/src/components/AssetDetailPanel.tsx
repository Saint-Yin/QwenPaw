...
export default function AssetDetailPanel({
  ...
  const promptEntries = useMemo(() => {
    ...
    const count = Math.max(
      ...
      prompts.length,
      storedImageCount,
      assetType === "characters" ? 0 : 1,
    );

    return Array.from({ length: count || 1 }, (_, index) => {
      const image =
        asset.images[index]?.url || (index === 0 ? asset.primaryUrl || "" : "");
      ...
      return {
        ...
        image,
        referenceImageUrls: (asset.referenceImageRefs[index] || []).filter(
          (url) => url.trim(),
        ),
      };
    });
  ...
  const activePromptIndex =
    promptState?.assetId === asset.id
      ? promptState.index
      : Math.min(initialPromptIndex, Math.max(promptEntries.length - 1, 0));
  ...
  const parseReferenceDraft = (value: string) =>
    value
      .split("\n")
      .map((url) => url.trim())
      .filter(Boolean);
  ...
  const getReferenceLabel = (value: string) => {
    return (
      referenceImageOptions.find((option) => option.value === value)?.label ||
      value
    );
  };
  ...
  const selectedMissingReferences = selectedReferenceValues.filter((value) => {
    const option = referenceImageOptions.find(
  ...
  const savePrompt = () => {
    onPromptConfigChange?.(
      ...
      activePromptIndex,
      draftPrompt,
      parseReferenceDraft(referenceDraft),
    );
  };
  ...
  const renderPromptEditor = (listTitle: string) => {
    ...
    return (
      <div>
        <h4 className="mb-3 text-sm font-medium text-[var(--color-text-secondary)]">
          {listTitle}
        </h4>
            ...
            return (
              ...
              >
                <div className="group relative h-12 w-12 shrink-0 overflow-hidden rounded-lg bg-[var(--color-bg-card)]">
                  {entry.image ? (
                    <img
                      src={displayMediaUrl(entry.image)}
                      alt={entry.title}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 text-xs text-[var(--color-text-secondary)]">
                    {entry.prompt ? "Prompt 已就绪" : "Prompt 待补"} · 引用图{" "}
          ...
          <div className="mt-4 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-card)] p-3">
            ...
            <div className="mt-3">
              <div className="mb-2 flex items-center justify-between">
                ...
                <Tag
                  color={
                    parseReferenceDraft(referenceDraft).length > 0
                      ? "blue"
                      : "default"
                  }
                >
              ...
              {selectedMissingReferences.length > 0 && (
                <div className="mb-2 rounded-md border border-[var(--color-warning)]/30 bg-[var(--color-warning-soft)] px-2 py-2 text-xs leading-5 text-[var(--color-warning)]">
                  依赖图片尚未生成：
                  {selectedMissingReferences.map(getReferenceLabel).join("、")}
                </div>
              )}
                ...
                {referenceImageOptions.length > 0 ? (
                  <div className="grid max-h-36 grid-cols-3 gap-2 overflow-y-auto">
                    {referenceImageOptions.map((option) => {
                      const selected = parseReferenceDraft(
                        referenceDraft,
                      ...
                      return (
                        ...
                        >
                          {option.url ? (
                            <img
                              src={displayMediaUrl(option.url)}
                              alt={option.label}
            ...
            <div className="mt-3 flex gap-2">
              ...
              {onGeneratePrompt && (
                <Button
                  ...
                  disabled={
                    !draftPrompt.trim() || selectedMissingReferences.length > 0
                  }
    ...
    <div className="space-y-6">
      ...
      {mat.primaryUrl && (
        <div>
          ...
          <p className="break-all rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-secondary)] p-2 text-xs text-[var(--color-text-secondary)]">
            {mat.primaryUrl}
          </p>
        </div>
      )}
    ...
    >
      ...
      {asset && projectId && (
        <NewAppearanceModal
          ...
          onGeneratePrompt={(refDescription) =>
            onGenerateAppearancePrompt
          ...
          onGenerateImage={(prompt) =>
            onGenerateAppearanceImage
              ...
              : Promise.resolve(
                  assetType === "characters" ? asset.images[0]?.url || "" : "",
                )
          }
        />
      )}
      ...
      <input
        ...
        onChange={(e) => {
          ...
          if (file && asset && onUploadReference) {
            // 使用异步处理等待上传完成
            Promise.resolve(onUploadReference(asset, file)).finally(() => {
              e.target.value = "";
              setUploadingReference(false);
            });
          }
