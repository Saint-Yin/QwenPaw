...
function navigateToResolved(projectId: string, resolved: ResolvedRef) {
  ...
  if (locator.page === "workbench" && locator.unitId)
    navigate(`/project/${projectId}/plan/unit/${locator.unitId}/workbench`);
  else if (locator.page === "section-compose" && locator.sectionId)
    navigate(`/project/${projectId}/plan/section/${locator.sectionId}`);
  else if (locator.page === "final-compose")
    navigate(`/project/${projectId}/plan?finalCompose=1`);
  else if (locator.page === "assets" && locator.assetId)
    navigate(
      `/project/${projectId}/assets?select=${locator.assetId}${
        locator.versionId ? `&version=${locator.versionId}` : ""
      }`,
    );
  else if (locator.sectionId)
    navigate(`/project/${projectId}/plan?section=${locator.sectionId}`);
  else if (locator.unitId)
    navigate(`/project/${projectId}/plan?unit=${locator.unitId}`);
}
...
function RefChip({
  ...
  return (
    <button
      ...
      className={`inline-flex max-w-full items-center gap-1 truncate rounded-md px-1.5 py-0.5 text-[11px] font-medium transition-colors ${
        resolved
...
function RelationBlock({
  ...
  return (
    <div className="rounded-lg bg-[var(--color-bg-secondary)]/60 px-3 py-2.5">
      <b className="block text-[11px] font-bold text-[var(--color-text-primary)]">
        {title}
      </b>
...
function relationRefs(view: AssetLibraryView, asset: AssetDisplayItem) {
  ...
  for (const relation of view.relations) {
    const from = String(
      relation.from || relation.sourceRef || relation.source || "",
    );
    const to = String(
      relation.to || relation.targetRef || relation.target || "",
    );
    const kind = String(relation.kind || relation.type || "references");
    if (from && to && sameRef(from, asset.sourceRef))
      outgoing.push({ ref: to, kind });
    if (from && to && sameRef(to, asset.sourceRef))
      incoming.push({ ref: from, kind });
  }
  return { outgoing, incoming };
}

function categoryLabel(category: string, scenario?: string): string {
  if (category === "subject_ref")
...
export default function AssetInspector({
  ...
  const referencedBy = incoming.filter(
    (item) => !/constraint|constrain|derive/i.test(item.kind),
  );
  const derivatives = incoming.filter((item) => /derive/i.test(item.kind));
  const constraintRefs = outgoing.filter((item) =>
    /constraint|constrain/i.test(item.kind),
  );
  const constrainsRefs = incoming.filter((item) =>
    /constraint|constrain/i.test(item.kind),
  );
  const assetNamePath =
    asset.kind === "visual"
      ...
      <div className="flex items-start justify-between gap-3 border-b border-[var(--color-border)] px-4 py-3">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-xs text-[var(--color-text-secondary)]">
            {categoryLabel(asset.category, header?.scenario)} ·{" "}
            {assetTypeTag(asset)}
          </p>
        </div>
        <div className="flex shrink-0 items-center gap-1.5">
          ...
          >
            {asset.planned ? "待生成" : STATUS_LABELS[asset.status]}
          </span>
          <button
            type="button"
            onClick={() =>
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-4">
        ...
        {asset.mediaType === "url" && asset.sourceUrl && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex items-center gap-1.5 truncate rounded-lg border border-[var(--color-border)] px-3 py-2 text-xs text-[var(--color-accent)] hover:bg-[var(--color-accent-soft)]"
          >
        ...
        {(asset.userNotes ||
          (asset.content && asset.generatedKind !== "storyboard_image")) && (
          <p
        ...
        {asset.detail?.prompts?.some((prompt) => prompt.trim()) && (
          <RelationBlock title="文生图 Prompt">
            <div className="max-h-40 space-y-2 overflow-y-auto whitespace-pre-wrap">
              {asset.detail.prompts.map((prompt, index) =>
                prompt.trim() ? (
        ...
        <RelationBlock
          title={`被引用（${referencedBy.length + derivatives.length}）`}
        >
          {referencedBy.length === 0 && derivatives.length === 0 ? (
            "暂未被任何对象引用。"
          ) : (
            <span className="flex flex-wrap items-center gap-1">
        ...
        <RelationBlock title="作用约束">
          {constraintRefs.length === 0 && constrainsRefs.length === 0 ? (
            "暂无关联约束。"
          ) : (
            <span className="flex flex-wrap items-center gap-1">
              {constraintRefs.length > 0 && "受"}
              {constraintRefs.map((item) => (
                <RefChip
                  key={`cb-${item.ref}`}
                  projectId={projectId}
                  view={view}
      ...
      <div className="flex items-center gap-2 border-t border-[var(--color-border)] p-3">
        {asset.generatedKind === "storyboard_image" && (
          <Button
            size="small"
            onClick={() => {
              const unitId = asset.ownerRef?.startsWith("unit:")
                ? asset.ownerRef.slice(5)
                : "";
              if (unitId)
                navigate(`/project/${projectId}/plan/unit/${unitId}/workbench`);
            }}
