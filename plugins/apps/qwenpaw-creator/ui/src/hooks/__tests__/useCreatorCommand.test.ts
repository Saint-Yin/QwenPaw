...
describe("semantic command builder", () => {
  it("keeps exact text payload and target CAS", () => {
    const command = buildCreatorCommand(
      ...
      "SET_UNIT_TEXT",
      "unit:u1",
      { field: "storyText", value: "新文本" },
      unit.targetVersion,
    );
    expect(command.arguments).toEqual({ field: "storyText", value: "新文本" });
    expect(command.expectedTargetVersions).toEqual([
      { ref: "unit:u1", objectVersion: "ov-u1" },
    ]);
  ...
  it("carries presentation/decision/overlay CAS during Pending", () => {
    const pending = envelope(
      {
        presentationVersion: "pv-2",
        reviewRevisionId: "rr-1",
        approvedRevisionId: "r1",
        overlayId: "o1",
        overlayHead: "oh-3",
        view: {},
        origins: { "unit:u1": "review_candidate" as const },
        targetVersions: {
          "unit:u1": {
            targetVersion: "ov-review",
            decisionGroupId: "g1",
            decisionToken: "dt-1",
          },
        },
    ...
    const command = buildCreatorCommand(pending, "SET_UNIT_TEXT", "unit:u1", {
      field: "storyText",
      value: "覆盖",
    });
    expect(command).toMatchObject({
      expectedPresentationVersion: "pv-2",
      expectedDecisionToken: "dt-1",
      expectedOverlayHead: "oh-3",
      expectedTargetVersions: [{ ref: "unit:u1", objectVersion: "ov-review" }],
    });
  });
  ...
  it("preserves seconds and snake_case AI Edit fields", () => {
    const command = buildCreatorCommand(
      ...
      "SET_EDIT_CLIP_OS",
      "unit:u1",
      { clipId: "clip-01", text: "出发", vibe: "action" },
      "ov-u1",
    );
    const range = buildCreatorCommand(
      envelope({}),
      "SET_EDIT_CLIP_RANGE",
      "unit:u1",
      { clipId: "clip-01", start: 1.25, end: 8.75 },
      "ov-u1",
    );
    ...
    expect(range.arguments).toEqual({
      clipId: "clip-01",
      start: 1.25,
      end: 8.75,
    });
  ...
  it.each([
    ...
    [
      "CREATE_UNIT",
      "section:s1",
      { title: "Unit 2", taskType: "edit", afterUnitId: "u1" },
    ],
    ...
    [
      ...
      {
        field: "characters",
        referenceSet: "storyboard_and_video",
    ...
    [
      ...
      "post:final",
      {
        fromSourceRef: "project://section/s1",
        toSourceRef: "project://section/s2",
        type: "fade",
        durationSeconds: 0.5,
      },
    ],
  ] as const)(
    "keeps the canonical %s argument envelope unchanged",
    (type, targetRef, args) => {
      const command = buildCreatorCommand(
        envelope({}),
        type,
