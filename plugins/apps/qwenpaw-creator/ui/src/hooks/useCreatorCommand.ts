...
function reviewPresentation<T>(
  envelope: ViewEnvelope<T>,
): ReviewPresentationView<T> | null {
...
export function buildCreatorCommand<T>(
  ...
  return {
    ...
    expectedTargetVersions:
      objectVersion || targetMetadata?.targetVersion
        ? [
            {
              ref: targetRef,
              objectVersion: objectVersion || targetMetadata!.targetVersion,
            },
          ]
        : [],
    ...
    expectedOverlayHead:
      presentation?.overlayHead || envelope.manualEditOverlay?.head,
  };
}

export function commandStatusCopy(
  result: CreatorCommandAccepted,
  uiPhase: string,
): string {
  if (result.status === "DEFERRED_UNTIL_REVIEW_RESOLVED")
    return "已记录，将在本轮审阅结束后执行";
  if (result.status === "QUEUED")
    return "已排队，Creator 会在合法消息边界继续处理";
...
export function useCreatorCommand<T>(
  ...
  const [lastResult, setLastResult] = useState<CreatorCommandAccepted | null>(
    null,
  );
  const retryIds = useRef(
    new Map<string, { clientCommandId: string; editSessionId?: string }>(),
  );
  const submit = useCallback(
    async (
      ...
      const stableIds = retryIds.current.get(signature) ?? {
        clientCommandId: newClientId("command"),
        editSessionId: isCoalescedAutosave(type)
          ? newClientId("edit")
          : undefined,
      };
      ...
      try {
        const result = await submitCreatorCommand(projectId, {
          ...command,
          ...stableIds,
        });
