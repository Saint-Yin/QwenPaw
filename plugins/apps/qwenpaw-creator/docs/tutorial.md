...
> 验证说明：本次已重新启动前端以清除旧 chunk 缓存，后端 `/api/qwenpaw-creator/health` 返回 ready；截图来自 `http://127.0.0.1:18110/creator` 当前真实页面。所有红框按页面实际控件边界复核后生成，没有触发真实视频生成任务。
...
| `http://127.0.0.1:18110/api/qwenpaw-creator/health` | 检查 Key、视频后端、FFmpeg |
...
QwenPaw 启动后会加载 `qwenpaw-creator/backend/main.py`，注册 `/api/qwenpaw-creator/*` 后端路由；前端由 `qwenpaw-creator/ui/dist/index.js` 注册 `/apps/qwenpaw-creator` 路由，在 App Center 中以独立 Creator 界面打开 `qwenpaw-creator/ui/dist/app/index.html`。

这一步不调用 AI。`/api/qwenpaw-creator/health` 会检查文本、图片、视频 Key，OSS/Reference 媒体配置，以及 FFmpeg 是否可用。
</details>
