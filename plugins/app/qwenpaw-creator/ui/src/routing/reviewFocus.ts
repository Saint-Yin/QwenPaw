...
type ReviewFocusRuntime = Window & {
  __creatorReviewFocus?: (request: { path: string; query: Record<string, string> }) => void;
};
...
export function findCreatorFieldElement(field: string, root: ParentNode = document): HTMLElement | null {
  return Array.from(root.querySelectorAll<HTMLElement>('[data-review-field], [data-creator-field]'))
    .find((element) => (
...
export function reviewFlashElementForField(field: string, root: ParentNode = document): HTMLElement | null {
  const fieldElement = findCreatorFieldElement(field, root);
  if (!fieldElement) return null;
  if (fieldElement.matches('textarea, input')) {
    return fieldElement.closest<HTMLElement>('section') ?? fieldElement.parentElement ?? fieldElement;
  }
  return fieldElement;
}

/** Immediate same-page fallback used after AgentDock closes on “查看”. */
export function flashCreatorReviewField(field: string, root: ParentNode = document): HTMLElement | null {
 ...
 */
export function useReviewFieldFocus({ path, field, enabled, pulse }: UseReviewFieldFocusOptions): void {
  ...
  const trigger = useCallback((targetField: string) => {
    clearTimers();
    ...
    const verifyMountedTarget = () => {
      ...
      const workspaceRoot = document.querySelector<HTMLElement>('[data-creator-workspace-root]');
      const mountedTarget = reviewFlashElementForField(targetField, workspaceRoot ?? document);
      if (mountedTarget && (mountedTarget !== activeTargetRef.current || !mountedTarget.classList.contains('review-flash'))) {
        applyFlash(mountedTarget);
      }
      if (settleChecks++ < 12) {
        settleTimerRef.current = window.setTimeout(verifyMountedTarget, RETRY_INTERVAL_MS);
      } else {
        settleTimerRef.current = null;
      }
    };
    ...
    const tryFocus = () => {
      if (sequence !== requestSequenceRef.current) return;
      const workspaceRoot = document.querySelector<HTMLElement>('[data-creator-workspace-root]');
      const target = reviewFlashElementForField(targetField, workspaceRoot ?? document);
      if (!target) {
        if (retries++ < MAX_RETRIES) {
          retryTimerRef.current = window.setTimeout(tryFocus, RETRY_INTERVAL_MS);
        }
        return;
      }
  ...
  useEffect(() => {
    ...
    if (reviewFocusRequest.query.review !== '1' || !reviewFocusRequest.query.field) return;
  ...
  useEffect(() => {
    ...
    const handler = (request: { path: string; query: Record<string, string> }) => {
      if (!enabled || request.path !== path || request.query.review !== '1' || !request.query.field) return;
      trigger(request.query.field);
    };
    runtime.__creatorReviewFocus = handler;
    return () => {
      if (runtime.__creatorReviewFocus === handler) delete runtime.__creatorReviewFocus;
    };
