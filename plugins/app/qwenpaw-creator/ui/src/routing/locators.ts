...
export function pathForLocator(projectId: string, locator: Record<string, string>): string {
  switch (locator.page) {
    case 'assets':
      return `/project/${projectId}/assets`;
    case 'workbench':
      return `/project/${projectId}/plan/unit/${encodeURIComponent(locator.unitId)}/workbench`;
    case 'section-compose':
      return `/project/${projectId}/plan/section/${encodeURIComponent(locator.sectionId)}`;
    case 'final-compose':
...
export function navigateToLocator(
  ...
  if (locator.sectionId && locator.page === 'plan') params.set('section', locator.sectionId);
  if (locator.unitId && locator.page === 'plan') params.set('unit', locator.unitId);
  ...
  const target = params.size ? `${base}${base.includes('?') ? '&' : '?'}${params}` : base;
  ...
  if (options.review && options.field) {
    ...
    [0, 180, 420, 800].forEach((delay) => window.setTimeout(() => {
...
export function navigateToRefItem(projectId: string, item: RefSearchItem): void {
  navigateToLocator(projectId, item.uiLocator, { description: `引用：${item.name}` });
}
