...
import { flashCreatorReviewField, useReviewFieldFocus } from '@/routing/reviewFocus';
import { useNavigationStore } from '@/store/navigationStore';

const PATH = '/project/p1/plan';
const FIELD = 'unit:unit-1/storyText';

function Harness({ field = FIELD, showTarget = true }: { field?: string; showTarget?: boolean }) {
...
function DockCollisionHarness() {
  useReviewFieldFocus({ path: PATH, field: FIELD, enabled: true, pulse: '1' });
  return <>
    <aside data-agent-dock><div data-creator-field={FIELD}>AgentDock diff</div></aside>
    <main data-creator-workspace-root><div data-creator-field={FIELD}>工作区正文</div></main>
  </>;
}
...
describe('useReviewFieldFocus', () => {
  ...
  it('URL 审阅定位会滚动并闪烁字段', () => {
    ...
    const target = container.querySelector<HTMLElement>('[data-creator-field]')!;
  ...
  it('同名字段同时存在时只定位工作区，不会闪烁 AgentDock', () => {
    ...
    const dockTarget = container.querySelector<HTMLElement>('[data-agent-dock] [data-creator-field]')!;
    const workspaceTarget = container.querySelector<HTMLElement>('[data-creator-workspace-root] [data-creator-field]')!;
  ...
  it('详情节点延迟挂载时会重试，不依赖固定 360ms', () => {
    ...
    expect(container.querySelector('[data-creator-field]')).toHaveClass('review-flash');
    expect(Element.prototype.scrollIntoView).toHaveBeenCalledTimes(1);
  });

  it('切换查看字段会清理上一个目标的闪烁类', () => {
    const { container } = render(<MultiFieldHarness />);
    const first = container.querySelector<HTMLElement>(`[data-creator-field="${FIELD}"]`)!;
    const second = container.querySelector<HTMLElement>('[data-creator-field="unit:unit-1/videoPrompt"]')!;
    ...
    act(() => {
      useNavigationStore.getState().setReviewFocus({
        path: PATH,
        ref: 'unit:unit-1',
        query: { review: '1', field: 'unit:unit-1/videoPrompt', reviewPulse: '2' },
      });
    });
