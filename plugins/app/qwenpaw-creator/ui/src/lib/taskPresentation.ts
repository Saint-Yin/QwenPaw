...
export function taskProgressPercent(progress: number | null | undefined): number | null {
...
export function taskErrorMessage(error: unknown, fallback: string): string {
  ...
  const direct = nonEmptyString(record.message) || nonEmptyString(record.detail);
  ...
  if (Array.isArray(record.items)) {
    const itemErrors = record.items.flatMap((item) => {
      ...
      const detail = nonEmptyString(itemRecord.error)
