...
describe('Creator conversation presentation', () => {
  it('keeps actual user authority sources and rejects Runtime control rows as user bubbles', () => {
    for (const source of ['initial_goal', 'agent_dock', 'frontend_action', 'frontend_manual_edit', 'user', 'user_continuation']) {
    ...
    for (const source of ['runtime_action_result', 'runtime_work_update', 'specialist_result', 'completion_context', 'completion_rejected']) {
      const message = creatorMessage({ source });
  ...
  it('never renders reserved Creator/Runtime control markers as user input', () => {
    const rejected = creatorMessage({
      ...
      content: [{ type: 'text', text: '[CREATOR_ACTION_REJECTED]\n\n必须先持久化 plan' }],
    });
    const malformedLegacyRuntimeRow = creatorMessage({
      source: 'user',
      content: [{ type: 'text', text: '[RUNTIME_EVENT: CREATOR_WAITING]\n\ninternal' }],
    });
    ...
    expect(shouldRenderConversationMessage(malformedLegacyRuntimeRow)).toBe(false);
  ...
  it('keeps legacy file Runtime tool rows out of the conversation', () => {
    expect(shouldRenderConversationMessage(creatorMessage({
      role: 'tool',
  ...
  it('hides only the exact legacy file Runtime tool placeholder and preserves narration', () => {
    const placeholder = creatorMessage({
      role: 'assistant',
      source: 'file_agent_runtime',
      content: [{ type: 'text', text: '准备调用工具：read_project、write_project' }],
      metadata: {
        toolCalls: [
    ...
    expect(actionAwareConversationContent(narration)).toEqual(narration.content);
  });
  ...
  it('preserves the complete raw assistant action envelope', () => {
    const message = creatorMessage({
      ...
      metadata: { parsedAction: { action: 'tool_call', tool: 'read_file', arguments: {} } },
    });
  ...
  it('preserves protocol-rejected assistant output without content rewriting', () => {
    const message = creatorMessage({
      ...
      metadata: { protocolError: { code: 'INVALID_ACTION', message: '必须先 plan' } },
    });
    expect(conversationContent(message)).toEqual(message.content);
  });
  ...
  it('keeps every in-flight assistant stream byte unchanged', () => {
    ...
    expect(conversationContent(blockedAfterNarration)).toEqual(blockedAfterNarration.content);
    ...
    expect(conversationContent(echoedGovernance)).toEqual(echoedGovernance.content);
  });
  ...
  it('recognizes a streamed Sub-agent function call and parses its parameters when complete', () => {
    ...
    expect(partial).toMatchObject({ action: 'tool_call', tool: 'read_project_file', complete: false });
  ...
  it('renders a final action message as conversation prose while retaining its raw envelope', () => {
    const message = creatorMessage({
      role: 'assistant',
      content: [{ type: 'text', text: '处理结束。\n```json\n{"action":"final","message":"全部完成。","awaitUserInput":false}\n```' }],
      metadata: { parsedAction: { action: 'final', message: '全部完成。', awaitUserInput: false } },
    });
    const envelope = creatorActionEnvelope(message);
    expect(actionAwareConversationContent(message, envelope)).toEqual([
      { type: 'text', text: '处理结束。\n\n全部完成。' },
  ...
  it('merges assistant arguments, Runtime result metadata/text and tool events by actionId', () => {
    ...
    const events = [
      creatorEvent({ data: { actionId: 'action-1', tool: 'read_project_file' } }),
      creatorEvent({ eventId: 'event-2', seq: 2, type: 'agent.tool_completed', data: { actionId: 'action-1' } }),
    ];
  ...
  it('uses failed Runtime result text as the origin tool card error', () => {
    const messages = [
      creatorMessage({
        ...
        metadata: {
          actionId: 'action-1',
          parsedAction: { action: 'tool_call', tool: 'write_file', arguments: { path: 'x' } },
        },
      }),
      creatorMessage({
        messageId: 'result-1',
  ...
  it('marks a standard failed tool completion as failed before the durable result arrives', () => {
    expect(toolCallPresentations([], [
      ...
      creatorEvent({
        ...
        data: {
          actionId: 'delegate-1', tool: 'delegate_to_agent', failed: true,
  ...
  it('rebuilds legacy file Runtime cards from plural tool calls and tool result rows', () => {
    const messages = [
      creatorMessage({
        ...
        metadata: {
          toolCalls: [
            { id: 'call-read', name: 'read_project', arguments: { include: ['story'] } },
            { id: 'call-write', name: 'write_project' },
          ],
        },
      }),
    ...
    const presentations = toolCallPresentations(messages, [
      creatorEvent({
        eventId: 'read-completed', seq: 1, type: 'agent.tool.completed',
        data: {
          toolCallId: 'call-read', toolName: 'read_project', messageId: 'tool-read',
        },
      }),
      creatorEvent({
        eventId: 'write-completed', seq: 2, type: 'agent.tool.completed',
        data: {
          toolCallId: 'call-write', toolName: 'write_project', messageId: 'tool-write',
        },
      }),
    ]);
    ...
    expect(presentations).toMatchObject([
      {
        actionId: 'call-read', anchorMessageId: 'assistant-file', status: 'succeeded',
        tool: 'read_project', arguments: { include: ['story'] },
        result: { ok: true, generation: 2 },
      },
      {
        actionId: 'call-write', anchorMessageId: 'assistant-file', status: 'succeeded',
        tool: 'write_project', result: '已写入 Project（generation 3）',
      },
    ]);
  });
  ...
  it('projects native AgentScope control tool metadata without parsing assistant text', () => {
    ...
    expect(envelope).toMatchObject({
      action: 'final',
      syntax: 'native',
      payload: { action: 'final', message: '原生工具回复', awaitUserInput: false },
    });
    expect(actionAwareConversationContent(message, envelope)).toEqual([
  ...
  it('assembles main-agent native tool deltas and removes rejected calls', () => {
    const deltas = [
      creatorEvent({
        eventId: 'delta-1', seq: 1, type: 'agent.tool_delta',
        data: {
          messageId: 'assistant-native', toolCallId: 'call-read', tool: 'read_file',
          deltaIndex: 0, argumentsDelta: '{"file_path":"story/',
        },
      }),
      creatorEvent({
        eventId: 'delta-2', seq: 2, type: 'agent.tool_delta',
        data: {
          messageId: 'assistant-native', toolCallId: 'call-read', tool: 'read_file',
          deltaIndex: 1, argumentsDelta: 'outline.md"}',
        },
      }),
    ];
    ...
    expect(toolCallPresentations([], [
      ...deltas,
      creatorEvent({
        eventId: 'rejected', seq: 3, type: 'assistant.output_rejected',
