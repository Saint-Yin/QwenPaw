...
export function reviewGroupForArtifactVersion(
  ...
  const comparison = manifest.mediaComparisons.find((item) => (
  ...
  return manifest.decisionGroups.find((group) => (
...
export function reviewGroupForArtifact(
  ...
  return operation
    ? manifest.decisionGroups.find((group) => group.id === operation.decisionGroupId) ?? null
    : null;
}
...
export function artifactPresentationStatus(
  ...
  if (!group) return artifact.freshnessStatus === 'stale' ? 'stale' : 'accepted';
  if (group.decision === 'PENDING' || group.decision === 'REVISION_REQUESTED') return 'pending';
  if (group?.decision === 'REJECTED' || group?.decision === 'SUPERSEDED_BY_USER_EDIT') return 'stale';
  return 'accepted';
}
