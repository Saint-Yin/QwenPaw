import type { CreatorContentPart, CreatorEvent, CreatorMessage } from '@/contracts/creator';
...
export function isRuntimeControlSource(source: string | undefined): boolean {
  ...
  return source.startsWith('runtime_')
...
export function isTechnicalControlText(raw: string): boolean {
  ...
  return Boolean(value) && (BARE_CONTROL_CODE.test(value) || RESERVED_RUNTIME_MARKER.test(value));
}

function containsReservedRuntimeMarker(message: CreatorMessage): boolean {
  return message.content.some((part) => (
    part.type === 'text'
    && (RESERVED_RUNTIME_MARKER.test(part.text) || BARE_CONTROL_CODE_LINE.test(part.text))
  ));
}
...
export function shouldRenderConversationMessage(message: CreatorMessage): boolean {
  ...
  if (message.role === 'tool' && message.source === 'file_agent_runtime') return false;
...
function legacyFileRuntimeToolCalls(message: CreatorMessage): Record<string, unknown>[] {
  if (message.role !== 'assistant' || message.source !== 'file_agent_runtime') return [];
...
function toolCallName(toolCall: Record<string, unknown>): string | undefined {
  ...
  if (typeof toolCall.toolName === 'string' && toolCall.toolName) return toolCall.toolName;
  const function_ = isRecord(toolCall.function) ? toolCall.function : undefined;
  return typeof function_?.name === 'string' && function_.name ? function_.name : undefined;
}

function withoutLegacyFileRuntimePlaceholder(message: CreatorMessage): CreatorContentPart[] {
  ...
  return message.content.filter((part) => part.type !== 'text' || part.text !== placeholder);
}

export function conversationContent(message: CreatorMessage): CreatorContentPart[] {
...
function parsedActionMetadata(message: CreatorMessage): Record<string, unknown> | undefined {
  return isRecord(message.metadata?.parsedAction) ? message.metadata.parsedAction : undefined;
}

function nativeToolCallMetadata(message: CreatorMessage): Record<string, unknown> | undefined {
  return isRecord(message.metadata?.toolCall) ? message.metadata.toolCall : undefined;
}
...
function nativeActionEnvelope(
  ...
  if (!name) return null;
  const arguments_ = isRecord(toolCall.arguments) ? toolCall.arguments : undefined;
  const control = ['plan', 'final', 'yield_until_runtime_event', 'complete_current_change'].includes(name);
  const action = control ? name : 'tool_call';
  const payload = arguments_
    ? control
      ? name === 'yield_until_runtime_event' || name === 'complete_current_change'
        ? { action, arguments: arguments_ }
        : { action, ...arguments_ }
      : { action, tool: name, arguments: arguments_ }
    : undefined;
  const rawPayload = typeof toolCall.argumentsDelta === 'string'
...
function partialJsonString(raw: string, key: string): string | undefined {
  ...
  const match = new RegExp(`"${escapedKey}"\\s*:\\s*"((?:\\\\.|[^"\\\\])*)"`, 'u').exec(raw);
...
function parsedFunctionArguments(raw: string): Record<string, unknown> {
  ...
  const parameterPattern = /<parameter=([A-Za-z_][A-Za-z0-9_.-]*)>\s*([\s\S]*?)\s*<\/parameter>/gu;
  for (const match of raw.matchAll(parameterPattern)) {
    ...
    if (match[1] === 'arguments' && isRecord(value)) Object.assign(arguments_, value);
...
function actionEnvelopeFromText(
  ...
  if (fence?.index !== undefined) {
    ...
    const rawPayload = (closingIndex >= 0 ? suffix.slice(0, closingIndex) : suffix).trim();
    ...
    return {
      ...
      action,
      tool,
      complete: Boolean(payload) && (Boolean(metadataPayload) || closingIndex >= 0),
      syntax: 'json',
    };
  }
  ...
  if (functionMatch?.index !== undefined) {
    ...
    return {
      ...
      rawPayload,
      payload: complete
        ? { action: 'tool_call', tool, arguments: parsedFunctionArguments(rawPayload) }
        : undefined,
      action: 'tool_call',
      tool,
      complete,
  ...
  if (metadataPayload && typeof metadataPayload.action === 'string') {
    return {
      ...
      tool: typeof metadataPayload.tool === 'string' ? metadataPayload.tool : undefined,
      complete: true,
      syntax: 'json',
    };
  }
...
export function actionEnvelopeFromStreamText(raw: string): CreatorActionEnvelope | null {
  return actionEnvelopeFromText(raw, 0);
}

/** Detect the final machine action while its SSE text is still incomplete. */
export function creatorActionEnvelope(message: CreatorMessage): CreatorActionEnvelope | null {
  const native = nativeToolCallMetadata(message);
  if (native) {
    const narration = message.content
      .filter((part): part is Extract<CreatorContentPart, { type: 'text' }> => part.type === 'text')
      .map((part) => part.text)
      .join('\n')
      .trim();
...
export function actionAwareConversationContent(
  ...
  if (envelope.syntax === 'native') {
    const finalMessage = envelope.action === 'final' && typeof envelope.payload?.message === 'string'
  ...
  return visibleContent.flatMap((part, index) => {
    ...
    const text = [envelope.narration, finalMessage]
      .filter((value, valueIndex, values) => Boolean(value) && values.indexOf(value) === valueIndex)
      .join('\n\n');
...
function runtimeResultText(message: CreatorMessage): string {
  return textContent(message).replace(/^\[[A-Z0-9_]+\]\s*/u, '').trim();
}
...
export function toolCallPresentations(
  ...
  [...messages]
    ...
    .forEach((message) => {
      legacyFileRuntimeToolCalls(message).forEach((toolCall, index) => {
        const actionId = typeof toolCall.id === 'string'
          ? toolCall.id
      ...
      const actionId = typeof message.metadata?.actionId === 'string'
      ...
      if (message.role === 'assistant' && nativeToolCall) {
        call.anchorMessageId = message.messageId;
        if (typeof nativeToolCall.name === 'string') call.tool = nativeToolCall.name;
        if (isRecord(nativeToolCall.arguments)) call.arguments = nativeToolCall.arguments;
      } else if (message.role === 'assistant' && parsedAction) {
        call.anchorMessageId = message.messageId;
        if (typeof parsedAction.tool === 'string') call.tool = parsedAction.tool;
        if (isRecord(parsedAction.arguments)) call.arguments = parsedAction.arguments;
      }
      if (message.role === 'tool' || message.source === 'runtime_action_result') {
        if (typeof message.metadata?.tool === 'string') call.tool = message.metadata.tool;
        else if (typeof message.metadata?.toolName === 'string') call.tool = message.metadata.toolName;
        call.completed = true;
        call.failed = message.metadata?.failed === true || message.metadata?.cancelledByHardStop === true;
  ...
  [...events]
    ...
    .forEach((event) => {
      if (event.type === 'assistant.output_rejected' || event.type === 'session.error') {
        const rejectedMessageId = typeof event.data.rejectedAssistantMessageId === 'string'
        ...
        if (rejectedMessageId) {
          for (const [actionId, call] of calls) {
            if (call.anchorMessageId === rejectedMessageId) calls.delete(actionId);
          }
        }
        return;
      }
      ...
      const actionId = typeof event.data.toolCallId === 'string'
        ? event.data.toolCallId
        : typeof event.data.actionId === 'string'
          ? event.data.actionId
          : undefined;
      if (!actionId) return;
      const call = ensure(actionId, Number.MAX_SAFE_INTEGER - 1_000_000 + event.seq);
      if (typeof event.data.tool === 'string') call.tool = event.data.tool;
      else if (typeof event.data.toolName === 'string') call.tool = event.data.toolName;
      const dottedCompletion = event.type === 'agent.tool.completed' || event.type === 'agent.tool.failed';
      if (typeof event.data.messageId === 'string' && (!dottedCompletion || !call.anchorMessageId)) {
        call.anchorMessageId = event.data.messageId;
      }
      if (event.type === 'agent.tool_delta') {
        const deltaIndex = Number(event.data.deltaIndex);
        if (Number.isInteger(deltaIndex) && deltaIndex >= 0 && typeof event.data.argumentsDelta === 'string') {
          call.argumentDeltas = {
            ...(call.argumentDeltas ?? {}),
      ...
      if (
        ...
        if (
          ...
          call.failed = true;
          call.error = typeof event.data.error === 'string'
            ? event.data.error
  ...
  return [...calls.values()]
    .filter((call): call is MutableToolCall & { tool: string } => (
      Boolean(call.tool)
      && !['plan', 'final', 'yield_until_runtime_event', 'complete_current_change'].includes(call.tool ?? '')
    ))
