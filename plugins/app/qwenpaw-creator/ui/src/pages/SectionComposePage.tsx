...
export default function SectionComposePage() {
  ...
  const envelope = useWorkspaceViewStore((state) => state.sectionCompose[sectionId]);
  const load = useWorkspaceViewStore((state) => state.loadSectionCompose);
  const view = presentationOf(envelope || null);
  const error = useWorkspaceViewStore((state) => state.errors[`section-compose:${sectionId}`]);
  ...
  useEffect(() => { void load(id, sectionId).catch(() => undefined); }, [id, load, sectionId, lastViewSeq]);
  ...
  if (error && !view) return <PageLoadError message={error} retry={() => void load(id, sectionId)} />;
  if (!envelope || !view) return <PageSkeleton type="list" />;
  return <ComposeWorkspace projectId={id} envelope={envelope} view={view} reload={() => load(id, sectionId)} onClose={() => router.push(`/project/${id}/plan?section=${sectionId}`)} focusVersion={query.get('version')} focusPulse={reviewPulse} />;
}
