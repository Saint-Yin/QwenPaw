...
import type { EditWorkbenchView, R2VWorkbenchView, RefSearchItem, ViewEnvelope } from '@/contracts/creator';
...
function isRefSearchItem(value: RefSearchItem | null | undefined): value is RefSearchItem {
  return value != null;
}
...
export default function UnitWorkbenchPage() {
  ...
  const error = useWorkspaceViewStore((state) => state.errors[`workbench:${unitId}`]);
  const focusVersion = query.get('version');
  const reviewField = query.get('field');
  const reviewEnabled = query.get('review') === '1';
  const reviewFocus = reviewEnabled && (query.get('focus') === 'storyboard' || query.get('focus') === 'video')
    ? query.get('focus') as 'storyboard' | 'video'
    : null;
  const pulse = query.get('reviewPulse');
  const lastViewSeq = [...events].reverse().find((event) => (
    event.type === 'workspace.head_changed'
    || ['task.completed', 'task.failed', 'task.quarantined', 'subagent.completed'].includes(event.type)
  ))?.seq;
  ...
  const locatedSection = useMemo(
    () => plan?.sections.find((section) => section.units.some((unit) => unit.id === unitId)),
    [plan, unitId],
  );
  ...
  const displayView = useMemo(() => (
    view && locatedUnit
      ? { ...view, unit: { ...view.unit, number: locatedUnit.number, title: locatedUnit.title } }
      : view
  ), [locatedUnit, view]);
  const availableAssetRefs = useMemo<Array<RefSearchItem & { mediaType?: string }>>(() => {
    if (!assets) return [];
    const logicalAssets = new Map(assets.availableAssets.map((asset) => [asset.assetId, asset]));
  ...
  if (error && !view) {
    return <PageLoadError message={error} retry={() => void loadWorkbench(id, unitId)} />;
  }
  if (!envelope || !view || !plan || !header || !assets) return <PageSkeleton type="editor" />;

  const typedEnvelope = envelope as ViewEnvelope<R2VWorkbenchView | EditWorkbenchView>;
