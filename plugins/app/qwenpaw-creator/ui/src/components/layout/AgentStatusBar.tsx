...
const ACTIVE_RUN_STATUSES = new Set(['QUEUED', 'QUEUED_CAPACITY', 'RUNNING_MODEL', 'WAITING_RUNTIME', 'WAITING_AUTHORIZATION']);
...
export default function AgentStatusBar() {
  ...
  const subagentActivities = useCreatorSessionStore((state) => state.subagentActivities);
  ...
  const active = Boolean(
    ...
    || (session && ['RUNNING', 'RESUMING', 'WAITING_RUNTIME', 'INTERRUPT_REQUESTED'].includes(session.status)),
  );
  const waitingInput = session?.status === 'WAITING_USER_INPUT';
  const hasActiveRuns = runs.some((run) => ACTIVE_RUN_STATUSES.has(run.status));
  const hasActiveSubagents = Object.values(subagentActivities).some((a) => !a.completed);
  const stoppable = hasActiveSubagents || hasActiveRuns || Boolean(session && [
    'RUNNING',
    'RESUMING',
    'WAITING_RUNTIME',
  ...
  const stoppableHideTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  useEffect(() => {
    if (stoppable || stopping) {
      if (stoppableHideTimerRef.current) clearTimeout(stoppableHideTimerRef.current);
      setStoppableVisible(true);
    } else {
      stoppableHideTimerRef.current = setTimeout(() => setStoppableVisible(false), 2000);
    }
    return () => { if (stoppableHideTimerRef.current) clearTimeout(stoppableHideTimerRef.current); };
  }, [stoppable, stopping]);
  const pendingCount = (status?.badges || []).reduce((sum, badge) => (
    ...
    >
      <div className="flex min-w-0 flex-1 items-center gap-2 pl-4">
        ...
        <span className="min-w-0 truncate whitespace-nowrap">
          <b className="font-semibold text-[var(--color-text-primary)]">Agent 状态：</b>
      ...
      <div className="flex shrink-0 self-stretch items-center gap-2 px-4">
        {stoppableVisible && (
          <button
            ...
            title={session?.status === 'INTERRUPT_REQUESTED' ? '停止请求已发送，点击再次停止' : '立即停止当前项目的主 Agent、子 Agent 与未完成任务'}
          >
