...
function setSession(status: CreatorSessionStatus) {
  useCreatorSessionStore.setState({
    ...
    agentStatusBar: {
      progress: { phase: 'story_planning', label: '正在制作', sourceEventSeq: 1, updatedAt: 'now' },
...
describe('AgentStatusBar origin/main state projection', () => {
  ...
  it('hides status controls and their separator while the AgentDock is closed', () => {
    ...
    expect(screen.getByRole('button', { name: '停止所有 Agent' })).toBeInTheDocument();
  ...
  it('exposes a global stop control and interrupts the whole Creator Session', async () => {
    const { calls } = installMockFetch([
      {
        ...
        method: 'POST',
        response: {
          json: { creatorSessionId: 'session-1', status: 'INTERRUPT_REQUESTED', stopRequested: true },
        },
      },
    ]);
    setSession('RUNNING');
    ...
    expect(useCreatorSessionStore.getState().session?.status).toBe('INTERRUPT_REQUESTED');
    await waitFor(() => expect(calls.some((call) => call.url.includes('/projects/p1/interrupt'))).toBe(true));
    expect(calls.find((call) => call.url.includes('/projects/p1/interrupt'))?.method).toBe('POST');
  });
  ...
  it('maps authorization wait to the origin decision-center interaction', () => {
    ...
    useCreatorSessionStore.setState((state) => ({
      agentStatusBar: {
        ...
        badges: [{ kind: 'execution_authorization', label: '等待执行授权', count: 1 }],
      },
    }));
    render(<AgentStatusBar />);
  ...
  it('does not reinterpret durable progress into client-side background or partial states', () => {
    ...
    useCreatorSessionStore.setState((state) => ({
      agentStatusBar: {
        ...state.agentStatusBar!,
        progress: { ...state.agentStatusBar!.progress, label: '素材理解等待工具结果' },
      },
    }));
    ...
    setSession('IDLE');
    useCreatorTaskViewStore.setState({ tasks: [task('ok', 'SUCCEEDED'), task('bad', 'FAILED')] });
