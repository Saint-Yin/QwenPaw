...
import { CreatorPanel, useCreatorInteractionStore } from '@/store/creatorInteractionStore';
...
function isProjectShellEvent(type: string): boolean {
  return type === 'workspace.head_changed'
...
function reviewIdsFromEvent(data: Record<string, unknown>): string[] {
  ...
  return data.reviewIds.filter((item): item is string => (
...
function LayoutSkeleton() {
  return (
    <div data-project-shell data-top-nav-height="58" data-agent-status-bar-height="42" className="app-shell grid h-screen grid-rows-[58px_42px_minmax(0,1fr)]">
      <TopNav />
      <AgentStatusBar />
      <main data-creator-workspace-root className="flex-1 overflow-hidden"><PageSkeleton type="list" /></main>
    </div>
  );
}
...
export default function ProjectLayout() {
  ...
  const revalidateLoadedViews = useWorkspaceViewStore((state) => state.revalidateLoaded);
  const bootstrap = useCreatorSessionStore((state) => state.bootstrap);
  const refreshSession = useCreatorSessionStore((state) => state.refreshSession);
  const disconnect = useCreatorSessionStore((state) => state.disconnect);
  const sessionStatus = useCreatorSessionStore((state) => state.session?.status ?? null);
  const events = useCreatorSessionStore(useShallow((state) => (
    state.events.filter((event) => isProjectShellEvent(event.type))
  )));
  const refreshTasks = useCreatorTaskViewStore((state) => state.refresh);
  const startProjectSnapshotPolling = useProjectSnapshotStore((state) => state.startPolling);
  const snapshotRevision = useProjectSnapshotStore(useShallow((state) => ({
    projectId: state.projectId,
    generation: state.generation,
    etag: state.etag,
  })));
  const startFileReviewPolling = useFileProjectReviewStore((state) => state.startPolling);
  const activeFileReview = useFileProjectReviewStore((state) => state.review);
  const fileReviewSyncStatus = useFileProjectReviewStore((state) => state.syncStatus);
  ...
  useEffect(() => {
    ...
    const poll = () => {
      void useReviewManifestStore.getState().loadFileAuthorizations(id).catch(() => undefined);
    };
    poll();
    ...
    return () => {
      ...
      if (current.projectId === id && current.transactionId === null) current.reset();
    };
  }, [id]);
  ...
  useEffect(() => {
    ...
    lastConsumedEvent.current = switchingProject ? 0 : sessionState.lastEventSeq;
    if (switchingProject) {
    ...
    void Promise.all([loadHeader(id), loadPlan(id), bootstrap(id), refreshTasks(id)]).catch(() => undefined);
    return () => disconnect();
  }, [bootstrap, disconnect, id, loadHeader, loadPlan, refreshTasks]);

  useEffect(() => {
    if (!id || !sessionStatus || sessionStatus === 'IDLE' || sessionStatus === 'CANCELLED') return;
  ...
  useEffect(() => {
    const pendingEvents = events.filter((event) => event.seq > lastConsumedEvent.current);
    if (!pendingEvents.length) return;
    lastConsumedEvent.current = pendingEvents.at(-1)!.seq;
    pendingEvents.forEach((event) => useCreatorTaskViewStore.getState().consumeEvent(event));
    if (pendingEvents.some((event) => (
      event.type === 'workspace.head_changed' || event.type === 'workspace.manual_edit_committed'
    ))) {
      void loadHeader(id);
    }
    ...
    if (completedReviewId) {
      ...
      void reviewStore.pollOnce(id)
        ...
        .then(() => setPendingReviewNavigation((current) => (
  ...
  useEffect(() => {
    ...
    void loadWorkbench(id, primary.unitId).then(() => {
      const workspace = useWorkspaceViewStore.getState();
      targets.forEach(({ unitId, clipIds }) => {
        workspace.setClipHighlights(unitId, clipIds);
      });
      const targetPath = `/project/${id}/plan/unit/${encodeURIComponent(primary.unitId)}/workbench`;
      if (!pathname.startsWith(targetPath)) router.push(targetPath);
    }).catch(() => undefined);
  }, [activeFileReview, fileReviewSyncStatus, id, loadWorkbench, pathname, pendingReviewNavigation, router]);
    ...
    <div data-project-shell data-top-nav-height="58" data-agent-status-bar-height="42" className="app-shell grid h-screen grid-rows-[58px_42px_minmax(0,1fr)]">
      ...
      <div className="flex min-h-0 overflow-hidden">
        <div className="relative flex min-w-0 flex-1 flex-col overflow-hidden">
          <ReturnBanner />
          <main data-creator-workspace-root key={pathname} className="panel-enter relative min-h-0 flex-1 overflow-hidden"><Outlet /></main>
        </div>
        <AgentDock sidebar />
      </div>
      <SelectionToolbar />
