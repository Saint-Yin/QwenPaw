...
import { navigate, useParams, usePathname, useSearchParams } from '@/routing/navigation';
...
function unitDisplayName(unit: { number: number; title?: string }): string {
  return `${String(unit.number).padStart(2, '0')} ${unit.title || `生成单元 ${String(unit.number).padStart(2, '0')}`}`;
}
...
export function useBreadcrumbs(): Crumb[] {
  ...
  if (routeSection === 'plan') {
    ...
    if (parts[3] === 'unit' && parts[4]) {
      ...
      if (located) {
        crumbs.push({
          label: `${String(located.section.number).padStart(2, '0')} ${located.section.title}`,
          path: `${base}/plan?section=${encodeURIComponent(located.section.id)}`,
        });
      }
      crumbs.push({
      ...
      crumbs.push({ label: searchParams.get('review') ? '深度审阅' : '制作工作台' });
      return crumbs;
    }
    ...
    if (unitId) {
      ...
      crumbs.push({ label: located ? unitDisplayName(located.unit) : '生成单元' });
    } else if (sectionId) {
  ...
  if (routeSection === 'assets') {
    ...
    if (selectedId) {
      const selected = [
        ...
        ...(assets?.visualAssets || []),
      ].find((item) => ('assetId' in item ? item.assetId : item.id) === selectedId);
...
export default function Breadcrumb() {
  ...
  return (
    <nav className="flex min-w-0 items-center gap-1.5 text-xs text-[var(--color-text-secondary)]" aria-label="面包屑">
        ...
        return (
          <Fragment key={`${crumb.label}-${index}`}>
            ...
            {crumb.path && !isLast ? (
              ...
              <span className={`max-w-[160px] truncate px-1 ${isLast ? 'font-medium text-[var(--color-text-primary)]' : ''}`}>
                {crumb.label}
              </span>
            )}
          </Fragment>
