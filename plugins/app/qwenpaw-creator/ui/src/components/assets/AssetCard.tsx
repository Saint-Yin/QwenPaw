...
export function assetTypeTag(asset: AssetDisplayItem): string {
  ...
  if (asset.generatedKind === 'unit_video' || asset.generatedKind === 'section_video') return 'MP4';
  switch (asset.mediaType) {
    ...
    default: return asset.visualKind === 'character' ? 'CHR' : 'IMG';
  }
}

export default function AssetCard({ asset, selected, onSelect, onGenerate, generating, flashing }: {
  ...
  const tone = asset.previewState === 'failed'
    ? STATUS_TONES.stale
    : asset.previewState === 'processing'
      ? STATUS_TONES.pending
      : STATUS_TONES[asset.status];
  const statusLabel = asset.previewState === 'failed'
    ? '入库失败'
    : asset.previewState === 'processing'
      ? '入库中'
      : asset.planned
  ...
  return (
    <article
      ...
      className={`group cursor-pointer overflow-hidden rounded-xl border bg-[var(--color-bg-card)] transition-all ${flashing ? 'review-flash ' : ''}${selected ? 'border-[var(--color-accent)] shadow-[0_0_0_1px_var(--color-accent)]' : asset.planned ? 'border-dashed border-[var(--color-border-strong)] hover:border-[var(--color-accent)]/50' : 'border-[var(--color-border)] hover:border-[var(--color-border-strong)] hover:shadow-sm'}`}
    >
      <div className="relative flex h-32 items-center justify-center overflow-hidden bg-[var(--color-bg-secondary)]">
        <AssetMediaPreview
          ...
          mediaClassName={asset.mediaType === 'image' ? 'h-full w-full object-cover transition-transform duration-300 group-hover:scale-[1.02]' : 'h-full w-full object-cover'}
        ...
        />
        <span className="absolute left-2 top-2 rounded bg-black/65 px-1.5 py-0.5 text-[10px] font-bold text-white">{assetTypeTag(asset)}</span>
        {asset.referenceCount > 0 && (
          <span className="absolute right-2 top-2 flex items-center gap-1 rounded bg-black/65 px-1.5 py-0.5 text-[10px] font-medium text-white"><Link2 className="h-2.5 w-2.5" />{asset.referenceCount}</span>
        )}
      ...
      <div className="p-3">
        <div className="flex items-center justify-between gap-2">
          <h3 className="min-w-0 truncate text-sm font-semibold text-[var(--color-text-primary)]">{asset.name}</h3>
          <span className={`shrink-0 rounded border px-1.5 py-px text-[10px] ${tone}`}>{statusLabel}</span>
        </div>
        <p className="mt-1 line-clamp-2 min-h-[2rem] text-[11px] leading-4 text-[var(--color-text-secondary)]">{asset.sourceLine}</p>
      </div>
    </article>
  );
}
