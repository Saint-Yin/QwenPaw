...
interface NewAppearanceModalProps {
  open: boolean;
  onClose: () => void;
  onConfirm: (refDescription: string, prompt: string, imageUrl: string) => Promise<void>;
  assetName: string;
  projectId: string;
  assetId: string;
      ...
      case 'prompt':
        return (
          <div className="space-y-4">
            <div>
              <label className="mb-2 block text-sm font-medium">需求描述</label>
              <div className="rounded-lg bg-gray-50 p-2 text-sm">{refDescription}</div>
            </div>
            <div>
              <label className="mb-2 block text-sm font-medium">生成的 Prompt（可编辑）</label>
              <Input.TextArea
                value={prompt}
      ...
      case 'preview':
        return (
          <div className="space-y-4">
            ...
            <div>
              <label className="mb-2 block text-sm font-medium">生成的图片</label>
      ...
      <div className="py-4">
        {loading || loadingImage ? (
          <div className="flex h-48 items-center justify-center">
            <Spin size="large" tip={loadingImage ? '生成图片中...' : '生成 Prompt 中...'} />
          </div>
        ) : (
          renderContent()
        )}
