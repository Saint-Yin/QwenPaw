...
import { ArrowLeft, Check, ListVideo, PlayCircle, Scissors, Sparkles, Zap } from 'lucide-react';
...
const AGENTDOCK_RUN_ORIGINS = new Set(['agentdock_idle_goal', 'agentdock_interrupt']);

function latestActiveAgentDockRun(events: CreatorEvent[]): { runId: string; startedAt: string } | null {
  const active = new Map<string, { runId: string; startedAt: string; seq: number }>();
  ...
  return [...active.values()].sort((left, right) => right.seq - left.seq)[0] ?? null;
}

function ClipPreview({ src, start, end, autoPlay = false }: {
  src: string;
  start: number;
  end: number;
  autoPlay?: boolean;
  ...
  const seekToStartWhenOutsideClip = useCallback((video: HTMLVideoElement) => {
  ...
  return (
    <div className="w-full bg-black" style={{ maxWidth: '400px' }}>
      ...
      <div className="flex items-center gap-2 bg-black/80 px-2 py-1.5">
        <button type="button" onClick={togglePlay} className="shrink-0 text-white hover:text-white/70">
          {playing ? (
            <svg width="11" height="11" viewBox="0 0 11 11" fill="currentColor">
              <rect x="1" y="1" width="3.5" height="9" rx="0.5" /><rect x="6.5" y="1" width="3.5" height="9" rx="0.5" />
            </svg>
          ) : (
            <svg width="11" height="11" viewBox="0 0 11 11" fill="currentColor">
        ...
        <button type="button" onClick={toggleMute} className="shrink-0 text-white hover:text-white/70" aria-label={muted ? '取消静音' : '静音'}>
          {muted ? (
        ...
        >
          ...
          <div className="absolute left-0 h-1 rounded bg-white" style={{ width: `${progress * 100}%` }} />
          <div
            className="absolute h-2.5 w-2.5 rounded-full bg-white shadow transition-transform group-hover:scale-110"
            style={{ left: `calc(${progress * 100}% - 5px)`, opacity: dragging ? 1 : undefined }}
          />
          {dragging && (
            <div
        ...
        <span className="shrink-0 tabular-nums text-[10px] text-white/60">{start}s – {end}s</span>
      </div>
    </div>
  );
}
...
function Panel({ title, badge, children, className }: {
  ...
  return (
    <section className={`rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-card)] ${className ?? ''}`}>
...
function currentVersion(versions: ArtifactVersionView[]): ArtifactVersionView | null {
...
function materialUrl(asset: AiEditMaterialAsset): string | null {
  return typeof asset.url === 'string' && asset.url ? getGeneratedMediaUrl(asset.url) : null;
}
...
export default function AiEditWorkbench({
  ...
  const agentRunEvents = useCreatorSessionStore(useShallow((state) => (
    state.events.filter((event) => AGENT_RUN_LIFECYCLE_EVENTS.has(event.type))
  )));
  ...
  const [activeAction, setActiveAction] = useState<'plan' | 'execute' | 'full' | null>(null);
  const [autoExecute, setAutoExecute] = useState(false);
  const [completionFlash, setCompletionFlash] = useState(false);
  const [reloading, setReloading] = useState(false);
  const [progressBarVisible, setProgressBarVisible] = useState(false);
  const progressBarHideTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  type ExecutionLogEntry = { index: number; title: string; start: number; end: number };
  ...
  const [activePreviewPanelId, setActivePreviewPanelId] = useState<string | null>(null);
  const [pendingRanges, setPendingRanges] = useState<Array<{ clipId: string; start: number; end: number }> | null>(null);
  const [refOptions, setRefOptions] = useState<RefSearchItem[]>(materialReferenceOptions ?? view.resolvedRefs);
  const [loadingRefs, setLoadingRefs] = useState(false);
  const timingDraftsRef = useRef<Record<string, { start: number; end: number }>>({});
  const rangeInFlightRef = useRef(false);
  const prevTimelineRef = useRef<Map<string, { start: number; end: number; fp: string }>>(new Map());
  const [changedClips, setChangedClips] = useState<Map<string, { oldStart: number; oldEnd: number } | null>>(new Map());
  const changedFlashTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const prevUnitIdForChangeRef = useRef(view.unit.id);
  const clipHighlights = useWorkspaceViewStore((state) => state.clipHighlights[view.unit.id]);
  const clipStateKey = `${view.unit.id}|${timeline.map((c) => {
    const os = c.overlay_copy;
    return `${c.clip_id}:${c.start}:${c.end}:${os?.text ?? ''}:${os?.vibe ?? ''}:${os?.appear_at ?? ''}:${os?.duration ?? ''}`;
  }).join('|')}`;

  const { schedule: scheduleUnitField } = useDebouncedUnitFieldSave<string>((field, value) => (
    submit('SET_UNIT_TEXT', `unit:${view.unit.id}`, { field, value }, view.unit.targetVersion)
  ));
  ...
  useEffect(() => {
    ...
    if (isUnitSwitch) {
      ...
      if (changedFlashTimerRef.current) { clearTimeout(changedFlashTimerRef.current); changedFlashTimerRef.current = null; }
    } else {
      const prevMap = prevTimelineRef.current;
      const newChanges = new Map<string, { oldStart: number; oldEnd: number } | null>();
      for (const clip of timeline) {
        const prev = prevMap.get(clip.clip_id);
        if (prev) {
          const os = clip.overlay_copy;
          const currentFp = `${clip.start}:${clip.end}:${os?.text ?? ''}:${os?.vibe ?? ''}:${os?.appear_at ?? ''}:${os?.duration ?? ''}`;
          if (prev.fp !== currentFp) {
            const timingChanged = Math.abs(prev.start - clip.start) > 0.04 || Math.abs(prev.end - clip.end) > 0.04;
            newChanges.set(clip.clip_id, timingChanged ? { oldStart: prev.start, oldEnd: prev.end } : null);
          }
        }
      }
      if (newChanges.size > 0) {
        setChangedClips(newChanges);
        if (changedFlashTimerRef.current) clearTimeout(changedFlashTimerRef.current);
        changedFlashTimerRef.current = setTimeout(() => setChangedClips(new Map()), 3500);
      }
    }
    const newMap = new Map<string, { start: number; end: number; fp: string }>();
    for (const clip of timeline) {
      const os = clip.overlay_copy;
      const fp = `${clip.start}:${clip.end}:${os?.text ?? ''}:${os?.vibe ?? ''}:${os?.appear_at ?? ''}:${os?.duration ?? ''}`;
      newMap.set(clip.clip_id, { start: clip.start, end: clip.end, fp });
    }
    prevTimelineRef.current = newMap;
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [clipStateKey]);
  useEffect(() => () => { if (changedFlashTimerRef.current) clearTimeout(changedFlashTimerRef.current); }, []);
  useEffect(() => {
    if (!clipHighlights?.length) return;
    const unitId = view.unit.id;
    setChangedClips(new Map(clipHighlights.map((clipId) => [clipId, null])));
    if (changedFlashTimerRef.current) clearTimeout(changedFlashTimerRef.current);
    changedFlashTimerRef.current = setTimeout(() => setChangedClips(new Map()), 5000);
  ...
  useEffect(() => {
    if (focusVersion && videoVersions.some((version) => version.id === focusVersion)) setViewedVideoId(focusVersion);
  ...
  const effectiveVideoId = viewedVideoId ?? selectedVideo?.id ?? videoVersions.at(-1)?.id ?? null;
  const viewedVideo = videoVersions.find((version) => version.id === effectiveVideoId) ?? null;
  const unitTasks = tasks.filter((task) => task.targetRef === `unit:${view.unit.id}`);
  const activeAgentDockRun = useMemo(() => latestActiveAgentDockRun(agentRunEvents), [agentRunEvents]);
  const planTaskRunning = unitTasks.some((task) => task.kind === 'ai_edit_plan' && ['QUEUED', 'RUNNING'].includes(task.status));
  const executeTaskRunning = unitTasks.some((task) => task.kind === 'ai_edit_execute' && ['QUEUED', 'RUNNING'].includes(task.status));
  const agentDockExecutionObserved = activeAgentDockRun != null && unitTasks.some((task) => (
    task.kind === 'ai_edit_execute'
    && task.createdAt != null
    && Date.parse(task.createdAt) >= Date.parse(activeAgentDockRun.startedAt)
  ));
  const agentDockThinkingRunning = activeAgentDockRun != null
    && !executeTaskRunning
    && !agentDockExecutionObserved;
  const agentDockPostExecute = activeAgentDockRun != null
    && !executeTaskRunning
    && agentDockExecutionObserved;
  const thinkingRunning = planTaskRunning || agentDockThinkingRunning;
  const executeTaskProgress = unitTasks.find((task) => (
    task.kind === 'ai_edit_execute' && ['QUEUED', 'RUNNING'].includes(task.status)
  ))?.progress ?? null;
  const processingClipIndex = executeTaskRunning && executeTaskProgress != null && timeline.length > 0
    ? Math.min(Math.floor(executeTaskProgress * timeline.length), timeline.length - 1)
    : -1;

  const shouldShowProgressBar = planTaskRunning || executeTaskRunning || autoExecute || completionFlash || reloading || agentDockPostExecute || changedClips.size > 0;
  useEffect(() => {
    if (stopping) {
    ...
    if (shouldShowProgressBar) {
      ...
      progressBarHideTimerRef.current = setTimeout(() => setProgressBarVisible(false), 500);
    }
    return () => {
      if (progressBarHideTimerRef.current) clearTimeout(progressBarHideTimerRef.current);
    };
  ...
  useEffect(() => {
    if (activeAction == null && !autoExecute && !thinkingRunning && !executeTaskRunning) return;
    const poll = () => {
  ...
  }, [activeAction, autoExecute, executeTaskRunning, projectId, refreshTasks, thinkingRunning]);
  ...
  useEffect(() => {
    if (!autoExecute || planTaskRunning || executeTaskRunning || !view.planVersion?.id) return;
  ...
  }, [autoExecute, executeTaskRunning, planTaskRunning, submit, view.planVersion, view.unit.id, view.unit.targetVersion]);

  useLayoutEffect(() => {
    if (prevExecuteRunningRef.current && !executeTaskRunning && !planTaskRunning && !autoExecute) {
  ...
  useEffect(() => {
    if (!executeTaskRunning) {
      if (prevClipIndexRef.current >= 0) {
        ...
        const panel = storyboard.find((item) => item.clip_id === clip?.clip_id)
          ...
          setExecutionLog((current) => (
            current.some((entry) => entry.index === prevClipIndexRef.current)
              ...
              : [...current, {
                ...
                title: panel?.title ?? `片段 ${prevClipIndexRef.current + 1}`,
                start: clip.start,
                end: clip.end,
              }]
          ));
    ...
    if (prevClipIndexRef.current === -1) {
      ...
      if (clip) {
        setExecutionLog((current) => [...current, {
          index: previousIndex,
  ...
  const storyboardImageUrl = view.storyboard_image_url || plan?.storyboard_image_url || null;
  const editDuration = timeline.length
    ? Math.round(timeline.reduce((sum, clip) => sum + Math.max(0, clip.end - clip.start), 0))
    : view.unit.duration;

  const selectArtifact = (version: ArtifactVersionView) => submit(
    'SELECT_ARTIFACT_VERSION',
    `unit:${view.unit.id}`,
    { slotId: version.slotId, artifactVersionId: version.artifactVersionId, artifactRef: version.sourceRef },
    view.unit.targetVersion,
  );
  ...
  const acceptArtifact = async (version: ArtifactVersionView) => {
    ...
    if (group?.decision === 'PENDING') {
      await decideReview(group.id, { decisionToken: group.decisionToken, decision: 'ACCEPT' });
  ...
  const undoArtifact = async (version: ArtifactVersionView) => {
    const group = reviewGroupForArtifact(useReviewManifestStore.getState().manifest, version);
    if (group?.decision !== 'PENDING') return;
    await decideReview(group.id, { decisionToken: group.decisionToken, decision: 'REJECT' });
  ...
  const buildEditPlan = async () => {
    ...
    try {
      const result = await submit('BUILD_EDIT_PLAN', `unit:${view.unit.id}`, {}, view.unit.targetVersion);
  ...
  const updateClipTiming = (clip: AiEditTimelineClip, index: number, field: 'start' | 'end', raw: string) => {
    const value = Number.parseFloat(raw);
    if (!Number.isFinite(value) || value < 0) return;
    const draft = timingDraftsRef.current[clip.clip_id] ?? { start: clip.start, end: clip.end };
  ...
  const confirmTimeline = () => {
    const ranges = timeline.flatMap((clip) => {
      ...
      if (!draft || (draft.start === clip.start && draft.end === clip.end)) return [];
  ...
  useEffect(() => {
    ...
    void submit(
      ...
      setPendingRanges((current) => result && current ? current.slice(1) : null);
    });
  }, [pendingRanges, submitting, submit, view.unit.id, view.unit.targetVersion]);

  const updateClipOs = (clip: AiEditTimelineClip, patch: Record<string, unknown>) => submit(
    'SET_EDIT_CLIP_OS',
    `unit:${view.unit.id}`,
    {
      clipId: clip.clip_id,
      ...
      ...(clip.overlay_copy?.duration == null ? {} : { duration: clip.overlay_copy.duration }),
      ...patch,
    },
    view.unit.targetVersion,
  );
  ...
  const changeMaterials = (next: string[]) => {
    ...
    if (removed) {
      void submit('UNBIND_REFERENCE', `unit:${view.unit.id}`, {
        field: 'sources',
        referenceSet: 'unit',
        sourceRef: removed,
      }, view.unit.targetVersion);
  ...
  const jumpToMaterial = (asset: AiEditMaterialAsset) => {
    ...
    const target = view.resolvedRefs.find((item) => item.logicalAssetId === id || item.ref.includes(id));
    ...
    <div className="flex h-full flex-col overflow-hidden bg-[var(--color-bg-layout)]">
      <div className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-[var(--color-border)] bg-[var(--color-bg-primary)]/60 px-5 py-3 backdrop-blur">
        <div className="flex min-w-0 items-center gap-2">
          <button type="button" onClick={onBack} className="icon-button shrink-0" aria-label="返回视频方案">
            <ArrowLeft className="h-3.5 w-3.5" />
          </button>
          <div className="min-w-0">
            <h2 className="truncate text-sm font-semibold text-[var(--color-text-primary)]">
              视频方案 / {`${String(sectionNumber).padStart(2, '0')} ${sectionTitle}`} / {unitDisplayName(view.unit)} / AI 剪辑工作台
            </h2>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            ...
            loading={activeAction === 'full' || activeAction === 'execute' || planTaskRunning || executeTaskRunning || autoExecute}
            disabled={planTaskRunning || executeTaskRunning || autoExecute}
      ...
      {progressBarVisible && (() => {
        ...
        const isPlanPhase = planTaskRunning && !executeTaskRunning && !agentDockPostExecute && !isDone;
        const progressLabel = executeTaskRunning
          ? (executeTaskProgress != null && executeTaskProgress > 0
            ...
            : 'AI 执行剪接…')
          : agentDockPostExecute ? 'AI 确认成果中…'
          : isDone ? '完成'
          : 'AI 生成方案中…';
        const executeFill = isDone || agentDockPostExecute ? 100
          : executeTaskRunning ? 40 + (executeTaskProgress ?? 0) * 60
          : 0;
        return (
          <div className="shrink-0 border-b border-[var(--color-border)] px-5 py-2.5">
            <div className="mb-1.5 flex items-center justify-between">
              <span className={`flex items-center gap-1.5 text-[11px] font-semibold transition-colors ${isDone ? 'text-[var(--color-success)]' : 'text-[var(--color-accent)]'}`}>
                {isDone
            ...
            <div className="relative h-1.5 overflow-hidden rounded-full bg-[var(--color-bg-primary)]">
              {isPlanPhase ? (
                <div className="absolute inset-y-0 left-0 overflow-hidden" style={{ width: '40%' }}>
              ...
              {agentDockPostExecute && !completionFlash && !reloading && changedClips.size === 0 && (
      ...
      {(executeTaskRunning || completionFlash) && (
        <div className="max-h-28 shrink-0 space-y-1 overflow-y-auto border-b border-[var(--color-border)] bg-[var(--color-bg-secondary)]/40 px-5 py-2">
          {executionLog.map((entry) => (
            <div key={entry.index} className="flex items-center gap-2 text-[11px]">
              <Check className="h-3 w-3 shrink-0 text-[var(--color-success)]" />
              <span className="font-medium text-[var(--color-text-primary)]">#{entry.index + 1} {entry.title}</span>
              <span className="tabular-nums text-[var(--color-text-tertiary)]">{entry.start.toFixed(1)}s → {entry.end.toFixed(1)}s</span>
            </div>
          ))}
            ...
            return (
              <div className="flex items-center gap-2 text-[11px]">
                ...
                <span className="font-medium text-[var(--color-text-primary)]">
                  #{processingClipIndex + 1} {panel?.title ?? `片段 ${processingClipIndex + 1}`}
                </span>
                <span className="text-[var(--color-text-tertiary)]">处理中…</span>
              </div>
            );
          })()}
          <div ref={logEndRef} />
        ...
        <div className="min-h-0 space-y-3 overflow-y-auto pr-1">
          <Panel title="剪辑目标">
            <div
              ...
              data-creator-path={projectJsonPointer('production', 'units_by_id', view.unit.id, 'intent')}
          ...
          <div className="scroll-mt-4">
            <Panel
              ...
              badge={plan ? <span className="text-[11px] font-medium text-[var(--color-text-tertiary)]">{storyboard.length} panels</span> : null}
            >
                ...
                {storyboard.length ? (
                  <>
                    <div className="flex flex-wrap gap-3">
                      {storyboard.map((panel, panelIndex) => {
                        const clip = panel.clip_id
                          ? timeline.find((item) => item.clip_id === panel.clip_id)
                          : timeline[panelIndex];
                        const clipIndex = clip ? timeline.indexOf(clip) : -1;
                        const assetIndex = materialAssets.findIndex((item) => String(item.id || '') === panel.source_asset_id);
                        const asset = assetIndex >= 0 ? materialAssets[assetIndex] : undefined;
                        ...
                        const frameTimestamp = Math.max(
                          clipStart,
                          Math.min(
                            Number.isFinite(panel.timestamp) ? panel.timestamp : clipStart,
                        ...
                        const keyframeSrc = panel.image_url
                          ...
                          : clip?.asset_version_id
                            ? getAssetVersionFrameUrl(clip.asset_version_id, frameTimestamp)
                            : null;
                        const previousClip = clipIndex > 0 ? timeline[clipIndex - 1] : undefined;
                        const nextClip = clipIndex >= 0 && clipIndex < timeline.length - 1
                          ? timeline[clipIndex + 1]
                          : undefined;
                        const isClipChanged = Boolean(clip && changedClips.has(clip.clip_id));
                        const isProcessing = clipIndex === processingClipIndex;
                        const isClipDone = executeTaskRunning && processingClipIndex >= 0
                          && clipIndex >= 0 && clipIndex < processingClipIndex;
                        const isClipPending = executeTaskRunning && processingClipIndex >= 0
                          && clipIndex > processingClipIndex;
                        const clipChangedInfo = clip ? changedClips.get(clip.clip_id) : undefined;
                        return (
                          <div
                            key={panel.panel_id}
                          ...
                          >
                            ...
                            {videoSrc ? (
                              <StoryboardClipPreview
                                ...
                                eager={panelIndex < 2}
                                onActivate={() => setActivePreviewPanelId(panel.panel_id)}
                                onShowKeyframe={() => setActivePreviewPanelId(null)}
                              />
                            ) : (
                              <div className="flex items-center justify-center bg-[var(--color-bg-secondary)] text-xs text-[var(--color-text-tertiary)]" style={{ aspectRatio: '16/9' }}>
                                无预览
                              </div>
                            )}
                            <div className="p-2">
                              <p
                                ...
                                data-creator-path={projectJsonPointer('production', 'units_by_id', view.unit.id, 'plan', 'storyboard', 'items', panel.panel_id, 'title')}
                              ...
                              <p
                                ...
                                data-creator-path={projectJsonPointer('production', 'units_by_id', view.unit.id, 'plan', 'storyboard', 'items', panel.panel_id, 'description')}
                              ...
                              <div className="mt-1.5 flex flex-wrap items-center gap-1 text-[10px] text-[var(--color-text-tertiary)]">
                                ...
                                <input
                                  ...
                                  type="number"
                                  step="0.5"
                                  min={previousClip?.end ?? view.previousUnitLastClipEnd ?? 0}
                                  defaultValue={clipStart}
                                  data-creator-field={`unit:${view.unit.id}/editPlan/timeline/clip:${clip?.clip_id ?? panel.clip_id}/sourceInSeconds`}
                                  data-creator-path={clip ? projectJsonPointer('production', 'units_by_id', view.unit.id, 'plan', 'timeline', 'items', clip.clip_id, 'source_in_seconds') : undefined}
                                  data-creator-field-label={`剪辑片段 ${clipIndex >= 0 ? clipIndex + 1 : panelIndex + 1} · 入点`}
                                  onBlur={(event) => { if (clip) updateClipTiming(clip, clipIndex >= 0 ? clipIndex : panelIndex, 'start', event.target.value); }}
                                ...
                                />
                                {(previousClip?.end ?? (clipIndex <= 0 ? view.previousUnitLastClipEnd ?? 0 : undefined)) != null && (
                                  <span className="text-[9px] text-[var(--color-text-tertiary)] opacity-60">(最早 {previousClip?.end ?? view.previousUnitLastClipEnd ?? 0}s)</span>
                                )}
                                到
                                <input
                                  ...
                                  step="0.5"
                                  min="0"
                                  max={nextClip?.start ?? view.nextUnitFirstClipStart ?? undefined}
                                  defaultValue={clipEnd}
                                  data-creator-field={`unit:${view.unit.id}/editPlan/timeline/clip:${clip?.clip_id ?? panel.clip_id}/sourceOutSeconds`}
                                  data-creator-path={clip ? projectJsonPointer('production', 'units_by_id', view.unit.id, 'plan', 'timeline', 'items', clip.clip_id, 'source_out_seconds') : undefined}
                                  data-creator-field-label={`剪辑片段 ${clipIndex >= 0 ? clipIndex + 1 : panelIndex + 1} · 出点`}
                                  onBlur={(event) => { if (clip) updateClipTiming(clip, clipIndex >= 0 ? clipIndex : panelIndex, 'end', event.target.value); }}
                                  className="w-16 rounded border border-[var(--color-border)] bg-[var(--color-bg-secondary)] px-1 py-0.5 text-center text-[10px] font-semibold text-[var(--color-text-primary)]"
                                />
                                秒
                                {(nextClip?.start ?? view.nextUnitFirstClipStart) != null && (
                                  <span className="text-[9px] text-[var(--color-text-tertiary)] opacity-60">(最長 {nextClip?.start ?? view.nextUnitFirstClipStart}s)</span>
                                )}
                              </div>
                              {isClipChanged && (
                                <div className="mt-1.5 inline-flex items-center gap-1 rounded bg-[var(--color-accent-soft)] px-1.5 py-0.5 text-[9px] font-medium text-[var(--color-accent)]">
                                  {clipChangedInfo ? (
                                    <>
                                      <span className="line-through opacity-60">{clipChangedInfo.oldStart}s–{clipChangedInfo.oldEnd}s</span>
                                      <span>→</span>
                                      <span>{clipStart}s–{clipEnd}s</span>
                                    </>
                                  ) : '已更新'}
                                </div>
                              )}
                    ...
                    <div className="flex justify-end pt-1">
                      <button
                        type="button"
                        disabled={activeAction === 'execute' || executeTaskRunning || pendingRanges != null}
                  ...
                  <img src={getGeneratedMediaUrl(storyboardImageUrl)} alt="AI 剪辑关键帧分镜" className="w-full rounded-lg border border-[var(--color-border)]" />
                ) : (
          ...
          <Panel
            ...
            badge={executeTaskRunning && processingClipIndex >= 0 ? (
              <span className="text-[11px] font-medium text-[var(--color-accent)]">
                正在处理第 {processingClipIndex + 1} 段 / 共 {timeline.length} 段
              </span>
            ) : null}
          >
            {timeline.length ? (
              <div className="space-y-2">
                {timeline.map((clip, index) => {
                  const clipPanel = storyboard.find((panel) => panel.clip_id && panel.clip_id === clip.clip_id) || storyboard[index];
                  const rowDone = executeTaskRunning && processingClipIndex >= 0 && index < processingClipIndex;
                  const rowProcessing = index === processingClipIndex;
                  const rowPending = executeTaskRunning && processingClipIndex >= 0 && index > processingClipIndex;
                  return (
                  <div
                  ...
                  >
                    <div className={`flex items-center gap-1 text-xs font-bold ${rowDone ? 'text-[var(--color-success)]' : 'text-[var(--color-accent)]'}`}>
                      {rowDone ? <Check className="h-3 w-3" /> : null}
                      #{index + 1}
                    </div>
                    <div className="min-w-0">
                      <p
                        data-creator-field={clipPanel ? `unit:${view.unit.id}/editPlan/storyboard/panel:${clipPanel.panel_id}/title` : `unit:${view.unit.id}/editPlan/timeline/clip:${clip.clip_id}/label`}
                        data-creator-path={clipPanel ? projectJsonPointer('production', 'units_by_id', view.unit.id, 'plan', 'storyboard', 'items', clipPanel.panel_id, 'title') : undefined}
                        data-creator-field-label={`剪辑片段 ${index + 1} · 标题`}
                      ...
                      <p
                        ...
                        data-creator-path={projectJsonPointer('production', 'units_by_id', view.unit.id, 'plan', 'timeline', 'items', clip.clip_id, 'reason')}
                        data-creator-field-label={`剪辑片段 ${index + 1} · 选择理由`}
                          ...
                          <div className="relative inline-block">
                            <div className="flex items-center gap-1 rounded-2xl border-2 border-black bg-yellow-50 px-2.5 py-1.5 shadow-[2px_2px_0_#000]">
                              <input
                                ...
                                data-creator-path={projectJsonPointer('production', 'units_by_id', view.unit.id, 'plan', 'timeline', 'items', clip.clip_id, 'overlay', 'text')}
                                data-creator-field-label={`剪辑片段 ${index + 1} · 画面文案`}
                                ref={(element) => {
                                  if (element) { element.style.width = '0'; element.style.width = `${Math.max(80, element.scrollWidth)}px`; }
                                }}
                                onInput={(event) => {
                                  const element = event.target as HTMLInputElement;
                                  element.style.width = '0';
                                  element.style.width = `${Math.max(80, element.scrollWidth)}px`;
                                }}
                                onBlur={(event) => {
                                  const value = event.target.value.trim();
                                  if (value !== clip.overlay_copy?.text.trim()) void updateClipOs(clip, { text: value });
                                }}
                            ...
                            <div style={{ position: 'absolute', bottom: -9, left: 14, width: 0, height: 0, borderLeft: '7px solid transparent', borderRight: '7px solid transparent', borderTop: '9px solid black' }} />
                            <div style={{ position: 'absolute', bottom: -6, left: 16, width: 0, height: 0, borderLeft: '5px solid transparent', borderRight: '5px solid transparent', borderTop: '7px solid #fefce8' }} />
                          </div>
                          <div className="mt-3 text-base leading-none">
                            {clip.overlay_copy.vibe === 'action' ? '😾' : clip.overlay_copy.vibe === 'surprise' ? '🙀' : clip.overlay_copy.vibe === 'curious' ? '🐱' : '😺'}
                          </div>
                        </div>
                      )}
                      ...
                      <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] text-[var(--color-text-tertiary)]">
                        <label className="flex items-center gap-1">
                          {`OS出现自第${'一二三四五六七八九十'.split('')[index] ?? (index + 1)}分段`}
                          <input
                            key={`os_at_${clip.clip_id}_${clip.overlay_copy?.appear_at ?? 0}`}
                            type="number"
                            step="0.5"
                            min="0"
                            ...
                            data-creator-path={projectJsonPointer('production', 'units_by_id', view.unit.id, 'plan', 'timeline', 'items', clip.clip_id, 'overlay', 'appear_at')}
                            data-creator-field-label={`剪辑片段 ${index + 1} · 画面文案出现时间`}
                            onBlur={(event) => {
                              const value = Number.parseFloat(event.target.value);
                              if (Number.isFinite(value) && value >= 0 && value !== (clip.overlay_copy?.appear_at ?? 0)) {
                                const remaining = Math.max(0.5, clip.duration - value);
                                void updateClipOs(clip, {
                                  appear_at: value,
                                  duration: Math.min(clip.overlay_copy?.duration ?? remaining, remaining),
                                });
                              }
                            }}
                        ...
                        <label className="flex items-center gap-1">
                          OS时长
                          <input
                            key={`os_dur_${clip.clip_id}_${clip.overlay_copy?.duration ?? ''}`}
                            type="number"
                            step="0.5"
                            min="0.5"
                            ...
                            data-creator-path={projectJsonPointer('production', 'units_by_id', view.unit.id, 'plan', 'timeline', 'items', clip.clip_id, 'overlay', 'duration')}
                            data-creator-field-label={`剪辑片段 ${index + 1} · 画面文案时长`}
                            onBlur={(event) => {
                              const raw = event.target.value.trim();
                              const value = raw === '' ? null : Number.parseFloat(raw);
                              if (value === null || (Number.isFinite(value) && value > 0)) {
                                if (value !== (clip.overlay_copy?.duration ?? null)) void updateClipOs(clip, { duration: value });
                              }
                            }}
              ...
              <p className="text-xs text-[var(--color-text-tertiary)]">生成剪辑方案后会显示素材取舍、时间段和转场建议。</p>
            )}
          </Panel>
        </div>
        ...
        <aside className="min-h-0 space-y-3 overflow-y-auto pr-1">
              ...
              <div className="space-y-2">
                {viewedVideo ? (
                  <video
                    ...
                    src={getArtifactVersionMediaUrl(viewedVideo.artifactVersionId)}
                    controls
                    preload="metadata"
                    playsInline
                    aria-label="剪辑成片预览"
                ...
                {viewedVideo && (
                  reviewGroupForArtifact(manifest, viewedVideo)?.decision === 'PENDING'
                  ...
                  <div className="flex items-center justify-between rounded-lg border border-[var(--color-warning)]/25 bg-[var(--color-warning-soft)] px-2.5 py-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      <Button size="small" type="primary" onClick={() => void acceptArtifact(viewedVideo)} className="!text-[11px]">接受</Button>
                      {reviewGroupForArtifact(manifest, viewedVideo)?.decision === 'PENDING' && (
                        <Button size="small" onClick={() => void undoArtifact(viewedVideo)} className="!text-[11px]">撤销</Button>
                      )}
                    </div>
                  </div>
                )}
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <div key={cell.label} className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-card)] p-3 text-center">
                <p className="text-[10px] text-[var(--color-text-tertiary)]">{cell.label}</p>
                <p className="mt-1 text-xs font-semibold text-[var(--color-text-primary)]">{cell.value}</p>
              </div>
            ))}
          </div>
          ...
          <Panel title="剪辑素材">
            <Select
              ...
              onDropdownVisibleChange={(open) => { if (open) void loadReferenceOptions(); }}
            ...
            <div className="mt-3 space-y-1.5">
              {materialAssets.length === 0 ? (
                <p className="text-xs text-[var(--color-text-tertiary)]">暂无视频素材。请在新建项目处选择文件夹或上传视频。</p>
                ...
                >
                  <span className="shrink-0 rounded border border-[var(--color-border)] bg-[var(--color-bg-primary)] px-1.5 py-px text-[10px] text-[var(--color-text-tertiary)]">VID</span>
                  <span className="min-w-0 flex-1 truncate text-xs font-medium text-[var(--color-accent)]">@{materialName(asset, index)}</span>
                </button>
              ))}
            </div>
          </Panel>
