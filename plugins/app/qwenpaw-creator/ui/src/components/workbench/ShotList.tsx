...
export default function ShotList({ shots, unitId, sectionId, disabled, onUpsert, onDelete }: ShotListProps) {
  const [drafts, setDrafts] = useState<Record<string, ShotView>>(() => Object.fromEntries(shots.map((shot) => [shot.id, shot])));
  ...
  useEffect(() => {
    setDrafts((current) => Object.fromEntries(shots.map((shot) => [
      shot.id,
  ...
  const commit = (shot: ShotView, draft = draftsRef.current[shot.id] || shot) => {
  ...
  const patchDraft = (shot: ShotView, patch: Partial<ShotView>) => {
    ...
    timersRef.current.set(shot.id, setTimeout(() => commit(shot, next), 600));
  };
  ...
  const addShot = () => {
    const maxNumber = shots.length > 0 ? Math.max(...shots.map((shot) => shot.number)) : 0;
    void onUpsert({
      id: `shot-${Date.now()}`,
      number: maxNumber + 1,
      duration: 3,
          ...
          >
            ...
            <div className="min-w-0 flex-1 space-y-1.5">
              <div
                ...
                data-creator-path={sectionId ? projectJsonPointer('story', 'sections', 'items', sectionId, 'units', 'items', unitId, 'shots', 'items', shot.id, 'description') : undefined}
                data-creator-field-label={`镜头${shot.number} · 描述`}
              >
                <TextArea
                  value={draft.description}
                  disabled={disabled}
                  onChange={(event) => patchDraft(shot, { description: event.target.value })}
              ...
              <div className="flex items-center gap-2">
                <span
                  ...
                  data-creator-path={sectionId ? projectJsonPointer('story', 'sections', 'items', sectionId, 'units', 'items', unitId, 'shots', 'items', shot.id, 'camera_description') : undefined}
                  data-creator-field-label={`镜头${shot.number} · 运镜`}
                  className="contents"
                >
                  <Input
                    value={draft.cameraDescription ?? `${draft.camera} · ${draft.framing}`}
                    disabled={disabled}
                    onChange={(event) => {
                      const value = event.target.value;
                      patchDraft(shot, {
                        cameraDescription: value,
                        ...(CANONICAL_CAMERAS.includes(value as ShotCamera) ? { camera: value as ShotCamera } : {}),
                      });
                    }}
                ...
                <span
                  ...
                  data-creator-path={sectionId ? projectJsonPointer('story', 'sections', 'items', sectionId, 'units', 'items', unitId, 'shots', 'items', shot.id, 'duration_seconds') : undefined}
                ...
                >
                  <InputNumber
                    ...
                    disabled={disabled}
                    onChange={(value) => patchDraft(shot, { duration: value ?? 1 })}
            ...
            <Popconfirm title="删除此镜头？" onConfirm={() => void onDelete(shot)} okText="删除" cancelText="取消">
              <Button
                type="text"
                size="small"
                disabled={disabled}
