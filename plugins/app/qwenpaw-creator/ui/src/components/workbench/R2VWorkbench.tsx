...
import { AlertTriangle, ArrowLeft, Clapperboard, Image as ImageIcon, Video, Wand2 } from 'lucide-react';
import type {
  ArtifactVersionView,
  CreatorCommandType,
  R2VWorkbenchView,
...
import { getArtifactVersionMediaUrl, getGeneratedMediaUrl, searchRefs } from '@/api/creator';
...
function currentVersion(versions: ArtifactVersionView[], selectedId?: string | null): ArtifactVersionView | null {
  return versions.find((version) => version.id === selectedId || version.selected) ?? null;
}

function referenceSetOf(view: R2VWorkbenchView, sourceRef: string): ReferenceSet {
  const sets = view.inputReferenceBindings.find((item) => item.sourceRef === sourceRef)?.referenceSets ?? [];
  if (sets.includes('storyboard') && sets.includes('video')) return 'storyboard_and_video';
...
export default function R2VWorkbench({
  ...
  const [activeAction, setActiveAction] = useState<CreatorCommandType | null>(null);
  const [storyboardPrompt, setStoryboardPrompt] = useState(view.storyboardPrompt || '');
  const [videoPrompt, setVideoPrompt] = useState(view.videoPrompt || '');
  const selectedStoryboard = currentVersion(view.storyboardVersions, view.selectedStoryboardVersionId);
  const selectedVideo = currentVersion(view.videoVersions, view.selectedVideoVersionId);
  const [viewedSbId, setViewedSbId] = useState<string | null>(null);
  const [viewedVideoId, setViewedVideoId] = useState<string | null>(null);
  const [refOptions, setRefOptions] = useState<RefSearchItem[]>(view.resolvedRefs);
  ...
  useEffect(() => setStoryboardPrompt(view.storyboardPrompt || ''), [view.storyboardPrompt]);
  ...
  useEffect(() => {
    ...
    if (view.storyboardVersions.some((version) => version.id === focusVersion)) setViewedSbId(focusVersion);
    if (view.videoVersions.some((version) => version.id === focusVersion)) setViewedVideoId(focusVersion);
  ...
  const effectiveSbId = viewedSbId ?? selectedStoryboard?.id ?? view.storyboardVersions.at(-1)?.id ?? null;
  const effectiveVideoId = viewedVideoId ?? selectedVideo?.id ?? view.videoVersions.at(-1)?.id ?? null;
  const viewedStoryboard = view.storyboardVersions.find((version) => version.id === effectiveSbId) ?? null;
  const viewedVideo = view.videoVersions.find((version) => version.id === effectiveVideoId) ?? null;
  const videoTask = [...tasks]
    .filter((task) => task.kind === 'r2v_generation' && task.targetRef === `unit:${view.unit.id}`)
    .sort((left, right) => Date.parse(right.updatedAt || right.createdAt || '') - Date.parse(left.updatedAt || left.createdAt || ''))[0];
  const videoGenerating = videoTask?.status === 'RUNNING' || videoTask?.status === 'QUEUED';
  const videoFailed = videoTask && ['FAILED', 'CANCELLED', 'QUARANTINED'].includes(videoTask.status);
  const videoTaskMessage = (() => {
    if (!videoTask) return '';
    if (videoGenerating) return '任务已提交，等待最新状态';
    const detail = videoTask.error?.message || videoTask.error?.detail || videoTask.error?.code;
  ...
  const bindingFor = (sourceRef: string) => view.inputReferenceBindings.find((item) => item.sourceRef === sourceRef);
  const refsForField = (field: ReferenceField) => [
    ...new Set(view.inputReferenceBindings.filter((item) => item.field === field).map((item) => item.sourceRef)),
  ];
  const selectOptionsFor = (field: ReferenceField) => {
    ...
    return [...byRef.values()].map((item) => ({ value: item.ref, label: item.name }));
  };

  const runAction = async (type: CreatorCommandType, args: Record<string, unknown> = {}) => {
  ...
  const bindReference = (field: ReferenceField, sourceRef: string) => submit(
    'BIND_REFERENCE',
    `unit:${view.unit.id}`,
  ...
  const unbindReference = (field: ReferenceField, sourceRef: string) => submit(
    'UNBIND_REFERENCE',
    `unit:${view.unit.id}`,
    ...
    <div className="flex h-full flex-col overflow-hidden bg-[var(--color-bg-layout)]">
      <div className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-[var(--color-border)] bg-[var(--color-bg-primary)]/60 px-5 py-3 backdrop-blur">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <div className="min-w-0">
            <h2 className="truncate text-sm font-semibold text-[var(--color-text-primary)]">
              视频方案 / {`${String(sectionNumber).padStart(2, '0')} ${sectionTitle}`} / {unitDisplayName(view.unit)} / 制作工作台
            </h2>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Tooltip title={view.blockers.length > 0 ? `Agent 将在计划阶段校验：${view.blockers.join('；')}` : undefined}>
            <Button
              size="small"
              type="primary"
              ...
              onClick={() => void runAction('GENERATE_R2V_VIDEO', {
                storyboardArtifactVersionId: selectedStoryboard?.artifactVersionId,
              })}
              className="!text-xs"
            >
      ...
      <div className="grid min-h-0 flex-1 gap-4 p-4 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="min-h-0 space-y-3 overflow-y-auto pr-1">
          <Panel
            ...
            badge={(
              <span className={`flex items-center gap-1 text-[11px] font-medium ${overLimit ? 'text-[var(--color-danger)]' : 'text-[var(--color-text-tertiary)]'}`}>
                {overLimit && <AlertTriangle className="h-3 w-3" />}
                合计 {totalDuration}s / 上限 {view.providerConstraints.maxDuration}s
              </span>
            )}
          >
            <ShotList
              ...
              unitId={view.unit.id}
              sectionId={sectionId}
              onUpsert={(shot) => submit('UPSERT_SHOT', `unit:${view.unit.id}`, {
                shot: {
                  id: shot.id,
              ...
              onDelete={(shot) => submit('DELETE_SHOT', `shot:${shot.id}`, {}, shot.targetVersion)}
            />
          </Panel>
          ...
          <div className="scroll-mt-4">
            ...
            >
              <div className="space-y-3">
                {viewedStoryboard && (
                  reviewGroupForArtifact(manifest, viewedStoryboard)?.decision === 'PENDING'
                  ...
                  <div className="flex items-center justify-between rounded-lg border border-[var(--color-warning)]/25 bg-[var(--color-warning-soft)] px-2.5 py-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      <Button size="small" type="primary" onClick={() => void acceptArtifact(viewedStoryboard)} className="!text-[11px]">接受</Button>
                      {reviewGroupForArtifact(manifest, viewedStoryboard)?.decision === 'PENDING' && (
                        <Button size="small" onClick={() => void undoArtifact(viewedStoryboard)} className="!text-[11px]">撤销</Button>
                      )}
                    </div>
                  </div>
                )}
                <div
                  ...
                  data-creator-path={projectJsonPointer('production', 'units_by_id', view.unit.id, 'storyboard_prompt')}
                  data-creator-field-label={`${unitDisplayName(view.unit)} · 分镜图 Prompt`}
                >
                  <p className="mb-1 text-[11px] font-medium text-[var(--color-text-tertiary)]">分镜图 Prompt</p>
                  <TextArea
                ...
                <div
                  ...
                  data-creator-path={projectJsonPointer('production', 'units_by_id', view.unit.id, 'video_prompt')}
                  data-creator-field-label={`${unitDisplayName(view.unit)} · 视频 Prompt`}
                >
                  <p className="mb-1 text-[11px] font-medium text-[var(--color-text-tertiary)]">视频 Prompt</p>
                  <TextArea
                    value={videoPrompt}
                    onChange={(event) => {
        ...
        <aside className="min-h-0 space-y-3 overflow-y-auto pr-1">
              ...
              <div className="space-y-2">
                {viewedVideo ? (
                  ...
                  <div className="flex h-32 flex-col items-center justify-center gap-1.5 rounded-lg border border-dashed border-[var(--color-border)] text-xs text-[var(--color-text-tertiary)]">
                    {videoGenerating ? (
                      <>
                        <span className="font-medium text-[var(--color-warning)]">R2V 任务生成中…</span>
                        <span>{videoTaskMessage}</span>
                        <Button
                          size="small"
                          onClick={() => void Promise.all([refreshTasks(projectId), reload()])}
                          className="!text-[11px]"
                        >
                          手动刷新
                        </Button>
                      ...
                      <span className="px-3 text-center text-[var(--color-danger)]">{videoTaskMessage}</span>
                    ) : (
                      '尚未生成视频'
                    )}
                  </div>
          ...
          <Panel title={`输入引用（${refCount}）`}>
            {inputRefs.length === 0 ? (
              <p className="text-xs text-[var(--color-text-tertiary)]">暂无引用资产。</p>
            ) : (
                ...
                {inputRefs.map((ref) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1 truncate text-xs font-medium text-[var(--color-accent)]">@{ref.name}</span>
                    </button>
                  );
                })}
              </div>
          ...
          <Panel title="资产绑定">
            <div className="space-y-3">
              <div>
                <p className="mb-1 text-[11px] font-medium text-[var(--color-text-tertiary)]">场景</p>
                <Select
                  size="small"
                  className="!w-full"
              ...
              <div>
                <p className="mb-1 text-[11px] font-medium text-[var(--color-text-tertiary)]">角色</p>
                <Select
                  size="small"
                  mode="multiple"
                  className="!w-full"
                  value={refsForField('characters')}
                  onChange={(value) => changeMultipleReferences('characters', value)}
                  onDropdownVisibleChange={(open) => { if (open) void loadReferenceOptions(); }}
                  loading={!referenceOptions && loadingRefs}
                  placeholder="选择角色"
              ...
              <div>
                <p className="mb-1 text-[11px] font-medium text-[var(--color-text-tertiary)]">道具</p>
                <Select
                  size="small"
                  mode="multiple"
                  className="!w-full"
              ...
              <div>
                <p className="mb-1 text-[11px] font-medium text-[var(--color-text-tertiary)]">素材</p>
                <Select
                  size="small"
                  mode="multiple"
                  className="!w-full"
                  value={refsForField('sources')}
                  onChange={(value) => changeMultipleReferences('sources', value)}
                  onDropdownVisibleChange={(open) => { if (open) void loadReferenceOptions(); }}
                  loading={!referenceOptions && loadingRefs}
                  placeholder="选择素材"
              ...
              {view.blockers.length > 0 && (
                <div className="rounded-lg border border-[var(--color-warning)]/25 bg-[var(--color-warning-soft)] px-2.5 py-2">
                  {view.blockers.slice(0, 4).map((blocker) => (
                    <p key={blocker} className="text-[11px] text-[var(--color-warning)]">· {blocker}</p>
                  ))}
                </div>
              )}
            </div>
