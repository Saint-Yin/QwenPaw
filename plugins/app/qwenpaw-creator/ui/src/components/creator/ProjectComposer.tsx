...
import { FileText, Film, FolderOpen, Link2, Paperclip, Rocket, X } from 'lucide-react';
...
type AttachmentDraft =
  | { kind: 'file'; id: string; file: File; source: 'file' | 'folder'; relativePath?: string }
...
const SCENARIO_TERMS: Record<CreatorScenario, { description: string }> = {
  general: { description: '例：以"我在马路边，捡到一分钱"的儿歌歌词为故事内容，做一个1分钟视频。' },
  short_drama: { description: '例：霸道总裁短剧。要突出快节奏，强化戏剧冲突，故事要有高频反转，最后要有个美好结局。' },
  video_edit: { description: '例：把我上传的这段黑白默片老电影，剪辑成一段新视频。新视频长度为30秒。新视频要用彩色图像替换到原有的黑白图像。新视频要给人物加上合适的中文配音。' },
};
...
async function waitForTask(projectId: string, taskId: string): Promise<string[]> {
  for (;;) {
    const task = await getTask(projectId, taskId);
    if (terminal.has(task.status)) {
      if (task.status !== 'SUCCEEDED') {
        throw new Error(taskErrorMessage(task.error, `素材处理失败（${task.status}）`));
      }
...
function chipLabel(att: AttachmentDraft): { tag: string; name: string } {
  ...
  if (att.source === 'folder') return { tag: 'DIR', name: att.relativePath || att.file.name };
  ...
  return { tag: suffix ? suffix.toUpperCase().slice(0, 4) : 'DOC', name: att.file.name };
}
...
export function ProjectComposer({ open, onClose }: ProjectComposerProps) {
  ...
  const addFiles = useCallback((files: FileList | File[], source: 'file' | 'folder' = 'file') => {
  ...
  const addUrl = () => {
    ...
    setAttachments((prev) => [...prev, { kind: 'url', id: `att-${Date.now()}`, url }]);
    setUrlDraft('');
  };
  ...
  const handleLaunch = async () => {
    ...
    setLaunching(true);
    try {
      const resolvedProjectName = projectName.trim() || projectNameFromDescription(projectDescription);
      const projectPayload = {
        ...
        initialGoal: attachments.length === 0 && !pendingUrl
      ...
      const fileAttachments = attachments.filter((att): att is Extract<AttachmentDraft, { kind: 'file' }> => att.kind === 'file');
      const folderFiles = fileAttachments.filter((att) => att.source === 'folder');
      const looseFiles = fileAttachments.filter((att) => att.source !== 'folder');
      const committedUrlAttachments = attachments.filter((att): att is Extract<AttachmentDraft, { kind: 'url' }> => att.kind === 'url');
      const urlAttachments = pendingUrl
        ? [...committedUrlAttachments, { kind: 'url' as const, id: `att-${Date.now()}`, url: pendingUrl }]
      ...
      if (folderFiles.length > 0) {
        try {
          ...
          const folderKey = files.map((file) => {
          ...
          for (;;) {
            const view = await getAssetImport(project.projectId, accepted.importId);
            if (terminal.has(view.status)) {
              if (view.status !== 'SUCCEEDED') throw new Error(`文件夹导入失败（${view.status}）`);
              refs.push(...view.items.map((item) => `asset-version:${item.assetVersionId}`));
              if (view.failures.length > 0) {
                message.warning(`文件夹导入完成，跳过 ${view.failures.length} 个文件`);
              }
              break;
            }
            await wait(800);
      ...
      for (const att of looseFiles) {
        try {
          const accepted = await ingestAssetFile(
            ...
            att.file,
            'NONE',
            requestIdFor(`file:${att.file.name}:${att.file.size}:${att.file.lastModified}`),
          );
          ...
          message.warning(`附件「${att.file.name}」入库失败：${(error as Error).message}`);
        }
      }
      ...
      for (const att of urlAttachments) {
        ...
        try {
          const accepted = await ingestAssetValue(
            project.projectId,
            { kind: 'url', name: att.url, value: att.url, postIngestAction: 'NONE' },
          ...
          message.warning(`附件「${att.url}」入库失败：${(error as Error).message}`);
        }
      }

      const uniqueRefs = [...new Set(refs)].filter((ref) => ref.startsWith('asset-version:'));
      ...
      const clientMessageId = initialMessageRequests.current.get(messageSignature) ?? newClientId('initial-message');
    ...
    >
      <div className="pt-2">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            <div className="flex items-center gap-1 rounded-full border border-[var(--color-border)] bg-[var(--color-bg-secondary)] p-1">
              {MODES.map((mode) => (
                <Tooltip key={mode.key} title={mode.enabled ? undefined : '即将推出'}>
                  <button
                    type="button"
        ...
        <div className="mt-4 flex flex-wrap items-center gap-1.5">
          <span className="text-[11px] font-medium text-[var(--color-text-tertiary)]">视频场景</span>
          ...
          {!isVideoEdit && (
            <div className="ml-auto flex items-center gap-2">
              <Select
                ...
                classNames={{
                  content: '!mr-0 !w-full !-translate-x-0.5 !justify-center !text-center !text-[11px] !font-semibold !text-[var(--color-text-secondary)]',
                  suffix: '!absolute !right-2 !text-[10px] !text-[var(--color-text-tertiary)]',
                  popup: {
                    listItem: '!text-[11px] !font-semibold !text-[var(--color-text-secondary)]',
                  },
                }}
              />
              <Select
        ...
        <div
          ...
          onDrop={(e) => {
            e.preventDefault();
            setDragOver(false);
            if (e.dataTransfer.files.length > 0) addFiles(e.dataTransfer.files, 'file');
          }}
        >
          <Input
            value={projectName}
            ...
            <div className="flex flex-wrap gap-1.5 px-4 pb-2">
              {attachments.map((att) => {
                ...
                return (
                  ...
                  >
                    <b className="shrink-0 text-[10px] text-[var(--color-accent)]">{tag}</b>
