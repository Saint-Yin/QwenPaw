...
import { SettingOutlined, EyeOutlined, PictureOutlined, VideoCameraOutlined, AudioOutlined, CloudOutlined } from '@ant-design/icons';
...
export default function ModelBadges() {
  ...
  const status = (type: ModelType): 'on' | 'off' | 'none' => {
    ...
    if (!item) return 'none';
    if (type === 'vlm' && config.vlm.use_llm && config.llm.model_name) return item.enabled ? 'on' : 'none';
          ...
          return (
            <div
              ...
              style={{
                ...
                fontSize: 11,
                fontWeight: 500,
                color: st === 'on' ? 'var(--color-text-primary)' : 'var(--color-text-tertiary)',
                background: st === 'on' ? 'var(--color-success-soft)' : 'transparent',
            ...
            >
              <span
                style={{
                  ...
                  height: 5,
                  borderRadius: '50%',
                  background: st === 'on' ? 'var(--color-success)' : st === 'off' ? 'var(--color-danger)' : 'var(--color-border)',
                  flexShrink: 0,
                }}
              />
              {meta.icon}
