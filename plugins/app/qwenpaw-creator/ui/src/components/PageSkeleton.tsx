        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {[1, 2, 3, 4, 5, 6].map(i => (
            <div key={i} className="surface overflow-hidden">
              <Skeleton.Image active style={{ width: '100%', height: 200 }} className="!w-full !h-[200px]" />
              <div className="p-4">
          ...
          <div className="space-y-4">
            {[1, 2, 3, 4, 5].map(i => (
              <div key={i} className="py-3 px-2">
                <Skeleton active paragraph={{ rows: 0 }} title={{ width: `${60 + i * 5}%` }} />
              </div>
            ))}
          </div>
        </div>
    ...
    return (
      <div className="h-full flex flex-col bg-[var(--color-bg-layout)]">
        <div className="flex h-14 items-center gap-3 border-b border-[var(--color-border)] bg-[var(--color-bg-primary)] px-4">
          {[1, 2, 3].map(i => (
            <Skeleton.Button key={i} active size="small" style={{ width: 32 }} />
          ))}
        </div>
