...
export type ReviewPresentationCategory = 'change' | 'storyboard' | 'video' | 'asset';
...
function groupOperations(group: ReviewDecisionGroup, operations: ReviewOperation[]): ReviewOperation[] {
  const ids = operationSet(group);
  return operations.filter((operation) => ids.has(operation.id));
}

function groupComparison(group: ReviewDecisionGroup, comparisons: MediaComparison[]): MediaComparison | undefined {
  const ids = operationSet(group);
  return comparisons.find((comparison) => comparison.operationIds.some((id) => ids.has(id)));
}
...
function pathFieldLabel(path: string | undefined): string | null {
  if (!path) return null;
  const semantic = path.match(/^(?:section|unit):[^/]+(?:\/shot:[^/]+)?\/([^/]+)$/);
...
function safeGroupTitle(title: string): string | null {
  if (!title || /(?:^|\s)(?:asset|unit|section|project|post):[^\s·]+/.test(title)) return null;
  if (title.includes('/') || title.includes('\\')) return null;
  return title;
}

function assetName(assetId: string | null, assets: AssetLibraryView | null): string | null {
  if (!assetId || !assets) return null;
  return assets.attachedSources.find((item) => item.assetId === assetId)?.name
...
function assetNameFromPath(path: string): string | null {
  const segment = path.split('/').find((part, index, values) => values[index - 1] === 'sources');
...
function reviewLocation(
  ...
  const semantic = operations.map(reviewFieldForOperation).find(Boolean) ?? null;
  ...
  const assetId = operations.map((item) => refId(item.targetRef, 'asset')).find(Boolean) ?? null;
  const field = pathFieldLabel(semantic ?? undefined)
    ?? operations.map((item) => pathFieldLabel(item.path)).find(Boolean)
    ?? (category === 'storyboard' ? '分镜图' : category === 'video' ? '视频' : category === 'asset' ? '素材信息' : null);
  ...
  if (comparison?.targetRef === 'post:final' || operations.some((item) => item.targetRef === 'post:final')) {
    return ['视频方案', '最终合成', ...(field ? [field] : [])];
  }

  const unitMatch = unitId
    ? plan?.sections.flatMap((section) => section.units.map((unit) => ({ section, unit })))
      .find((item) => item.unit.id === unitId)
    : undefined;
  const section = unitMatch?.section
    ?? (sectionId && sectionId !== 'final' ? plan?.sections.find((item) => item.id === sectionId) : undefined);
  const unit = unitMatch?.unit;
  const shot = shotId ? unit?.shots.find((item) => item.id === shotId) : undefined;
  const segments = ['视频方案'];
  if (section) segments.push(`${paddedNumber(section.number)} ${section.title || '段落'}`);
  if (unit) segments.push(`${paddedNumber(unit.number)} ${unit.title || '剪辑片段'}`);
  if (shotId) segments.push(shot ? `镜头 ${paddedNumber(shot.number)}` : '镜头');
  if (!section && !unit) {
    const fallback = safeGroupTitle(group.title);
    if (fallback) segments.push(fallback);
    else if (operations.some((item) => item.targetRef.startsWith('project:'))) segments.push('项目设置');
  }
...
function presentationCategory(
  ...
  if (version?.versionKind === 'asset' && operations.some((operation) => (
    operation.targetRef.startsWith('asset:') || operation.path?.includes('/refs/sources/')
  ))) return 'asset';
  ...
  if (paths.some((path) => /(?:rendered-video|video)\.ref$/i.test(path))) return 'video';
  if (paths.some((path) => path.startsWith('sources/') || path.includes('/understanding/'))) return 'asset';
  return 'change';
}

function presentationDetail(category: ReviewPresentationCategory, operations: ReviewOperation[]): string {
  if (category === 'storyboard') return '生成的分镜图待确认，请前往对应位置查看。';
...
export function presentReviewGroup(
  ...
  const locationSegments = reviewLocation(group, operations, category, plan, assets, comparison);
  const mediaVersion = comparison?.after ?? comparison?.before ?? comparison?.candidates[0];
  return {
    group,
    category,
    ...
    mediaVersion,
    showPreview: mediaVersion?.mediaType === 'image' || mediaVersion?.mediaType === 'video',
  };
}
...
export function groupReviewPresentations(items: ReviewGroupPresentation[]): Array<{
  id: ReviewPresentationCategory;
  label: string;
  items: ReviewGroupPresentation[];
}> {
  const order: ReviewPresentationCategory[] = ['change', 'storyboard', 'video', 'asset'];
  return order
    .map((id) => ({ id, label: CATEGORY_LABELS[id], items: items.filter((item) => item.category === id) }))
