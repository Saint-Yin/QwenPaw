...
import type { ExecutionAuthorizationApproval, ExecutionAuthorizationView } from '@/contracts/creator';
...
export function authorizationDetail(authorization: ExecutionAuthorizationView): string {
  const messageText = authorization.scope.message;
  if (typeof messageText === 'string' && messageText.trim()) return messageText;
  const operation = typeof authorization.scope.operation === 'string'
    ? authorization.scope.operation
    : '高成本媒体执行';
  return `${operation} · ${authorization.targetRef} · ${authorization.provider}/${authorization.model}`;
}

export default function ExecutionAuthorizationCard({ authorization }: { authorization: ExecutionAuthorizationView }) {
  ...
  return (
    <article data-execution-authorization-card={authorization.id} className="rounded-xl border border-[var(--color-warning)]/50 bg-[var(--color-warning-soft)]/40 p-2.5">
      <div className="flex items-start gap-2.5">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            ...
            <span className="min-w-0 flex-1 truncate text-xs font-semibold text-[var(--color-text-primary)]">端到端生产等待确认</span>
          </div>
          <p className="mt-0.5 line-clamp-2 text-[11px] leading-4 text-[var(--color-text-tertiary)]">{authorizationDetail(authorization)}</p>
        </div>
      </div>
      <div className="mt-2 flex flex-wrap items-center gap-1.5">
        <button type="button" disabled={busy} onClick={() => void continueRun()} className={`flex-1 ${BUTTON_PRIMARY}`}>继续</button>
        <button type="button" disabled={busy} onClick={() => void cancelRun()} className={`flex-1 ${BUTTON_GHOST}`}>取消</button>
      </div>
    </article>
  );
}
