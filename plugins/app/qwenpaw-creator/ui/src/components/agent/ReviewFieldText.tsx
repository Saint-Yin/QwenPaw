...
const ReviewFocusPulseContext = createContext<{ field: string | null; pulse: string | null }>({ field: null, pulse: null });
...
export function ReviewFocusPulseProvider({
  ...
  return <ReviewFocusPulseContext.Provider value={{ field, pulse }}>{children}</ReviewFocusPulseContext.Provider>;
}
...
export function diffSegments(before: string, after: string): DiffSeg[] {
  ...
  if (m === 0 || n === 0 || m * n > MAX_DIFF_CHAR_CELLS) {
    push('del', left.join(''));
    push('add', right.join(''));
    push('equal', suffix ? beforeTokens.slice(beforeTokens.length - suffix).join('') : '');
    return segments;
  }
...
function semanticField(field: string): SemanticField | null {
  ...
  if (shot) return { scope: 'shot', ownerId: shot[1], shotId: shot[2], field: shot[3] };
  const entity = /^(section|unit):([^/]+)\/([^/]+)$/.exec(field);
  if (!entity) return null;
  return { scope: entity[1] as 'section' | 'unit', ownerId: entity[2], field: entity[3] };
}

function pathSegmentMatchesId(segment: string | undefined, id: string): boolean {
  ...
  return decoded === id || decoded.includes(`--${id}--`) || decoded.endsWith(`--${id}`);
}

function pathHasEntity(path: string, collection: 'sections' | 'units' | 'shots', id: string): boolean {
  const parts = path.split('/').filter(Boolean);
  const index = parts.lastIndexOf(collection);
  return index >= 0 && pathSegmentMatchesId(parts[index + 1], id);
}

function entityIdFromPath(path: string, collection: 'sections' | 'units' | 'shots'): string | null {
  const parts = path.split('/').filter(Boolean);
  const index = parts.lastIndexOf(collection);
...
function targetMatches(operation: ReviewOperation, targetRef: string): boolean {
  return operation.targetRef === targetRef || operation.targetRef.startsWith(`${targetRef}/`);
}
 ...
 */
export function reviewFieldMatchesOperation(field: string, operation: ReviewOperation): boolean {
  if (operation.path === field || operation.uiLocator?.field === field) return true;
  const semantic = semanticField(field);
  if (!semantic) return false;
  const paths = [operation.path, operation.uiLocator?.field]
...
function refId(targetRef: string, kind: 'section' | 'unit' | 'shot'): string | null {
  const prefix = `${kind}:`;
...
export function reviewFieldForOperation(operation: ReviewOperation): string | null {
  ...
  const shotId = refId(operation.targetRef, 'shot') ?? entityIdFromPath(path, 'shots');
  if (shotId && path.includes('/shots/')) {
    const unitId = refId(operation.targetRef, 'unit') ?? entityIdFromPath(path, 'units');
    if (!unitId) return null;
    const field = endsWithAny(path, ['description.md']) ? 'description'
      : endsWithAny(path, ['camera-source.md', 'camera.md']) ? 'cameraDescription'
        : endsWithAny(path, ['dialogue.md']) ? 'dialogue'
          : endsWithAny(path, ['duration.txt']) ? 'duration'
            : null;
  ...
  if (unitId && path.includes('/units/')) {
    const field = endsWithAny(path, ['production/edit/intent.md']) ? 'goal'
      : endsWithAny(path, ['production/r2v/storyboard/prompt.md']) ? 'storyboardPrompt'
      : endsWithAny(path, ['production/r2v/video/prompt.md']) ? 'videoPrompt'
        : endsWithAny(path, ['refs/scene.ref']) ? 'sceneRef'
          : endsWithAny(path, ['title.txt']) ? 'title'
            : endsWithAny(path, ['narrative.md']) ? 'storyText'
              : endsWithAny(path, ['duration.txt']) ? 'duration'
                : null;
    return field ? `unit:${unitId}/${field}` : null;
  }

  const sectionId = refId(operation.targetRef, 'section') ?? entityIdFromPath(path, 'sections');
  if (!sectionId) return null;
  const field = endsWithAny(path, ['title.txt']) ? 'title'
    : endsWithAny(path, ['summary.md']) ? 'summary'
      : endsWithAny(path, ['pacing.md']) ? 'pacing'
        : endsWithAny(path, ['narrative.md']) ? 'narrative'
      : endsWithAny(path, ['script.md']) ? 'script'
        : endsWithAny(path, ['constraints.md']) ? 'constraints'
          : endsWithAny(path, ['transition.md']) ? 'transitionNote'
            : endsWithAny(path, ['duration-budget.txt']) ? 'durationBudget'
              : endsWithAny(path, ['dialogue.md']) ? 'dialogue'
                : null;
...
function DiffSegments({ segs }: { segs: DiffSeg[] }) {
  return <>
    {segs.map((seg, index) => {
      ...
      if (seg.type === 'del') return <span key={index} className="agent-diff-del">{seg.text}</span>;
      return <span key={index} className="agent-diff-add">{seg.text}</span>;
    })}
  </>;
}

function InlineReviewActions({ field, group }: { field: string; group: ReviewDecisionGroup }) {
  const [busy, setBusy] = useState<'ACCEPT' | 'REJECT' | null>(null);
  const decide = async (decision: 'ACCEPT' | 'REJECT') => {
    const current = useReviewManifestStore.getState().manifest?.decisionGroups
      .find((item) => item.id === group.id && item.decision === 'PENDING');
    if (!current) return;
    setBusy(decision);
    try {
  ...
  return (
    <span className="ml-2 inline-flex items-center gap-1 align-middle" data-review-inline-actions>
      <button
        type="button"
        disabled={busy !== null}
...
export default function ReviewFieldText({ field, children }: { field: string; children: ReactNode }) {
  ...
  useEffect(() => {
    if (pulse && segs) targetRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
  }, [pulse, segs]);
...
export function ReviewDiffPreview({
  ...
  return (
    <div
      ...
      {...focusAttrs}
      data-review-pulse={pulse || undefined}
      className={`mt-2 rounded-lg border border-[var(--color-accent)]/25 bg-[var(--color-accent-soft)]/45 px-2.5 py-2${pulse ? ' review-flash-query' : ''}`}
      data-agent-diff-preview
    >
      {label && <p className="mb-1 text-[10px] font-semibold text-[var(--color-text-tertiary)]">{label}</p>}
