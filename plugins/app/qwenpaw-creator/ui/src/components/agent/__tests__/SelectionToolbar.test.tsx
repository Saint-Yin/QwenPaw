...
describe('SelectionToolbar project locator', () => {
  ...
  it('opens for a Creator field and keeps its semantic locator and exact project.json path', async () => {
    ...
    const path = '/production/units_by_id/u1/plan/storyboard/items/panel-1/description';
    const { container } = render(
      <>
        <p data-creator-field={field} data-creator-path={path} data-creator-field-label="VLM 分镜 1 · 描述">
          源视频关键帧
        </p>
        <SelectionToolbar />
      </>,
    ...
    expect(useAgentDockUiStore.getState()).toMatchObject({ open: true, tab: 'conversation' });
  });
});
