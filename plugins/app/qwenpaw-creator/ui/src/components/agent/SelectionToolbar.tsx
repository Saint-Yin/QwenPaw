...
import { useAgentDockUiStore, type SelectionAttachment } from '@/store/agentDockUiStore';
...
function locateField(element: Element | null): { field: string | null; path: string | null; label: string } {
  const region = element?.closest?.('[data-creator-field]') as HTMLElement | null;
...
function shouldIgnoreSelectionIn(element: Element | null): boolean {
  return Boolean(element?.closest?.('[data-agent-dock], [data-agent-status-bar]'));
}
...
export default function SelectionToolbar() {
  ...
  useEffect(() => {
    const compute = (clientX: number, clientY: number): ToolbarState | null => {
      ...
      if (selection && selection.rangeCount > 0 && !selection.isCollapsed) {
        ...
        if (text.trim()) {
          ...
          const label = region?.getAttribute('data-creator-field-label') || '选中文本';
          if (!region || !field) return null;
          if (!region.contains(range.startContainer) || !region.contains(range.endContainer)) return null;
    ...
    const onMouseUp = (event: MouseEvent) => {
      ...
      window.setTimeout(() => setState(compute(event.clientX, event.clientY)), 0);
    };
  ...
  return (
    <div
      ref={barRef}
      style={{ position: 'fixed', top: Math.max(8, y - 46), left: x, transform: 'translateX(-50%)', zIndex: 50 }}
