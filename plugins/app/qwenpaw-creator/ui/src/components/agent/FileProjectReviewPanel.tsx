      ...
      {long ? (
        <details>
          <summary className="cursor-pointer list-none text-[10px] leading-4 text-[var(--color-text-secondary)]">
            <span className="line-clamp-3 whitespace-pre-wrap break-all">{preview}</span>
            <span className="mt-1 inline-block text-[var(--color-accent)]">展开完整值</span>
          </summary>
...
function operationLocation(operation: FileProjectReviewOperation): string {
  return operation.json_pointer
    ?? (operation.file_id ? `file:${operation.file_id}` : null)
    ?? operation.target_ref
    ?? 'unknown';
}

export default function FileProjectReviewPanel({ projectId }: { projectId: string }) {
  ...
  const decisionInFlight = useFileProjectReviewStore((state) => state.decisionInFlight);
  ...
  if (storeProjectId !== projectId || !review || review.status !== 'PENDING') return null;
  const pending = review.operations.filter((operation) => operation.decision === 'PENDING');
  ...
  const submit = async (
    ...
    setLocalBusy(true);
    try {
      await decide(projectId, operations.map((operation) => ({
        operation_id: operation.operation_id,
        decision,
      })));
      message.success(decision === 'ACCEPT'
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 truncate font-mono text-[9px] text-[var(--color-text-tertiary)]" title={review.round_id}>
            round {review.round_id} · generation {review.baseline_generation} → {review.candidate_generation}
          </p>
        </div>
      ...
      {syncError && (
        <p role="alert" className="mt-2 rounded-md bg-[var(--color-warning-soft)] px-2 py-1 text-[10px] text-[var(--color-warning)]">
