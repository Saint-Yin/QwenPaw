...
export function getSpecialistRun(projectId: string, runId: string): Promise<SpecialistRunView> {
  return creatorRequest(`${project(projectId)}/specialist-runs/${encodeURIComponent(runId)}`);
}
...
export function getTask(projectId: string, taskId: string): Promise<TaskView> {
  return creatorRequest(`${project(projectId)}/tasks/${encodeURIComponent(taskId)}`);
}

export function cancelTask(projectId: string, taskId: string): Promise<TaskView> {
  const id = newClientId('cancel-task');
  return creatorRequest(`${project(projectId)}/tasks/${encodeURIComponent(taskId)}/cancel`, {
    method: 'POST',
...
export function listFileExecutionAuthorizations(
  ...
  return creatorRequest(
    `${project(projectId)}/execution-authorizations?status=${encodeURIComponent(status)}`,
  );
}
...
export function approveFileExecutionAuthorization(
  ...
  return creatorRequest(
    `${project(projectId)}/execution-authorizations/${encodeURIComponent(authorizationId)}/approve`,
    {
      method: 'POST',
...
export function declineFileExecutionAuthorization(
  ...
  return creatorRequest(
    `${project(projectId)}/execution-authorizations/${encodeURIComponent(authorizationId)}/decline`,
    {
      method: 'POST',
      headers: { 'Idempotency-Key': clientRequestId },
      body: jsonBody(request),
