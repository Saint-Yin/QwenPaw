...
describe('file-native Project Review API', () => {
  ...
  it('treats 204 and conditional 304 as protocol results', async () => {
    ...
    await expect(getActiveFileProjectReview('p1')).resolves.toEqual({ kind: 'empty' });
    await expect(getActiveFileProjectReview('p1', '"token-1"')).resolves.toEqual({
      kind: 'not_modified',
      etag: '"token-1"',
    });
  ...
  it('posts operation decisions with camel request metadata and an Idempotency-Key', async () => {
    ...
    await expect(decideFileProjectReview('p1', 'review-1', {
      decisionId: 'decision-1',
      decisionToken: 'token-1',
      decisions: [{ operation_id: 'operation-1', decision: 'ACCEPT' }],
    }, 'idempotency-1')).resolves.toMatchObject({ status: 'RESOLVED' });

    const [url, init] = fetchMock.mock.calls[0];
    expect(url).toBe('/api/creator/projects/p1/runtime/reviews/review-1/decisions');
