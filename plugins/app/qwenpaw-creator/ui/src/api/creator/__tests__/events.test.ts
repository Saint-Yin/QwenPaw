...
describe('Creator event stream', () => {
  it('consumes every file-native Runtime event as a named SSE event', () => {
    ...
    const sources = (globalThis as unknown as {
      __testEventSources: Array<{ emit: (type: string, value: unknown) => void }>;
    }).__testEventSources;
    ...
    expect(onEvent.mock.calls.map(([event]) => event.type)).toEqual(FILE_RUNTIME_EVENT_TYPES);
    stream.close();
  });
});
