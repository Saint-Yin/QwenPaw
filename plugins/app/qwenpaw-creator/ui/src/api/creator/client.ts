...
export const CREATOR_API_BASE = '/api/creator';
...
function hostToken(): string | null {
  try {
    ...
    return parent?.QwenPaw?.host?.getApiToken?.() || current.QwenPaw?.host?.getApiToken?.() || null;
  } catch {
    return null;
  }
}
...
export async function creatorFetch(path: string, init: RequestInit = {}): Promise<Response> {
...
export async function creatorRequest<T>(path: string, init: RequestInit = {}): Promise<T> {
