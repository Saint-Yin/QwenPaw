...
function parseOperation(value: unknown): FileProjectReviewOperation {
  ...
  if (
    ...
    || !OPERATION_DECISIONS.has(item.decision as FileProjectReviewOperationDecision)
  ) {
...
export function parseFileProjectReview(value: unknown): FileProjectReviewRecord {
  const item = record(value);
  if (
    !item
  ...
  const hasPending = operations.some((operation) => operation.decision === 'PENDING');
  if (
