...
export function getCreatorSession(projectId: string): Promise<CreatorSessionResponse> {
  return creatorRequest(`/projects/${encodeURIComponent(projectId)}/session`);
}

export function listConversations(projectId: string, cursor?: number): Promise<ConversationPage> {
  const query = cursor ? `?cursor=${encodeURIComponent(cursor)}` : '';
  return creatorRequest(`/projects/${encodeURIComponent(projectId)}/conversations${query}`);
}
...
export function createConversation(
  ...
  return creatorRequest(`/projects/${encodeURIComponent(projectId)}/conversations`, {
    method: 'POST',
...
export function interruptCreator(projectId: string): Promise<{ creatorSessionId: string; status: string; stopRequested: boolean }> {
  const id = newClientId('interrupt');
  return creatorRequest(`/projects/${encodeURIComponent(projectId)}/interrupt`, {
    method: 'POST',
