...
import { isTechnicalControlText, isUserAuthorityMessage } from '@/lib/creatorMessagePresentation';

const conversationRetryIds = new Map<string, string>();

async function createStableConversation(projectId: string) {
  const requestId = conversationRetryIds.get(projectId) ?? newClientId('conversation');
...
export interface SubagentStreamTool {
  ...
  state?: string;
  taskId?: string;
  outputEvents: Array<{ seq: number; type: string; data: Record<string, unknown> }>;
}
...
interface CreatorSessionState {
  ...
  sendMessage: (request: Omit<SendCreatorMessageRequest, 'clientMessageId' | 'conversationId'> & {
    message?: string;
    clientMessageId?: string;
  }) => Promise<void>;
...
function mergeMessages(current: CreatorMessage[], incoming: CreatorMessage[]): CreatorMessage[] {
...
function eventStringArray(data: Record<string, unknown>, key: string): string[] | undefined {
  const value = data[key];
...
function subagentActivityBase(
  ...
  return {
    ...
    roleDisplayName: eventString(event.data, 'roleDisplayName')
      ?? eventString(event.data, 'displayName')
      ?? existing?.roleDisplayName,
    delegationText: eventString(event.data, 'delegationText')
      ?? eventString(event.data, 'task')
      ?? existing?.delegationText,
    targetRefs: eventStringArray(event.data, 'targetRefs') ?? existing?.targetRefs ?? [],
    firstEventSeq: runChanged ? event.seq : existing?.firstEventSeq ?? event.seq,
...
function subagentTerminalKind(event: CreatorEvent): SubagentActivity['terminalKind'] | undefined {
  const marker = eventString(event.data, 'marker');
  if (marker === 'SUCCESS' || marker === 'BLOCKED' || marker === 'FAILED') return marker;
...
function subagentSummary(event: CreatorEvent): string | undefined {
  for (const key of ['summaryText', 'summary', 'reason', 'cancelReason', 'staleReason']) {
...
function settleSubagentMessages(
  ...
  let changed = false;
  const messages = Object.fromEntries(Object.entries(activity.messages).map(([key, message]) => {
    if (
      message.runId !== runId
...
export const useCreatorSessionStore = create<CreatorSessionState>((set, get) => {
  ...
  let messageRefreshRunner: { epoch: number; promise: Promise<void> } | null = null;
  ...
  const sessionResponsePatch = (
    ...
    const currentProgressSeq = current.agentStatusBar?.progress.sourceEventSeq ?? -1;
    if (response.agentStatusBar.progress.sourceEventSeq >= currentProgressSeq) {
      patch.agentStatusBar = response.agentStatusBar;
    }
    return patch;
  };

  const waitForMessageRefreshRetry = (delay: number, epoch: number) => new Promise<boolean>((resolve) => {
    const waiter = {
      timer: 0,
      resolve,
    };
  ...
  const pullMessages = async (after?: number, expectedEpoch = messageRefreshEpoch) => {
    const initial = get();
    const projectId = initial.projectId;
    const conversationId = initial.activeConversationId;
    if (!projectId || !conversationId) return;
    let cursor = Math.max(0, after ?? initial.messages.at(-1)?.messageSeq ?? 0);
    while (expectedEpoch === messageRefreshEpoch) {
      ...
      set((state) => {
        if (state.projectId !== projectId || state.activeConversationId !== conversationId) return {};
        const existingMessageSeqs = new Set(state.messages.map((item) => item.messageSeq));
        const appendedUserCount = page.items.filter((item) => (
          !existingMessageSeqs.has(item.messageSeq) && isUserAuthorityMessage(item)
        )).length;
        ...
        return {
          ...
          queuedUi: state.queuedUi.filter((item) => !removable.has(item.clientMessageId)),
        };
      });
  ...
  const scheduleMessageRefresh = (after: number) => {
    ...
    if (pendingMessageRefresh?.epoch === epoch) {
      pendingMessageRefresh.after = Math.min(pendingMessageRefresh.after, after);
    } else {
      ...
      while (pendingMessageRefresh?.epoch === epoch) {
        ...
        for (let attempt = 0; attempt <= retryDelays.length; attempt += 1) {
          try {
            ...
            if (attempt === retryDelays.length || epoch !== messageRefreshEpoch) break;
            const current = await waitForMessageRefreshRetry(retryDelays[attempt], epoch);
            if (!current) break;
          }
        }
      }
  ...
  bootstrap: async (projectId) => {
    ...
    const preferredConversationId = sameProject ? state.activeConversationId : null;
    state.stream?.close();
    ...
    try {
      ...
      let activeConversationId = preferredConversationId
        && conversations.some((item) => item.conversationId === preferredConversationId)
      ...
      if (!activeConversationId) {
        ...
        conversations = [{ conversationId: created.conversationId, title: created.title, isDefault: false, createdAt: created.createdAt }];
      }
      const page = await listMessages(projectId, activeConversationId, 0, 50);
      if (
        bootstrapGeneration !== lifecycleGeneration
        || get().projectId !== projectId
      ) return;
      set((current) => {
        ...
        const messages = sameProject && activeConversationId === preferredConversationId
      ...
      const durableStream = openCreatorEvents(
        ...
        (event) => {
          ...
          if (flushTimer == null) flushTimer = window.setTimeout(flushEvents, 16);
        },
        () => set({ connected: false }),
      );
  ...
  setConversation: async (conversationId) => {
    ...
    try {
      ...
      if (get().projectId === projectId && get().activeConversationId === conversationId) {
        set({ messages: page.items, hasMoreMessages: page.nextAfter != null, loadingOlder: false });
      }
    } catch (error) {
      set({
        activeConversationId,
  ...
  loadOlderMessages: async () => {
    const { projectId, activeConversationId, messages, loadingOlder } = get();
    if (!projectId || !activeConversationId || loadingOlder) return;
    const latest = messages.at(-1)?.messageSeq ?? 0;
    set({ loadingOlder: true });
    try {
      const page = await listMessages(projectId, activeConversationId, latest, 50);
      set((state) => {
        const messages = mergeMessages(state.messages, page.items);
        return {
          messages,
  ...
  refreshSession: async () => {
    ...
    set((current) => sessionResponsePatch(projectId, response, generation, current));
  },
  ...
  sendMessage: async (input) => {
    ...
    if (!projectId || !session || !activeConversationId) throw new Error('Creator Session 尚未初始化');
    const text = input.message ?? input.content?.find((part) => part.type === 'text')?.text ?? '';
    ...
    const failedRetry = get().queuedUi.find((item) => (
      item.state === 'failed' && item.requestSignature === requestSignature
    ));
    const clientMessageId = input.clientMessageId ?? failedRetry?.clientMessageId ?? newClientId('message');
    set((state) => ({
      queuedUi: [
        ...state.queuedUi.filter((item) => item.clientMessageId !== clientMessageId),
    ...
    try {
      ...
      set((state) => ({
        queuedUi: state.queuedUi.map((item) => item.clientMessageId === clientMessageId
          ? { ...item, state: 'queued' }
          : item),
      }));
      if (accepted.appendState === 'appended') {
        ...
        set((state) => ({
          ...
          queuedUi: state.queuedUi.filter((item) => item.clientMessageId !== clientMessageId),
        }));
      }
    } catch (error) {
      set((state) => ({
  ...
  stopAllAgents: async () => {
    ...
    try {
      ...
      set((state) => ({
        stopping: false,
        session: sessionId && state.session?.id === sessionId
          && state.session.status === 'INTERRUPT_REQUESTED'
  ...
  ingestEvents: (incoming) => {
    ...
    set((current) => {
      ...
      incoming.forEach((event) => {
        if (event.projectId === current.projectId && event.seq > current.lastEventSeq) {
          bySeq.set(event.seq, event);
        }
      });
      const accepted = [...bySeq.values()].sort((left, right) => left.seq - right.seq);
      ...
      accepted.forEach((event) => {
        ...
        const subagentDetail = event.type === 'subagent.message_delta'
        ...
        const delegateBoundary = (
          event.type === 'agent.tool_started' || event.type === 'agent.tool_completed'
        ) && Boolean(
          actionId
          && (
        ...
        const parentActionId = subagentDetail || subagentLifecycle
          ? eventString(event.data, 'parentActionId')
          : delegateBoundary
            ? actionId
            : undefined;
        if (parentActionId && (subagentDetail || subagentLifecycle || delegateBoundary)) {
          const existingActivity = subagentActivities[parentActionId];
          let activity = subagentActivityBase(existingActivity, parentActionId, event);
          ...
          if (
            ...
            activity = {
              ...
              completed: true,
              terminalKind: 'FAILED',
              summaryText: eventString(event.data, 'error')
          ...
          if (event.type === 'subagent.message_delta' || event.type === 'subagent.message_completed') {
            const messageId = eventString(event.data, 'messageId');
            const runId = eventString(event.data, 'runId') ?? activity.runId;
            if (messageId && runId) {
              const messageKey = `${runId}:${messageId}`;
              const existingMessage = activity.messages[messageKey];
              if (event.type === 'subagent.message_delta' && !existingMessage) {
                activity = settleSubagentMessages(
                  activity,
                  runId,
              ...
              if (event.type === 'subagent.message_delta') {
                ...
                const streamKind = eventString(event.data, 'streamKind') ?? 'text';
                if (
                  Number.isInteger(deltaIndex)
                  && deltaIndex >= 0
                  && typeof delta === 'string'
                  && !(deltaIndex in (streamKind === 'thinking' ? nextMessage.thinkingDeltas : nextMessage.deltas))
                ) {
                  nextMessage = {
                    ...nextMessage,
                    ...(streamKind === 'thinking'
                      ? { thinkingDeltas: { ...nextMessage.thinkingDeltas, [deltaIndex]: delta } }
                      : { deltas: { ...nextMessage.deltas, [deltaIndex]: delta } }),
                  };
                }
              } else {
                nextMessage = {
                  ...nextMessage,
                  completedText: typeof event.data.text === 'string'
                    ? event.data.text
                    : nextMessage.completedText,
                  completedThinking: typeof event.data.thinking === 'string'
                    ? event.data.thinking
                    : nextMessage.completedThinking,
                  completed: true,
                  finishReason: eventString(event.data, 'finishReason') ?? nextMessage.finishReason,
                };
              }
              if (nextMessage !== existingMessage) {
                activity = {
                  ...activity,
                  messages: { ...activity.messages, [messageKey]: nextMessage },
                };
              }
            }
          }
            ...
            if (toolCallId && runId) {
              ...
              if (event.type === 'subagent.tool_delta') {
                ...
                if (
                  ...
                  argumentDeltas = { ...argumentDeltas, [deltaIndex]: argumentsDelta };
                }
              }
              let parsedArguments = isRecord(event.data.arguments)
                ? event.data.arguments
                : existingTool?.arguments;
              if (!parsedArguments && Object.keys(argumentDeltas).length > 0) {
              ...
              const nextTool: SubagentStreamTool = {
                toolCallId,
                runId,
                tool: eventString(event.data, 'tool') ?? existingTool?.tool ?? 'tool',
                firstEventSeq: existingTool?.firstEventSeq ?? event.seq,
                status: event.type !== 'subagent.tool_completed'
                  ? 'started'
                  : subagentToolStatus(event.data.state),
                argumentDeltas,
                arguments: parsedArguments,
                result: event.type === 'subagent.tool_completed'
                  ? event.data.result
                  : existingTool?.result,
                state: eventString(event.data, 'state') ?? existingTool?.state,
                taskId: eventString(event.data, 'taskId') ?? existingTool?.taskId,
          ...
          subagentActivities = { ...subagentActivities, [parentActionId]: activity };
        }
        const clearsStreamingAssistant = event.type === 'assistant.output_rejected'
        ...
        if (clearsStreamingAssistant) {
          const assistantMessageId = typeof event.data.assistantMessageId === 'string'
            ? event.data.assistantMessageId
            : typeof event.data.messageId === 'string'
              ? event.data.messageId
              : undefined;
          if (assistantMessageId && streamingAssistantMessages[assistantMessageId]) {
        ...
        if (event.type === 'agent.run.failed' || event.type === 'agent.run.cancelled') {
          ...
          if (terminalRunId) {
            const remaining = Object.fromEntries(
              Object.entries(streamingAssistantMessages)
                .filter(([, message]) => message.runId !== terminalRunId),
            );
            if (Object.keys(remaining).length !== Object.keys(streamingAssistantMessages).length) {
              streamingAssistantMessages = remaining;
            }
          }
        }
        if (event.type === 'agent.message_delta') {
          const messageId = typeof event.data.messageId === 'string'
            ? event.data.messageId
            : undefined;
          const deltaIndex = Number(event.data.deltaIndex);
          const delta = event.data.delta;
          ...
          if (
            ...
            const target = streamKind === 'thinking' ? existing?.thinkingDeltas : existing?.deltas;
            if (!existing || !(deltaIndex in (target ?? {}))) {
              streamingAssistantMessages = {
                ...
                [messageId]: {
                  ...
                  deltas: streamKind === 'thinking'
                    ? existing?.deltas ?? {}
                    : { ...(existing?.deltas ?? {}), [deltaIndex]: delta },
                  thinkingDeltas: streamKind === 'thinking'
                    ? { ...(existing?.thinkingDeltas ?? {}), [deltaIndex]: delta }
        ...
        if (event.type === 'agent.tool_delta') {
          ...
          if (
            ...
            const argumentDeltas = {
              ...(currentTool?.id === toolCallId ? currentTool.argumentDeltas : {}),
        ...
        if (event.type.startsWith('task.')) {
          ...
          if (runId) {
            const activityEntry = Object.entries(subagentActivities)
              .find(([, activity]) => activity.runId === runId);
            if (activityEntry) {
              ...
              const candidates = Object.entries(currentActivity.tools)
                .filter(([, tool]) => tool.status === 'started' && (!tool.taskId || tool.taskId === taskId))
              ...
              if (candidate) {
                ...
                const updatedTool: SubagentStreamTool = {
                  ...
                  outputEvents: [...tool.outputEvents, { seq: event.seq, type: event.type, data: event.data }],
                };
                subagentActivities = {
                  ...subagentActivities,
                  [activityKey]: {
                    ...currentActivity,
                    tools: { ...currentActivity.tools, [toolKey]: updatedTool },
                  },
                };
              }
            }
        ...
        const persistsConversationMessage = !event.type.endsWith('message_delta')
        ...
        if (persistsConversationMessage) {
          ...
          const clientMessageId = event.data.clientMessageId as string | undefined;
          if (clientMessageId) queuedUi = queuedUi.filter((item) => item.clientMessageId !== clientMessageId);
          const rawMessageSeq = event.data.messageSeq ?? event.data.seq;
          const cursor = typeof rawMessageSeq === 'number'
            ? Math.max(0, rawMessageSeq - 1)
            : messages.at(-1)?.messageSeq ?? 0;
          messageRefreshAfter = messageRefreshAfter == null
            ? cursor
            : Math.min(messageRefreshAfter, cursor);
        }
        if (event.type === 'session.status_changed' || event.type === 'creator.woken') {
          const status = event.type === 'creator.woken'
            ? 'RUNNING'
            : (event.data.status ?? event.data.to) as CreatorSessionView['status'] | undefined;
          if (status && session) session = { ...session, status, lastEventSeq: event.seq };
        }
      });
      const lastEventSeq = accepted.at(-1)!.seq;
      if (session && session.lastEventSeq !== lastEventSeq) session = { ...session, lastEventSeq };
      ...
      if (streamingAssistantMessages !== current.streamingAssistantMessages) {
    ...
    if (messageRefreshAfter != null) scheduleMessageRefresh(messageRefreshAfter);
  },
