...
export const useAgentDockUiStore = create<AgentDockUiState>((set) => ({
  ...
  setSize: (width, height) => set({ width: Math.max(440, width), height: Math.max(320, height) }),
  setRunFilter: (runFilter) => set({ runFilter }),
  setDraft: (draft) => set({ draft }),
  setSelection: (selection) => set({ selection, tab: 'conversation', open: true }),
  setReviewContext: (reviewContext) => set({ reviewContext }),
  requestReviewRevision: (context) => set({
    reviewContext: context,
    reviewRevisionHandoff: { ...context, prepared: false },
    tab: 'conversation',
    open: true,
  }),
  markReviewRevisionPrepared: () => set((state) => ({
    reviewRevisionHandoff: state.reviewRevisionHandoff
      ? { ...state.reviewRevisionHandoff, prepared: true }
      : null,
  })),
  clearReviewRevisionHandoff: () => set({ reviewRevisionHandoff: null, reviewContext: null }),
  reset: () => set({
    open: true,
    tab: 'conversation',
    width: 440,
