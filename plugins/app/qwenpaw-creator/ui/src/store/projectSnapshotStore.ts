...
function jitteredDelay(delay: number, options: ProjectSnapshotPollOptions): number {
...
export const useProjectSnapshotStore = create<ProjectSnapshotState>((set, get) => {
  let projectEpoch = 0;
    ...
    const request = (async () => {
      try {
        ...
        set((state) => {
          ...
          const finished = state.issuedRequestSequence === token.sequence
            ? { requestInFlight: false }
            : {};
          const applied = { appliedRequestSequence: token.sequence, ...finished };
          ...
          if (result.kind === 'not_modified') {
            if (!state.project || state.generation === null || !state.etag) {
              return {
                ...
                syncError: 'Project snapshot returned 304 before a last-good snapshot was loaded',
                consecutiveFailures: state.consecutiveFailures + 1,
              };
            }
            if (
              (result.generation !== null && result.generation !== state.generation)
              || (result.etag !== null && !sameEtag(result.etag, state.etag))
            ) {
              return {
                ...applied,
                syncStatus: 'degraded' as const,
                syncError: 'Project snapshot 304 metadata does not match the last-good snapshot',
                consecutiveFailures: state.consecutiveFailures + 1,
              };
            }
            return {
              ...
              syncError: result.syncStatus === 'healthy' ? null : state.syncError,
              consecutiveFailures: result.syncStatus === 'healthy'
                ? 0
        ...
        const projectDeleted = error instanceof CreatorHttpError && error.status === 404;
        set((state) => {
          if (
            token.epoch !== projectEpoch
            || state.projectId !== token.projectId
