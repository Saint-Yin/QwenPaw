...
function jitteredDelay(delay: number, options: FileProjectReviewPollOptions): number {
...
export const useFileProjectReviewStore = create<FileProjectReviewState>((set, get) => {
  let projectEpoch = 0;
  ...
  const stopController = (projectId?: string) => {
    if (!controller || (projectId && controller.projectId !== projectId)) return;
    ...
    controller = null;
    set((state) => state.projectId === stoppedProjectId ? { polling: false } : {});
  };
  ...
  const pollOnce = (projectId: string): Promise<void> => {
    ...
    const request = (async () => {
      try {
        const result = await getActiveFileProjectReview(projectId, requestEtag);
        set((state) => {
          if (epoch !== projectEpoch || state.projectId !== projectId) return {};
          ...
          if (result.kind === 'not_modified') {
            if (!state.review || !state.etag) {
              return {
                ...
                syncError: 'Active Review returned 304 before a last-good Review was loaded',
                consecutiveFailures: state.consecutiveFailures + 1,
              };
            }
            if (result.etag && semanticEtag(result.etag) !== semanticEtag(state.etag)) {
              return {
                requestInFlight: false,
                syncStatus: 'degraded' as const,
                syncError: 'Active Review 304 ETag does not match the last-good decision token',
                consecutiveFailures: state.consecutiveFailures + 1,
              };
            }
            return {
          ...
          if (
            ...
            && semanticEtag(incoming.decision_token) === semanticEtag(current.decision_token)
        ...
        const notFound = error instanceof CreatorHttpError && error.status === 404;
        set((state) => {
          if (epoch !== projectEpoch || state.projectId !== projectId) return {};
          if (notFound) {
            return {
        ...
        if (notFound && epoch === projectEpoch && get().projectId === projectId) {
    ...
    void request.finally(() => {
      if (inFlightByProject.get(projectId) === request) inFlightByProject.delete(projectId);
    });
    return request;
  };
  ...
  const decide = async (
    ...
    if (!review || review.status !== 'PENDING') throw new Error('没有可处理的文件 Review');
    ...
    if (ids.size !== decisions.length) throw new Error('Review 决策不能包含重复 operation_id');
    ...
    try {
      ...
      set((current) => {
        if (epoch !== projectEpoch || current.projectId !== projectId) return {};
        if (
          current.review
  ...
  const schedule = (active: PollController) => {
    ...
    const failures = get().projectId === active.projectId ? get().consecutiveFailures : 0;
  ...
  return {
    ...
    startPolling: (projectId, overrides = {}) => {
      ...
      const active: PollController = {
        ...
        projectId,
        options: { ...DEFAULT_FILE_PROJECT_REVIEW_POLL_OPTIONS, ...overrides },
        timer: null,
        running: false,
      ...
      controller = active;
      set((state) => state.projectId === projectId ? { polling: true } : {});
      if (typeof document !== 'undefined') {
        document.addEventListener('visibilitychange', active.visibilityHandler);
      }
      void run(active);
      return () => {
