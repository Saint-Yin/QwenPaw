...
export function presentationOf<T>(envelope: ViewEnvelope<T> | null): T | null {
  ...
  const candidate = envelope.view as T | { presentationVersion: string; view: T };
  return candidate && typeof candidate === 'object' && 'presentationVersion' in candidate
    ? candidate.view
    : candidate as T;
}
...
interface WorkspaceViewState {
  ...
  workbenches: Record<string, ViewEnvelope<R2VWorkbenchView | EditWorkbenchView>>;
...
export const useWorkspaceViewStore = create<WorkspaceViewState>((set, get) => {
  ...
  const beginRequest = (projectId: string, key: string): WorkspaceRequestToken => {
  ...
  const loadView = async <T,>(
    ...
    try {
      ...
      if (result.projectId !== projectId) {
        throw new Error(`View project mismatch: expected ${projectId}, received ${result.projectId}`);
      }
  ...
  return {
    ...
    loadHeader: (projectId) => loadView(
      projectId,
      'header',
      () => getHeaderView(projectId),
      (_state, result) => ({ header: result }),
    ),
    loadPlan: (projectId) => loadView(
      projectId,
      'plan',
      () => getPlanView(projectId),
      (_state, result) => ({ plan: result }),
    ),
    loadSection: (projectId, sectionId) => loadView(
      projectId,
      `section:${sectionId}`,
      () => getSectionView(projectId, sectionId),
      (state, result) => ({ sections: { ...state.sections, [sectionId]: result } }),
    ),
    loadWorkbench: (projectId, unitId) => loadView(
      projectId,
      `workbench:${unitId}`,
      () => getWorkbenchView(projectId, unitId),
      (state, result) => ({ workbenches: { ...state.workbenches, [unitId]: result } }),
    ),
    loadAssets: (projectId) => loadView(
      projectId,
      'assets',
      () => getAssetView(projectId),
      (_state, result) => ({ assets: result }),
    ),
    loadSectionCompose: (projectId, sectionId) => loadView(
      projectId,
    ...
    loadFinalCompose: (projectId) => loadView(
      projectId,
      'final-compose',
      () => getFinalComposeView(projectId),
      (_state, result) => ({ finalCompose: result }),
    ),
    setClipHighlights: (unitId, clipIds) => set((state) => ({
      clipHighlights: {
        ...state.clipHighlights,
        [unitId]: [...new Set(clipIds)],
      },
    })),
    clearClipHighlights: (unitId) => set((state) => ({
    ...
    revalidateLoaded: async (projectId) => {
      ...
      if (current.finalCompose !== null) requests.push(current.loadFinalCompose(projectId));
