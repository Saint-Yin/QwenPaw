...
import type { CreatorEvent, SpecialistRunView, TaskView } from '@/contracts/creator';
...
export const useCreatorTaskViewStore = create<CreatorTaskViewState>((set, get) => ({
  projectId: null,
  runs: [],
  tasks: [],
  loading: false,
  ...
  refresh: async (projectId) => {
    ...
    try {
      const [runs, tasks] = await Promise.all([listSpecialistRuns(projectId), listTasks(projectId)]);
      if (generation !== refreshGeneration || get().projectId !== projectId) return;
  ...
  consumeEvent: (event) => {
    ...
    if (run) set((state) => ({ runs: [run, ...state.runs.filter((item) => item.id !== run.id)] }));
    if (task) set((state) => ({ tasks: [task, ...state.tasks.filter((item) => item.id !== task.id)] }));
  },
  cancel: async (taskId) => {
    const { projectId } = get();
    if (!projectId) return;
  ...
  reset: () => {
    refreshGeneration += 1;
    set({ projectId: null, runs: [], tasks: [], loading: false, error: null });
  },
}));
