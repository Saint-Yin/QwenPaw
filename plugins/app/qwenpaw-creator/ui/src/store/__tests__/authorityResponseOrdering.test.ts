...
function creatorSession(
  ...
  return {
    ...
    agentStatusBar: {
      progress: {
        phase: sessionStatus === 'PENDING_REVIEW' ? 'review' : 'unit_production',
        label: sessionStatus,
...
function authorization(statusValue: ExecutionAuthorizationView['status']): ExecutionAuthorizationView {
  return {
    id: 'authorization-1',
    transactionId: 'tx-1',
...
function reviewManifest(transactionId: string, reviewRound: number): ReviewManifest {
  return {
...
describe('authoritative response ordering', () => {
  ...
  it('keeps the newest Session refresh when an older authorization snapshot arrives last', async () => {
    ...
    let index = 0;
    vi.stubGlobal('fetch', vi.fn(() => requests[index++].promise));
    ...
    expect(useCreatorSessionStore.getState().session?.status).toBe('PENDING_REVIEW');
    expect(useCreatorSessionStore.getState().session?.lastEventSeq).toBe(10);
    expect(useCreatorSessionStore.getState().agentStatusBar?.progress.sourceEventSeq).toBe(10);
  });
  ...
  it('keeps the newest working-head snapshot for each independent View key', async () => {
    ...
    newer.resolve(response(envelope(
      { ...planView, title: 'new plan' },
      { workingHead: 'head-new' },
    )));
    await newerLoad;
    older.resolve(response(envelope(
    ...
    expect(useWorkspaceViewStore.getState().plan?.view).toMatchObject({ title: 'new plan' });
  ...
  it('discards an old-project View response after navigation', async () => {
    ...
    expect(useWorkspaceViewStore.getState().header?.view).toMatchObject({ name: 'project two' });
  });

  it('revalidates every loaded View key without loading absent Views', async () => {
    const fetchMock = vi.fn((input: RequestInfo | URL) => {
      const url = String(input);
      if (url.endsWith('/header')) return Promise.resolve(response(envelope(headerView)));
      if (url.endsWith('/plan')) return Promise.resolve(response(envelope(planView)));
      if (url.endsWith('/sections/s1')) {
        return Promise.resolve(response(envelope(planView.sections[0])));
      }
      if (url.endsWith('/units/u1/workbench')) return Promise.resolve(response(envelope(r2vView)));
      if (url.endsWith('/assets')) return Promise.resolve(response(envelope(assetView)));
      if (url.endsWith('/post/sections/s1')) return Promise.resolve(response(envelope(composeView)));
  ...
  it('fences an in-flight PENDING authorization list with the approve response', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
      const url = String(input);
      if (init?.method === 'POST' && url.endsWith('/approve')) return approved.promise;
      if (url.includes('/execution-authorizations?status=PENDING')) return staleList.promise;
      throw new Error(`Unexpected request: ${url}`);
    }));
    useReviewManifestStore.getState().bindTransaction('p1', 'tx-1');
    useReviewManifestStore.setState({ authorizations: [authorization('PENDING')] });

    const oldList = useReviewManifestStore.getState().loadAuthorizations();
    const approve = useReviewManifestStore.getState().approveAuthorization(
      'authorization-1',
      {
        authorizationToken: 'authorization-token',
        provider: 'dashscope',
  ...
  it('discards Review and authorization responses from a previously bound transaction', async () => {
    ...
    for (const path of ['tx-1/review', 'tx-1/auth', 'tx-2/review', 'tx-2/auth']) {
    ...
    responses.get('tx-1/auth')!.resolve(response({ items: [authorization('PENDING')] }));
    await oldLoad;

    expect(useReviewManifestStore.getState().transactionId).toBe('tx-2');
    expect(useReviewManifestStore.getState().manifest?.transactionId).toBe('tx-2');
