...
describe('file-native Project Review store', () => {
  ...
  it('clears active Review state on both 204 and Project 404', async () => {
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce(response(404, { code: 'NOT_FOUND', message: 'gone' }));
    vi.stubGlobal('fetch', fetchMock);
  ...
  it('does not apply a lower candidate generation over the current Review', async () => {
    ...
    await useFileProjectReviewStore.getState().pollOnce('p1');
    await useFileProjectReviewStore.getState().pollOnce('p1');
    expect(useFileProjectReviewStore.getState().review?.candidate_generation).toBe(5);
    expect(useFileProjectReviewStore.getState().review?.decision_token).toBe('token-5');
  });
  ...
  it('ignores a late response after switching Project scope', async () => {
    ...
    resolveP2(activeResponse(review('p2-token', 8, { review_id: 'review-p2' })));
    await p2;
    resolveP1(activeResponse(review('p1-token', 9, { review_id: 'review-p1' })));
    await p1;

    expect(useFileProjectReviewStore.getState().projectId).toBe('p2');
    expect(useFileProjectReviewStore.getState().review?.review_id).toBe('review-p2');
    expect(useFileProjectReviewStore.getState().review?.decision_token).toBe('p2-token');
  });
  ...
  it('reuses decisionId and Idempotency-Key after an ambiguous failed response', async () => {
    ...
    await expect(useFileProjectReviewStore.getState().decide('p1', decisions))
      .rejects.toThrow('response lost');
    await expect(useFileProjectReviewStore.getState().decide('p1', [...decisions].reverse()))
    ...
    expect(new Headers(firstInit.headers).get('Idempotency-Key')).toBe(firstBody.decisionId);
    expect(new Headers(secondInit.headers).get('Idempotency-Key')).toBe(firstBody.decisionId);
    expect(useFileProjectReviewStore.getState().review).toBeNull();
  });
});
