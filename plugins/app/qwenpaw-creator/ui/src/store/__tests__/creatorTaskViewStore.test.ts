...
import type { SpecialistRunListResponse, TaskListResponse } from '@/contracts/creator';
...
describe('creator task view refresh ordering', () => {
  ...
  it('does not let an older pending snapshot overwrite a newer terminal refresh', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn((input: RequestInfo | URL) => {
      ...
      if (url.endsWith('/specialist-runs')) return runResponses[runRequest++].promise;
    ...
    const staleRuns: SpecialistRunListResponse = {
      items: [{ ...terminalRuns.items[0], status: 'WAITING_AUTHORIZATION', finalMarker: undefined }],
    };
