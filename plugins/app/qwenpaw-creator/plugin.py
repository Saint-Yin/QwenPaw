# -*- coding: utf-8 -*-
"""QwenPaw Creator plugin entry point."""
...
def configure_creator_runtime_environment(
    *,
    ...
    if not data_root.is_absolute():
        raise CreatorDataRootError("CREATOR_DATA_ROOT must be an absolute path")
    ...
    if not model_config_path.is_absolute():
        raise CreatorDataRootError("CREATOR_MODEL_CONFIG_PATH must be an absolute path")
    ...
    if configured_binary_dir:
        ...
        if not binary_dir.is_absolute():
            raise CreatorDataRootError("CREATOR_BINARY_DIR must be an absolute path")
...
class QwenPawCreatorPlugin:
    """Register Creator UI and REST API routes."""

    def __init__(self) -> None:
        self._file_services: CreatorFileServices | None = None
    ...
    def register(self, api: PluginApi) -> None:
        ...
        api.register_startup_hook(
            hook_name="qwenpaw_creator_startup",
            callback=self._startup,
            priority=90,
        )
        api.register_shutdown_hook(
            hook_name="qwenpaw_creator_shutdown",
            callback=self._shutdown,
            priority=90,
        )
        logger.info("QwenPaw Creator plugin registered")
    ...
    async def _startup(self) -> None:
        ...
        self._ensure_config_tools_registered()
        configured = [
            key
            for key in (
                "TEXT_API_KEY",
        ...
        if configured:
            logger.info("QwenPaw Creator env-backed config available: %s", configured)
        else:
            logger.warning(
    ...
    def _ensure_config_tools_registered(self) -> None:
