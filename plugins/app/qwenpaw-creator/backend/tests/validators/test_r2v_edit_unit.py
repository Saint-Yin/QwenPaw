...
from services.validators.edit_timeline import EditExecutionRequest, EditTimelineClip, validate_edit_execution
from services.validators.r2v import ProviderCapabilitySnapshot, R2VExecutionRequest, validate_r2v_execution
from services.validators.unit import ActionBeat, ShotDefinition, UnitDefinition, validate_unit
...
def test_r2v_execution_is_storyboard_first_and_at_most_15_seconds() -> None:
    valid = R2VExecutionRequest(
        ...
        video_prompt="人物走入雪夜",
        reference_image_refs=("artifact://u1-storyboard@v2", "asset://hero@v3"),
    ...
    invalid = replace(
        valid,
        duration_seconds=16,
        reference_image_refs=("asset://hero@v3", "artifact://u1-storyboard@v2"),
    )
    codes = {issue.code for issue in validate_r2v_execution(invalid).issues}
    assert {"R2V_DURATION_MAX", "R2V_PROVIDER_DURATION", "R2V_DURATION_DISCRETE", "R2V_STORYBOARD_FIRST"} <= codes
...
def test_r2v_storyboard_first_compares_same_exact_ref_shape() -> None:
    ...
    other_reference = "artifact://asset%3Agirl%2Fappearance@artifact-version-girl"
    valid = R2VExecutionRequest(
...
def test_edit_over_15_seconds_is_valid_and_does_not_require_asr_or_ocr() -> None:
...
def test_unit_duration_limit_only_applies_to_r2v_and_shot_detail_rules_are_disabled() -> None:
    edit = UnitDefinition("unit:e1", "edit", 45, material_refs=("asset://source@v1",))
    ...
    assert validate_unit(UnitDefinition("unit:r1", "r2v", 5, (broken_shot,))).valid
