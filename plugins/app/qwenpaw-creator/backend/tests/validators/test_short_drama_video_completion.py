...
def _snapshot(files: dict[str, str]) -> TextWorkspaceSnapshot:
    return TextWorkspaceSnapshot(
        ...
        files={path: WorkspaceFile(text, f"ov-{index}") for index, (path, text) in enumerate(files.items())},
        target_versions={},
    )
...
def test_short_drama_allows_edit_route_when_duration_topology_matches() -> None:
    files = _files()
...
def test_short_drama_ignores_project_target_but_keeps_section_local_rules() -> None:
...
def test_video_completion_chain_is_disabled_for_empty_video_edit_tree() -> None:
...
def test_exact_section_and_final_video_chain_passes_completion() -> None:
    ...
    assert report.valid
    assert derive_missing_post_work_refs(
        scenario="short_drama",
        project_id="project-short",
        snapshot=snapshot,
        sections=parse_sections(snapshot),
...
def test_disabled_video_completion_never_exposes_post_or_unit_work() -> None:
    ...
    assert derive_missing_video_work_refs(
        scenario="short_drama",
        project_id="project-short",
        snapshot=upstream_missing,
        sections=parse_sections(upstream_missing),
...
def test_disabled_completion_ignores_metadata_provenance_and_final_duration() -> None:
    artifacts = deepcopy(_artifacts())
    artifacts[2]["metadata"]["sourceSelections"][1]["artifactVersionId"] = "old-unit"
