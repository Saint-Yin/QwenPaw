...
def test_duration_balancer_supplements_plan_below_completion_tolerance(
    ...
    monkeypatch.setattr(core, "_heuristic_plan", lambda *args, **kwargs: filler)
...
def test_duration_balancer_preserves_plan_at_completion_tolerance(monkeypatch) -> None:
    monkeypatch.setattr(
        core,
        "_heuristic_plan",
    ...
    assert core._ensure_target_duration(
        deepcopy(plan),
        {},
        {"duration": 60},
        [],
