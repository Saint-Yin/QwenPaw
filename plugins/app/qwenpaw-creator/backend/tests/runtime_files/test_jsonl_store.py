...
def test_jsonl_restart_retains_durable_sequence_and_expected_seq_cas(tmp_path):
    ...
    assert (
        first_store.append(
            EventRecord(worker=0, value=2), expected_next_seq=2
        ).seq
        == 2
    )
    ...
    assert (
        restarted.append(
            EventRecord(worker=0, value=3), expected_next_seq=3
        ).seq
        == 3
    )
...
def test_jsonl_normal_append_reads_only_the_durable_tail(tmp_path, monkeypatch):
    path = tmp_path / "events.jsonl"
    store = DurableJsonlStore(path, EventRecord)
    store.append(EventRecord(worker=1, value=1))

    def unexpected_full_recovery():
        raise AssertionError("normal append must not rescan the full JSONL stream")
