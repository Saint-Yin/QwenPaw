...
def test_run_task_and_attempt_heads_use_atomic_json_and_jsonl(tmp_path):
    ...
    assert store.get_task(PROJECT_ID, task.task_id).status is TaskStatus.RUNNING
    ...
    assert (runtime_root / "tasks" / task.task_id / "attempts.jsonl").read_text(
        encoding="utf-8"
    ).count("\n") == 2
...
def test_message_and_continuation_streams_are_ordered_and_inherit_provenance(
    ...
    assert (
        store.get_run(PROJECT_ID, RUN_ID).status is SpecialistRunStatus.WAITING_RUNTIME
    )
    ...
    assert store.get_run(PROJECT_ID, RUN_ID).status is SpecialistRunStatus.RUNNING_MODEL
    ...
    assert [
        item.state for item in store.list_continuation_events(PROJECT_ID, RUN_ID)
    ] == [
...
def test_rejected_attempt_or_continuation_does_not_append_a_history_line(
    ...
    continuation_path = (
        tmp_path / PROJECT_ID / "runtime" / "runs" / RUN_ID / "continuations.jsonl"
    )
    ...
    attempt_path = (
        tmp_path / PROJECT_ID / "runtime" / "tasks" / "task-1" / "attempts.jsonl"
    )
    assert not attempt_path.exists()


def test_attempt_replay_repairs_append_then_head_crash_gap(tmp_path, monkeypatch):
    store = _store(tmp_path)
    store.create_run(_run())
    store.create_task(_task())
    ...
    monkeypatch.setattr(store, "_apply_attempt_head_unlocked", crash_after_append)
...
def test_authorization_decision_is_token_and_status_cas(tmp_path):
    ...
    assert store.get_authorization(
        PROJECT_ID, authorization.authorization_id
    ).status in {
        ExecutionAuthorizationStatus.APPROVED,
        ExecutionAuthorizationStatus.REJECTED,
    }
...
def test_all_path_identifiers_reject_directory_traversal(tmp_path, project_id, run_id):
    store = _store(tmp_path)
    candidate = _run(run_id=run_id).model_copy(update={"project_id": project_id})
...
def test_execution_store_has_no_legacy_database_dependency():
    source = (
        Path(__file__).parents[2] / "services" / "runtime_files" / "execution_store.py"
