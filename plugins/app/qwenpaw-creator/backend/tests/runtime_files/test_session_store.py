...
def test_bootstrap_atomically_creates_session_and_default_conversation_only(
    ...
    assert (session_root / "conversations" / f"{CONVERSATION_ID}.json").is_file()
    assert all(
...
def test_message_jsonl_sequence_and_client_id_payload_drift(tmp_path):
    ...
    assert [
        item.message_seq for item in store.list_messages(PROJECT_ID, SESSION_ID)
    ] == [1, 2]
...
def test_current_state_and_all_session_streams_are_durable(tmp_path):
    ...
    assert [item.event_seq for item in store.list_events(PROJECT_ID, SESSION_ID)] == [
        1,
        2,
    ]
    assert [
        item.queue_seq for item in store.list_queued_messages(PROJECT_ID, SESSION_ID)
    ] == [1, 2]
    assert [item.outbox_seq for item in store.list_outbox(PROJECT_ID, SESSION_ID)] == [
        1,
        2,
    ]
    assert store.get_session(PROJECT_ID, SESSION_ID).queued_user_message_count == 0
...
def test_review_resolution_event_is_published_before_idle_and_is_idempotent(
    ...
    monkeypatch.setattr(store, "_write_session_unlocked", assert_event_precedes_idle)
    ...
    assert store.get_goal(PROJECT_ID, "goal-1").status is CreatorGoalStatus.COMPLETED
...
def test_competing_review_resolution_supervisors_emit_one_event(tmp_path):
    ...
    stores = [ProjectRuntimeSessionStore(root), ProjectRuntimeSessionStore(root)]
...
def test_event_append_and_poll_skip_full_aggregate_recovery_on_consistent_head(
    ...
    def unexpected_full_recovery(*_args, **_kwargs):
        raise AssertionError("consistent event operations must not recover all streams")

    monkeypatch.setattr(store, "_recover_session_unlocked", unexpected_full_recovery)
    ...
    events = store.list_events(PROJECT_ID, SESSION_ID, after_seq=first.event_seq)
...
def test_event_append_recovers_a_stale_aggregate_head_after_a_crash(tmp_path):
    root, snapshot, store, _bootstrap = _runtime(tmp_path)
    _activate_runtime(root, snapshot, store)
    first = store.append_event(PROJECT_ID, SESSION_ID, event_type="runtime.started")
    session_path = (
        root / PROJECT_ID / "runtime" / "sessions" / SESSION_ID / "session.json"
    )
    ...
    second = store.append_event(PROJECT_ID, SESSION_ID, event_type="runtime.continued")
...
def test_review_resolution_reuses_event_after_final_session_write_failure(
    ...
    monkeypatch.setattr(store, "_write_session_unlocked", fail_first_idle_write)
...
def test_only_active_agentdock_mutation_persists_review_boundary(tmp_path):
    ...
    assert store.review_boundary_path(
        PROJECT_ID, SESSION_ID, "request-review"
    ).is_file()
...
def test_concurrent_admission_has_one_message_and_one_boundary(tmp_path):
    ...
    boundary_root = (
        root / PROJECT_ID / "runtime" / "sessions" / SESSION_ID / "review-boundaries"
    )
...
def test_review_admission_waits_for_project_commit_baseline_finalization(
    ...
    monkeypatch.setattr(boundary, "_update_runtime_state", delayed_update_state)
    ...
    assert admitted[0].review_boundary.accepted_etag == project_store.read(
        PROJECT_ID
    ).etag
...
def test_restart_repairs_heads_and_restores_missing_boundary(tmp_path):
    ...
    AtomicJsonRecordStore(session_path, type(stale)).write(stale)
    boundary_path = store.review_boundary_path(PROJECT_ID, SESSION_ID, "request-review")
