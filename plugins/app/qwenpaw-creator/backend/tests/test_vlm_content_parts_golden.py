...
def test_local_image_and_video_are_encoded_as_typed_data_urls(tmp_path, monkeypatch):
    image = tmp_path / "reference.png"
    video = tmp_path / "clip.mp4"
    image.write_bytes(b"png")
    video.write_bytes(b"mp4")
    monkeypatch.setattr(vlm_model.model_config, "get_vlm_max_inline_bytes", lambda: 1024)

    assert vlm_model.multimodal_media_part(image.as_uri(), "image") == {
        "type": "image_url",
        "image_url": {"url": "data:image/png;base64,cG5n"},
    }
    assert vlm_model.multimodal_media_part(video.as_uri(), "video", fps=1.0) == {
        "type": "video_url",
...
def test_chat_completion_preserves_mixed_content_parts_in_request(monkeypatch):
    ...
    monkeypatch.setattr(vlm_model, "get_capability_cache", lambda: FakeCapabilityCache())
    monkeypatch.setattr(vlm_model, "model_slot", fake_model_slot)
    monkeypatch.setattr(vlm_model.httpx, "AsyncClient", FakeAsyncClient)
    monkeypatch.setattr(vlm_model.model_config, "get_vlm_api_key", lambda: "test-vlm-key")
    monkeypatch.setattr(vlm_model.model_config, "get_vlm_model_name", lambda: "qwen3.7-plus")
    monkeypatch.setattr(
        vlm_model.model_config,
        "get_vlm_chat_url",
        lambda: "https://provider.example.com/compatible-mode/v1/chat/completions",
    )
    monkeypatch.setattr(vlm_model.model_config, "get_vlm_timeout_seconds", lambda: 31.0)
...
def test_invalid_media_payload_does_not_poison_model_capability_cache() -> None:
