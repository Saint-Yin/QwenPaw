...
def test_streaming_stage_and_immutable_publish_are_fsynced(tmp_path, monkeypatch):
    ...
    assert len(fsync_calls) >= 5  # staging file/dir, created dirs, final/staging dirs
...
def test_publish_rejects_noncanonical_or_reserved_paths(tmp_path, relative_uri):
    store = _store(tmp_path)
...
def test_index_validation_and_reads_verify_presence_size_and_sha256(tmp_path):
    ...
    assert store.read_verified(index.files_by_id["file-valid"]) == valid_content
...
def test_orphan_scan_marks_candidates_and_requires_persisted_grace(tmp_path):
    ...
    assert staged.path.exists()  # .staging is excluded from orphan publication GC
