...
def _transaction_root(store: ProjectStore, transaction_id: str):
    return store.project_root("project-1") / "runtime" / "transactions" / transaction_id
...
def _crash_with_project_replaced(
    ...
    *,
    ...
    with monkeypatch.context() as patch:
        ...
        with pytest.raises(SimulatedProcessDeath, match="after Project replace"):
    ...
    assert _journal(store, transaction_id).state is CommitJournalState.PROJECT_REPLACED


def test_startup_recovery_reports_corrupt_project_without_blocking_healthy_one(tmp_path):
    store = ProjectStore(tmp_path.resolve())
...
def test_error_after_project_replace_keeps_journal_recoverable(tmp_path, monkeypatch):
    ...
    report = ProjectCommitRecoveryCoordinator(store).recover_project("project-1")
    assert report.ok
    assert _journal(store, "post-replace-error").state is CommitJournalState.RUNTIME_FINALIZED


def test_new_commit_waits_for_unfinalized_publication_recovery(tmp_path, monkeypatch):
    store, base = _project_store(tmp_path)
    ...
    ProjectCommitRecoveryCoordinator(store).recover_project("project-1").raise_for_integrity()
...
def test_recovery_uses_prepared_latest_snapshot_for_stale_disjoint_base(
    ...
    assert [change.json_pointer for change in changeset.changes] == ["/description"]


def test_prepared_unpublished_transaction_is_aborted_safely(tmp_path, monkeypatch):
    store, base = _project_store(tmp_path)
    ...
    def leave_prepared(self, value):
        if (
            ...
            raise SimulatedProcessDeath("process died before abort journal fsync")
...
def test_prepared_but_published_project_is_proved_and_fully_recovered(
    ...
    with monkeypatch.context() as patch:
        patch.setattr(AtomicJsonRecordStore, "write", crash_before_journal_transition)
    ...
    current = store.read("project-1")
    assert current.project.description == "published in the narrow crash window"
    assert _journal(store, transaction_id).state is CommitJournalState.PREPARED

    report = ProjectCommitRecoveryCoordinator(store).recover_project("project-1")
    assert report.ok
    assert report.outcomes[0].action is RecoveryAction.FINALIZED_RUNTIME
    assert _journal(store, transaction_id).state is CommitJournalState.RUNTIME_FINALIZED
    changeset = AtomicJsonRecordStore(
        store.project_root("project-1")
        / "runtime"
        / "change-rounds"
...
def test_restart_facade_runs_recovery_once_and_exposes_report(tmp_path, monkeypatch):
    store, base = _project_store(tmp_path)
...
def test_startup_accepts_finalized_disjoint_merge_with_newer_published_base(tmp_path):
...
def test_recovery_recreates_pending_review_from_durable_boundary(tmp_path, monkeypatch):
    store, base = _project_store(tmp_path)
    ...
    assert [operation.json_pointer for operation in review.operations] == ["/name"]
...
def test_finalized_journal_repairs_missing_runtime_records(tmp_path):
    ...
    assert report.outcomes[0].action is RecoveryAction.REPAIRED_FINALIZED_RUNTIME
    assert (
...
def test_prepared_no_change_crash_recovers_runtime_without_rewriting_project(
    ...
    monkeypatch.setattr(AtomicJsonRecordStore, "create", fail_transaction_changeset_once)
    ...
    assert _round(store, "round-no-change-crash").status is TransactionStatus.NO_CHANGE
...
def test_recovery_recreates_aggregate_after_transaction_publish_crash(
    ...
    with monkeypatch.context() as patch:
        patch.setattr(commit_module, "fsync_directory", fail_after_transaction_rename)
    ...
    assert (_transaction_root(store, transaction_id) / "journal.json").is_file()
...
def test_recovery_aborts_dangling_publish_snapshots_before_journal_update(
    ...
    transaction_round_store.write(
        transaction_round.model_copy(update={"status": TransactionStatus.ACTIVE})
    )
    ...
    aggregate_store.write(
        aggregate_round.model_copy(update={"status": TransactionStatus.ACTIVE})
    )
