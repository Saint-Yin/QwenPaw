...
def test_poller_keeps_last_good_when_external_write_is_invalid(tmp_path) -> None:
...
def test_note_commit_cannot_roll_cache_back_to_an_older_snapshot(tmp_path) -> None:
    store = ProjectStore(tmp_path.resolve())
...
def test_poller_evicts_last_good_snapshot_after_project_deletion(tmp_path) -> None:
    store = ProjectStore(tmp_path.resolve())
