...
from services.project_files.jq_transform import JqProjectTransformer, JqTransformError
...
def test_disjoint_object_changes_merge_without_losing_latest() -> None:
    ...
    merged, changes = merge_candidate(base=base, candidate=candidate, latest=latest)
...
def test_jq_does_not_depend_on_a_mutable_sidecar_path(
    ...
    missing_module = (
        tmp_path / "installed" / "services" / "project_files" / "jq_transform.py"
    )
    monkeypatch.setattr(jq_transform_module, "__file__", str(missing_module))

    result = JqProjectTransformer().transform({"name": "before"}, '.name = "after"')
