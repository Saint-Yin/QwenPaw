...
def test_create_read_list_replace_and_delete(tmp_path):
    ...
    assert [item.project_id for item in store.list()] == ["project-a", "project-b"]

    replacement = _next(created.project)
    updated = store.replace("project-b", replacement, expected_etag=created.etag)
...
def test_create_staging_initializer_is_published_atomically_or_rolled_back(tmp_path):
    store = ProjectStore(tmp_path.resolve())
    ...
    store.create(
        _project("project-ready"), initialize_staged_project=initialize
    )
    assert (
    ...
    with pytest.raises(RuntimeError, match="injected bootstrap failure"):
        store.create(_project("project-failed"), initialize_staged_project=fail)
...
def test_replace_and_delete_enforce_etag_and_generation(tmp_path):
    ...
    with pytest.raises(ProjectConflict, match="ETag"):
        store.replace("project-1", next_project, expected_etag="sha256:" + "0" * 64)
...
@pytest.mark.parametrize(
    "project_id",
    ["../escape", "nested/project", r"nested\\project", ".", "..", "has:colon", " bad"],
)
...
def test_read_rejects_indexed_asset_symlink_escape(tmp_path):
    ...
    with pytest.raises(UnsafeProjectPath, match="escapes"):
        store.replace("project-1", candidate, expected_etag=store.read("project-1").etag)


def test_atomic_replace_failure_preserves_old_project_and_cleans_temp(tmp_path, monkeypatch):
    store = ProjectStore(tmp_path.resolve())
    ...
    assert store.read("project-1") == current
    assert list((store.project_root("project-1") / "runtime" / "temp").iterdir()) == []


def test_store_fsyncs_file_and_directories_during_publish(tmp_path, monkeypatch):
    calls: list[int] = []
...
def test_discovery_ignores_internal_incomplete_and_symlink_directories(tmp_path):
    store = ProjectStore((tmp_path / "store").resolve())
...
def test_discovery_reports_project_directory_with_invalid_project_file(tmp_path):
    store = ProjectStore((tmp_path / "store").resolve())
