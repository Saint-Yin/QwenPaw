...
def _store(tmp_path) -> tuple[ProjectStore, object]:
    ...
    return store, store.create(Project.new(project_id="project-1", name="Initial"))


def test_commit_publishes_project_changeset_and_accepted_state(tmp_path) -> None:
    ...
    assert [change.json_pointer for change in result.changeset.changes] == ["/name"]
...
def test_same_round_keeps_immutable_changeset_per_transaction(tmp_path) -> None:
    store, base = _store(tmp_path)
    boundary = ProjectCommitBoundary(store)
    first_candidate = base.project.model_dump(mode="json")
    first_candidate["name"] = "First"
    ...
    assert [change.json_pointer for change in first_changeset.changes] == ["/name"]
...
def test_transaction_is_published_only_after_initial_journal_is_durable(
    ...
    assert ProjectCommitRecoveryCoordinator(store).recover_project("project-1").ok


def test_no_change_promotes_aborted_same_round_projection(tmp_path, monkeypatch) -> None:
    store, base = _store(tmp_path)
    boundary = ProjectCommitBoundary(store)
    candidate = base.project.model_dump(mode="json")
...
def test_no_change_does_not_downgrade_committed_same_round_projection(tmp_path) -> None:
    store, base = _store(tmp_path)
    boundary = ProjectCommitBoundary(store)
    candidate = base.project.model_dump(mode="json")
...
def test_no_change_does_not_downgrade_pending_review_projection(tmp_path) -> None:
    store, base = _store(tmp_path)
    boundary = ProjectCommitBoundary(store)
    candidate = base.project.model_dump(mode="json")
...
def test_identical_retry_archives_safe_aborted_transaction(tmp_path, monkeypatch) -> None:
    store, base = _store(tmp_path)
    boundary = ProjectCommitBoundary(store)
    candidate = base.project.model_dump(mode="json")
...
def test_commit_boundary_enforces_field_block_at_publish_time(tmp_path) -> None:
    store, base = _store(tmp_path)
...
def test_candidate_cannot_change_runtime_metadata(tmp_path) -> None:
    ...
    try:
        ...
        raise AssertionError("protected metadata change unexpectedly succeeded")
...
def test_only_agentdock_active_interrupt_creates_review(tmp_path) -> None:
    ...
    assert [operation.json_pointer for operation in result.review.operations] == [
        "/description"
    ]
...
def test_one_review_round_accumulates_multiple_transactions_safely(tmp_path) -> None:
    store, base = _store(tmp_path)
    commit = ProjectCommitBoundary(store)
    review_boundary = ReviewBoundary(
        request_message_seq=2,
...
def test_review_round_net_zero_clears_review_and_recovers_cleanly(tmp_path) -> None:
    store, base = _store(tmp_path)
    commit = ProjectCommitBoundary(store)
    review_boundary = ReviewBoundary(
        request_message_seq=2,
