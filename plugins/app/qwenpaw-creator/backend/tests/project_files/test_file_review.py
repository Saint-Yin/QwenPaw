...
from services.runtime_files.models import ReviewBoundary, ReviewStatus, RuntimeProjectState
...
def _pending_review(tmp_path):
    ...
    base = store.create(Project.new(project_id="project-1", name="Before", description="old"))
...
def test_accept_does_not_rewrite_project_and_reject_is_compensating_cas(tmp_path) -> None:
...
def test_review_decision_refuses_to_overwrite_newer_user_value(tmp_path) -> None:
    store, _base, committed = _pending_review(tmp_path)
    review = committed.review
    assert review is not None
    operation = next(item for item in review.operations if item.json_pointer == "/name")
    current = store.read("project-1")
...
def test_frontend_edit_supersedes_only_overlapping_pending_operation(tmp_path) -> None:
    store, _base, committed = _pending_review(tmp_path)
    current = store.read("project-1")
    candidate = current.project.model_dump(mode="json")
    ...
    assert operations["/description"].decision is ReviewOperationDecision.PENDING
    state = AtomicJsonRecordStore(
        tmp_path / "project-1" / "runtime" / "state.json",
        RuntimeProjectState,
    ).read()
...
def test_reject_hashes_opaque_decision_and_round_ids_before_path_use(tmp_path) -> None:
    store, _base, committed = _pending_review(tmp_path)
    review = committed.review
    assert review is not None
    operation = next(item for item in review.operations if item.json_pointer == "/name")
...
def test_missing_project_review_decision_does_not_create_runtime_tree(tmp_path) -> None:
    store = ProjectStore(tmp_path.resolve())
...
def test_reject_retry_recovers_when_crash_follows_compensating_commit(
    ...
    monkeypatch.setattr(service, "_persist_project_applied", crash_before_applied_journal)
...
def test_reject_retry_finishes_project_applied_journal_after_review_write_crash(
    ...
    assert review is not None
    operation = next(
        item for item in review.operations if item.json_pointer == "/description"
    )
    decision = ReviewDecisionItem(
        operation_id=operation.operation_id,
        decision="REJECT",
...
def test_startup_recovery_preserves_user_edit_after_prepared_decision_crash(
    ...
    recovered_operation = next(
        item for item in recovered.operations if item.operation_id == operation.operation_id
    )
    assert (
...
def test_new_decision_id_is_rejected_while_same_review_journal_is_active(
    ...
    with pytest.raises(ReviewDecisionConflict, match="active decision journal"):
        service.decide(
            project_id="project-1",
            review_id=review.review_id,
            decision_token=review.decision_token,
    ...
    decision_dirs = [item for item in transactions_root.iterdir() if item.is_dir()]
