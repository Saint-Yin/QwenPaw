...
def test_waiting_runtime_maps_to_executing_and_uses_working_snapshot(projection_fixture):
    ...
    working = TextWorkspaceSnapshot(
        ...
        files={
            **fixture.snapshot.files,
            "title.txt": replace(fixture.snapshot.files["title.txt"], content="Working title"),
        },
...
@pytest.mark.parametrize(
    ...
    [("INTERRUPT_REQUESTED", "interrupting"), ("WAITING_USER_INPUT", "waiting_input")],
)
def test_non_mutating_boundary_phase_keeps_approved_view_readable(
    ...
    assert envelope["view"] == {"title": fixture.snapshot.text("title.txt", required=True)}
...
def test_waiting_review_returns_presentation_with_origin_and_cas_metadata(projection_fixture):
    fixture = projection_fixture
    presentation_snapshot = TextWorkspaceSnapshot(
        project_id="p1",
        revision_id="review-revision-18",
        files=fixture.snapshot.files,
        target_versions={**fixture.snapshot.target_versions, "unit:u-r2v": "ov-review-u-r2v"},
    )
    runtime = _runtime(
        session_status="PENDING_REVIEW",
        transaction_status="PENDING_REVIEW",
        review_revision_id="review-revision-18",
        manual_edit_overlay=ManualEditOverlayRecord("overlay-2", "overlay-head-3", 1),
    )
    review_state = ReviewPresentationState(
        ...
        target_versions={
            "unit:u-r2v": ReviewTargetMetadata("ov-review-u-r2v", "group-1", "token-1"),
    ...
    envelope = build_view_envelope(
        runtime,
        ProjectionSnapshots(approved=fixture.snapshot, review_presentation=presentation_snapshot),
    ...
    assert envelope["manualEditOverlay"] == {"id": "overlay-2", "head": "overlay-head-3", "mutationCount": 1}
    ...
    assert presentation["targetVersions"]["unit:u-edit"] == {"targetVersion": "ov-overlay-u-edit"}
...
def test_ref_index_search_is_server_projected_and_bounded(projection_fixture):
    ...
    assert result[0]["uiLocator"] == {"page": "assets", "assetId": "scene-a", "versionId": "av-scene-2"}
    assert index.resolve(fixture.artifacts["storyboard"].source_ref)["artifactVersionId"] == "sb-v4"
...
def test_ref_index_resolves_current_source_analysis(projection_fixture):
    ...
    resolved = build_ref_index(snapshot, fixture.catalogs).resolve(analysis_ref)
...
def test_review_view_keeps_manifest_locators_and_does_not_duplicate_operations():
    manifest = {
        ...
        "decisionGroups": [
            {"id": "group-1", "operationIds": ["op-1"], "decisionToken": "token-1", "decision": "PENDING"}
        ],
        "operations": [
            {
                "id": "op-1",
                ...
                "uiLocator": {"page": "workbench", "unitId": "u-r2v", "focus": "storyboard"},
            }
        ],
    }
    view = build_review_view(manifest)
    assert len(view["manifest"]["operations"]) == 1
    assert view["manifest"]["operations"][0]["mutationIds"] == ["mutation-1", "mutation-2"]
