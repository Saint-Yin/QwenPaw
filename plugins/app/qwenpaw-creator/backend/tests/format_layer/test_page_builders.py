...
def test_header_and_hybrid_plan_are_deterministic(projection_fixture):
    ...
    metadata = ProjectPresentationMetadata(
        scenario="general", content_type="brand_video"
    )
    ...
    assert units[0]["storyboardImageUrl"] == fixture.artifacts["storyboard"].url
    assert units[0]["storyboardImageVersionId"] == fixture.artifacts["storyboard"].id
...
def test_reference_target_duration_never_blocks_header_or_plan(projection_fixture):
    fixture = projection_fixture
    ...
    assert build_project_header_view(snapshot, metadata)["targetDuration"] is None
    assert build_plan_view(snapshot, fixture.catalogs)["targetDuration"] is None
...
def test_edit_shots_do_not_require_camera_and_guide_ai_edit_inputs(
    ...
    edit_unit = build_plan_view(snapshot, fixture.catalogs)["sections"][0]["units"][1]
...
def test_assets_separate_ingest_tasks_from_attached_versions(projection_fixture):
    fixture = projection_fixture
    ...
    assert all(
        item["assetVersionId"] != "task-new-upload" for item in view["attachedSources"]
    )
    assert "presentationAssets" in view
    assert isinstance(view["presentationAssets"], list)
    video = next(
        item for item in view["attachedSources"] if item["assetId"] == "upload-video-01"
    )
    ...
    generated = [
        item for item in view["presentationAssets"] if item["category"] == "generated"
    ]
    ...
    presentation = next(
        item for item in view["presentationAssets"] if item["id"] == "scene-main"
    )
    ...
    source = next(
        item for item in view["presentationAssets"] if item["id"] == "upload-video-01"
    )
    assert source["assetVersionId"] == "av-video-7"
    assert source["checksum"] == fixture.assets["video"].checksum
    assert source["durationSeconds"] == fixture.assets["video"].duration_seconds
...
def test_compose_views_use_explicit_immutable_version_selections(projection_fixture):
    fixture = projection_fixture
    ...
    assert section["renderedVideoUrl"] == fixture.artifacts["section_video"].url
    assert {item["name"] for item in section["candidates"]} == {
        "U01 video v6",
        "U02 edit v3",
    }
    assert all(
        item["sourceRef"].startswith("artifact://") for item in section["candidates"]
    )
    ...
    assert final["renderedVideoRef"] == fixture.artifacts["final_video"].source_ref
    ...
    assert all(item["freshnessStatus"] == "current" for item in final["candidates"])
...
def test_final_compose_uses_selected_unit_videos_when_section_video_is_absent(
    ...
    catalogs = replace(
        fixture.catalogs,
        artifacts=tuple(
            item for item in fixture.catalogs.artifacts if item.kind != "section_video"
        )
        + (old_r2v,),
    )
    selected_versions = dict(fixture.selections.artifact_versions)
    selected_versions.pop(fixture.artifacts["section_video"].slot_id)
    selections = replace(fixture.selections, artifact_versions=selected_versions)
    ...
    assert [item["sourceKind"] for item in final["selections"]] == ["unit", "unit"]
    assert [item["artifactVersionId"] for item in final["candidates"]] == [
        "r2v-v6",
        "edit-v3",
    ]
...
def test_final_compose_partial_section_or_unit_subset_is_ready(projection_fixture):
    fixture = projection_fixture
    files = dict(fixture.snapshot.files)
    files.pop("post/final/sequence/001000--s1.ref")
    files["post/final/sequence/001000--unit--u-r2v.ref"] = WorkspaceFile(
    ...
    assert not any("FINAL_COMPOSE_SOURCE_MISSING" in item for item in final["blockers"])
