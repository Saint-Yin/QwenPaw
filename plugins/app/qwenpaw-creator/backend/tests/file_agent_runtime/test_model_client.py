...
def test_vlm_client_uses_creator_vlm_configuration(
    ...
    monkeypatch.setattr(model_client.model_config, "get_vlm_api_key", lambda: "vlm-key")
    monkeypatch.setattr(
        model_client.model_config,
        "get_vlm_base_url",
...
def test_agentscope_client_streams_native_blocks_and_raw_argument_deltas() -> None:
...
def test_agentscope_client_withholds_split_textual_tool_markup() -> None:
    ...
    async def scenario() -> None:
        ...
        with pytest.raises(AgentModelError, match="AgentScope model request failed"):
            await client.complete(
                messages=[{"role": "user", "content": "读取"}],
                tools=_tools(),
                on_text_delta=collect,
    ...
    assert all("<function" not in delta and "<parameter" not in delta for delta in text_deltas)


def test_callback_client_replays_complete_turn_through_stream_callbacks() -> None:
    ...
    async def callback(messages, tools) -> AgentModelTurn:
        callback_inputs.append(
            ([dict(item) for item in messages], [dict(item) for item in tools])
        )
...
def test_callback_client_keeps_stream_callback_failures_outside_model_errors() -> None:
