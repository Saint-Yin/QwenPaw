...
from services.runtime_files.execution_models import ExecutionAuthorizationStatus
from services.source_analysis import SourceAnalyzerOutput, SourceMediaAnalysisService
...
def test_message_text_includes_exact_project_json_selection_locator() -> None:
    ...
    assert '\"field\":\"unit:unit-1/editPlan/storyboard/panel:panel-1/description\"' in rendered
    assert '\"path\":\"/production/units_by_id/unit-1/plan/storyboard/items/panel-1/description\"' in rendered
...
def test_initial_creation_runs_auto_fix_tool_loop_without_review(tmp_path) -> None:
    async def scenario():
    ...
    project, session, goal, runs, review, messages, events = asyncio.run(scenario())
...
def test_parent_authors_story_units_and_production_without_planning_specialists(
    ...
    async def scenario():
        services, _snapshot = _create_project(
            tmp_path, initial_goal="请创建一个剪辑项目结构"
        )
        driver = FileCreatorAgentRuntime(
            services,
            model_client=CallbackAgentChatClient(callback),
...
def test_source_intelligence_receives_every_user_media_part_directly(tmp_path) -> None:
    parent_turn = 0
    ...
    async def source_callback(messages, tools):
        assert "analyze_source_media" in {item["function"]["name"] for item in tools}
...
def test_parent_reads_persisted_source_intelligence_and_links_project_structure(
    ...
    async def scenario():
        ...
        source_id = next(
            iter(services.projects.read(PROJECT_ID).project.sources.sources.items)
        )
        ...
        message = services.sessions.append_message(
            ...
            channel=MessageChannel.COMPOSER,
            classification=MessageClassification.MUTATION_INSTRUCTION,
            metadata={"assetVersionRefs": [f"asset-version:{asset_version_id}"]},
        ).message
        services.sessions.create_goal(
            PROJECT_ID,
            SESSION_ID,
    ...
    unit = project.story.sections.items["section-source"].units.items["unit-source"]
...
def test_agentdock_boundary_is_carried_into_run_and_creates_review(tmp_path) -> None:
    async def scenario():
    ...
    admitted, runs, review, pending_session, session, goal, events = asyncio.run(
        scenario()
    )
    run = runs[-1]
...
def test_redundant_supersede_preserves_pending_replacement_message(tmp_path) -> None:
    async def scenario():
        ...
        events_before_start = services.sessions.list_events(PROJECT_ID, SESSION_ID)
...
def test_costly_specialist_tool_waits_for_file_authorization(
    ...
    async def scenario():
        ...
        driver.notify(PROJECT_ID)
        await _wait_for(
            lambda: bool(driver.executions.list_execution_authorizations(PROJECT_ID))
        )
        authorization = driver.executions.list_execution_authorizations(PROJECT_ID)[0]
        await _wait_for(
            lambda: driver.executions.get_specialist_run(
                PROJECT_ID, authorization.run_id
            ).status.value
