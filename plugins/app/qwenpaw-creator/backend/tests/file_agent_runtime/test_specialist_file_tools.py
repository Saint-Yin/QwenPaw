...
def test_source_analysis_tool_explains_single_transient_retry(tmp_path) -> None:
    registry = FileSpecialistToolRegistry(
        CreatorFileServices.create(tmp_path.resolve())
    )
    ...
    tool = next(
        item for item in manifest if item["function"]["name"] == "analyze_source_media"
    )["function"]

    assert "首次分析使用 force=false" in tool["description"]
    assert "ReadTimeout" in tool["description"]
    assert "重试一次" in tool["description"]
    force = tool["parameters"]["properties"]["arguments"]["properties"]["force"]
...
def test_project_assets_scope_does_not_expand_for_r2v_image_tool(tmp_path) -> None:
    registry = FileSpecialistToolRegistry(
        CreatorFileServices.create(tmp_path.resolve())
    )
    spec = registry.spec_for(
