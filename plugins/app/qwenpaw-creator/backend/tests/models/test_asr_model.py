...
def test_fun_asr_result_normalizes_sentence_timestamps() -> None:
    result = _sentences({
        "transcripts": [{
            "sentences": [{"begin_time": 100, "end_time": 900, "text": "欢迎使用。"}]
        }]
    })
...
def test_fun_asr_can_reuse_llm_key(monkeypatch) -> None:
    ...
    monkeypatch.setattr(config, "_get_user_config", lambda: {"asr": {"enabled": True, "reuse_llm_key": True}})
...
def test_whisper_posts_extracted_audio_and_normalizes_chunk_offsets(
    ...
    monkeypatch.setattr(config, "get_asr_base_url", lambda: "https://api.openai.test/v1")
    ...
    monkeypatch.setattr(asr_model, "_extract_audio_chunks", lambda *_args: chunks)
