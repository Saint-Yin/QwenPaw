...
from services.project_files.models import IndexedFile, Project, SourceAssetVersion
...
def _services_with_source(tmp_path: Path) -> tuple[CreatorFileServices, str, str]:
...
def _output(evidence_ref: str) -> SourceAnalyzerOutput:
    coverage = {
        "visual": {"mode": "available", "producer": "model_native", "ratio": 1.0},
...
def _dispatch(service: SourceMediaAnalysisService, asset_id: str, command_id: str):
...
def test_fake_provider_publishes_one_canonical_index_and_read_apis(tmp_path) -> None:
    ...
    assert {item.kind for item in query.items} >= {"summary", "shot", "semantic"}
...
def test_url_backed_source_is_analyzed_before_runtime_cache_exists(tmp_path) -> None:
    services = CreatorFileServices.create(tmp_path.resolve())
    services.projects.create(Project.new(project_id="project-1", name="One"))
...
def test_materialize_after_project_delete_does_not_recreate_project(tmp_path) -> None:
...
def test_changed_project_head_quarantines_late_provider_result(tmp_path) -> None:
    services, asset_id, _ = _services_with_source(tmp_path)
    ...
    assert service.executions.get_run(
        "project-1", dispatch.job.run_id
    ).status is SpecialistRunStatus.STALE
    quarantine = service.executions.get_quarantine_record(
        "project-1", dispatch.job.task_id
    )
    assert "generation/ETag" in quarantine.reason
    project = services.projects.read("project-1").project
    assert not project.assets.intelligence_versions_by_id
    assert next(iter(project.sources.sources.items.values())).current_intelligence_version_id is None


def test_cancelled_task_quarantines_provider_result_without_project_commit(tmp_path) -> None:
    services, asset_id, _ = _services_with_source(tmp_path)
    analyzer = BlockingAnalyzer()
    service = SourceMediaAnalysisService(services, analyzer=analyzer)
    ...
    assert service.executions.get_run(
        "project-1", dispatch.job.run_id
    ).status is SpecialistRunStatus.CANCELLED
...
def test_default_provider_fails_explicitly_without_vlm_configuration(
    ...
    with pytest.raises(SourceAnalyzerConfigurationError, match="creator_vlm_model"):
        asyncio.run(
...
def test_missing_media_tools_fail_task_and_run_once_without_retry(
    ...
    assert service.executions.get_run(
        "project-1", dispatch.job.run_id
    ).status is SpecialistRunStatus.FAILED
    assert [
        attempt.status
    ...
    class Dispatcher:
        async def dispatch(self, **kwargs):
            ...
            return SimpleNamespace(
                job=SimpleNamespace(round_id="round-source", input_etag="sha256:input")
            )
...
def test_source_admission_replay_repairs_run_created_before_task(
    ...
    monkeypatch.setattr(service.executions, "create_task", fail_first_task_create)
...
def test_startup_recovery_repairs_then_fails_closed_orphan_source_run(
    ...
    monkeypatch.setattr(service.executions, "create_task", interrupted_task_create)
...
def test_startup_recovery_converges_project_commit_before_task_completion(
    ...
    project = services.projects.read("project-1").project
    assert (
        next(iter(project.sources.sources.items.values())).current_intelligence_version_id
        == dispatch.job.intelligence_version_id
    )
    assert service.executions.get_task(
        "project-1", dispatch.job.task_id
    ).status is TaskStatus.RUNNING
    assert service.executions.get_run(
        "project-1", dispatch.job.run_id
    ).status is SpecialistRunStatus.RUNNING_MODEL
...
def test_startup_recovery_fails_closed_queued_and_running_source_tasks(
    ...
    async def prepare():
        ...
        service.executions.append_attempt(
            ...
            input={"sourceAssetVersionId": running.job.source_version.version_id},
        )
    ...
    assert [
        ...
        for item in service.executions.list_attempts(
            "project-1", running.job.task_id
        )
    ] == [TaskAttemptStatus.RUNNING, TaskAttemptStatus.FAILED]


def test_shutdown_cancels_and_awaits_registered_source_workers(tmp_path) -> None:
    clear_source_analysis_service_registry()
    services, asset_id, _ = _services_with_source(tmp_path)
    analyzer = BlockingAnalyzer()
