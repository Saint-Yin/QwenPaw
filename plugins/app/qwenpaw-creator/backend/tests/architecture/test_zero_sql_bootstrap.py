...
def _run_sql_blocked(
    ...
    *,
    ...
    environment["QWENPAW_SECRET_DIR"] = str(data_root.parent / "qwenpaw-secrets")
...
def test_import_and_startup_closure_is_sql_free(tmp_path: Path) -> None:
    result = _run_sql_blocked(
        ...
        async def exercise_startup():
            instance = plugin.QwenPawCreatorPlugin()
            await instance._startup()
            assert instance._file_services is not None
            assert instance._file_services.startup_recovery.ok
            assert instance._file_services.startup_review_recovery.ok
            ...
            async with dev_main.lifespan(dev_main.app):
                ...
                async with AsyncClient(
                    ...
                    response = await client.get("/api/creator/health")
...
def test_distribution_metadata_declares_no_sql_packages() -> None:
    ...
    assert {_requirement_name(name) for name in SQL_MODULES}.isdisjoint(normalized)
...
def test_wheel_and_desktop_payload_exclude_sql_runtime() -> None:
    setup_paths = _literal_string_tuple(QWENPAW_ROOT / "setup.py", "_CREATOR_RUNTIME_PATHS")
    desktop_paths = _literal_string_tuple(
        QWENPAW_ROOT / "scripts" / "pack-tauri" / "qwenpaw.spec",
        "CREATOR_RUNTIME_PATHS",
    )
    assert setup_paths == desktop_paths
    ...
    assert forbidden_payload_roots.isdisjoint(setup_paths)
    assert {
    ...
    }.issubset(setup_paths)

    selected_python: set[Path] = set()
    for relative in setup_paths:
    ...
    manifest = (QWENPAW_ROOT / "MANIFEST.in").read_text(encoding="utf-8")
    for relative in setup_paths:
        assert f"plugins/app/qwenpaw-creator/{relative}" in manifest
    assert "backend/alembic.ini" not in manifest
    assert "backend/migrations" not in manifest
    assert "recursive-include plugins/app/qwenpaw-creator/backend/api *" not in manifest
    assert "recursive-include plugins/app/qwenpaw-creator/backend/services *" not in manifest

    launcher = (QWENPAW_ROOT / "scripts" / "run_creator_app.sh").read_text(
        encoding="utf-8"
    )
    assert all(name not in launcher for name in ("SQLAlchemy", "aiosqlite", "alembic"))


def test_materialized_distribution_payload_starts_without_sql(tmp_path: Path) -> None:
    runtime_paths = _literal_string_tuple(
        QWENPAW_ROOT / "setup.py", "_CREATOR_RUNTIME_PATHS"
    )
