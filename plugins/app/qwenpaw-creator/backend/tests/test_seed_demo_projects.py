...
SPEC = importlib.util.spec_from_file_location("seed_demo_projects", SCRIPT_PATH)
...
def test_seed_uses_project_asset_and_message_http_boundaries() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path == "/api/creator/projects":
            return httpx.Response(201, json=_project_response())
        if request.url.path == "/api/creator/projects/project-demo/assets":
        ...
        if request.url.path == "/api/creator/projects/project-demo/messages":
            return httpx.Response(
                202,
                json={
                    "messageSeq": 1,
    ...
    with httpx.Client(
        base_url="http://creator.test/api/creator",
    ...
    assert [request.url.path for request in requests] == [
        ...
        "/api/creator/projects/project-demo/assets",
        "/api/creator/projects/project-demo/messages",
    ]
    assert all(request.headers["idempotency-key"] for request in requests)
    project_body = json.loads(requests[0].content)
    assert project_body["clientRequestId"] == requests[0].headers["idempotency-key"]
    ...
    assert message_body["content"] == [{"type": "text", "text": seed.DEMOS[0].goal}]
...
def test_seed_polls_persisted_task_when_ingest_is_not_complete() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path.endswith("/assets"):
            return httpx.Response(
                202,
                json={"assetId": "asset-demo", "taskId": "task-demo", "status": "RUNNING"},
            )
