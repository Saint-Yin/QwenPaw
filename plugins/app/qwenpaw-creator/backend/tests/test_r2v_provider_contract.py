...
def test_unit_normalization_golden_summary_and_renumbering() -> None:
    source = [
        {
            ...
            "title": "连续动作",
            "shots": [
                {"shot_number": 7, "title": "起身", "duration": 8, "dialogue": ""},
                {"shot_number": 8, "title": "奔跑", "duration": 8, "dialogue": "快走"},
            ],
        }
    ]
    ...
    assert [segment["shots"][0]["shot_number"] for segment in normalized] == [1, 1]
    assert normalized[1]["shots"][0]["dialogue"] == "快走"
    assert all(
        segment["split_reason"] == "r2v_15_second_limit" for segment in normalized
    )
