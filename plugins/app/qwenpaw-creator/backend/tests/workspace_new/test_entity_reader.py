...
def test_entity_reader_reads_ordered_and_plain_entities_without_raw_io(workspace_stack):
    files = {
    ...
    branch = workspace_stack.text.open_working_tree(
        project_id="project-1", base_revision_id=revision.id, branch_id="branch-1"
    )
    ...
    assert section.file_map()["units/001000--unit-1--shot/title.txt"].text == "镜头一"
    ...
    assert workspace_stack.entities.read_ref(
        "project-1",
...
def test_entity_reader_keeps_revision_snapshot_isolated_from_working_change(workspace_stack):
    revision = workspace_stack.revisions.create_initial_approved_revision(
    ...
    assert workspace_stack.entities.read_text(
        "project-1", path, branch_id=branch.id
    ) == "Working 标题"
    assert workspace_stack.entities.read_text(
        "project-1", path, revision_id=revision.id
    ) == "旧标题"


def test_entity_reader_rejects_path_escape_and_ambiguous_snapshot_selector(workspace_stack):
    revision = workspace_stack.revisions.create_initial_approved_revision(
