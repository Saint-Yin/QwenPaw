...
def test_workspace_path_escape_private_roots_and_non_text_authority_are_rejected(path):
...
def test_workspace_glob_has_segment_aware_double_star_semantics():
    ...
    assert not glob_matches("story/sections/s1/script.txt", "story/**/script.md")
...
def test_each_domain_file_has_one_exact_role_owner(path, owner):
    ...
    if owner == CREATOR_ROLE:
        ...
        assert not any(registry.can_write_path(role, path) for role in SpecialistRole)
        return
    allowed = [role for role in SpecialistRole if registry.can_write_path(role, path)]
    if owner in {
        SpecialistRole.SOURCE_INTELLIGENCE,
        SpecialistRole.AI_EDITING_DIRECTOR,
    }:
        ...
        assert not registry.can_project_path(SpecialistRole.R2V_GENERATION_DIRECTOR, path)
    else:
...
def test_unit_planning_does_not_gain_r2v_story_or_section_wide_writes():
    ...
    assert registry.can_write_path(role, "story/sections/s1/units/u1/route.txt")
    assert not registry.can_write_path(
        role, "story/sections/s1/units/u1/production/r2v/video/prompt.md"
    )
