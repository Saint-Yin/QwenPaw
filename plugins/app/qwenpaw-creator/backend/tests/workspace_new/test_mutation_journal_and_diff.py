...
from services.workspace.mutations import MutationActor, MutationKind, new_mutation
...
def _record_for_result(result, *, target_ref: str):
    ...
    return new_mutation(
        ...
        target_ref=target_ref,
        path=path,
        before_blob_hash=(result.before_entry.blob_hash if result.before_entry else None),
        after_blob_hash=(result.after_entry.blob_hash if result.after_entry else None),
    )


def test_typed_journal_and_full_tree_diff_cover_every_change_kind(workspace_stack):
    base = workspace_stack.revisions.create_initial_approved_revision(
        project_id="project-1", files=BASE_FILES
    )
    branch = workspace_stack.text.open_working_tree(
        project_id="project-1", base_revision_id=base.id, branch_id="branch-1"
    )
    ...
    results = [
        ...
        workspace_stack.text.delete(
            project_id="project-1", branch_id=branch.id, path="delete.md"
        ),
    ...
    diff = DiffService(workspace_stack.content).compare(base, target, journal=records)
    ...
    assert by_kind[MutationKind.CHANGE_REFERENCE].before_ref == "project://unit/unit-1"
    assert by_kind[MutationKind.CHANGE_REFERENCE].after_ref == "project://unit/unit-2"
    assert by_kind[MutationKind.REPLACE_MEDIA].before_ref == "asset://video-a@asset-v1"
    assert by_kind[MutationKind.REPLACE_MEDIA].after_ref == "artifact://unit-video@artifact-v2"
...
def test_journal_hash_detects_private_metadata_tampering(workspace_stack):
    ...
    result = workspace_stack.text.write_text(
        project_id="project-1", branch_id=branch.id, path="title.txt", text="after"
    )
    journal = MutationJournal(workspace_stack.content.root)
