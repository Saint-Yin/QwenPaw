...
def _app(tmp_path):
    ...
    project.story = Story(
        ...
        sections=EntityCollection(
            items={section.section_id: section}, order=[section.section_id]
        ),
    )
...
def test_asset_view_exposes_pending_ingest_and_normalizes_uploaded_video(tmp_path) -> None:
...
def test_file_views_return_structured_not_found(tmp_path) -> None:
    ...
    async def scenario():
        ...
        async with httpx.AsyncClient(
            ...
            missing_section = await client.get("/projects/project-1/sections/missing")
    ...
    async def scenario():
        ...
        async with httpx.AsyncClient(
            ...
            return await client.get(
                "/projects/project-1/refs", params={"types": "analysis"}
            )
