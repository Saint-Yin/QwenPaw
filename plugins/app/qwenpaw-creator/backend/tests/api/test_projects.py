...
def test_project_create_is_atomic_file_native_idempotent_and_has_no_goal(
    ...
    assert [item["projectId"] for item in listed.json()["items"]] == [project_id]
    assert listed.json()["items"][0]["name"] == "创建后改名"

    project = Project.model_validate_json(
        (api_runtime_root / project_id / "project.json").read_text(encoding="utf-8")
    )
    ...
    assert [item.conversation_id for item in conversations] == [body["conversationId"]]
    assert session.active_goal_id is None
    assert list((api_runtime_root / project_id / "runtime" / "goals").glob("*.json")) == []
    assert not list(api_runtime_root.rglob("*.sqlite*"))


def test_project_create_can_atomically_bootstrap_initial_goal(app, api_runtime_root):
    async def scenario():
        async with AsyncClient(
            transport=ASGITransport(app=app), base_url="http://test"
        ) as client:
    ...
    async def scenario():
        async with AsyncClient(
            ...
            deleted = await client.delete(
                delete_url, headers={"Idempotency-Key": "delete-project-request"}
            )
...
def test_project_runtime_bootstrap_failure_never_publishes_half_project(
    app, api_runtime_root, monkeypatch
):
...
def test_removed_whole_project_put_is_404_and_not_registered(app):
    async def scenario():
        async with AsyncClient(
            ...
            removed_put = await client.put(project_url, json={"name": "legacy"})
            ordinary_method_mismatch = await client.patch(
                project_url, json={"name": "unsupported patch"}
            )
