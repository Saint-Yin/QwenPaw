...
def test_text_asset_is_published_indexed_attached_and_replayed(tmp_path) -> None:
    ...
    async def scenario():
        ...
        async with httpx.AsyncClient(
            ...
            media = await client.get(f"/media/assets/{first.json()['assetVersionId']}")
...
def test_indexed_media_supports_byte_ranges_and_utf8_filenames(tmp_path) -> None:
    ...
    assert "filename*=UTF-8''%E7%8C%AB%E5%92%AA%E6%88%90%E7%89%87.mp4" in disposition
...
def test_remote_asset_streams_to_file_and_project_json_only_indexes_it(
    ...
    async def scenario():
        ...
        async with httpx.AsyncClient(
            ...
            await asyncio.gather(*list(file_asset_routes._REMOTE_INGEST_TASKS.values()))
            replay = await client.post(
                "/projects/project-1/assets",
                headers={"Idempotency-Key": "remote-asset-1"},
                json=request_payload,
    ...
    first, replay, before_cache, after_cache, keyframe = asyncio.run(scenario())
    assert first.status_code == 202
    assert replay.status_code == 202
    ...
    assert (project_root / cache_item["cacheRelativeUri"]).read_bytes() == payload
...
def test_registered_remote_asset_is_indexed_while_uncached_media_is_unavailable(
    ...
    version = services.projects.read("project-1").project.assets.source_versions_by_id[
        item["assetVersionId"]
    ]
    assert version.file_id is None
...
def test_remote_asset_download_does_not_hold_project_lifecycle_lock(
    ...
    def staged_download(
        ...
        *,
        ...
        staged = file_store.stage_bytes(b"locked-remote", staging_id=staging_id)
...
def test_folder_import_commits_all_files_in_one_generation(tmp_path) -> None:
    ...
    assert [item["name"] for item in loaded.json()["items"]] == ["a.txt", "b.txt"]
    project = services.projects.read("project-1").project
    assert project.generation == 1
...
def test_remote_asset_url_rejects_private_and_resolved_private_addresses() -> None:
...
def test_remote_asset_url_accepts_only_public_http_without_credentials() -> None:
