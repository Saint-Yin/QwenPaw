...
def _app(tmp_path):
    ...
    snapshot = services.projects.create(Project.new(project_id="project-1", name="One"))
...
def test_file_execution_authorization_can_be_polled_and_approved(tmp_path) -> None:
    app, services, snapshot, _bootstrap = _app(tmp_path)
    executions = ProjectExecutionStore(services.root)
