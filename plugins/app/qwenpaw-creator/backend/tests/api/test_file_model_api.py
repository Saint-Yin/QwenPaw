...
def test_model_config_is_single_file_native_and_idempotent(
    ...
    assert list((tmp_path / "config" / "runtime" / "idempotency").rglob("*.json"))
    assert not any(
...
def test_concurrent_single_file_save_is_atomic_and_last_writer_wins(
    ...
    first = threading.Thread(target=save, args=(updater,), name="secret-updater")
    second = threading.Thread(target=save, args=(second_writer,), name="second-writer")
    first.start()
    ...
    persisted = json.loads(
        (tmp_path / "config" / "model_config.json").read_text(encoding="utf-8")
    )
