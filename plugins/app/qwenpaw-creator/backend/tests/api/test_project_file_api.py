...
def test_project_snapshot_etag_patch_and_disjoint_stale_merge(tmp_path) -> None:
    ...
    assert patch_description.json()["project"]["description"] == "User description"
...
def test_invalid_external_project_keeps_last_good_and_reports_sync_error(tmp_path) -> None:
    ...
    services.projects.project_path("project-1").write_text("{invalid", encoding="utf-8")
...
def test_deleted_project_snapshot_does_not_serve_stale_last_good(tmp_path) -> None:
    app, services, _base = _app(tmp_path)
    services.poller.open("project-1")
...
def test_active_review_poll_is_created_only_from_review_boundary(tmp_path) -> None:
...
def test_project_patch_replays_success_and_rejects_payload_drift(tmp_path) -> None:
    app, _services, base = _app(tmp_path)
    payload = {
...
def test_project_patch_recovers_crash_before_idempotency_completion(
    ...
    def crash_before_completion(self, **kwargs):
        nonlocal crashed
        if not crashed and kwargs["scope"] == "PATCH /projects/{projectId}/project":
            crashed = True
            raise SimulatedCompletionWriteFailure
        return original_complete(self, **kwargs)

    monkeypatch.setattr(IdempotencyRecordStore, "complete", crash_before_completion)
    ...
    transactions_root = (
        services.projects.project_root("project-1") / "runtime" / "transactions"
    )
    assert sorted(
...
def test_failed_project_patch_is_terminal_and_replays_exact_error(tmp_path) -> None:
    app, services, base = _app(tmp_path)
    stale_payload = {
...
def test_review_decision_replays_success_and_rejects_payload_drift(tmp_path) -> None:
    app, services, base = _app(tmp_path)
...
def test_review_decision_recovers_crash_before_idempotency_completion(
    ...
    async def scenario():
        ...
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
            ...
            assert services.reviews.get(
                "project-1", review.review_id
...
def test_failed_review_decision_is_terminal_and_replays_exact_error(tmp_path) -> None:
    app, services, base = _app(tmp_path)
    review = _pending_review(services, base)
    payload = {
...
def test_missing_project_writes_do_not_create_a_phantom_directory(tmp_path) -> None:
    services = CreatorFileServices.create(tmp_path.resolve())
    app = FastAPI()
    app.add_exception_handler(CreatorError, creator_error_handler)
    app.include_router(router)
    ...
    assert [result.status_code for result in results] == [404, 404, 404, 404, 404]
