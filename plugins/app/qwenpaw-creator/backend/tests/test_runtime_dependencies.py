...
def test_resolvers_prefer_explicit_creator_paths(
    ...
    monkeypatch.setenv(dependencies.CREATOR_FFPROBE_PATH_ENV, os.fspath(ffprobe))
...
def test_system_jq_remains_usable_on_an_unsupported_auto_install_platform(
    ...
    monkeypatch.setattr(dependencies, "_platform_key", lambda: ("freebsd", "riscv64"))

    status = dependencies.CreatorBinaryManager(binary_dir=tmp_path / "bin").ensure_jq()
...
def test_manager_downloads_and_verifies_pinned_jq(
    ...
    monkeypatch.setattr(dependencies, "_platform_key", lambda: ("linux", "amd64"))
    monkeypatch.setitem(
    ...
    monkeypatch.setattr(dependencies.shutil, "which", lambda _name: None)
    monkeypatch.setattr(
        dependencies, "urlopen", lambda *_args, **_kwargs: io.BytesIO(payload)
    )
    monkeypatch.setenv(
        dependencies.CREATOR_JQ_BASE_URL_ENV, "https://mirror.invalid/jq"
    )
    ...
    assert os.environ[dependencies.CREATOR_JQ_PATH_ENV] == os.fspath(manager.jq_path)
...
def test_manager_rejects_a_jq_checksum_mismatch(
    ...
    monkeypatch.setattr(dependencies.shutil, "which", lambda _name: None)
    monkeypatch.setattr(
        dependencies, "urlopen", lambda *_args, **_kwargs: io.BytesIO(b"tampered")
    )
    manager = dependencies.CreatorBinaryManager(binary_dir=tmp_path / "bin")
...
def test_ffprobe_is_optional_when_managed_ffmpeg_is_ready(
    ...
    status = dependencies.CreatorBinaryManager(binary_dir=tmp_path / "bin").ensure_all()
