...
def _creator_data_root(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
...
def _install_storyboard(services: CreatorFileServices) -> None:
    ...
    candidate["assets"]["artifact_versions_by_id"][artifact.version_id] = (
        artifact.model_dump(mode="json")
    )
    candidate["assets"]["artifact_slots_by_id"][slot.slot_id] = slot.model_dump(
        mode="json"
    )
...
async def _seed_materialized_descriptor(
    ...
    *,
    ...
    indexed, published = worker._build_materialized_publication(  # noqa: SLF001
        task,
        seeded,
        stable=stable,
...
def test_r2v_dispatch_submits_polls_and_commits_exact_project_versions(
    ...
    async def scenario():
        ...
        task = await worker.wait_for_task(
            "project-1", dispatch.task_id, timeout_seconds=3
        )
    ...
    assert runtime.get_run("project-1", dispatch.run_id).status is SpecialistRunStatus.SUCCEEDED
    ...
    assert project.assets.artifact_slots_by_id[
        "unit:unit-1:video"
    ].selected_version_id == selected
    assert AssetFileStore(services.projects.project_root("project-1")).inspect(
...
def test_r2v_remote_provider_has_task_scratch_before_materialization(
    ...
    monkeypatch.setattr(r2v_execution, "materialize_r2v_video", materialize_remote)
...
def test_r2v_replay_recreates_missing_active_task_scratch(tmp_path: Path) -> None:
    services = _services(tmp_path)
    provider = RemoteURLR2VProvider()
...
def test_r2v_project_change_quarantines_published_unindexed_output(
    ...
    async def scenario():
        worker = FileR2VExecutionService(
            services, provider=provider, poll_interval_seconds=0.01
        )
        dispatch = await worker.dispatch(
            project_id="project-1",
            target_ref="unit:unit-1",
    ...
    runtime = ProjectExecutionStore(services.root)
    assert runtime.get_quarantine_record(
        "project-1", task.task_id
    ).reason == "PROJECT_INPUT_SNAPSHOT_STALE"
    assert runtime.get_run("project-1", task.run_id).status is SpecialistRunStatus.STALE
    project = services.projects.read("project-1").project
    assert project.production.units_by_id[
        "unit-1"
    ].selected_video_artifact_version_id is None
    assert all(
...
def test_r2v_cancel_during_success_quarantines_late_output(tmp_path: Path) -> None:
    services = _services(tmp_path)
    runtime = ProjectExecutionStore(services.root)
    ...
    assert runtime.get_run("project-1", task.run_id).status is SpecialistRunStatus.CANCELLED
    assert services.projects.read("project-1").project.production.units_by_id[
        "unit-1"
    ].selected_video_artifact_version_id is None
...
def test_r2v_materialize_heartbeat_covers_stage_publish_and_state_cas(
    ...
    monkeypatch.setattr(r2v_execution, "_stage_materialized_video", blocking_stage)
    ...
    assert list((services.projects.project_root("project-1") / "assets" / ".staging").iterdir()) == []
...
def test_r2v_cancel_during_published_file_waits_for_owner_then_quarantines(
    ...
    assert indexed["file_id"] not in services.projects.read(
        "project-1"
    ).project.assets.files_by_id
    asset_path = services.projects.project_root("project-1") / indexed["relative_uri"]
...
def test_r2v_submit_heartbeat_starts_before_attempt_transition(
    ...
    async def scenario():
        ...
        monkeypatch.setattr(first.executions, "append_attempt", blocking_append)
        first.start_task("project-1", dispatch.task_id)
        assert await asyncio.to_thread(entered.wait, 2)
        await asyncio.sleep(0.2)
        second = FileR2VExecutionService(
...
def test_r2v_cancelled_task_adopts_asset_from_expired_materialize_descriptor(
    ...
    async def unexpected_materialize(*_args, **_kwargs):
        raise AssertionError("published descriptor recovery must not redownload")
    ...
    async def scenario():
        ...
        file_store = AssetFileStore(services.projects.project_root("project-1"))
        with source.open("rb") as stream:
            staged = file_store.stage_stream(
                stream,
                staging_id=dispatch.task_id[:80],
...
def test_r2v_expired_submit_claim_without_provider_id_fails_closed(
    ...
    async def scenario():
        ...
        worker._update_state_sync(  # noqa: SLF001 - explicit crash fixture
            "project-1", dispatch.task_id, expire
        )
...
def test_r2v_admission_recovers_run_without_task_or_state(
    ...
    async def scenario():
        ...
        state = worker._read_state_sync("project-1", replay.task_id)  # noqa: SLF001
        await worker.shutdown()
...
def test_r2v_terminal_reaper_waits_for_live_materialize_lease_then_quarantines(
    ...
    async def unexpected_materialize(*_args, **_kwargs):
        raise AssertionError("cancelled descriptor recovery must not redownload")
    ...
    async def scenario():
        ...
        dispatch, _source, indexed, _published = (
        ...
        await worker.recover_all()
        deferred = worker._read_state_sync("project-1", dispatch.task_id)  # noqa: SLF001
        assert deferred.phase == "PROVIDER_SUCCEEDED"
        deadline = asyncio.get_running_loop().time() + 2
        while True:
            state = worker._read_state_sync("project-1", dispatch.task_id)  # noqa: SLF001
            if state.phase == "QUARANTINED":
                break
            if asyncio.get_running_loop().time() >= deadline:
                raise TimeoutError("terminal materialize reaper did not converge")
            await asyncio.sleep(0.01)
        quarantine = worker.executions.get_quarantine_record(
            "project-1",
            dispatch.task_id,
...
def test_r2v_cancel_during_live_submit_preserves_and_binds_provider_id(
    ...
    async def scenario():
        ...
        protected = second._read_state_sync("project-1", dispatch.task_id)  # noqa: SLF001
        assert protected.phase == "SUBMIT_CLAIMED"
        assert protected.submit_claim_token is not None
        provider.submit_release.set()
        deadline = asyncio.get_running_loop().time() + 2
        while True:
            state = first._read_state_sync("project-1", dispatch.task_id)  # noqa: SLF001
...
def test_r2v_cancelled_missing_descriptor_output_never_redownloads(
    ...
    async def scenario():
        ...
        dispatch, source, _indexed, _published = (
            await _seed_materialized_descriptor(
                worker,
...
def test_r2v_active_recovery_reuses_verified_descriptor_scratch_before_url(
    ...
    async def scenario():
        ...
        def stale_provider_url(current):
            ...
            provider_result["result_url"] = (source.parent / "gone.mp4").as_uri()
...
def test_r2v_terminal_quarantined_task_repairs_missing_quarantine_record(
    ...
    async def scenario():
        ...
        dispatch, _source, _indexed, published = (
            await _seed_materialized_descriptor(
                worker,
        ...
        with pytest.raises(RecordNotFoundError):
            worker.executions.get_quarantine_record("project-1", dispatch.task_id)
        await worker.recover_all()
