...
@pytest.mark.parametrize(
    ...
    [
        ...
        (_QUICKTIME, "video/quicktime", "quicktime", "video/quicktime", ".mov"),
...
@pytest.mark.parametrize(
    "source_factory",
    [
        lambda root, _scratch: str(root / "runtime" / "task-work" / "task-2" / "v.mp4"),
        lambda root, _scratch: (
            root.parent / "project-2" / "runtime" / "task-work" / "task-1" / "v.mp4"
        ).as_uri(),
...
@_run_async
async def test_openat_rejects_symlink_parent_and_hard_link(tmp_path: Path) -> None:
    project_root, scratch = _scope(tmp_path)
...
async def test_size_mime_checksum_and_magic_failures_remove_partial_file(
    ...
    cases = [
        ...
        (
            SecureR2VVideoMaterializer(),
            {"path": str(source), "media_type": "video/mp4", "checksum": "0" * 64},
...
@_run_async
async def test_remote_stream_pins_peer_to_preresolved_public_dns(tmp_path: Path) -> None:
    project_root, scratch = _scope(tmp_path)
...
@_run_async
async def test_remote_rejects_peer_outside_dns_set_and_private_dns(tmp_path: Path) -> None:
    project_root, scratch = _scope(tmp_path)
    requests = 0
    ...
    with pytest.raises(ValidationError, match="私有|保留"):
        await SecureR2VVideoMaterializer(
            ...
            {"url": "https://private.example/result", "media_type": "video/mp4"},
            project_root=project_root,
            project_id="project-1",
            task_id="task-1",
        )
...
@_run_async
async def test_each_redirect_hop_is_resolved_and_peer_pinned_again(tmp_path: Path) -> None:
...
@_run_async
async def test_shared_process_semaphore_bounds_materializers(tmp_path: Path) -> None:
    project_root, _scratch = _scope(tmp_path)
