...
def _sentences(payload: Mapping[str, Any]) -> tuple[ASRSegment, ...]:
    ...
    for transcript in payload.get("transcripts") or ():
        ...
        if not sentences and transcript.get("text"):
            duration = int(transcript.get("content_duration_in_milliseconds") or 1)
            sentences = ({"begin_time": 0, "end_time": duration, "text": transcript["text"]},)
        for sentence in sentences:
            ...
            if text and end > start:
                values.append(ASRSegment(start, end, text, speaker=str(sentence.get("speaker_id") or "") or None))
    return tuple(values)
...
async def _fun_asr(media_url: str) -> ASRResult:
    ...
    if not key:
        raise ValueError("Fun-ASR requires ASR API key or enabled LLM key reuse")
    headers = {
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(30, read=60)) as client:
        response = await client.post(
            ...
            headers=headers,
            json={"model": model, "input": {"file_urls": [media_url]}, "parameters": {}},
        )
        ...
        while True:
            ...
            succeeded = next((item for item in results if isinstance(item, Mapping) and item.get("subtask_status") == "SUCCEEDED"), None)
            if status != "SUCCEEDED" or not succeeded:
                raise RuntimeError(f"Fun-ASR failed: {output}")
            with tempfile.TemporaryDirectory(prefix="creator-fun-asr-") as directory:
            ...
            for seg in segments:
                logger.debug(
                    "  [%d-%d] %s", seg.start_ms, seg.end_ms, seg.text,
                )
            return result
...
def _local_media_path(media_url: str, directory: Path) -> Path:
    ...
    if parsed.scheme not in {"http", "https"}:
        raise ValueError("Whisper input must be a local file or HTTP(S) media URL")
...
def _extract_audio_chunks(source: Path, directory: Path) -> list[Path]:
    ...
    command = [
        ...
        "-segment_time", "3600", "-reset_timestamps", "1", str(pattern),
    ]
    result = subprocess.run(command, capture_output=True, text=True, timeout=600)
    if result.returncode != 0:
        raise RuntimeError(f"ffmpeg audio extraction failed: {(result.stderr or result.stdout)[-500:]}")
    chunks = sorted(directory.glob("audio-*.mp3"))
    if not chunks or any(path.stat().st_size >= 25 * 1024 * 1024 for path in chunks):
        raise RuntimeError("Whisper audio extraction did not produce <25 MB chunks")
    return chunks
...
async def _whisper(media_url: str) -> ASRResult:
    ...
    with tempfile.TemporaryDirectory(prefix="creator-asr-") as raw_directory:
        ...
        source = await asyncio.to_thread(_local_media_path, media_url, directory)
        chunks = await asyncio.to_thread(_extract_audio_chunks, source, directory)
        async with httpx.AsyncClient(timeout=config.get_asr_timeout_seconds()) as client:
            for index, chunk in enumerate(chunks):
                ...
                with chunk.open("rb") as handle:
                    response = await client.post(
                        _endpoint(config.get_asr_base_url(), "audio/transcriptions"),
                        headers=headers,
                        data=data,
...
async def transcribe(media_url: str) -> ASRResult:
    ...
    logger.info("ASR transcribe started: provider=%s url=%s", provider, media_url[:120])
    if provider == "whisper":
