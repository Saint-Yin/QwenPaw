...
def infer_visual_media_type(filename: str, content_type: str = "") -> str | None:
...
async def chat_completion(
    ...
    *,
    ...
    if has_media and get_capability_cache().get(f"vlm:{model_name}", "rejects_media"):
        raise ModelError(
            "该模型已知不支持多模态输入",
            model_name=model_name,
        )
    ...
    media_parts = [p for p in content if isinstance(p, dict) and p.get("type") in ("image_url", "video_url")]
    ...
    logger.info(
        ...
        model_name, image_count, video_count, max_tokens,
    )
    start_ts = time.perf_counter()
    actual_timeout = timeout if timeout is not None else model_config.get_vlm_timeout_seconds()
    try:
        ...
        if has_media and _is_media_related_error(exc.response.text):
            get_capability_cache().learn(f"vlm:{model_name}", "rejects_media", True)
        elapsed = time.perf_counter() - start_ts
        logger.error(
            "VLM request failed: %s %s elapsed=%.2fs",
            exc.response.status_code, exc.response.text[:500], elapsed,
        )
        raise ModelError(
        ...
        logger.error(
            ...
            type(exc).__name__, exc, model_config.get_vlm_chat_url(),
            model_config.get_vlm_timeout_seconds(), elapsed,
            exc_info=True,
        )
        raise ModelError(
    ...
    logger.info(
        ...
        model_name, elapsed, output_chars, finish_reason or "unknown",
    )
    if not choices:
        raise ModelError(f"No VLM choices in response: {payload}", model_name=model_name)
    ...
    if not isinstance(content_text, str) or not content_text.strip():
        raise ModelError(f"Empty VLM response: {payload}", model_name=model_name)
