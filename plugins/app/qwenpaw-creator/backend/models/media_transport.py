...
_dashscope_temp_upload_cache: dict[tuple[str, int, int, str, str], tuple[str, float]] = {}
_dashscope_temp_upload_locks: dict[
    tuple[str, int, int, str, str], asyncio.Lock
] = {}
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        resp = await client.post(
            ...
            data={
                ...
                "x-oss-object-acl": policy_data.get("x_oss_object_acl", "default"),
                "x-oss-forbid-overwrite": policy_data.get("x_oss_forbid_overwrite", "true"),
                "key": key,
...
def _upload_local_file_to_dashscope_temp_sync(
    ...
    *,
    ...
    with httpx.Client(timeout=timeout, follow_redirects=True) as client:
        ...
        try:
            max_policy_bytes = int(float(policy["max_file_size_mb"]) * 1024 * 1024)
        except (KeyError, TypeError, ValueError) as exc:
            raise RuntimeError("DashScope upload policy missing max_file_size_mb") from exc
...
async def upload_local_file_to_dashscope_temp(
    ...
    *,
    ...
    if not resolved.is_file():
        raise FileNotFoundError(f"selected local media does not exist: {resolved}")
    if not api_key.strip() or not model_name.strip():
        raise RuntimeError("DashScope temporary upload requires API key and model name")
...
def creator_oss_readiness() -> dict:
    ...
    if not access_key_id:
        blockers.append("creator_media_oss.access_key_id or OSS_ACCESS_KEY_ID is required")
    if not access_key_secret:
        blockers.append("creator_media_oss.access_key_secret or OSS_ACCESS_KEY_SECRET is required")
    if not endpoint:
        blockers.append("creator_media_oss.endpoint or OSS_ENDPOINT is required")
    return {
...
def _upload_with_oss2(file_content: bytes, filename: str, backend: str) -> str:
    try:
        ...
        raise RuntimeError("oss2 package is required for creator-media uploads") from exc
...
async def upload_reference_media_to_creator_oss(file_content: bytes, filename: str, backend: str) -> str:
    """Upload reference media to creator-media and return a public HTTPS URL."""
    if backend not in {"wan", "seedance2"}:
        raise ValueError(f"Unsupported video backend for OSS upload: {backend}")
    return await asyncio.to_thread(_upload_with_oss2, file_content, filename, backend)
...
async def read_reference_media(url: str) -> tuple[bytes, str]:
    ...
    if url.startswith("/generated/"):
        ...
        return media_path.read_bytes(), media_path.name or f"reference-{uuid.uuid4().hex}.bin"
