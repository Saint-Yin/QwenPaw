...
from models.media_transport import read_reference_media, upload_reference_media_to_creator_oss
...
async def _resolve_reference_media_url(url: str, backend: str) -> str:
    ...
    if url.startswith("/generated/"):
        filename = Path(urlparse(url).path).name or f"reference-{uuid.uuid4().hex}.bin"
    ...
    try:
        ...
        public_url = await upload_reference_media_to_creator_oss(content, filename, backend)
        logger.info(
...
def _normalize_seedance_resolution(resolution: str, model_name: str) -> str:
    ...
    if value not in SEEDANCE_RESOLUTIONS:
        raise ModelError(f"Seedance2 resolution must be one of {sorted(SEEDANCE_RESOLUTIONS)}", model_name=model_name)
    return value
...
def _normalize_seedance_ratio(ratio: str, model_name: str) -> str:
    ...
    if value not in SEEDANCE_RATIOS:
        raise ModelError(f"Seedance2 ratio must be one of {sorted(SEEDANCE_RATIOS)}", model_name=model_name)
    return value


def _normalize_seedance_duration(duration: int, model_name: str) -> int:
    if duration < 4 or duration > 15:
        raise ModelError("Seedance2 duration must be between 4 and 15 seconds", model_name=model_name)
    return duration
...
async def submit_video_task(
    ...
    for img_url in dict.fromkeys(all_images):
        if img_url and img_url.strip():
            resolved_url = await _resolve_reference_media_url(img_url.strip(), upload_backend)
            media.append({
    ...
    if uses_seedance:
        ...
        body = {
            ...
            "watermark": watermark,
            "model": model_name,
            "resolution": _normalize_seedance_resolution(resolution, model_name),
            "content": content,
    ...
    try:
            ...
            async with model_slot("video"):
                for attempt in range(MAX_RETRIES):
                    ...
                    if resp.status_code == 429:
                        ...
                        logger.warning(f"Video submit rate limited (429), retrying in {wait}s (attempt {attempt+1}/{MAX_RETRIES})")
        ...
        task_id = output.get("task_id") or data.get("task_id") or data.get("taskId")
        ...
        logger.error(f"Video task submission HTTP error: {e.response.status_code} - {e.response.text[:500]}")
        ...
        if isinstance(obj, dict):
            if str(obj.get("status", "")).lower() == "failed":
                ...
                if isinstance(error, dict):
                    ...
                    code = ""
                    message = str(error) if error else str(obj.get("message") or "Task failed")
                found["code"] = code
...
async def check_task_status(task_id: str) -> dict:
    ...
    try:
        ...
        status_raw = str(payload.get("task_status") or payload.get("status") or "unknown").upper()
        ...
        if status == "SUCCEEDED":
            ...
            logger.info(f"Video task succeeded | task_id={task_id}, url={video_url[:80] if video_url else ''}")
        elif status == "FAILED":
            error_payload = payload.get("error") or {}
            error_msg = error_payload.get("message") if isinstance(error_payload, dict) else str(error_payload)
            error_msg = error_msg or payload.get("message") or "Task failed"
            result["error"] = error_msg
            logger.warning(f"Video task failed | task_id={task_id}: {error_msg}")
        else:
        ...
        if failure is not None:
            ...
            return {"task_id": task_id, "status": "FAILED", "error": failure["message"]}
        logger.error(
        ...
        raise ModelError(f"Task status check failed: {str(e)}", model_name=model_name)
