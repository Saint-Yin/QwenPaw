...
VLM_MAX_INLINE_BYTES = _positive_int_env("VLM_MAX_INLINE_BYTES", 20 * 1024 * 1024)

# ── ASR Model (OpenAI Whisper / DashScope Fun-ASR) ───────────────────────────
ASR_BASE_URL = os.environ.get("ASR_BASE_URL", "https://dashscope.aliyuncs.com/api/v1")
...
def get_asr_api_key() -> str:
    configured = _explicit_configured_value(
        CREATOR_ASR_CONFIG_TOOL, "api_key", ("ASR_API_KEY",)
    )
    if configured:
        return configured
...
def get_asr_base_url() -> str:
    return _configured_value(
        CREATOR_ASR_CONFIG_TOOL, ("base_url", "endpoint"), "ASR_BASE_URL", ASR_BASE_URL
    )


def get_asr_model_name() -> str:
    return _configured_value(
        CREATOR_ASR_CONFIG_TOOL, "model", "ASR_MODEL_NAME", ASR_MODEL_NAME
    )


def get_asr_provider() -> str:
    value = _configured_value(
        CREATOR_ASR_CONFIG_TOOL, "provider", "ASR_PROVIDER", ASR_PROVIDER
    ).casefold()
...
def get_asr_language() -> str:
    return _configured_value(
        CREATOR_ASR_CONFIG_TOOL, "language", "ASR_LANGUAGE", ASR_LANGUAGE
    )
...
def is_asr_enabled() -> bool:
    ...
    if isinstance(section, dict) and section.get("enabled") is True:
        return True
    return os.environ.get("ASR_ENABLED", "").casefold() in {"1", "true", "yes", "on"}
...
def get_oss_access_key_id() -> str:
    ...
    if persisted:
        return persisted
    return _first_env(
        "OSS_ACCESS_KEY_ID", "ALIYUN_ACCESS_KEY_ID", "ALIBABA_CLOUD_ACCESS_KEY_ID"
    )
