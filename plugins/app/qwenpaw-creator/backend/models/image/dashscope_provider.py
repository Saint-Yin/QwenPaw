...
class DashScopeImageModel(BaseImageModel):
    ...
    def from_config(cls) -> "DashScopeImageModel":
        ...
        return cls(
            ...
            timeout=_configured_int(
                ...
                int(os.environ.get("DASHSCOPE_IMAGE_TIMEOUT", os.environ.get("IMAGE_TIMEOUT", "120")) or 120),
            ),
    ...
    async def _request(
        ...
        body = await self._build_body(prompt, aspect_ratio, clean_reference_urls)
            ...
            if isinstance(content, list):
                for block in content:
                    if isinstance(block, dict) and block.get("image"):
                        return await download_remote_image(str(block["image"]), self.model_name)
            if isinstance(content, dict) and content.get("image"):
                return await download_remote_image(str(content["image"]), self.model_name)
        raise ModelError(
