...
async def build_reference_image_files(
    ...
    for index, raw_url in enumerate(dict.fromkeys(reference_image_urls[:16]), start=1):
        url = raw_url.strip()
        if not url:
            continue
...
class OpenAIImageModel(BaseImageModel):
    ...
    def from_config(cls) -> "OpenAIImageModel":
        ...
        base_url = _configured_value(
            ...
            os.environ.get("OPENAI_IMAGE_BASE_URL", os.environ.get("IMAGE_BASE_URL", DEFAULT_BASE_URL)),
        )
        return cls(
            model_name=_configured_value(
                "model",
                "OPENAI_IMAGE_MODEL_NAME",
                os.environ.get("OPENAI_IMAGE_MODEL_NAME", os.environ.get("IMAGE_MODEL_NAME", DEFAULT_MODEL_NAME)),
            ),
            api_key=_configured_value(
                "api_key",
                "OPENAI_IMAGE_API_KEY",
                os.environ.get("OPENAI_IMAGE_API_KEY", os.environ.get("IMAGE_API_KEY", "")),
            ),
            base_url=base_url,
            quality=_configured_value(
                "quality",
                "OPENAI_IMAGE_QUALITY",
                os.environ.get("OPENAI_IMAGE_QUALITY", os.environ.get("IMAGE_QUALITY", "low")),
            ),
            timeout=_configured_int(
                "timeout",
                "OPENAI_IMAGE_TIMEOUT",
                int(os.environ.get("OPENAI_IMAGE_TIMEOUT", os.environ.get("IMAGE_TIMEOUT", "120")) or 120),
            ),
    ...
    def _url(self, clean_reference_urls: list[str]) -> str:
        ...
        suffix = "/v1/images/edits" if clean_reference_urls else "/v1/images/generations"
    ...
    async def _decode(self, data: dict | list) -> str:
        ...
        if not items:
            raise ModelError(f"Empty data in response: {data}", model_name=self.model_name)
        ...
        if item.get("b64_json"):
            ...
            return persist_image_bytes(img_bytes, self.model_name, "base64→file")
        if item.get("url"):
            return await download_remote_image(str(item["url"]), self.model_name)
        raise ModelError(
