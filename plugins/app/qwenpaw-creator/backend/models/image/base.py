...
def persist_image_bytes(img_bytes: bytes, model_name: str, source: str) -> str:
    ...
    logger.info(f"Image saved ({source}) | path={image_path}, size={len(img_bytes)} bytes")
    return result_url
...
async def download_remote_image(remote_url: str, model_name: str) -> str:
    ...
    try:
        ...
        temporary = unique_task_work_path("images", ".remote", prefix="download_")
        try:
            await asyncio.to_thread(download_remote_file, remote_url, str(temporary))
...
def _configured_int(fields: str | tuple[str, ...], env_name: str, default: int) -> int:
...
class BaseImageModel(ABC):
    ...
    async def generate(
        ...
        clean_reference_urls = [
            url.strip() for url in (reference_image_urls or []) if url and url.strip()
        ]
        ...
        try:
                ...
                for attempt in range(MAX_RETRIES):
                    async with httpx.AsyncClient(timeout=self.timeout) as client:
                        resp = await self._request(
                            client, prompt, aspect_ratio, clean_reference_urls
                        )
            ...
            logger.error(f"Image generation timed out after {self.timeout}s: {type(e).__name__}")
            raise ModelError(
            ...
            logger.error(f"Image generation HTTP error: {e.response.status_code} - {detail[:500]}")
            raise ModelError(
