...
_MEDIA_TASK_ID: ContextVar[str | None] = ContextVar("creator_media_task_id", default=None)
_MEDIA_PROJECT_ID: ContextVar[str | None] = ContextVar(
    "creator_media_project_id", default=None
)
...
@contextmanager
def media_task_scope(task_id: str, *, project_id: str | None = None) -> Iterator[None]:
...
def task_work_root(task_id: str | None = None) -> Path:
    ...
    if project_id is not None:
        ...
        if project_file.is_symlink() or not project_file.is_file():
            raise ValueError("Project-scoped media scratch requires project.json")
...
def media_url_for(path: Path) -> str:
    ...
    if relative.parts and relative.parts[0] in _MEDIA_ROOTS:
        ...
        and relative.parts[1:3] == ("runtime", "task-work")
    ):
        url_parts = ("projects", relative.parts[0], *relative.parts[2:])
    else:
        raise ValueError(f"Path is not in a served Creator media namespace: {path}")
...
def media_path_from_url(url: str) -> Path:
    ...
    if parts[0] == "projects":
        ...
        raise ValueError("Creator media URL must name a canonical media namespace")
