...
class DelegateToAgentInput(BaseModel):
    ...
    def validate_contract(self, *, project_id: str) -> None:
        ...
        if self.role not in _DELEGATABLE_ROLES:
            raise ValueError(f"specialist role is not delegatable: {self.role.value}")
            ...
            "parameters": deepcopy(
                {
                    ...
                    "properties": {
                        "role": {
                            "type": "string",
                            "enum": [role.value for role in _DELEGATABLE_ROLES],
                        },
                        "target_refs": {
                            "type": "array",
                            "items": {"type": "string"},
...
def specialist_system_prompt(
    ...
    *,
    ...
    values: dict[str, str] = {
        ...
        "workspace_schema": workspace_schema or build_project_schema_prompt().text,
    }
    if role is SpecialistRole.AI_EDITING_DIRECTOR:
        content_type = project.settings.content_type if project is not None else None
        target_duration = (
            project.settings.target_duration_seconds if project is not None else None
        )
        values.update(
            {
                "content_type": content_type or "general",
                "target_duration_seconds": (
                    f"{target_duration:g}" if target_duration is not None else "null"
                ),
            }
        )
