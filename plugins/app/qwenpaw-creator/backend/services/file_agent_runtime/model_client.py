...
class AgentStreamCallbackError(RuntimeError):
    ...
    def __init__(self, cause: Exception) -> None:
        self.cause = cause
        super().__init__(f"Creator stream callback failed: {type(cause).__name__}: {cause}")
...
def _text_content(value: Any, *, field: str) -> str:
    ...
    if isinstance(value, Sequence) and not isinstance(value, (str, bytes, bytearray)):
        pieces: list[str] = []
        for item in value:
            if not isinstance(item, Mapping) or item.get("type") not in {None, "text"}:
                raise AgentModelError(f"Creator Agent history {field} contains non-text content")
...
def _message_content_blocks(value: Any, *, field: str) -> list[Any]:
    ...
    raise AgentModelError(f"Creator Agent history {field} must be text or content parts")


def _history_tool_call(value: Any) -> AgentToolCall:
    if not isinstance(value, Mapping):
        raise AgentModelError("Creator Agent history contains an invalid tool call")
    function = value.get("function")
    if not isinstance(function, Mapping):
        raise AgentModelError("Creator Agent history tool call has no function")
    ...
    if not isinstance(arguments, dict):
        raise AgentModelError("Creator Agent history tool arguments must be an object")
...
def records_to_agentscope_messages(
    ...
    for index, record in enumerate(records):
        ...
        if role == "user":
            ...
            if not blocks:
                raise AgentModelError("Creator Agent history contains an empty user turn")
        ...
        if role == "assistant":
            ...
            if not isinstance(raw_calls, Sequence) or isinstance(
                ...
                raise AgentModelError("Creator Agent history tool_calls must be a list")
            ...
            if not blocks:
                raise AgentModelError("Creator Agent history contains an empty assistant turn")
        ...
        if role == "tool":
            ...
            continue
        raise AgentModelError(f"Creator Agent history has unsupported role: {role!r}")
    return messages
    ...
    async def complete(
        ...
        *,
        ...
        try:
                    ...
                    async for item in response:
                        ...
                        for block in item.content:
                            if isinstance(block, TextBlock):
                                ...
                                if block.thinking and guarded_thinking_delta is not None:
                                    await guarded_thinking_delta(block.thinking)
                                ...
                                effective_name = streamed_tool_names.get(block.id, raw_name)
                                if block.input:
                                    if effective_name in allowed_names:
                                        ...
                                        pending_tool_inputs.setdefault(block.id, []).append(
                                            block.input
                                        )
            ...
            raise AgentModelError(f"Creator AgentScope model request failed: {exc}") from exc
        ...
        for block in response.content:
            if isinstance(block, TextBlock):
                ...
                if not call_id or not name:
                    raise AgentModelError("Creator AgentScope ToolCallBlock has no id/name")
        ...
        if thinking and on_thinking_delta is not None and not streamed_thinking:
