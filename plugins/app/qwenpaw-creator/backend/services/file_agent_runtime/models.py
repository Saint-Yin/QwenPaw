    ...
    def validate_review_provenance(self) -> CreatorAgentRunRecord:
        if self.review_policy is ReviewPolicy.REQUIRE_REVIEW:
            if self.review_boundary is None:
                raise ValueError("review-required Agent run needs a ReviewBoundary")
            if self.caused_by_request_id != self.review_boundary.request_id:
                raise ValueError("Agent run request does not match ReviewBoundary")
            if self.caused_by_message_seq != self.review_boundary.request_message_seq:
                raise ValueError("Agent run message does not match ReviewBoundary")
        elif self.review_boundary is not None:
            raise ValueError("auto-fix Agent run cannot carry a ReviewBoundary")
