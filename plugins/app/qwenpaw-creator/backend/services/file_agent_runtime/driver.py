...
class FileCreatorAgentRuntime:
    ...
    async def start(self) -> None:
        ...
        self._dispatcher = asyncio.create_task(
            self._dispatch_loop(), name="creator-file-agent-dispatcher"
        )
        self._wake.set()
    ...
    async def stop(self) -> None:
        ...
        for handle in handles:
            await asyncio.to_thread(
                self._revoke_epoch, handle.project_id, handle.run_id, handle.epoch
            )
            handle.task.cancel()
        if handles:
    ...
    async def interrupt(
        ...
        *,
        ...
        await asyncio.to_thread(
            self._revoke_epoch, project_id, handle.run_id, handle.epoch
        )
        handle.task.cancel()
        self.notify(project_id)
        return True
        ...
        *,
        ...
        while True:
            ...
            if time.monotonic() >= deadline:
                raise TimeoutError(f"Creator Agent did not become idle: {project_id}")
    ...
    async def _reconcile_project(self, project_id: str) -> None:
        ...
        session = await asyncio.to_thread(
            self.sessions.get_project_session_snapshot, project_id
        )
    ...
    async def _run_message(
        ...
        *,
        ...
        snapshot = await asyncio.to_thread(self.services.projects.read, project_id)
    ...
    async def _model_loop(
        ...
        *,
        ...
        for _turn_number in range(1, self.max_model_turns + 1):
            ...
            async def persist_message_delta(stream_kind: str, delta: str) -> None:
                ...
                try:
                    ...
                    if call.name == DELEGATE_TOOL_NAME:
                        ...
                        result = await asyncio.to_thread(
                            tools.invoke, call.name, call.arguments
                        )
                    self._assert_epoch(project_id, run_id, epoch)
        ...
        try:
            for _turn_number in range(1, self.max_model_turns + 1):
                ...
                await self._event(
                    ...
                    parent_run_id,
                    request,
                    {**common, "messageId": message_id, "text": turn.content or ""},
                )
                assistant_wire: dict[str, Any] = {
                    "role": "assistant",
                    "content": turn.content,
    ...
    async def _await_execution_authorization(
        ...
        *,
        ...
        execution_request_id = _specialist_tool_request_id(
            specialist_run_id, spec.name, arguments
        )
        authorization_id = (
            "authorization-"
            + uuid5(
    ...
    async def _goal_for_message(self, session: Any, message: CreatorMessageRecord):
    ...
    async def _converge_resolved_review(self, project_id: str, session: Any):
        ...
        active_review = await asyncio.to_thread(
            self.services.reviews.active, project_id
        )
        ...
        try:
            ...
            return await asyncio.to_thread(
                self.sessions.get_project_session, project_id
            )
    ...
    async def _cancel_run(
        ...
        try:
            await asyncio.to_thread(
                ...
                project_id,
                run_id,
                expected_status={AgentRunStatus.QUEUED, AgentRunStatus.RUNNING},
    ...
    async def _record_idle_interrupt(self, project_id: str, *, reason: str) -> None:
        try:
            session = await asyncio.to_thread(
                self.sessions.get_project_session, project_id
            )
...
def _require_source_intelligence_associations(
    ...
    for target_ref in target_refs:
        ...
        intelligence = project.assets.intelligence_versions_by_id.get(intelligence_id)
        if (
