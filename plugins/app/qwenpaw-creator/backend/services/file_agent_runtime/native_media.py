    ...
    *,
    ...
    for content_part in request.content_parts:
        ...
        if content_part.type in _MEDIA_PART_TYPES:
            ...
            if not isinstance(nested, Mapping) or not str(nested.get("url") or ""):
        ...
        if isinstance(attachment, Mapping):
            version_id = _version_id_from_ref(attachment.get("assetVersionRef"))
            if version_id:
    ...
    for version_id in dict.fromkeys(referenced_versions):
        ...
        if public_url is None:
            ...
            if not api_key:
                raise ValidationError("本地素材传给素材理解 Agent 需要配置 Creator VLM API key")
