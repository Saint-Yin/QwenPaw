...
def _secure_create(path: Path, data: ObservabilityConfigData) -> None:
    ...
    encoded = (
        json.dumps(data.model_dump(mode="json", by_alias=True), ensure_ascii=False, indent=2)
        + "\n"
    ).encode("utf-8")
    try:
...
def _load_cached(path_text: str, modified_ns: int, size: int) -> ObservabilityConfigData:
    del modified_ns, size
    return ObservabilityConfigData.model_validate(_read_payload(Path(path_text)))
...
def load_observability_config() -> ObservabilityConfigData:
    ...
    if not path.exists():
        try:
            migrated = ObservabilityConfigData.model_validate(_legacy_payload())
...
def trace_root(config: ObservabilityConfigData | None = None) -> Path:
    configured = (config or load_observability_config()).trace_directory.strip()
    candidate = Path(configured).expanduser()
    root = candidate if candidate.is_absolute() else require_creator_data_root() / candidate
