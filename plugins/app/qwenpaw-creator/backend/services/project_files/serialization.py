...
def canonical_data(value: Any) -> Any:
    ...
    if isinstance(value, float):
        if not math.isfinite(value):
            raise CanonicalJsonError("NaN and Infinity are not valid Project values")
        return value
...
def load_project_json(payload: bytes | str) -> Project:
    ...
    try:
        ...
        raise CanonicalJsonError("project.json does not match the Project schema") from exc
