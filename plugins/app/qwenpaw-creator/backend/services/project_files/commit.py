...
from services.runtime_files.atomic_store import AtomicJsonRecordStore, fsync_directory
...
def _reset_derived_plan_hashes(project: dict[str, Any]) -> None:
    ...
    units = production.get("units_by_id") if isinstance(production, dict) else None
...
class ProjectCommitBoundary:
    ...
    def commit(
        ...
        *,
        ...
        if not isinstance(candidate, Mapping):
            raise ProjectCommitError("Project candidate must be one JSON object")
        ...
        protected = sorted({item.pointer for item in requested if is_protected_pointer(item.pointer)})
        if protected:
        ...
        try:
            ...
            with CrossProcessFileLock(
                ...
                if not actual_changes:
                    ...
                    return ProjectCommitResult(
                        ...
                        round=round_record.model_copy(
                            update={"status": TransactionStatus.NO_CHANGE, "updated_at": _now()}
                        ),
                        review=None,
                        transaction_id=transaction_id,
                    )
                ...
                with field_blocks.guard_write(
                    project_id=project_id,
                    json_pointers=tuple(item.pointer for item in actual_changes),
                    token=block_token,
                ):
            ...
            self._update_runtime_state(
                ...
                active_round_id=(
                    round_id
                    if review is not None
                    else existing_review_round_id if existing_review_pending else None
                ),
            )
            journal_store.write(
                journal.model_copy(
    ...
    def _assert_no_pending_publication(
        *,
        ...
        if not transactions_root.is_dir():
            return
        for entry in sorted(transactions_root.iterdir(), key=lambda item: item.name):
            if (
                ...
                continue
            journal = AtomicJsonRecordStore(
                entry / "journal.json", ProjectCommitJournal
            ).read_or_none()
            if journal is None:
                raise PendingProjectRecoveryError(
    ...
    def _finalize_no_change(
        ...
        *,
        ...
        AtomicJsonRecordStore(transaction_root / "latest.json").write(latest_data)
        AtomicJsonRecordStore(transaction_root / "final.json").write(latest_data)
        journal = journal.model_copy(
            update={
        ...
        pending_review, resolved_review, active_review_round = self._reconcile_existing_review(
            runtime_root=runtime_root,
            origin=round_record.origin,
            snapshot=latest,
            changes=[],
    ...
    def _record_review(
        ...
        *,
        ...
        operations_by_pointer = {
            ...
            for operation in (existing.operations if existing is not None else [])
            if operation.json_pointer is not None
        }
        for change in changes:
            pointer = change.json_pointer
            previous = operations_by_pointer.get(pointer) if pointer is not None else None
        ...
        operations = sorted(operations_by_pointer.values(), key=lambda item: item.operation_id)
        if not operations:
