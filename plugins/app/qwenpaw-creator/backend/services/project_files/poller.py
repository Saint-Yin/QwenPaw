...
class ProjectPoller:
    ...
    def note_commit(self, snapshot: ProjectSnapshot) -> ProjectSnapshotCacheEntry:
        ...
        if current.generation == snapshot.generation and current.etag != snapshot.etag:
            raise ProjectStoreError(
    ...
    def poll_once(self, project_id: str, *, force: bool = False) -> ProjectSnapshotCacheEntry:
        ...
        while not self._stop_event.is_set():
            ...
            if opened:
                await asyncio.gather(
                    *(asyncio.to_thread(self.poll_once, project_id) for project_id in opened),
