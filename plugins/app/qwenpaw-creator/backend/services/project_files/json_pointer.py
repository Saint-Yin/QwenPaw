...
def split_pointer(pointer: str) -> tuple[str, ...]:
    ...
    return tuple(unescape_pointer_token(token) for token in pointer[1:].split("/"))
...
def diff_json(before: Any, after: Any, *, pointer: str = "") -> tuple[JsonChange, ...]:
    ...
    def visit(left: Any, right: Any, path: str) -> None:
        ...
        if isinstance(left, dict) and isinstance(right, dict):
            ...
            for key in sorted(left_keys - right_keys):
                ...
                changes.append(JsonChange("delete", child, copy.deepcopy(left[key]), MISSING))
            for key in sorted(right_keys - left_keys):
                child = join_pointer(path, str(key))
                changes.append(JsonChange("create", child, MISSING, copy.deepcopy(right[key])))
            for key in sorted(left_keys & right_keys):
                visit(left[key], right[key], join_pointer(path, str(key)))
            return
        kind: JsonChangeKind = "reorder" if path.endswith("/order") else "update"
        changes.append(JsonChange(kind, path, copy.deepcopy(left), copy.deepcopy(right)))
...
def _parent_and_token(document: Any, pointer: str) -> tuple[Any, str]:
    ...
    if not tokens:
        raise JsonPointerError("root replacement is not a supported patch operation")
    parent: Any = document
    for token in tokens[:-1]:
        if isinstance(parent, dict) and token in parent:
            ...
            try:
                ...
                raise JsonPointerError(f"missing JSON Pointer parent: {pointer}") from exc
        else:
            raise JsonPointerError(f"missing JSON Pointer parent: {pointer}")
    return parent, tokens[-1]


def apply_changes(document: Any, changes: tuple[JsonChange, ...] | list[JsonChange]) -> Any:
    ...
    ordered = sorted(
        ...
        key=lambda item: (
            ...
            -len(split_pointer(item.pointer)) if item.kind == "delete" else len(split_pointer(item.pointer)),
            item.pointer,
        ),
    )
    for change in ordered:
        ...
        if isinstance(parent, dict):
            if change.kind == "delete":
                if token not in parent:
                    raise JsonPointerError(f"cannot delete missing value: {change.pointer}")
                del parent[token]
            elif change.kind == "create":
                if token in parent:
                    raise JsonPointerError(f"cannot create existing value: {change.pointer}")
                parent[token] = copy.deepcopy(change.after)
            else:
                if token not in parent:
                    raise JsonPointerError(f"cannot replace missing value: {change.pointer}")
                parent[token] = copy.deepcopy(change.after)
            continue
        if isinstance(parent, list):
            try:
                ...
                raise JsonPointerError(f"invalid array index: {change.pointer}") from exc
            if change.kind == "delete":
                ...
                if index < 0 or index > len(parent):
                    raise JsonPointerError(f"invalid array insertion: {change.pointer}")
            ...
            continue
        raise JsonPointerError(f"JSON Pointer parent is not a container: {change.pointer}")
    return result


def merge_candidate(*, base: Any, candidate: Any, latest: Any) -> tuple[Any, tuple[JsonChange, ...]]:
