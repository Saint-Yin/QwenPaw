...
class ProjectStore:
    ...
    def __init__(
        ...
        *,
        ...
        if not raw_root.is_absolute():
            raise UnsafeProjectPath("ProjectStore root must be an absolute path")
    ...
    def read(self, project_id: str) -> ProjectSnapshot:
        ...
        if stat.S_ISLNK(target_stat.st_mode) or not stat.S_ISREG(target_stat.st_mode):
            raise ProjectIntegrityError("project.json must be a regular, non-symlink file")
        try:
            payload = target.read_bytes()
        except OSError as exc:
            raise ProjectIntegrityError(f"Cannot read Project: {safe_id}") from exc
        ...
        try:
            ...
            raise ProjectIntegrityError(f"Invalid project.json for {safe_id}") from exc
    ...
    def replace(
        ...
        if candidate.project_id != safe_id:
            raise ProjectConflict("candidate project_id does not match target Project")
        payload = self._checked_payload(candidate)
        ...
        with self._lock:
            ...
            if candidate.updated_at < current.project.updated_at:
                raise ProjectConflict("candidate updated_at cannot move backwards")
    ...
    def list(self) -> list[ProjectSummary]:
        ...
        try:
            ...
            raise ProjectIntegrityError("Cannot scan ProjectStore root") from exc
        for entry in entries:
            if entry.name.startswith(".deleted-") or entry.is_symlink() or not entry.is_dir():
                continue
            try:
    ...
    def delete(self, project_id: str, *, expected_etag: str | None = None) -> None:
        ...
        with self.lifecycle_lock(safe_id), self._lock:
            ...
            try:
                ...
                raise ProjectStoreError(f"Cannot delete Project: {safe_id}") from exc
            try:
    ...
    def lifecycle_lock(self, project_id: str) -> CrossProcessFileLock:
        ...
        return CrossProcessFileLock(
            self.root / ".locks" / f"project-{_safe_project_id(project_id)}.lock",
    ...
    def _require_real_project_directory(self, project_root: Path, project_id: str) -> None:
        try:
        ...
        try:
            ...
            raise UnsafeProjectPath("Project directory escapes the store root") from exc

    def _validate_asset_paths(self, project_root: Path, project: Project) -> None:
        assets_root = project_root / "assets"
        try:
            assets_stat = assets_root.lstat()
        except FileNotFoundError as exc:
            raise ProjectIntegrityError("Project assets directory is missing") from exc
        if stat.S_ISLNK(assets_stat.st_mode) or not stat.S_ISDIR(assets_stat.st_mode):
...
def _validated_project(project: Project) -> Project:
    try:
        ...
        raise ProjectIntegrityError("candidate does not match the Project schema") from exc
