...
class ReviewDecisionJournal(BaseModel):
    ...
    def validate_phase(self) -> ReviewDecisionJournal:
        if self.compensation_required and (
            ...
            raise ValueError("a compensating decision needs stable Runtime IDs")
        if not self.compensation_required and (
            ...
            raise ValueError("an accept-only decision cannot have compensation IDs")
        if self.state is not ReviewDecisionJournalState.PREPARED and (
            ...
            raise ValueError("an applied Review decision needs its durable result")
        return self
...
class ProjectReviewService:
    ...
    def active(self, project_id: str) -> ReviewRecord | None:
        ...
        state = AtomicJsonRecordStore(
            runtime_root / "state.json", RuntimeProjectState
        ).read_or_none()
        if state is not None and state.active_round_id:
            review = self._review_store(project_id, f"review-{state.active_round_id}").read_or_none()
        ...
        for child in reviews_root.iterdir():
            ...
            review = AtomicJsonRecordStore(
                child / "review.json", ReviewRecord
            ).read_or_none()
            if review is not None and review.status is ReviewStatus.PENDING:
        ...
        with lifecycle_lock:
            ...
            for journal_store, journal in sorted(
                ...
                try:
                    ...
                    if recovered.state is not ReviewDecisionJournalState.FINALIZED:
    ...
    def _discover_decision_journals(
        ...
        for review_root in sorted(reviews_root.iterdir(), key=lambda item: item.name):
    ...
    def decide(
        ...
        *,
        ...
        self.store.read(project_id)
        if not decisions:
            raise ReviewDecisionError("at least one Review decision is required")
        if len({item.operation_id for item in decisions}) != len(decisions):
            raise ReviewDecisionError("an operation can be decided only once per request")
        canonical_decisions = sorted(
            (ReviewDecisionItem.model_validate(item) for item in decisions),
            key=lambda item: item.operation_id,
        )
        try:
            review_id = require_safe_runtime_segment(review_id, label="Review id")
        ...
        if (
            ...
            raise ReviewDecisionError("decision id must contain 1 to 192 characters")
        ...
        *,
        ...
        if journal is not None:
            ...
            if journal.state is ReviewDecisionJournalState.FINALIZED:
                if journal.final_review is None:  # pragma: no cover - model invariant
                    raise ReviewDecisionConflict("finalized decision has no Review result")
            ...
            if not secrets.compare_digest(review.decision_token, decision_token):
    ...
    def _assert_no_other_active_decision(
        *,
        ...
        for decision_root in sorted(
            ...
            if journal.project_id != project_id or journal.review_id != review_id:
                raise ReviewDecisionConflict(
                    "Review decision journal identity is inconsistent"
                )
            expected_key = hashed_runtime_segment("decision", journal.decision_id)
            if decision_root.name != expected_key:
                raise ReviewDecisionConflict(
    ...
    def _finalize_decision(
        ...
        *,
        ...
        if (
            ...
            raise ReviewDecisionConflict("Review decision journal is not finalizable")
    ...
    def _validate_idempotent_request(
        *,
        ...
        if (
            ...
            or not secrets.compare_digest(journal.decision_token, decision_token)
        ...
        *,
        ...
        for operation_id, item in requested.items():
            ...
            if operation.json_pointer is None:
                raise ReviewDecisionError("file-only Review operations are not implemented")
            ...
            if operation.kind.value == "create":
                ...
                restored = operation.before
                kind = (
                    "reorder" if operation.kind.value == "reorder" else "update"
                )
            changes.append(
                JsonChange(
                    kind=kind,
    ...
    def _completed_compensation_snapshot(
        ...
        *,
        ...
        transaction_id = journal.compensation_transaction_id
        round_id = journal.compensation_round_id
        if transaction_id is None or round_id is None:  # pragma: no cover - invariant
            raise ReviewDecisionConflict("compensating decision has no Runtime identity")
    ...
    def _current_at_or_after(
        ...
        if current.generation < applied.generation or (
            current.generation == applied.generation and current.etag != applied.etag
        ):
            raise ReviewDecisionConflict(
    ...
    def _build_final_review(
        *,
        ...
        if (
            ...
            raise ReviewDecisionConflict("Review identity changed after decision prepare")
        requested = {item.operation_id: item for item in journal.decisions}
        before_operations = {
            operation.operation_id: operation for operation in before.operations
        }
        current_operations = {
            operation.operation_id: operation for operation in current.operations
        }
        if set(before_operations) != set(current_operations):
            raise ReviewDecisionConflict("Review operation set changed after decision prepare")
        ...
        for operation in current.operations:
            ...
            for field_name in (
                ...
                if getattr(operation, field_name) != getattr(original, field_name):
                    raise ReviewDecisionConflict(
        ...
        updated = current.model_copy(
            update={
                ...
                "status": ReviewStatus.PENDING if pending else ReviewStatus.RESOLVED,
    ...
    def _assert_review_can_finalize(
        *,
        ...
        for operation_id, current_operation in current_by_id.items():
            ...
            if current_operation.model_copy(update={"decision": final_operation.decision}) != final_operation:
                raise ReviewDecisionConflict(
                    f"Review operation changed after decision result: {operation_id}"
                )
            item = requested.get(operation_id)
            if item is None and current_operation.decision != final_operation.decision:
                raise ReviewDecisionConflict(
    ...
    def _update_state(
        ...
        *,
        ...
        state_store.write(
            state.model_copy(
                update={
                    "active_round_id": None if resolved else state.active_round_id,
                    "last_project_generation": snapshot.generation,
                    "last_project_etag": snapshot.etag,
                    "accepted_generation": (
                        snapshot.generation if resolved else state.accepted_generation
                    ),
                    "accepted_etag": snapshot.etag if resolved else state.accepted_etag,
                    "updated_at": datetime.now(UTC),
                }
            )
        )
