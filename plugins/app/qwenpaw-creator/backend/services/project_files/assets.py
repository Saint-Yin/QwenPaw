...
class AssetFileStore:
    ...
    def stage_stream(
        ...
        *,
        ...
        if staging_id is not None and not _SAFE_STAGING_ID.fullmatch(staging_id):
    ...
    def stage_bytes(
        ...
        *,
        ...
        return self.stage_stream(BytesIO(bytes(content)), staging_id=staging_id)
    ...
    def publish(
        ...
        *,
        ...
        self._validate_roots()
        actual_sha256, actual_size, verified_fingerprint = self._verify_staged(staged)
        if expected_sha256 is not None and actual_sha256 != expected_sha256:
            raise StagedAssetError("Staged Asset does not match expected sha256")
        ...
        if target_stat is not None:
            raise AssetAlreadyExists(f"Immutable Asset already exists: {relative_uri}")
        ...
        try:
            try:
                ...
                raise AssetFileError(f"Cannot publish Asset: {relative_uri}") from exc
            ...
            if (
                ...
                or not _matches_published_file(source_stat, verified_fingerprint)
                or not _matches_published_file(target_stat, verified_fingerprint)
            ):
                raise StagedAssetError("Staged Asset changed during publication")
    ...
    def inspect(self, indexed: IndexedFile) -> AssetInspection:
        ...
        try:
            ...
            return _inspection(indexed, AssetFileStatus.MISSING, detail=str(exc))
        except AssetFileNotRegular as exc:
            return _inspection(indexed, AssetFileStatus.NOT_REGULAR, detail=str(exc))
        except AssetPathError as exc:
            return _inspection(indexed, AssetFileStatus.UNSAFE, detail=str(exc))
        ...
        with stream:
            ...
            actual_sha256, streamed_size = _hash_stream(stream, self.chunk_size)
    ...
    def require_valid_index(self, index: AssetIndex) -> AssetValidationReport:
        ...
        if not report.valid:
            summary = ", ".join(
                f"{item.file_id}={item.status.value}" for item in report.failures
            )
            raise AssetFileCorrupt(f"Asset Index contains unavailable files: {summary}")
        return report
    ...
    def scan_orphans(
        ...
        *,
        ...
        observed_at = _utc_datetime(now or datetime.now(timezone.utc), label="now")
        previous = {
    ...
    def _verify_staged(
        ...
        try:
            stream, size_bytes = _open_regular(
                staged.path, missing_error=StagedAssetError
            )
        except AssetFileError as exc:
            raise StagedAssetError("Staged Asset is not a regular file") from exc
        with stream:
    ...
    def _validate_staged_handle(self, staged: StagedAsset) -> None:
        if (
            ...
            raise StagedAssetError("Staged Asset belongs to a different Project")
    ...
    def _ensure_final_parent(self, directory_parts: tuple[str, ...]) -> Path:
        ...
        for part in directory_parts:
            child = current / part
            created = _mkdir_real(child, parent=current, label="Asset directory")
            if created:
    ...
    def _require_final_parent(self, directory_parts: tuple[str, ...]) -> Path:
        ...
        for part in directory_parts:
            ...
            try:
                ...
                raise AssetFileMissing(f"Asset directory is missing: {child}") from exc
            current = child
        try:
            current.resolve(strict=True).relative_to(self._resolved_assets_root)
        except (OSError, ValueError) as exc:
            raise AssetPathError(
    ...
    def _validate_roots(self) -> None:
        ...
        try:
            self.assets_root.resolve(strict=True).relative_to(self.project_root)
            self.staging_root.resolve(strict=True).relative_to(
                self._resolved_assets_root
            )
        except (OSError, ValueError) as exc:
            raise AssetPathError("Asset storage root escapes its Project") from exc
        ...
        def visit(directory: Path, relative_parts: tuple[str, ...]) -> None:
            ...
            for entry in entries:
                if not relative_parts and entry.name == ".staging":
                    continue
                uri = PurePosixPath("assets", *relative_parts, entry.name).as_posix()
                try:
...
def _asset_uri_parts(value: str) -> tuple[str, ...]:
    ...
    if (
        ...
        raise AssetPathError("Asset URI must name a canonical file below assets/")
...
def _open_directory(path: Path) -> int:
    ...
    try:
        ...
        raise AssetPathError(f"Cannot securely open Asset directory: {path}") from exc
    ...
    *,
    ...
    try:
        ...
        if exc.errno in {errno.ELOOP, errno.EMLINK}:
            raise AssetPathError(f"Asset file cannot be a symlink: {path}") from exc
