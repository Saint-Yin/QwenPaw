...
def remote_cache_relative_uri(version: SourceAssetVersion) -> str:
    ...
    if not suffix or not suffix.replace(".", "").isalnum():
        suffix = mimetypes.guess_extension(version.media_type.split(";", 1)[0]) or ".bin"
    ...
    *,
    ...
    try:
        ...
        if stat.S_ISLNK(value.st_mode) or not stat.S_ISREG(value.st_mode):
            raise StorageIntegrityError("Runtime cache target is not a regular file")
        if value.st_size != staged.size_bytes or _hash_file(target) != staged.sha256:
            raise StorageIntegrityError("Runtime cache target has different content")
...
def resolve_remote_cache(
    ...
    for task in tasks:
        ...
        try:
            ...
            raise StorageIntegrityError("Remote Asset cache result is malformed")
        ...
        try:
            ...
            return None
        if stat.S_ISLNK(value.st_mode) or not stat.S_ISREG(value.st_mode):
            raise StorageIntegrityError("Remote Asset cache is not a regular file")
        if value.st_size != size_bytes:
            raise StorageIntegrityError("Remote Asset cache size does not match Runtime task")
        return RemoteCacheEntry(
            path=target,
