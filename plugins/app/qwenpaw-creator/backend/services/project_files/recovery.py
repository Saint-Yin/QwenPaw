...
class CreatorRecoveryReport:
    ...
    def integrity_errors(self) -> tuple[TransactionRecoveryOutcome, ...]:
        return tuple(
            outcome for project in self.projects for outcome in project.integrity_errors
        )
...
class ProjectCommitRecoveryCoordinator:
    ...
    def _recover_project_lifecycle_locked(
        ...
        if transactions_root.is_symlink() or not transactions_root.is_dir():
            ...
            return ProjectRecoveryReport(project_id=project_id, outcomes=(outcome,))
        ...
        with (
            ...
            outcomes = tuple(
                ...
                for entry in sorted(
                    transactions_root.iterdir(), key=lambda item: item.name
                )
    ...
    def _recover_entry(
        ...
        try:
            ...
            if stat.S_ISLNK(entry_stat.st_mode) or not stat.S_ISDIR(entry_stat.st_mode):
                raise _IntegrityProblem("transaction entry must be a real directory")
            ...
            inputs = self._load_inputs(project_id, runtime_root, transaction_root)
            ...
            if inputs.journal.state is CommitJournalState.PROJECT_REPLACED:
                ...
                self._finalize_runtime(
                    inputs, current, changes=changes, no_change=False
                )
                return TransactionRecoveryOutcome(
                    transaction_id=transaction_id,
                    action=RecoveryAction.FINALIZED_RUNTIME,
            ...
            if inputs.journal.state is CommitJournalState.RUNTIME_FINALIZED:
                ...
                if inputs.round_record.status is TransactionStatus.NO_CHANGE:
                    self._finalize_runtime(inputs, current, changes=(), no_change=True)
                else:
                    changes = self._prove_current_is_published(inputs, current)
                    self._finalize_runtime(
                        inputs, current, changes=changes, no_change=False
    ...
    def _load_inputs(
        ...
        if journal.project_id != project_id:
            raise _IntegrityProblem("journal project_id does not match its Project")
        if journal.round_id is None or journal.advance_accepted_baseline is None:
            raise _IntegrityProblem(
                "legacy journal lacks explicit round_id/accepted-baseline semantics"
            )

        base_value = AtomicJsonRecordStore(transaction_root / "base.json").read()
        ...
        if not isinstance(base_value, Mapping) or not isinstance(
            ...
            raise _IntegrityProblem("base.json and candidate.json must be JSON objects")
        ...
        if base_project.project_id != project_id:
            raise _IntegrityProblem("base Project identity does not match journal")
        if project_etag(base_project) != journal.base_etag:
            raise _IntegrityProblem("base.json does not match journal base_etag")
        ...
        if [change.pointer for change in requested] != journal.touched_pointers:
            raise _IntegrityProblem(
        ...
        transaction_round_snapshot = _read_optional_snapshot(transaction_round_store)
        ...
        if (
            ...
            raise _IntegrityProblem("ChangeRound identity does not match journal")
        aggregate_round_snapshot = _read_optional_snapshot(aggregate_round_store)
    ...
    def _prove_current_is_published(
        ...
        candidate_changes = _business_changes(inputs.base_data, inputs.candidate_data)
        current_changes = _business_changes(
            inputs.base_project.model_dump(mode="json"),
            current.project.model_dump(mode="json"),
        )
        if _change_signatures(candidate_changes) != _change_signatures(current_changes):
            raise _IntegrityProblem(
    ...
    def _runtime_metadata_complete(
        ...
        if not inputs.journal.final_etag:
            raise _IntegrityProblem("RUNTIME_FINALIZED journal has no final_etag")
        ...
        if state.project_id != inputs.journal.project_id:
            raise _IntegrityProblem("RuntimeProjectState belongs to another Project")
    ...
    def _finalize_runtime(
        ...
        *,
        ...
        if (
            ...
            new_review_plan = self._prepare_new_review(inputs, current, runtime_changes)
        ...
        state = _build_runtime_state(
            inputs=inputs,
            current=current,
            existing=existing_state.value if existing_state is not None else None,
            accepted=accepted,
            active_round_id=active_round_id,
        )
        terminal_status = TransactionStatus.NO_CHANGE if no_change else (
        ...
        if existing_state is None:
            ...
            if created is None:
                raise _IntegrityProblem("RuntimeProjectState appeared during recovery")
        else:
    ...
    def _prepare_new_review(
        ...
        if existing is not None:
            ...
            return _NewReviewPlan(store=store, existing=existing, value=existing.value)
        operations = [
            ReviewOperation(
    ...
    def _prepare_existing_review_update(
        ...
        if review.status is not ReviewStatus.PENDING:
            raise _IntegrityProblem("active Runtime review has an invalid status")
        touched = [change.json_pointer for change in changes if change.json_pointer]
        ...
        for operation in review.operations:
            if (
                ...
                operation = operation.model_copy(
                    update={"decision": ReviewOperationDecision.SUPERSEDED_BY_USER_EDIT}
                )
...
def _prepared_business_changes(inputs: _RecoveryInputs) -> tuple[JsonChange, ...]:
...
def _validate_changeset_identity(
    ...
    if (
        ...
        or changeset.caused_by_request_id != inputs.round_record.caused_by_request_id
        or changeset.caused_by_message_seq != inputs.round_record.caused_by_message_seq
        or changeset.final_etag != inputs.journal.final_etag
    ):
        raise _IntegrityProblem("ChangeSet identity/provenance conflicts with journal")
    if changeset.changes:
        ...
        if changeset.base_generation < inputs.base_project.generation:
            raise _IntegrityProblem("ChangeSet base generation precedes journal base")
        if (
...
def _validate_existing_new_review(
    ...
    *,
    ...
    if (
        ...
        raise _IntegrityProblem("Review identity/baseline conflicts with journal")
    if (
    ...
    if (
        ...
        if len(actual_by_locator) != len(review.operations):
            raise _IntegrityProblem("Review contains duplicate operation locators")
        for change in changes:
...
def _build_runtime_state(
    *,
    ...
    if existing is not None and existing.last_project_generation > current.generation:
        raise _IntegrityProblem("RuntimeProjectState is ahead of current Project")
    boundary = inputs.round_record.review_boundary
    if accepted:
        ...
        raise _IntegrityProblem("accepted Project baseline cannot be reconstructed")
