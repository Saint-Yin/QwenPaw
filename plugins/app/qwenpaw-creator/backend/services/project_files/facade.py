...
class CreatorFileServices:
    ...
    def blocks(self, project_id: str) -> FieldBlockStore:
        return FieldBlockStore(
            self.projects.project_root(project_id) / "runtime" / "locks" / "fields"
        )

    async def snapshot(
        self, project_id: str, *, force: bool = False
    ) -> ProjectSnapshotCacheEntry:
        return await asyncio.to_thread(self.poller.poll_once, project_id, force=force)
    ...
    async def apply_jq(
        ...
        **metadata: Any,
        ...
        return await self.commit_candidate(base=base, candidate=candidate, **metadata)
