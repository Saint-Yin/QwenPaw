...
class ProjectRuntimeSessionStore:
    ...
    def create_project_runtime(
        ...
        *,
        ...
        requested_session_id = (
            _safe_segment(session_id, "session_id") if session_id is not None else None
        )
        ...
        with self._project_lock(project_id):
            ...
            if existing is not None:
                ...
                recovered = self._recover_session_unlocked(
                    project_id, existing.session.session_id
                )
    ...
    def initialize_staged_project(
        ...
        *,
        ...
        if not staged_root.is_absolute() or staged_root.is_symlink() or not staged_root.is_dir():
        ...
        normalized_goal = initial_goal.strip() if initial_goal is not None else None
        ...
        if normalized_goal is not None:
            ...
            initial_client_message_id = _safe_nonempty(
                initial_client_message_id, "initial_client_message_id"
            )
        ...
        atomic_create_bytes(
            session_root / "session.json", json_document_bytes(session)
        )
        atomic_create_bytes(
        ...
        if normalized_goal is not None:
            content_parts = [MessageContentPart(type="text", text=normalized_goal)]
            ...
            DurableJsonlStore(
                session_root / "messages.jsonl", CreatorMessageRecord
            ...
            atomic_create_bytes(
                goals_root / f"{goal_id}.json", json_document_bytes(goal)
            )
    ...
    def get_project_session(self, project_id: str) -> CreatorSessionRecord:
        ...
        with self._project_lock(project_id):
            ...
            return self._recover_session_unlocked(
                project_id, bootstrap.session.session_id
            )
    ...
    def get_session(self, project_id: str, session_id: str) -> CreatorSessionRecord:
        project_id = _safe_segment(project_id, "project_id")
        session_id = _safe_segment(session_id, "session_id")
        self._require_project(project_id)
        with self._project_lock(project_id):
    ...
    def clear_active_run(
        ...
        *,
        ...
        with self._project_lock(project_id):
            session = self._recover_session_unlocked(project_id, session_id)
            if expected_run_id is not None and session.active_run_id != expected_run_id:
                raise SessionStateConflict(
                    "Active run conflict: "
    ...
    def create_conversation(
        ...
        *,
        ...
        with self._project_lock(project_id):
            ...
            existing = self._list_conversations_unlocked(project_id, session_id)
            if is_default and any(item.is_default for item in existing):
                raise SessionStateConflict("Project already has a default Conversation")
            ...
            store = self._conversation_store(project_id, session_id, conversation_id)
            try:
                store.create(record)
    ...
    def create_goal(
        ...
        *,
        ...
        with self._project_lock(project_id):
            ...
            self._assert_conversation_ownership(conversation, session_id=session_id)
            if not 1 <= root_message_seq <= session.last_message_seq:
                raise SessionStateConflict(
                    "Goal root_message_seq must reference a durable message"
                )
            root_message = self._message_records_unlocked(project_id, session_id)[
                root_message_seq - 1
            ]
    ...
    def set_goal_status(
        ...
        *,
        ...
        resolved_expected = (
            CreatorGoalStatus(expected_status) if expected_status is not None else None
        )
        with self._project_lock(project_id):
            goal = self._read_goal_unlocked(project_id, goal_id)
            if resolved_expected is not None and goal.status is not resolved_expected:
                raise SessionStateConflict(
    ...
    def append_message(
        ...
        *,
        ...
        if client_message_id is not None:
            client_message_id = _safe_nonempty(client_message_id, "client_message_id")
        ...
        with self._project_lifecycle_lock(project_id):
            ...
            with (
                ...
                existing = self._message_by_client_id_unlocked(
                    project_id, session_id, client_message_id
                )
                if existing is not None:
                ...
                if requires_review:
                    self._assert_review_active_goal_unlocked(project_id, session)
                    project_state = self._runtime_state_store(project_id).read_or_none()
                ...
                if boundary is not None:
                    self._ensure_review_boundary_unlocked(
                        project_id, session_id, boundary
                    )
                return RequestAdmissionResult(
    ...
    def append_event(
        ...
        *,
        ...
        with self._project_lock(project_id):
            ...
            def append_for(current: CreatorSessionRecord) -> SessionEventRecord:
            ...
            try:
                ...
                record = append_for(session)
            updated = session.model_copy(
                update={"last_event_seq": record.event_seq, "updated_at": utc_now()}
            )
            self._write_session_unlocked(updated)
            return record
    ...
    def append_queued_message(
        ...
        *,
        ...
        with self._project_lock(project_id):
            session = self._recover_session_unlocked(project_id, session_id)
            self._read_conversation_unlocked(project_id, session_id, conversation_id)
            records = self._queued_records_unlocked(project_id, session_id)
            matches = [
                item for item in records if item.client_message_id == client_message_id
            ]
            if matches:
                existing = matches[-1]
    ...
    def transition_queued_message(
        ...
        *,
        ...
        project_id, session_id = self._safe_session_ids(project_id, session_id)
        queued_message_id = _safe_segment(queued_message_id, "queued_message_id")
            ...
            session_updated = session.model_copy(
                update={
                    "queued_user_message_count": max(
                        0, session.queued_user_message_count - 1
                    ),
                    "updated_at": utc_now(),
                }
            )
    ...
    def transition_outbox(
        ...
        *,
        ...
        with self._project_lock(project_id):
            ...
            records = self._outbox_records_unlocked(project_id, session_id)
            existing = next(
                (item for item in reversed(records) if item.outbox_id == outbox_id),
                None,
            )
            if existing is None:
                raise SessionStateConflict(f"Outbox record not found: {outbox_id}")
    ...
    def _append_message_unlocked(
        ...
        *,
        ...
        seq = store.last_seq() + 1
        if review_boundary is not None and review_boundary.request_message_seq != seq:
            raise SessionStateConflict(
    ...
    def _assert_review_active_goal_unlocked(
        self, project_id: str, session: CreatorSessionRecord
    ) -> None:
    ...
    def _ensure_review_boundary_unlocked(
        ...
        path = self.review_boundary_path(project_id, session_id, boundary.request_id)
    ...
    def _recover_session_unlocked(
        ...
        queue_count = sum(
            item.state is QueuedMessageState.QUEUED for item in latest_queued.values()
        )
        ...
        for message in messages:
            if message.review_boundary is not None:
                self._ensure_review_boundary_unlocked(
                    project_id, session_id, message.review_boundary
                )
        if (
    ...
    def _message_by_client_id_unlocked(
        ...
        return next(
            (
                item
                for item in self._message_records_unlocked(project_id, session_id)
    ...
    def _existing_bootstrap_unlocked(
        ...
        session = AtomicJsonRecordStore(session_files[0], CreatorSessionRecord).read()
    ...
    def _assert_bootstrap_retry(
        ...
        *,
        ...
        if session_id is not None and existing.session.session_id != session_id:
            raise SessionStateConflict(
                "Project Runtime already exists with another session_id"
            )
        if (
            conversation_id is not None
            and existing.default_conversation.conversation_id != conversation_id
        ):
            raise SessionStateConflict(
    ...
    def _read_session_unlocked(
        ...
        record = AtomicJsonRecordStore(path, CreatorSessionRecord).read_or_none()
        if record is None:
            raise RuntimeSessionNotFound(f"Runtime Session not found: {session_id}")
    ...
    def _write_session_unlocked(
        self, session: CreatorSessionRecord
    ) -> CreatorSessionRecord:
        validated = CreatorSessionRecord.model_validate(session)
        self._session_store(validated.project_id, validated.session_id).write(validated)
        return validated
    ...
    def _list_conversations_unlocked(
        ...
        for path in sorted(root.glob("*.json")):
            ...
            record = AtomicJsonRecordStore(path, CreatorConversationRecord).read()
            if (
                record.project_id != project_id
                or record.creator_session_id != session_id
    ...
    def _read_goal_unlocked(self, project_id: str, goal_id: str) -> CreatorGoalRecord:
        ...
        if record.project_id != project_id or record.goal_id != goal_id:
            raise SessionStoreIntegrityError("Goal path and record identity disagree")
        return record

    @staticmethod
    def _assert_conversation_ownership(
        conversation: CreatorConversationRecord, *, session_id: str
    ) -> None:
    ...
    @staticmethod
    def _assert_goal_ownership(goal: CreatorGoalRecord, *, session_id: str) -> None:
        if goal.creator_session_id != session_id:
            raise SessionStateConflict("Goal belongs to another Runtime Session")

    def _safe_session_ids(self, project_id: str, session_id: str) -> tuple[str, str]:
        project_id = _safe_segment(project_id, "project_id")
        session_id = _safe_segment(session_id, "session_id")
        self._require_project(project_id)
    ...
    def _safe_message_ids(
        self, project_id: str, session_id: str, conversation_id: str
    ) -> tuple[str, str, str]:
        project_id, session_id = self._safe_session_ids(project_id, session_id)
        conversation_id = _safe_segment(conversation_id, "conversation_id")
    ...
    def _require_project(self, project_id: str) -> Path:
        ...
        if stat.S_ISLNK(project_stat.st_mode) or not stat.S_ISREG(project_stat.st_mode):
            raise UnsafeSessionPath("project.json must be a regular, non-symlink file")
        return project_root
    ...
    def _project_commit_order_lock(self, project_id: str) -> CrossProcessFileLock:
        return CrossProcessFileLock(
    ...
    def _goal_store(
        self, project_id: str, goal_id: str
    ...
    def _queued_store(
        ...
        return DurableJsonlStore(
            self._session_root(project_id, session_id) / "queued-messages.jsonl",
            QueuedMessageRecord,
        )
...
def _safe_segment(value: str, name: str) -> str:
    try:
        ...
        raise UnsafeSessionPath(f"{name} must be a safe filesystem identifier") from exc
...
def _request_hash(value: Mapping[str, Any]) -> str:
    digest = hashlib.sha256(canonical_json_bytes(_json_ready(value))).hexdigest()
...
def _assert_internal_sequence(
    records: Sequence[Any], *, attribute: str, stream_name: str
) -> None:
...
def _window(records: Sequence[Any], *, after_seq: int, limit: int | None) -> list[Any]:
