...
_MEASURABLE_TASK_KINDS = frozenset({TaskKind.ASSET_INGEST, TaskKind.ASSET_IMPORT})
...
def _run_progress(run: SpecialistRunRecord) -> _ProjectedProgress:
    ...
    if run.status in {SpecialistRunStatus.QUEUED, SpecialistRunStatus.QUEUED_CAPACITY}:
        label = f"{base}等待执行"
...
def _session_progress(session: CreatorSessionRecord | None) -> _ProjectedProgress:
    if session is None:
    ...
    if status is CreatorSessionStatus.WAITING_EXECUTION_AUTH:
        return _ProjectedProgress("unit_production", "等待执行授权", session.updated_at)
    if status is CreatorSessionStatus.WAITING_USER_INPUT:
        return _ProjectedProgress("creative_strategy", "等待用户输入", session.updated_at)
    if status is CreatorSessionStatus.INTERRUPT_REQUESTED:
        return _ProjectedProgress("creative_strategy", "正在停止所有 Agent", session.updated_at)
    if status is CreatorSessionStatus.WAITING_RUNTIME:
        return _ProjectedProgress("creative_strategy", "Agent 等待工具结果", session.updated_at)
    if status is CreatorSessionStatus.RESUMING:
        return _ProjectedProgress("creative_strategy", "Agent 正在恢复执行", session.updated_at)
    if status is CreatorSessionStatus.RUNNING:
        return _ProjectedProgress("creative_strategy", "Agent 正在处理", session.updated_at)
    if status is CreatorSessionStatus.ERROR:
        return _ProjectedProgress("creative_strategy", "Runtime 处理失败", session.updated_at)
    if status is CreatorSessionStatus.CANCELLED:
        return _ProjectedProgress("creative_strategy", "Agent 已停止", session.updated_at)
    return _ProjectedProgress("creative_strategy", "待命中，可继续编辑项目", session.updated_at)
...
def build_agent_status_bar(
    ...
    *,
    ...
    active_tasks = [item for item in task_items if item.status in _ACTIVE_TASK_STATUSES]
    active_runs = [item for item in run_items if item.status in _ACTIVE_RUN_STATUSES]
    session_status = session.status if session is not None else CreatorSessionStatus.IDLE
    ...
    if session_status in gate_statuses:
        ...
        projected = max(terminal, key=lambda item: item[0])[1] if terminal else _session_progress(session)
    ...
    waiting_authorization_count = sum(
        item.status is SpecialistRunStatus.WAITING_AUTHORIZATION for item in active_runs
    )
    ...
    if session_status is CreatorSessionStatus.ERROR:
        ...
        if (
            ...
            and latest_task.status in {TaskStatus.FAILED, TaskStatus.QUARANTINED}
        ):
            badges.append(
                {
                    "kind": "error",
