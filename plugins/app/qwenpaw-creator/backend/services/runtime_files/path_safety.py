...
def require_safe_runtime_segment(value: str, *, label: str = "Runtime id") -> str:
    """Return one normalized path segment or reject traversal/control input."""

    if not isinstance(value, str) or not _SAFE_RUNTIME_SEGMENT.fullmatch(value):
...
def hashed_runtime_segment(prefix: str, *opaque_parts: str) -> str:
    ...
    safe_prefix = require_safe_runtime_segment(prefix, label="Runtime id prefix")
    digest = hashlib.sha256()
    for part in opaque_parts:
        if not isinstance(part, str):
            raise RuntimeFileValidationError("opaque Runtime id parts must be strings")
