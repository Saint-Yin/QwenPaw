...
class ProjectExecutionStore:
    ...
    def create_specialist_run(
        self, record: SpecialistRunRecord | Mapping[str, Any]
    ) -> SpecialistRunRecord:
    ...
    def get_specialist_run(self, project_id: str, run_id: str) -> SpecialistRunRecord:
        project_id, run_id = self._safe_project_child(
            project_id, run_id, child_label="run_id"
        )
    ...
    def list_specialist_runs(self, project_id: str) -> list[SpecialistRunRecord]:
        ...
        with self._project_lock_read(project_id):
            ...
            if root.is_symlink() or not root.is_dir():
                raise UnsafeExecutionPath("Runtime runs path must be a real directory")
        ...
        *,
        ...
        with self._project_lock(project_id):
            ...
            candidate = SpecialistMessageRecord(
                ...
                run_id=run_id,
                message_seq=(existing.message_seq if existing else len(records) + 1),
                role=role,
                content_parts=list(content_parts),
                provider_message_id=provider_message_id,
                metadata=dict(metadata or {}),
                created_at=(
                    existing.created_at if existing else (created_at or utc_now())
                ),
                **provenance,
            )
    ...
    def list_specialist_messages(
        ...
        self._assert_run_identity(
            self._run_store(project_id, run_id).read(), project_id, run_id
        )
        records = self._messages_store(project_id, run_id).read_records()
        self._assert_stream_ownership(records, project_id=project_id, run_id=run_id)
        return records
    ...
    def append_continuation_event(
        ...
        *,
        ...
        with self._project_lock(project_id):
            ...
            records = stream.read_records()
            existing = next(
                (item for item in records if item.event_id == event_id), None
            )
            history = [
                item for item in records if item.continuation_id == continuation_id
            ]
            if any(
            ...
            if history:
                ...
                if first.continuation_token != continuation_token:
                    raise ExecutionPayloadConflict("continuation token cannot change")
                if task_id is not None and first.task_id != task_id:
                    raise ExecutionPayloadConflict("continuation task cannot change")
            ...
            if existing is not None:
                if existing == candidate:
                    if (
                        ...
                        and run.last_continuation_seq < existing.continuation_seq
                    ):
            ...
            self._validate_continuation_transition(history, target_state=target_state)
            ...
            stream.append(candidate, expected_next_seq=candidate.continuation_seq)
            self._apply_continuation_head_unlocked(
                project_id,
                run_id,
                run_snapshot,
    ...
    def create_task(self, record: TaskRecord | Mapping[str, Any]) -> TaskRecord:
        ...
        if candidate.last_attempt_seq != 0:
            raise ExecutionStateConflict("new Task Attempt head must start at zero")
        self._require_project(project_id)
        with self._project_lock(project_id):
    ...
    def get_task(
        ...
        *,
        ...
        project_id, task_id = self._safe_project_child(
            project_id, task_id, child_label="task_id"
        )
    ...
    def list_tasks(self, project_id: str) -> list[TaskRecord]:
        ...
        with self._project_lock_read(project_id):
            ...
            if not root.exists():
                return []
            if root.is_symlink() or not root.is_dir():
                raise UnsafeExecutionPath("Runtime tasks path must be a real directory")
    ...
    def append_task_attempt(
        ...
        *,
        ...
        with self._project_lock(
            ...
            history = [item for item in records if item.attempt_id == attempt_id]
            max_attempt = max((item.attempt_number for item in records), default=0)
            if history:
                first = history[0]
                resolved_number = first.attempt_number
                if attempt_number is not None and attempt_number != resolved_number:
                    raise ExecutionPayloadConflict(
                ...
                resolved_provider_id = provider_task_id or first.provider_task_id
                resolved_input = dict(input) if input is not None else first.input
            ...
            candidate = TaskAttemptEvent(
                ...
                attempt_seq=(existing.attempt_seq if existing else len(records) + 1),
            ...
            self._validate_attempt_transition(history, target_status=target_status)
    ...
    def list_task_attempts(
        ...
        self._assert_task_identity(
            self._task_store(project_id, task_id).read(), project_id, task_id
        )
        records = self._attempts_store(project_id, task_id).read_records()
        self._assert_stream_ownership(records, project_id=project_id, task_id=task_id)
        return records
    ...
    def create_execution_authorization(
        ...
        project_id = self._safe(candidate.project_id, "project_id")
        authorization_id = self._safe(candidate.authorization_id, "authorization_id")
        self._safe(candidate.round_id, "round_id")
        ...
        with self._project_lock(project_id):
            for existing in self._authorization_records(project_id):
                ...
                if existing.execution_request_id == candidate.execution_request_id:
            ...
            if created is not None:
                return created.value
            existing = store.read()
            self._assert_authorization_identity(existing, project_id, authorization_id)
            if self._equivalent(existing, candidate):
                return existing
            raise ExecutionPayloadConflict(
    ...
    def get_execution_authorization(
        ...
        self._assert_authorization_identity(record, project_id, authorization_id)
        return record
    ...
    def get_authorization_snapshot(
        ...
        self._assert_authorization_identity(
            snapshot.value, project_id, authorization_id
        )
        return snapshot
    ...
    def decide_execution_authorization(
        ...
        *,
        ...
        metadata: Mapping[str, Any] | None = None,
        expected_status: ExecutionAuthorizationStatus | str = (
        ...
        with self._project_lock(project_id):
            ...
            self._assert_authorization_identity(current, project_id, authorization_id)
            ...
            if (
                ...
                raise ExecutionStateConflict("expired authorization cannot be approved")
            if target is ExecutionAuthorizationStatus.APPROVED and decision is None:
                raise ExecutionStateConflict(
            ...
            return store.compare_and_swap(
                expected_checksum=checksum, value=candidate
            ).value
        ...
        *,
        ...
        with self._project_lock(project_id):
            ...
            candidate = QuarantinedTaskResult(
                ...
                run_id=task.run_id,
                reason=reason,
                request_fingerprint=(request_fingerprint or task.request_fingerprint),
                input_generation=(
                    input_generation
                    if input_generation is not None
                    else task.input_generation
                ),
                input_etag=input_etag if input_etag is not None else task.input_etag,
                result=dict(result),
    ...
    def _apply_attempt_head_unlocked(
        ...
        if current.last_attempt_seq + 1 != event.attempt_seq:
            raise ExecutionStoreIntegrityError("Task Attempt head is not contiguous")
    ...
    def _transition_run_unlocked(
        ...
        *,
        ...
        dumped.update({"status": target, "updated_at": updated_at or utc_now()})
        ...
        *,
        ...
        if current.status is target:
            if current.status not in {TaskStatus.QUEUED, TaskStatus.RUNNING}:
                raise ExecutionStateConflict("terminal Task state is immutable")
    ...
    @staticmethod
    def _run_statuses(
        value: SpecialistRunStatus | str | Collection[SpecialistRunStatus | str],
    ...
    def _assert_provenance_inherits(
        ...
        if any(getattr(child, name) != getattr(parent, name) for name in fields):
            raise ExecutionStoreIntegrityError(
    ...
    def _assert_run_identity(
        record: SpecialistRunRecord, project_id: str, run_id: str
    ) -> None:
    ...
    def _assert_task_identity(
        record: TaskRecord, project_id: str, task_id: str
    ) -> None:
        if record.project_id != project_id or record.task_id != task_id:
            raise ExecutionStoreIntegrityError("Task path and record identity disagree")
    ...
    def _assert_quarantine_identity(
        record: QuarantinedTaskResult, project_id: str, task_id: str
    ) -> None:
        if record.project_id != project_id or record.task_id != task_id:
            raise ExecutionStoreIntegrityError(
    ...
    def _safe_project_child(
        self, project_id: str, child_id: str, *, child_label: str
    ) -> tuple[str, str]:
        project_id = self._safe(project_id, "project_id")
    ...
    def _require_project(self, project_id: str) -> Path:
        ...
        try:
            ...
            raise ExecutionStoreError(f"Project does not exist: {project_id}") from exc
    ...
    def _messages_store(
        ...
        return DurableJsonlStore(
            self._runtime_root(project_id) / "runs" / run_id / "messages.jsonl",
            SpecialistMessageRecord,
        )
    ...
    def _continuations_store(
        ...
        return DurableJsonlStore(
            self._runtime_root(project_id) / "runs" / run_id / "continuations.jsonl",
            SpecialistContinuationEvent,
        )
    ...
    def _attempts_store(
        ...
        return DurableJsonlStore(
            self._runtime_root(project_id) / "tasks" / task_id / "attempts.jsonl",
            TaskAttemptEvent,
        )
    ...
    def _authorization_records(
        ...
        for path in sorted(root.glob("*.json")):
            record = AtomicJsonRecordStore(path, ExecutionAuthorizationRecord).read()
            self._assert_authorization_identity(
                record,
                project_id,
