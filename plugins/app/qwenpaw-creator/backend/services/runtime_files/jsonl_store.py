...
class DurableJsonlStore(Generic[T]):
    ...
    def _validate_record(
        self, record: Any, *, line_number: int | None = None
    ) -> T:
        try:
        ...
        with self._lock():
            ...
            if expected_next_seq is not None and expected_next_seq != next_seq:
                raise SequenceConflictError(
                    expected=expected_next_seq, actual=next_seq
                )
            envelope = JsonlEnvelope(
                seq=next_seq, written_at=timestamp, record=dumped
            )
    ...
    def read_records_after(
        ...
        *,
        ...
        return [
            self._validate_record(e.record, line_number=e.seq) for e in envelopes
        ]
