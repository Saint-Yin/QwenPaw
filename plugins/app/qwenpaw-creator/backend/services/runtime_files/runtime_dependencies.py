...
JQ_VERSION = "1.8.2"
_JQ_RELEASE_BASE_URL = f"https://github.com/jqlang/jq/releases/download/jq-{JQ_VERSION}"
...
def _auto_install_enabled() -> bool:
    value = os.environ.get(CREATOR_AUTO_INSTALL_BINARIES_ENV, "1").strip().casefold()
...
class CreatorBinaryManager:
    ...
    def __init__(self, *, binary_dir: Path | None = None) -> None:
        ...
        if not binary_dir.is_absolute():
            raise CreatorBinaryDependencyError("CREATOR_BINARY_DIR must be absolute")
    ...
    def _download_jq(self) -> Path:
        ...
        base_url = os.environ.get(CREATOR_JQ_BASE_URL_ENV, "").strip().rstrip("/")
        ...
        try:
            ...
            with temporary_path.open("wb") as temporary:
                request = Request(url, headers={"User-Agent": "QwenPaw-Creator/0.1"})
    ...
    def ensure_jq(self) -> BinaryStatus:
        ...
        if _is_executable(managed) and _file_sha256(managed) == expected_sha256:
        ...
        with _DOWNLOAD_LOCK:
            if not (
                _is_executable(managed) and _file_sha256(managed) == expected_sha256
            ):
                try:
                    self._download_jq()
    ...
    def ensure_ffmpeg(self) -> BinaryStatus:
        ...
        source = (
            "configured"
            if configured
            else ("system" if shutil.which("ffmpeg") == path else "imageio-ffmpeg")
        )
    ...
    def ensure_ffprobe(self, *, ffmpeg_path: str) -> BinaryStatus:
        ...
        if path:
            ...
            return BinaryStatus("ffprobe", "ok", path, "system-or-paired", False)
...
def creator_runtime_dependency_health() -> dict[str, Any]:
    ...
    if snapshot is None:
        ...
        jq = resolve_jq()
        snapshot = {
            "jq": BinaryStatus("jq", "ok" if jq else "missing", jq, "resolved", True),
            "ffmpeg": BinaryStatus(
                "ffmpeg", "ok" if ffmpeg else "missing", ffmpeg, "resolved", True
            ),
