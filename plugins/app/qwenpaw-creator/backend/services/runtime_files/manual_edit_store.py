...
class ManualEditBuffer(StrictRuntimeModel):
    ...
    def validate_net_buffer(self) -> ManualEditBuffer:
        if self.head_generation <= self.base_generation:
            raise ValueError("Manual Edit Buffer head generation must follow its base")
        keys = [_change_key(change) for change in self.changes]
        if len(keys) != len(set(keys)):
            raise ValueError("Manual Edit Buffer has duplicate change targets")
        if any(change.before_hash == change.after_hash for change in self.changes):
            raise ValueError("Manual Edit Buffer cannot persist a zero-net change")
        return self
...
class ManualEditBufferStore:
    ...
    def __init__(
        ...
        *,
        ...
        if not raw_root.is_absolute():
            raise UnsafeManualEditPath("Manual Edit data root must be absolute")
        ...
        if not self.data_root.is_dir():
            raise UnsafeManualEditPath("Manual Edit data root must be a directory")
    ...
    def record_frontend_commit(
        ...
        *,
        ...
        if head_generation <= base_generation:
            raise ValueError("head_generation must be greater than base_generation")
        incoming = [ProjectChange.model_validate(item) for item in changes]
        _require_unique_changes(incoming)
        with self._lock(
            project_id, lifecycle_lock_held=_lifecycle_lock_held
        ):
            existing = self._read_unlocked(project_id)
            if existing is not None and existing.head_generation == head_generation:
                # Recovery may replay a commit after the Buffer write but
                # before the enclosing Project journal was finalized.
                return existing
            if existing is not None and existing.head_generation != base_generation:
            ...
            merged = _merge_changes(
                existing.changes if existing is not None else (), incoming
            )
            if not merged:
        ...
        *,
        ...
        with self._lock(
            ...
            retained = [
                ...
                if not any(_changes_overlap(buffered, change) for change in incoming)
            ]
            if not retained:
                durable_unlink(self._buffer_path(project_id))
                return None
    ...
    def consume(
        self, project_id: str, *, consumer_id: str
    ...
    def _safe_project_id(self, project_id: str) -> str:
        try:
            safe_id = require_safe_runtime_segment(project_id, label="project_id")
    ...
    def _buffer_store(self, project_id: str) -> AtomicJsonRecordStore[ManualEditBuffer]:
        return AtomicJsonRecordStore(self._buffer_path(project_id), ManualEditBuffer)
...
def _merged_kind(
    previous: ProjectChangeKind, incoming: ProjectChangeKind
) -> ProjectChangeKind:
    ...
    if previous is ProjectChangeKind.DELETE and incoming is ProjectChangeKind.CREATE:
