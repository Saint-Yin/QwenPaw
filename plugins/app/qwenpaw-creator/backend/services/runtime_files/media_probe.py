...
from services.runtime_files.runtime_dependencies import resolve_ffmpeg, resolve_ffprobe
...
_AUDIO_CHANNEL_COUNT_RE = re.compile(r"Audio:.*?\b(\d+)\s+channels?\b", re.IGNORECASE)
...
def _probe_with_ffprobe(executable: str, target: str, timeout: float) -> MediaProbe:
    ...
    if duration is None:
        duration = _positive_float(video.get("duration") or audio.get("duration"))
    return MediaProbe(
...
def _probe_with_ffmpeg(executable: str, target: str, timeout: float) -> MediaProbe:
    completed = subprocess.run(
    ...
    result = MediaProbe(
        ...
        sample_rate_hz=_positive_int(audio_rate.group(1)) if audio_rate else None,
