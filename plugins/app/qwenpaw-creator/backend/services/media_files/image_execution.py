...
def _explicit_references(arguments: Mapping[str, Any]) -> tuple[list[str], list[str]]:
...
def _resolve_request(
    *,
    ...
    if command is CreatorCommandType.GENERATE_STORYBOARD_IMAGE:
        ...
        if production is not None and not isinstance(production, R2VProduction):
        ...
        if not prompt:
            ...
            prompt = "，".join(
                item
                for item in (unit.title.strip(), unit.narrative.strip(), shot_text)
                if item
            )
        if not prompt:
...
class FileImageExecutionService:
        ...
        *,
        ...
        if existing_task is not None:
            ...
            if existing_task.status in {
                ...
                raise ConflictError(f"图片 Task 已终止: {existing_task.status.value}")
        ...
        try:
            ...
            await self._fail_if_running(project_id, ids, "IMAGE_GENERATION_FAILED")
            raise
    ...
    async def _materialize_and_publish(
        ...
        *,
        ...
        content, media_type = await _provider_bytes(
            output,
            allowed_local_root=(project_root / "runtime" / "task-work" / task.task_id),
    ...
    async def _converge(
        ...
        *,
        ...
        await self._finish_run(
            ...
            summary=(
                "已生成并选择 "
                + ArtifactVersion.model_validate(success["artifactVersion"]).name
            ),
        )
    ...
    def _result_is_converged(project: Project, result: Mapping[str, Any]) -> bool:
        ...
        if existing_file != indexed or existing_version != artifact or slot is None:
            return False
    ...
    def _apply_result(candidate: dict[str, Any], result: Mapping[str, Any]) -> None:
        ...
        if command is CreatorCommandType.GENERATE_STORYBOARD_IMAGE:
            ...
            production["selected_storyboard_artifact_version_id"] = artifact.version_id
        else:
            entity_id = _target_id(target_ref, "asset")
            ...
            if variant_id:
                ...
                if artifact.version_id not in variant["generated_artifact_version_ids"]:
