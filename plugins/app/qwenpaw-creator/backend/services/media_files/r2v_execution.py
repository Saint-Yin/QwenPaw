...
def _ids(project_id: str, key: str) -> dict[str, str]:
    return {
        ...
        "attempt_started_event_id": _stable_id("attempt-started", project_id, key),
        "attempt_succeeded_event_id": _stable_id("attempt-succeeded", project_id, key),
        "attempt_failed_event_id": _stable_id("attempt-failed", project_id, key),
        "attempt_quarantined_event_id": _stable_id(
            "attempt-quarantined", project_id, key
        ),
...
def _json_mapping(value: Any, *, label: str) -> dict[str, Any]:
    ...
    if not isinstance(decoded, dict):  # pragma: no cover - mapping encodes object
        raise ValidationError(f"{label} 必须是 JSON object")
    return decoded
...
def _resolve_request(
    *,
    ...
    ratio = str(arguments.get("ratio") or project.settings.aspect_ratio).strip()
    resolution = str(
    ...
    *,
    ...
    try:
        ...
        if not stat.S_ISREG(details.st_mode) or details.st_nlink != 1:
            raise StorageIntegrityError("R2V materialized output is not a private file")
...
class FileR2VExecutionService:
    ...
    def __init__(
        ...
        *,
        ...
        if poll_timeout_seconds is not None and (
            poll_timeout_seconds <= 0 or poll_timeout_seconds >= poll_lease_seconds
        ):
            raise ValueError("poll timeout must be positive and below its lease")
        if submit_claim_seconds <= submit_timeout_seconds:
            raise ValueError("submit claim must exceed the total submit timeout")
    ...
    def _state_store(self, project_id: str, task_id: str) -> AtomicJsonRecordStore[R2VTaskState]:
    ...
    def _frozen_request(
        ...
        if not isinstance(raw, Mapping):
            raise StorageIntegrityError("R2V admission 缺少 frozen request snapshot")
        request = _json_mapping(raw, label="R2V frozen request snapshot")
        fingerprint = _fingerprint(request)
        expected = task.request_fingerprint if task is not None else run.request_fingerprint
    ...
    def _task_from_frozen_run(
        ...
        if not key or stable["run_id"] != run.run_id:
            raise StorageIntegrityError("R2V Run 缺少可恢复的 stable idempotency key")
    ...
    async def _ensure_state_for_task(
        ...
        try:
            state = await asyncio.to_thread(
                self._read_state_sync, task.project_id, task.task_id
            )
    ...
    async def _recover_run_admission_gap(
        ...
        try:
            ...
            try:
                ...
                return None
        if task.kind is not TaskKind.R2V_GENERATION or task.run_id != run.run_id:
            await self._fail_orphan_run(
                run,
        ...
        return await asyncio.to_thread(
            self.executions.get_task, run.project_id, task.task_id
        )
        ...
        *,
        ...
        try:
            existing = await asyncio.to_thread(
                self.executions.get_task, project_id, stable["task_id"]
            )
        except RecordNotFoundError:
            existing = None
    ...
    async def _admit(
        ...
        *,
        ...
        try:
            ...
            run = await asyncio.to_thread(self.executions.create_run, run_candidate)
        if (
        ...
        try:
            ...
            try:
                task = await asyncio.to_thread(
                    self.executions.create_task, task_candidate
                )
    ...
    def start_task(self, project_id: str, task_id: str) -> asyncio.Task[None] | None:
    ...
    def notify_terminal_task(self, task: TaskRecord) -> asyncio.Task[None] | None:
        """Wake terminal convergence after an API-side Task cancellation."""

        if task.kind is not TaskKind.R2V_GENERATION or task.status not in _TERMINAL_TASKS:
            return None
    ...
    async def wait_for_task(
        ...
        *,
        ...
        while True:
            task = await asyncio.to_thread(
                self.executions.get_task, project_id, task_id
            )
            if task.status in _TERMINAL_TASKS:
    ...
    async def _ensure_terminal_quarantine_record(
        ...
        state_error = state.last_error if isinstance(state.last_error, Mapping) else {}
        reason = str(
        ...
        stable = _ids(task.project_id, str(task.idempotency_key or task.task_id))
        await asyncio.to_thread(
        ...
        *,
        ...
        with self.services.projects.lifecycle_lock(task.project_id):
            ...
            if latest.status is TaskStatus.RUNNING:
                try:
                    state = self._read_state_sync(task.project_id, task.task_id)
                except RecordNotFoundError:
                    state = None
                try:
                    self.executions.append_attempt(
                        ...
                        provider_task_id=(state.provider_task_id if state else None),
                        error=dict(error),
                        _lifecycle_lock_held=True,
                    )
                except ExecutionStateConflict:
    ...
    async def _terminalize_task_from_state(
        ...
        if state.phase == "QUARANTINED":
            ...
            await self._quarantine(
                ...
                stable,
                result=result,
                reason=str((state.last_error or {}).get("code") or "QUARANTINED"),
    ...
    async def _drive(self, project_id: str, task_id: str) -> None:
        try:
            while True:
                ...
                try:
                    state = await asyncio.to_thread(
                        self._read_state_sync, project_id, task_id
                    )
                except RecordNotFoundError:
                    await self._fail(
                        task,
                ...
                if state.phase == "FAILED":
                    ...
                    await self._fail(
                        ...
                        message=str(detail.get("message") or "R2V state failed"),
                    )
                    return
                if state.phase == "CANCELLED":
                ...
                if state.phase == "ADMITTED":
                    ...
                    if claimed:
                        continue
                elif state.phase == "SUBMIT_CLAIMED" and not state.provider_task_id:
                    deadline = state.submit_claim_expires_at_epoch or 0.0
                    if self.clock() >= deadline:
                        accepted = await self._expire_submit_claim_if_still_stale(
                            task
                        )
                        if accepted:
                            await self._fail(
    ...
    async def _submit(self, task: TaskRecord, state: R2VTaskState) -> bool:
        ...
        claimed = await asyncio.to_thread(
            self._update_state_sync, task.project_id, task.task_id, claim
        )
        if claimed.phase != "SUBMIT_CLAIMED" or claimed.submit_owner != self.owner_id:
            return False
        ...
        latest = await asyncio.to_thread(
            self.executions.get_task, task.project_id, task.task_id
        )
        if latest.status is TaskStatus.RUNNING:
            await asyncio.to_thread(
                self.executions.transition_task,
        ...
        run = await asyncio.to_thread(
            self.executions.get_run, task.project_id, str(task.run_id)
        )
    ...
    async def _expire_submit_claim_if_still_stale(self, task: TaskRecord) -> bool:
        accepted = False
        now = float(self.clock())
    ...
    async def _claim_poll(
        ...
        def claim(current: R2VTaskState) -> Mapping[str, Any]:
            ...
            if current.phase != "POLLING" or not current.provider_task_id:
                return current.model_dump(mode="python")
            if current.next_poll_at_epoch is not None and now < current.next_poll_at_epoch:
                return current.model_dump(mode="python")
            if (
            ...
            dumped.update(
                {
                    ...
                    "poll_lease_expires_at_epoch": now + self.poll_lease_seconds,
                }
            )
            return dumped
    ...
    async def _poll_once(self, task: TaskRecord, state: R2VTaskState) -> None:
        ...
        try:
            ...
            if getattr(error, "retryable", True) is False:
                if await self._mark_poll_failed_if_owned(task, claim, str(error)):
                    await self._fail(
                        task,
        ...
        state = await asyncio.to_thread(
            self._update_state_sync, task.project_id, task.task_id, success
        )
        if accepted:
            await self._complete_success(task, state)
    ...
    async def _record_running_poll(
        ...
        await asyncio.to_thread(
            self._update_state_sync, task.project_id, task.task_id, update
        )
        if not accepted:
            return
        latest = await asyncio.to_thread(
    ...
    def _owns_live_materialize_claim(
        ...
        return bool(
            ...
            and current.materialize_claim_token
            and current.materialize_claim_token == claim.materialize_claim_token
            and (current.materialize_claim_expires_at_epoch or 0.0) > self.clock()
        )
    ...
    def _build_materialized_publication(
        ...
        *,
        ...
        if state.provider_result is None:
            raise StorageIntegrityError("R2V materialize claim has no provider result")
    ...
    def _validated_materialized_publication(
        ...
        if not isinstance(raw_published, Mapping):
            raise StorageIntegrityError("R2V materialized descriptor is corrupt")
        published = _json_mapping(raw_published, label="R2V materialized publication")
        if (
            published.get("taskId") != task.task_id
            or published.get("runId") != task.run_id
            or published.get("transactionId") != stable["transaction_id"]
        ):
            raise StorageIntegrityError("R2V materialized publication ownership mismatch")
        indexed = IndexedFile.model_validate(published.get("indexedFile"))
        if indexed.file_id != stable["file_id"]:
            raise StorageIntegrityError("R2V materialized IndexedFile id mismatch")
        try:
    ...
    async def _materialize_descriptor_scratch(
        ...
        try:
            recovered = await materialize_r2v_video(
                ...
                project_root=self.services.projects.project_root(task.project_id),
                project_id=task.project_id,
                task_id=task.task_id,
    ...
    async def _adopt_cancelled_materialized_output(self, task: TaskRecord) -> bool:
    ...
    async def _converge(
        ...
        success = latest.result if isinstance(latest.result, dict) else {
            **dict(result),
            "projectEtag": snapshot.etag,
            "projectGeneration": snapshot.generation,
        }
    ...
    @staticmethod
    def _result_is_converged(project: Project, result: Mapping[str, Any]) -> bool:
        try:
    ...
    @staticmethod
    def _apply_result(candidate: dict[str, Any], result: Mapping[str, Any]) -> None:
        indexed = IndexedFile.model_validate(result["indexedFile"])
        artifact = ArtifactVersion.model_validate(result["artifactVersion"])
        unit_id = _target_unit_id(str(result["targetRef"]))
        ...
        if artifact.version_id in versions and versions[artifact.version_id] != artifact_json:
        ...
        if latest.status is TaskStatus.RUNNING:
            try:
                await asyncio.to_thread(
                    ...
                    provider_task_id=str(result.get("providerTaskId") or "") or None,
                    output=dict(result),
    ...
    async def _finish_run(
        ...
        *,
        ...
        try:
            run = await asyncio.to_thread(
                self.executions.get_run, project_id, run_id
            )
        except RecordNotFoundError:
            return
        if run.status in {
            ...
            for task in self.executions.list_tasks(project_id):
                ...
                try:
                    if task.status in _TERMINAL_TASKS:
                        ...
                        continue
                    if task.status in {TaskStatus.QUEUED, TaskStatus.RUNNING}:
                        run = self.executions.get_run(project_id, str(task.run_id))
...
async def recover_interrupted_image_tasks(services: CreatorFileServices) -> int:
    ...
    for project_id in services.projects.discover_project_ids():
        for task in executions.list_tasks(project_id):
            if task.kind is not TaskKind.IMAGE_GENERATION or task.status not in {
                TaskStatus.QUEUED,
                TaskStatus.RUNNING,
            }:
                continue
            stable = worker._ids(  # noqa: SLF001 - same package recovery boundary
                project_id,
                str(task.idempotency_key or task.task_id),
            )
    ...
    *,
    ...
    with _registry_lock:
        ...
        if current is None:
            ...
            raise RuntimeError("R2V execution service already uses another provider")
...
def clear_file_media_execution_registry_for_tests() -> None:
    with _registry_lock:
        if any(  # noqa: SLF001
            ...
            raise RuntimeError("shutdown file media services before clearing registry")
        _registry.clear()
