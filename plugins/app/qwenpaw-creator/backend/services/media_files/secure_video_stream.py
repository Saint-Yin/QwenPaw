...
def _open_directory_at(parent_fd: int, name: str, *, label: str) -> int:
    ...
    if not stat.S_ISDIR(details.st_mode):  # defensive for platforms without O_DIRECTORY
        os.close(descriptor)
...
class _TaskScratch:
    ...
    def __init__(self, project_root: Path, project_id: str, task_id: str) -> None:
    ...
    def __enter__(self) -> _TaskScratch:
        ...
        opened = [project_fd]
        try:
            project_file = os.open("project.json", _file_read_flags(), dir_fd=project_fd)
            try:
            ...
            runtime_fd = _open_directory_at(project_fd, "runtime", label="runtime")
    ...
    def write(self, chunk: bytes) -> None:
        ...
        for offset in range(0, len(view), _COPY_CHUNK_BYTES):
            ...
            while written_total < len(piece):
                ...
                if written <= 0:  # pragma: no cover - regular files either write or fail
...
def _parse_remote_url(value: str) -> tuple[str, str, int]:
    ...
    sanitized = urlunsplit((scheme, parsed.netloc, parsed.path or "/", parsed.query, ""))
...
def _verify_media_types(detected: _DetectedVideo, declarations: Sequence[str]) -> None:
...
def _local_relative_parts(
    ...
    *,
    ...
    if scheme == "file":
        ...
        if len(decoded) <= len(expected) or decoded[: len(expected)] != expected:
            raise ValidationError("provider generated URL 跨越 Project 或 Task scope")
        return tuple(
    ...
    if candidate.is_absolute():
        try:
            ...
            raise ValidationError("provider 本地视频跨越 Project 或 Task scope") from error
    else:
        relative = candidate
    ...
    async def _resolve(self, value: str, deadline: float) -> _ResolvedRemote:
        ...
        try:
            addresses = await asyncio.wait_for(
                asyncio.to_thread(_resolver_addresses, host, port, self.resolver),
                timeout=remaining,
            )
        ...
        async with httpx.AsyncClient(
            ...
            for redirect_index in range(self.max_redirects + 1):
                ...
                try:
                    ...
                    raise ValidationError("provider 视频下载超过总 deadline") from error
            ...
            with _TaskScratch(project_root, project_id, task_id) as scratch:
                ...
                try:
                    ...
                    _verify_media_types(
                        detected,
                        (
                            str(output.get("media_type") or output.get("content_type") or ""),
                            response_media_type,
                        ),
                    )
                    declared_checksum = _declared_sha256(output.get("checksum"))
