...
class FfmpegLocalMediaRunner:
    ...
    def __init__(
        ...
        *,
        ...
        termination_grace_seconds: float = (_DEFAULT_FFMPEG_TERMINATION_GRACE_SECONDS),
    ) -> None:
    ...
    def _render_sync(self, spec: LocalMediaExecutionSpec) -> list[str]:
        ...
        if spec.command is CreatorCommandType.EXECUTE_EDIT:
            segment_dir = _ensure_real_directory_chain(spec.work_dir, "segments")
    ...
    def _apply_overlay(self, item: LocalMediaInput, segment: Path) -> str | None:
    ...
    def _validate_supported_directives(spec: LocalMediaExecutionSpec) -> None:
        ...
        for transition in spec.transitions:
            ...
            if kind != "cut" or duration_ms != 0:
                unsupported_transitions.append(f"{kind or '<empty>'}:{duration_ms}ms")
        ...
        unsupported_original_sound = sorted(
            {
                ...
                if item.original_sound.strip().casefold() not in {"", "preserve"}
            }
        )
    ...
    def _concat(self, inputs: Sequence[Path], output: Path, *, work_dir: Path) -> None:
...
def _require_real_directory(path: Path) -> None:
    try:
        ...
        raise StorageIntegrityError(f"Runtime task-work 父目录不存在: {path}") from exc
...
def _ensure_real_directory_chain(root: Path, *segments: str) -> Path:
    ...
    for segment in segments:
        if not segment or segment in {".", ".."} or "/" in segment or "\\" in segment:
            raise StorageIntegrityError(f"Runtime task-work 路径段不安全: {segment!r}")
...
def _indexed_version(
    ...
    *,
    ...
    media_type = indexed.media_type if indexed is not None else version.media_type
...
def _read_set_item(
    kind: str, version_id: str, indexed: IndexedFile | None, checksum: str
) -> dict[str, Any]:
    return {
...
def _composition_execution(
    *,
    ...
    for order, selection_id in enumerate(composition.sequence.order, 1):
        ...
        read_set.append(
            _read_set_item(
                "artifact-version", version.version_id, indexed, version.checksum
            )
        )
        selections.append(
            {
    ...
    return _ResolvedExecution(
        ...
        inputs=tuple(inputs),
        transitions=tuple(
            transition.model_dump(mode="json") for transition in composition.transitions
        ),
...
def _resolve_execution(
    *,
    ...
    if command is CreatorCommandType.EXECUTE_EDIT:
        ...
        if unit.route.value != "edit" or not isinstance(production, EditProduction):
        ...
        for clip_id in plan.timeline.order:
            ...
            inputs.append(
                _FrozenInput(
                    ...
                    duration_seconds=clip.source_out_seconds - clip.source_in_seconds,
            ...
            read_set.append(
                _read_set_item(
                    "asset-version", version.version_id, indexed, version.checksum
                )
            )
    ...
    if command is CreatorCommandType.COMPOSE_FINAL_VIDEO:
        if target_ref != "post:final":
            raise ValidationError("COMPOSE_FINAL_VIDEO targetRef 必须是 post:final")
        return _composition_execution(
            project=project,
            command=command,
            ...
            "inputs": [
                {
                    ...
                    "fileId": (
                        item.indexed.file_id if item.indexed is not None else None
                    ),
...
class FileLocalMediaExecutionService:
        ...
        *,
        ...
        try:
            existing_task = await asyncio.to_thread(
                self.executions.get_task, project_id, ids["task_id"]
            )
        ...
        if any(
            f"project:{base.etag}:" not in value for value in expected_object_versions
        ):
        ...
        task = await self._start(run=run, task=task, resolved=resolved, ids=ids)
        ...
        try:
            ...
            latest = await asyncio.to_thread(
                self.executions.get_task, project_id, task.task_id
            )
    ...
    def _ids(project_id: str, key: str) -> dict[str, str]:
        return {
            ...
            "attempt_succeeded_event_id": _stable_id(
                "attempt-succeeded", project_id, key
            ),
        ...
        *,
        ...
        try:
            run = await asyncio.to_thread(
                self.executions.get_run, base.project.project_id, ids["run_id"]
            )
        except RecordNotFoundError:
        ...
        try:
            task = await asyncio.to_thread(
                self.executions.get_task, base.project.project_id, ids["task_id"]
            )
        except RecordNotFoundError:
            try:
                task = await asyncio.to_thread(
    ...
    async def _claim_runner(self, task: TaskRecord) -> bool:
        ...
        def claim_sync():
            with self.services.projects.lifecycle_lock(task.project_id):
                ...
                claim_store = AtomicJsonRecordStore(task_root / "runner-claim.json")
    ...
    def _prepare_spec(
        ...
        project_root = self.services.projects.project_root(base.project.project_id)
        ...
        for frozen in resolved.inputs:
            ...
            if destination is None:
                ...
                if frozen.indexed is not None:
                    ...
                    source_version = base.project.assets.source_versions_by_id.get(
                        frozen.version_id
                    )
                    cached = (
                        resolve_remote_cache(
                            project_root, source_version, runtime_tasks
                        )
                    ...
                    if cached is not None:
                        ...
                        download_remote_file(frozen.source_url, str(destination))
                else:
            ...
            local_inputs.append(
                LocalMediaInput(
                    ...
                    file_id=(
                        frozen.indexed.file_id if frozen.indexed is not None else None
                    ),
    ...
    def _materialize_and_publish(
        ...
        if spec.output_path.is_symlink() or not stat.S_ISREG(output_stat.st_mode):
            raise StorageIntegrityError("本地媒体输出不是普通文件")
        resolved_output = spec.output_path.resolve(strict=True)
        if not resolved_output.is_relative_to(spec.work_dir.resolve(strict=True)):
            raise StorageIntegrityError("本地媒体输出逃逸 task-work")
        if output_stat.st_size <= 0 or output_stat.st_size > self.max_output_bytes:
        ...
        with spec.output_path.open("rb") as stream:
            staged = file_store.stage_stream(stream, staging_id=task.task_id[:80])
        relative_uri = PurePosixPath(
            "assets", "artifacts", f"{ids['file_id']}.mp4"
        ).as_posix()
        indexed = IndexedFile(
            file_id=ids["file_id"],
    ...
    async def _converge(
        ...
        *,
        ...
        outcome, current_task, snapshot = await asyncio.to_thread(commit_if_live)
    ...
    def _result_is_converged(project: Project, result: Mapping[str, Any]) -> bool:
        ...
        if command is CreatorCommandType.EXECUTE_EDIT:
            ...
            return (
                ...
                and production.rendered_video_artifact_version_id == artifact.version_id
            )
        if command is CreatorCommandType.STITCH_SECTION:
            section_id = _target_id(target_ref, "post")
            composition = project.post_production.sections_by_id.get(section_id)
        ...
        return (
            ...
            and composition.rendered_video_artifact_version_id == artifact.version_id
        )
    ...
    def _apply_result(candidate: dict[str, Any], result: Mapping[str, Any]) -> None:
        ...
        existing_version = assets["artifact_versions_by_id"].get(artifact.version_id)
        ...
        if command is CreatorCommandType.EXECUTE_EDIT:
            ...
            production["rendered_video_artifact_version_id"] = artifact.version_id
            return
        if command is CreatorCommandType.STITCH_SECTION:
            section_id = _target_id(target_ref, "post")
            composition = candidate["post_production"]["sections_by_id"].get(section_id)
        else:
        ...
        *,
        ...
        if task.status is TaskStatus.RUNNING:
            try:
                ...
                pass
        await self._finish_run(project_id, ids["run_id"], SpecialistRunStatus.FAILED)
    ...
    async def _finish_run(
        ...
        *,
        ...
        try:
            run = await asyncio.to_thread(self.executions.get_run, project_id, run_id)
        except RecordNotFoundError:
            return
        if run.status in {
            SpecialistRunStatus.SUCCEEDED,
    ...
    async def recover_project(self, project_id: str) -> list[str]:
        ...
        recovered: list[str] = []
        for task in await asyncio.to_thread(self.executions.list_tasks, project_id):
            ...
            await self._finish_run(
                project_id, ids["run_id"], SpecialistRunStatus.FAILED
            )
            recovered.append(task.task_id)
        return recovered
...
async def recover_file_local_media_project(
    ...
    return await FileLocalMediaExecutionService(services).recover_project(project_id)
