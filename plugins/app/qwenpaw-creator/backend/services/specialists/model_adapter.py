...
class SpecialistModelPort(Protocol):
    ...
    async def complete(
        ...
        *,
        ...
        on_tool_call_delta: Callable[[str, str, str], Awaitable[None]] | None = None,
    ) -> SpecialistModelTurn: ...
    ...
    for part in parts:
        ...
        if part_type in {"audio", "document"}:
            ...
            if not isinstance(attachment, Mapping):
                raise ValidationError(f"{part_type} content part 缺少 attachment")
            ...
            canonical_text = attachment.get("canonicalText") or attachment.get("text")
            if part_type == "document" and canonical_text is not None:
                provenance = attachment.get("versionId") or attachment.get("url") or "document"
                blocks.append(
                    TextBlock(text=f"[规范文档提取物 provenance={provenance}]\n{canonical_text}")
                )
                continue
            if part_type == "document":
                raise ValidationError("当前 Specialist 模型只接受带 provenance 的规范文档提取物")
            url = str(attachment.get("url") or "")
            if not url:
                raise ValidationError(f"{part_type} attachment 缺少 URL/规范提取物")
            fallback = "audio/wav" if part_type == "audio" else "application/pdf"
            blocks.append(_data_block(url, media_type=_media_type(url, attachment, fallback)))
            continue
        raise ValidationError(f"未知 Specialist content part: {part_type!r}")
    return blocks


def records_to_agentscope_messages(records: Sequence[Mapping[str, Any]]) -> list[Msg]:
    ...
    for record in records:
        ...
        normalized_parts = [dict(item) for item in parts if isinstance(item, Mapping)]
        metadata = dict(record.get("metadata") or {})
        if role == "system":
            text = "\n".join(str(item.get("text") or "") for item in normalized_parts)
            ...
            if isinstance(tool_call, Mapping):
                ...
                try:
                    ...
                    raise ValidationError(f"assistant action boundary metadata 损坏：{exc}") from exc
                blocks.append(
                    ToolCallBlock(
                        id=call.id,
                        name=call.name,
            ...
            if not call_id or not tool_name:
                raise ValidationError("tool message 缺少 action boundary metadata")
            output = "\n".join(str(item.get("text") or "") for item in normalized_parts)
            messages.append(
                AssistantMsg(
                    "specialist_runtime",
...
class AgentScopeSpecialistModel:
    ...
    async def complete(
        ...
        *,
            ...
            if inspect.isasyncgen(response):
                ...
                async for item in response:
                    ...
                    if item.is_last:
                        final = item
                if final is None:
                    raise SpecialistModelProtocolError("AgentScope stream 缺少 final response")
                response = final
        ...
        for block in response.content:
            if isinstance(block, TextBlock):
                ...
                try:
                    ...
                    raise SpecialistModelProtocolError("tool call arguments 不是合法 JSON") from exc
        ...
        if len(calls) > 1:
            raise SpecialistModelProtocolError("Specialist 每轮只允许一个 control action")
        ...
        if on_text_delta is not None and raw_visible_text and not streamed_text:
            await on_text_delta(raw_visible_text)
        if on_thinking_delta is not None and thinking and not streamed_thinking:
