...
RUNTIME_ACTION_SPECS: dict[str, RuntimeActionSpec] = {
    ...
    "move_path": RuntimeActionSpec(
        ...
        frozenset({SpecialistRole.UNIT_PLANNING_ROUTING, SpecialistRole.STORY_PLANNING}),
        "mutation",
        _object_schema(
            {
        ...
        _object_schema(
            {
                ...
                "displayName": {
                    "type": ["string", "null"],
                    "description": (
                        "Visual Development 生成实体锚点时必填的用户可读语义名称；"
                        "R2V 分镜不得填写。"
                    ),
                },
            },
        ...
        _object_schema(
            {
                "selections": {
                    ...
                    "items": {
                        ...
                        "required": ["sourceRef", "artifactVersionId", "order", "transition"],
                        "additionalProperties": False,
                    },
                },
                "outputPath": {"type": "string"},
                "transitionDuration": {"type": ["number", "null"], "minimum": 0},
...
def _require_arguments(spec: RuntimeActionSpec, arguments: Mapping[str, Any]) -> None:
    schema = spec.arguments_schema
    required = tuple(schema.get("required") or ())
    missing = [name for name in required if name not in arguments]
    extras = sorted(set(arguments) - set((schema.get("properties") or {}).keys()))
...
def _validate_schema(value: Any, schema: Mapping[str, Any], *, path: str) -> None:
    ...
    if isinstance(value, Mapping):
        ...
        extras = sorted(set(value) - set(properties)) if schema.get("additionalProperties") is False else []
        if missing or extras:
            raise ValidationError(
...
class RuntimeRequestRegistry:
    ...
    def __init__(self, handlers: Mapping[str, RuntimeActionHandler] | None = None) -> None:
        self._handlers = dict(handlers or {})
        unknown = sorted(set(self._handlers) - set(RUNTIME_ACTION_SPECS))
        if unknown:
            raise ValidationError(f"未定义 Runtime action handler: {', '.join(unknown)}")

    def implemented_for(self, role: SpecialistRole) -> tuple[RuntimeActionSpec, ...]:
        return tuple(
            spec
    ...
    def manifest_for(
        ...
        *,
        ...
        manifest = tuple(spec.provider_schema() for spec in self.implemented_for(role))
        if not admitted_target_refs:
            return manifest

        admitted = tuple(dict.fromkeys(str(item) for item in admitted_target_refs if item))
        if not admitted:
            raise ValidationError(
        ...
        for item in manifest:
            if (
                ...
                continue
            target_schema = item["function"]["parameters"]["properties"]["targetRef"]
    ...
    async def dispatch(
        ...
        *,
        ...
        if parsed_target.kind == "project" and parsed_target.identifier not in {
        ...
        if spec.category == "execution" and (
            not isinstance(input_refs, Sequence) or isinstance(input_refs, (str, bytes))
        ):
        ...
        for input_ref in input_refs:
            try:
                ...
                if (
                    ...
                    raise ValidationError("Runtime action inputRef 不属于当前 Project")
...
class DurableTaskExecutionHandler:
    ...
    def __init__(self, *, task_registry: Any, task_kind: TaskKind, prepare: ExecutionPreparer) -> None:
    ...
    async def __call__(
        ...
        if task["status"] == TaskStatus.SUCCEEDED.value:
            return RuntimeActionResult(
                ...
                {"taskId": task["id"], "status": task["status"], "result": task.get("result")},
            )
        if task["status"] not in {TaskStatus.QUEUED.value, TaskStatus.RUNNING.value}:
            raise ConflictError(f"Runtime Task 当前不可 continuation: {task['status']}")
...
class AuthorizationGatedExecutionHandler:
    ...
    def __init__(
        ...
        *,
        ...
        if not provider.strip() or not model.strip():
            raise ValidationError("execution authorization provider/model 不能为空")
    ...
    async def __call__(
        ...
        resolved_arguments = dict(prepared.resolved_arguments or draft.arguments)
...
def _expectation_for(run_input: SpecialistRunInput, target_ref: str) -> TargetExpectation:
    matches = [item for item in run_input.target_expectations if item.ref == target_ref]
    if len(matches) != 1:
        raise ConflictError(
...
def _execution_idempotency(context: RuntimeActionContext, draft: RuntimeActionDraft) -> str:
    payload = json.dumps(
        {
            "projectId": context.run_input.project_id,
            "transactionId": context.run_input.transaction_id,
...
def compose_execution_preparer(
    ...
    if orders != list(range(1, len(orders) + 1)) or len(orders) != len(set(orders)):
...
def _capability_payload(capability: ProviderCapabilitySnapshot) -> dict[str, Any]:
    return {
...
def r2v_execution_preparer(
    ...
    def prepare(context: RuntimeActionContext, draft: RuntimeActionDraft) -> PreparedExecution:
        expectation = _expectation_for(context.run_input, draft.target_ref)
        arguments = draft.arguments
        reference_versions = tuple(str(item) for item in arguments["referenceVersionIds"])
        ...
        fingerprint = canonical_json_hash(
            {
                ...
                "videoPromptHash": (
                    "sha256:"
                    + hashlib.sha256(str(arguments["videoPrompt"]).encode("utf-8")).hexdigest()
                ),
                "referenceVersionIds": [str(item) for item in arguments["referenceVersionIds"]],
                "referenceUrls": [str(item) for item in arguments["referenceUrls"]],
                "referenceVersionRefs": [],
        ...
        report = validate_r2v_execution(
            R2VExecutionRequest(
                ...
                storyboard_version_ref=str(arguments["storyboardVersionId"] or ""),
        ...
        reference_urls = tuple(str(item) for item in arguments["referenceUrls"])
        if reference_versions[0:1] != (str(arguments["storyboardVersionId"]),):
            raise ValidationError("R2V storyboard version 必须位于 referenceVersionIds 第一位")
