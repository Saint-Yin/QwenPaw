...
def validate_r2v_execution(request: R2VExecutionRequest) -> ValidationReport:
    ...
    if request.route != UnitTaskType.R2V.value:
        issues.append(
            ValidationIssue("R2V_ROUTE_REQUIRED", "Unit route 必须为 r2v", target)
        )
    if request.duration_seconds > MAX_R2V_UNIT_DURATION_SECONDS:
        issues.append(
            ValidationIssue("R2V_DURATION_MAX", "R2V Unit 总时长不能超过 15 秒", target)
        )
    if (
    ...
    if (
        ...
        and request.duration_seconds not in capability.allowed_durations_seconds
    ):
        issues.append(
            ValidationIssue(
                "R2V_DURATION_DISCRETE", "时长不在 provider 离散时长集合中", target
            )
        )
    ...
    if len(request.reference_image_refs) > capability.max_reference_images:
        issues.append(
            ValidationIssue(
                "R2V_REFERENCE_LIMIT", "参考图数量超过 provider 上限", target
            )
        )
    if request.storyboard_version_ref and (
        ...
        issues.append(
            ValidationIssue(
                "R2V_STORYBOARD_FIRST", "storyboard 必须是第一张参考图", target
            )
        )
    ...
    if (
        ...
        issues.append(
            ValidationIssue(
                "SLOT_GENERATION_STALE", "目标 video Slot generation 已变化", target
            )
        )
    if not request.authorization_valid:
        issues.append(
            ValidationIssue(
                "EXECUTION_AUTH_REQUIRED", "execution authorization 无效", target
            )
        )
...
def validate_actual_r2v_duration(
    duration_seconds: float, capability: ProviderCapabilitySnapshot
) -> ValidationReport:
...
def evaluate_video_prompt(prompt: str, context: dict[str, Any]) -> PromptEvaluation:
    ...
    forbidden = [term for term in FORBIDDEN_CONTEXT_TERMS if term in normalized]
    if forbidden:
    ...
    missing_characters = [name for name in character_names if name not in normalized]
    ...
    if re.search(
        r"\b\d+\s*(?:-|–|—|到)\s*\d+\s*秒|\[\s*\d+\s*s", normalized, re.IGNORECASE
    ):
    ...
    if abstract_hits and not _contains_any(normalized, VISIBLE_EMOTION_DETAILS):
    ...
    if dialogues:
        missing_dialogues = [
            dialogue for dialogue in dialogues if f"{{{dialogue}}}" not in normalized
        ]
