...
def validated_source_analysis_versions(
    *,
    ...
    if asset is None:
        raise StorageIntegrityError("Source Intelligence AssetVersion authority 丢失")
    ...
    if run_id not in model_run_ids or not model_run_ids.issubset(allowed_model_run_ids):
        raise StorageIntegrityError(
    ...
    primary_pairs = {
        (item.provider, item.model) for item in index.model_runs if item.id == run_id
    }
