        ...
        return "[CREATOR_CONTEXT]\n" + json.dumps(
            {
                ...
                "creatorPlan": dict(self.creator_plan) if self.creator_plan else None,
                ...
                "activeWaitableRuns": [
                    {
                        **dict(run),
                        "status": (
                            "WAITING_RESULT"
                            if str(run.get("status") or "") == "WAITING_RUNTIME"
                            else run.get("status")
                        ),
                    }
        ...
        if role == "system":
            ...
            if isinstance(tool_call, Mapping):
                ...
                if not call_id or not name or not isinstance(arguments, Mapping):
                    raise ValidationError("Creator assistant toolCall metadata 损坏")
                blocks.append(
                    ToolCallBlock(
                        id=call_id,
                        name=name,
        ...
        *,
        ...
        for block in response.content:
            if isinstance(block, TextBlock):
                ...
                if not call_id or not name:
                    raise CreatorModelProtocolError("Creator tool call id/name 不能为空")
