...
def _fail_closed(reason_code: str, *, model_name: str | None = None) -> MessageClassificationDecision:
...
def _strict_payload(raw_text: str) -> dict[str, Any]:
    ...
    if set(payload) != required:
        raise ValueError("classification response keys do not match the contract")
...
class ModelBackedMessageIntentClassifier:
    ...
    async def classify_waiting_input(
        ...
        *,
        ...
        if any(str(part.get("type") or "") != "text" for part in content_parts):
            return _fail_closed(
        ...
        try:
            ...
            return _fail_closed("classifier_unavailable", model_name=model_name)
