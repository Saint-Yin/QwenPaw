...
class ContentStore:
    ...
    def __init__(
        ...
        *,
        ...
        self.root = resolve_creator_data_root(root, provider=data_root_provider)
    ...
    def _namespace_root(self, namespace: ContentNamespace) -> Path:
        try:
            ...
            raise ValidationError(f"非法 content namespace: {namespace!r}") from exc
        return self.root / directory / "sha256"

    def path_for(self, digest: str, *, namespace: ContentNamespace = "blob") -> Path:
    ...
    def put_stream(
        ...
        *,
        ...
        descriptor, raw_temp = tempfile.mkstemp(
            prefix="content-", suffix=".tmp", dir=self._temp_root
        )
        temp_path = Path(raw_temp)
    ...
    def _verify_file(self, path: Path, digest: str, *, expected_size: int | None = None) -> None:
        hasher = hashlib.sha256()
        size = 0
        try:
        ...
        if hasher.hexdigest() != digest or (expected_size is not None and size != expected_size):
            raise StorageIntegrityError(
    ...
    def verify(self, digest: str, *, namespace: ContentNamespace = "blob") -> bool:
        normalized = validate_sha256(digest)
    ...
    def exists(self, digest: str, *, namespace: ContentNamespace = "blob") -> bool:
    ...
    def read_text(
        ...
        *,
        ...
        try:
            return self.read_bytes(digest, namespace=namespace).decode(encoding)
    ...
    def iter_digests(self, *, namespace: ContentNamespace = "blob") -> Iterator[str]:
