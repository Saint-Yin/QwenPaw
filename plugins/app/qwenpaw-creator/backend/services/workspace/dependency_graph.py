...
class DependencyGraph:
    ...
    def edges(self) -> tuple[DependencyEdge, ...]:
        return tuple(sorted(self._edges, key=lambda item: (item.source_ref, item.target_ref, item.strength.value, item.reason)))
    ...
    def remove_for(self, ref: str) -> None:
        self._edges = {edge for edge in self._edges if ref not in {edge.source_ref, edge.target_ref}}

    def direct_dependents(self, ref: str, *, strength: DependencyStrength | None = None) -> frozenset[str]:
        return frozenset(
            edge.target_ref
            for edge in self._edges
            if edge.source_ref == ref and (strength is None or edge.strength == strength)
        )

    def impact_closure(self, roots: Iterable[str], *, include_soft: bool = True) -> frozenset[str]:
        visited = set(roots)
        queue = list(visited)
        while queue:
            current = queue.pop(0)
            for edge in self._edges:
                if edge.source_ref != current or (not include_soft and edge.strength != DependencyStrength.HARD):
                    continue
    ...
    def hard_components(self, refs: Iterable[str]) -> tuple[frozenset[str], ...]:
        remaining = set(refs)
    ...
    def dangling_edges(self, existing_refs: Iterable[str]) -> tuple[DependencyEdge, ...]:
        known = set(existing_refs)
        return tuple(edge for edge in self.edges if edge.source_ref not in known or edge.target_ref not in known)
