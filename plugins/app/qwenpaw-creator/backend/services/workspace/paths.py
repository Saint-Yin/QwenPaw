...
_PRIVATE_ROOTS = frozenset({"runs", "runtime", "revisions", "blobs", "artifacts"})
...
def workspace_text_path(value: str) -> str:
    ...
    if path.parts[0].lower() in _PRIVATE_ROOTS or any(part.startswith(".") for part in path.parts):
    ...
    if path.suffix.lower() not in TEXT_WORKSPACE_SUFFIXES:
        raise ValidationError(
            ...
            details={"path": value, "allowedSuffixes": sorted(TEXT_WORKSPACE_SUFFIXES)},
        )
    return normalized
...
def workspace_glob(value: str) -> str:
    ...
    if literal_root.lower() in _PRIVATE_ROOTS or any(part.startswith(".") for part in parts):
        raise ValidationError("workspace glob 指向系统私有命名空间", details={"pattern": value})
    return raw
...
def glob_matches(path: str, pattern: str) -> bool:
    return compile_workspace_glob(pattern).fullmatch(workspace_text_path(path)) is not None
...
def scopes_overlap(left: str, right: str) -> bool:
    ...
    left_root = workspace_directory(left[:-3]) if left_is_tree else workspace_text_path(left)
    right_root = workspace_directory(right[:-3]) if right_is_tree else workspace_text_path(right)
