...
class RevisionManifest:
    ...
    def __post_init__(self) -> None:
        ...
        if self.parent_revision_id is not None:
            safe_storage_segment(self.parent_revision_id, label="parent revision id")
...
class RevisionRepository(Protocol):
    ...
    def get_pointer(self, project_id: str, pointer: RevisionPointer) -> str | None: ...
...
def tree_hash_for_entries(entries: Iterable[RevisionEntry]) -> str:
    ...
    return hashlib.sha256(_canonical_json([_entry_payload(item) for item in ordered])).hexdigest()
...
def _manifest_core_payload(
    *,
    ...
    return {
        ...
        "treeHash": tree_hash,
        "entries": [_entry_payload(item) for item in sorted(entries, key=lambda item: item.path)],
...
def _manifest_from_payload(payload: Mapping[str, Any]) -> RevisionManifest:
    ...
    if actual_tree_hash != manifest.tree_hash:
        raise StorageIntegrityError(
            ...
            details={"expected": manifest.tree_hash, "actual": actual_tree_hash},
        )
    ...
    if actual_manifest_hash != manifest.manifest_hash:
        raise StorageIntegrityError(
            ...
            details={"expected": manifest.manifest_hash, "actual": actual_manifest_hash},
        )
    return manifest
...
class FileRevisionRepository:
    ...
    def _project_root(self, project_id: str) -> Path:
        return self.root / "projects" / safe_storage_segment(project_id, label="project id")
    ...
    def _pointer_path(self, project_id: str, pointer: RevisionPointer) -> Path:
        return self._project_root(project_id) / "pointers" / f"{pointer.value}.json"
    ...
    def get(self, project_id: str, revision_id: str) -> RevisionManifest:
        ...
        try:
            ...
            raise StorageIntegrityError(f"revision manifest 无法读取: {target}") from exc
    ...
    def get_pointer(self, project_id: str, pointer: RevisionPointer) -> str | None:
        ...
        try:
            ...
            raise StorageIntegrityError(f"revision pointer 无法读取: {target}") from exc
    ...
    def compare_and_set_pointer(
        ...
        *,
        ...
        with self._lock:
            ...
            atomic_replace_bytes(target, _canonical_json({"revisionId": revision_id}))
...
class RevisionStore:
    def __init__(
        ...
        *,
        ...
        self.repository = repository or FileRevisionRepository(content_store.root)

    def _normalize_entries(
        self, entries: Mapping[str, RevisionEntry] | Iterable[RevisionEntry]
    ) -> tuple[RevisionEntry, ...]:
    ...
    def entries_from_files(
        self, files: Mapping[str, str | bytes | bytearray | memoryview]
    ) -> tuple[RevisionEntry, ...]:
    ...
    def create_revision(
        ...
        *,
        ...
        if parent_revision_id is not None:
            ...
            if parent.project_id != project_id:  # defensive for custom repositories
    ...
    def create_initial_approved_revision(
        ...
        *,
        project_id: str,
        files: Mapping[str, str | bytes | bytearray | memoryview] | None = None,
    ) -> RevisionManifest:
        entries = self.entries_from_files(files or {})
        requested_tree_hash = tree_hash_for_entries(entries)
        current_id = self.repository.get_pointer(project_id, RevisionPointer.APPROVED)
    ...
    def get_approved(self, project_id: str) -> RevisionManifest:
        revision_id = self.repository.get_pointer(project_id, RevisionPointer.APPROVED)
        if revision_id is None:
            raise NotFoundError("Project 尚无 Approved Revision", details={"projectId": project_id})
