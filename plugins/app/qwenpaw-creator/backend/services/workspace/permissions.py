...
if len(FILE_TOOL_NAMES) != 7 or len(set(FILE_TOOL_NAMES)) != 7:
    raise RuntimeError("File tool registry must contain exactly seven unique names")
...
def path_matches(path: str, patterns: Iterable[str]) -> bool:
    ...
    return any(compile_workspace_glob(pattern).fullmatch(normalized) for pattern in patterns)
...
def _normalize_role(role: SpecialistRole | str) -> str:
    ...
    if value != CREATOR_ROLE:
        try:
            ...
            raise ValidationError("未知 workspace role", details={"role": value}) from exc
    return value
...
def tools_for_role(role: SpecialistRole | str) -> tuple[str, ...]:
    ...
    if spec is None:
        raise ValidationError("该历史 specialist role 已停用", details={"role": value})
...
class PermissionRegistry:
    ...
    def ensure_project_path(self, role: SpecialistRole | str, path: str) -> None:
        value = role.value if isinstance(role, SpecialistRole) else str(role)
    ...
    def ensure_run_write_path(
        ...
        *,
        ...
        if value == SpecialistRole.SOURCE_INTELLIGENCE.value:
            normalized = workspace_text_path(path)
            if path_owner(normalized) is not SpecialistRole.SOURCE_INTELLIGENCE:
                raise WorkspacePermissionDenied(value, "写入 Workspace 文件", normalized)
            return
