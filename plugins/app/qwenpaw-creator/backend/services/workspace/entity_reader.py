...
_ORDERED_ENTITY_DIR_RE = re.compile(r"^(?P<order>\d{6})--(?P<id>[^/]+?)(?:--(?P<slug>.+))?$")
...
class EntityReader:
    ...
    @staticmethod
    def _selector(
        *, branch_id: str | None, revision_id: str | None
    ) -> dict[str, str | None]:
    ...
    def read_text(
        ...
        *,
        ...
        return self.text_store.read_text(project_id, safe_workspace_path(path), **selector)
        ...
        for entry in entries:
            ...
            files.append(
                EntityFile(
                    ...
                    text=self.text_store.read_text(project_id, entry.path, **selector),
        ...
        for directory_name in sorted(directory_names):
            ...
            if not match:
                raise ValidationError(
                    ...
                    details={"root": normalized_root, "directory": directory_name},
                )
            entity_id = safe_storage_segment(match.group("id"), label="entity id")
            ...
            entities.append(
                WorkspaceEntity(
                    ...
                    files=self.read_directory(project_id, directory, **selector),
                )
            )
        return tuple(
            sorted(
