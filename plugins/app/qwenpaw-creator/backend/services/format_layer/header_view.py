...
def milliseconds_to_seconds(value: int) -> float:
    if isinstance(value, bool) or not isinstance(value, int) or value < 0:
        raise ProjectionInputError("millisecond value must be a non-negative integer")
...
def seconds_to_milliseconds(value: int | float | str | Decimal) -> int:
    ...
    if milliseconds < 0 or milliseconds != milliseconds.to_integral_value():
        raise ProjectionInputError("second value must have exact millisecond precision")
    return int(milliseconds)


def _page_base(*, target_version: str, locator: Mapping[str, str], blockers: Sequence[str]) -> dict[str, Any]:
    ...
    view.update(
        {
            ...
            "resolution": snapshot.text("settings/resolution.txt", required=True),
