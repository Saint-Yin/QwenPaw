...
def _parse_ai_edit_plan_ref(raw_ref: str) -> tuple[str, str]:
    ...
    if parsed.scheme != "ai-edit-plan" or parsed.path not in {"", "/"} or not separator:
...
def build_edit_workbench_view(
    ...
    *,
    ...
    if raw_plan_ref:
        ...
        selected_plan = next((item for item in plan_versions if item.id == plan_version_id), None)
        if selected_plan is None:
            blockers.append(f"AI_EDIT_PLAN_VERSION_NOT_FOUND:{plan_version_id}")
        elif selected_plan.unit_id != unit.id or selected_plan.source_ref != raw_plan_ref:
            ...
            if collisions:
                raise ProjectionInputError(f"AI Edit envelope uses reserved projection keys: {collisions}")
    else:
    ...
    blockers.extend(_selection_consistency_blockers(rendered_ref, video, label="EDIT_VIDEO"))
    if (
    ...
    payload.update(
        {
            ...
            "relations": [
                {"from": f"project://unit/{unit.id}", "to": ref, "kind": "uses_version"}
                for ref in source_refs
            ],
            "readiness": {"ready": not unique_blockers},
            "blockers": unique_blockers,
            "targetVersion": snapshot.target_version(f"unit:{unit.id}"),
            "uiLocator": {"page": "workbench", "unitId": unit.id, "route": "edit"},
    ...
    ordered_units = [candidate for section in sections for candidate in section.units]
    unit_index = next(index for index, candidate in enumerate(ordered_units) if candidate.id == unit.id)

    def adjacent_boundary(candidate: Any, *, first: bool) -> float | None:
        if candidate is None or candidate.route != "edit":
            return None
        adjacent_ref = snapshot.text(f"{candidate.root}/production/edit/plan.ref")
        if not adjacent_ref:
            return None
        try:
            adjacent_unit_id, adjacent_version_id = _parse_ai_edit_plan_ref(adjacent_ref)
        ...
        adjacent = next(
            (
                ...
                if version.id == adjacent_version_id and version.unit_id == adjacent_unit_id
            ),
            None,
        )
        if adjacent is None:
    ...
    following = ordered_units[unit_index + 1] if unit_index + 1 < len(ordered_units) else None
    payload["previousUnitLastClipEnd"] = adjacent_boundary(previous, first=False)
    payload["nextUnitFirstClipStart"] = adjacent_boundary(following, first=True)
    return payload
...
def build_workbench_view(
    ...
    *,
    ...
    if unit.route == "r2v":
        if provider is None:
            raise ProjectionInputError("r2v projection requires a provider capability snapshot")
