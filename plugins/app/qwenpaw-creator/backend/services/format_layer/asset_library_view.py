...
def _source_directories(snapshot: TextWorkspaceSnapshot) -> tuple[tuple[str, str, str], ...]:
    ...
    for path in snapshot.paths("sources/"):
        ...
        if len(identity) != 2 or not all(identity):
            raise ProjectionInputError(f"invalid source directory: {segment!r}")
        result.add((identity[0], identity[1], f"sources/{segment}"))
    return tuple(sorted(result))


def _visual_directories(snapshot: TextWorkspaceSnapshot) -> tuple[tuple[str, str, str, str], ...]:
    ...
    for category in ("characters", "scenes", "props"):
        ...
        for path in snapshot.paths(prefix):
            ...
            identity = segment.split("--", 1)
            if len(identity) != 2 or not all(identity):
                raise ProjectionInputError(f"invalid visual asset directory: {segment!r}")
            result.add((category, identity[0], identity[1], f"visual/{category}/{segment}"))
    return tuple(sorted(result))
...
def _reference_count(snapshot: TextWorkspaceSnapshot, source_ref: str, *, selection_path: str) -> int:
    return sum(
        1
        for path, entry in snapshot.files.items()
        if path.endswith(".ref") and path != selection_path and entry.content.strip() == source_ref
    )
...
def _visual_detail(
    ...
    *,
    ...
    orders = sorted({*prompts, *requirements, *reference_groups, *image_refs} or {1000})
    defaults = {
        "characters": "正面基准形象",
        "scenes": "环境基准图",
        "props": "道具基准图",
    ...
    result = {
        ...
        "mediaType": "image",
        "images": images,
        "refsNeeded": [requirements.get(order) or defaults[category] for order in orders],
        "prompts": [prompts.get(order, "") for order in orders],
        "referenceImageRefs": [reference_groups.get(order, []) for order in orders],
    }
    if kind == "character":
...
def _source_detail(item, *, user_notes: str) -> dict[str, Any]:
    media_type = item.media_type if item.media_type in {"image", "video", "audio"} else None
    images = (
        [
            {
                "id": item.id,
...
def build_asset_library_view(
    ...
    *,
    ...
    for asset_id, slug, root in _source_directories(snapshot):
        ...
        count = _reference_count(snapshot, raw_ref, selection_path=selection_path)
        attached.append(
            {
                "assetId": asset_id,
                ...
                "understandingRef": snapshot.text(f"{root}/understanding/current.ref") or None,
        ...
        if count:
            relations.append({"from": f"project://asset/{asset_id}", "to": raw_ref, "kind": "referenced_by_project"})
        ...
        visual_assets.append(
            {
                ...
                "presentationStatus": "accepted" if item is not None else "draft",
                "mediaType": item.get("mediaType") if item is not None else "image",
                "assetVersionId": item.get("assetVersionId") if item is not None else None,
                "artifactVersionId": item.get("artifactVersionId") if item is not None else None,
                "url": item.get("url") if item is not None else None,
                "thumbnailUrl": item.get("thumbnailUrl") if item is not None else None,
                "referenceRefs": references,
                "referenceCount": _reference_count(snapshot, raw_ref, selection_path=selected_path) if raw_ref else 0,
                "targetVersion": snapshot.target_version(f"asset:{asset_id}"),
                "detail": detail,
                "uiLocator": {"page": "assets", "assetId": asset_id},
            }
    ...
    available = [
        {
            ...
            "category": "upload",
            "existence": "available",
            "presentationStatus": "accepted" if any(entry["assetVersionId"] == item.id for entry in attached) else "draft",
            ...
            "referenceCount": _reference_count(snapshot, item.source_ref, selection_path=""),
            "attached": any(entry["assetVersionId"] == item.id for entry in attached),
            "uiLocator": {"page": "assets", "assetId": item.logical_asset_id, "versionId": item.id},
        }
        for item in sorted(catalogs.assets, key=lambda record: (record.logical_asset_id, record.created_at, record.id))
    ]
    ...
    for item in sorted(catalogs.assets, key=lambda record: (record.created_at, record.id)):
    ...
    for item in latest_sources.values():
        ...
        attached_entry = next(
            (entry for entry in attached if entry["assetVersionId"] == item.id),
            None,
        )
        ...
        presentation_assets.append(
            {
                "id": item["id"],
                "name": item["name"],
                "category": "env_ref" if item["category"] == "scene" else "subject_ref",
    ...
    for item in sorted(catalogs.artifacts, key=lambda record: (record.created_at, record.id)):
        selected_by_visual_entity = any(
            visual.get("selectedRef") == item.source_ref for visual in visual_assets
        )
        if item.source_ref not in selected_snapshot_refs or item.kind not in {
        ...
        presentation_assets.append(
            {
                ...
                "mediaType": "image" if item.kind == "r2v_storyboard_image" else "video",
                "url": item.url,
                "thumbnailUrl": item.thumbnail_url,
                "description": str(item.metadata.get("generationPrompt") or ""),
                "sourceDescription": f"生成资产，来自 {item.owner_ref}",
                "sourceRef": item.source_ref,
                "referenceCount": _reference_count(snapshot, item.source_ref, selection_path=""),
                "generatedKind": "storyboard_image" if item.kind == "r2v_storyboard_image" else item.kind,
    ...
    return {
        ...
        "resolvedRefs": list({item["ref"]: item for item in resolved_refs}.values()),
        "relations": relations,
