...
def _artifact_locator(owner_ref: str, version_id: str) -> dict[str, str]:
    if owner_ref.startswith("unit:"):
        return {"page": "workbench", "unitId": owner_ref.split(":", 1)[1], "versionId": version_id}
    if owner_ref.startswith("section:"):
        return {"page": "section-compose", "sectionId": owner_ref.split(":", 1)[1], "versionId": version_id}
    return {"page": "final-compose", "versionId": version_id}


def _entity_items(project_id: str, sections: Iterable[ParsedSection]) -> list[dict[str, Any]]:
    del project_id
...
class RefIndex:
    ...
    def resolve(self, raw_ref: str) -> dict[str, Any] | None:
        ...
        if parsed.scheme == "project":
            ...
            item = next((candidate for candidate in self.items if candidate["ref"] == canonical), None)
            if item is not None:
        ...
        if analysis_identity is not None:
            ...
            item = next(
                (
                    ...
                    and candidate.get("analysisVersionId") == analysis_version_id
                ),
                None,
            )
            return {**item, "ref": raw_ref} if item is not None else None
    ...
    def search(
        ...
        *,
        ...
        if limit < 1 or limit > 100:
            raise ProjectionInputError("ref search limit must be between 1 and 100")
        allowed = set(types or {"section", "unit", "asset", "artifact", "analysis"})
        unknown = allowed - {"section", "unit", "asset", "artifact", "analysis"}
        if unknown:
            raise ProjectionInputError(f"unknown ref search types: {sorted(unknown)}")
        needle = query.casefold().strip()
        selected = [
            dict(item)
            for item in self.items
            if item["type"] in allowed
            and (not needle or needle in item["name"].casefold() or needle in item["ref"].casefold())
        ]
        selected.sort(key=lambda item: (item["type"], item["name"].casefold(), item["ref"]))
        return selected[:limit]


def build_ref_index(snapshot: TextWorkspaceSnapshot, catalogs: ProjectionCatalogs) -> RefIndex:
        ...
        items.append(
            {
                ...
                "uiLocator": {"page": "assets", "assetId": record.logical_asset_id, "versionId": record.id},
            }
        )
...
def resolve_many(index: RefIndex, raw_refs: Iterable[str]) -> tuple[list[dict[str, Any]], list[str]]:
