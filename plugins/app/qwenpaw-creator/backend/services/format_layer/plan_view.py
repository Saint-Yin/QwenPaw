...
def _unit_view(
    ...
    *,
    ...
    if unit.route == "r2v":
        ...
        selected_storyboard_ref = snapshot.text(f"{r2v_root}/storyboard/selected.ref")
        ...
        selected_video_ref = snapshot.text(f"{unit.root}/production/edit/rendered-video.ref")
        if selected_video_ref:
            selected_video = index.resolve(selected_video_ref)
            if selected_video is not None:
                video_url = selected_video.get("url")
    view = {
        ...
        "relations": [
            {"from": f"project://unit/{unit.id}", "to": raw_ref, "kind": "references"}
            for raw_ref in raw_refs
        ],
        "readiness": {"ready": not blockers, "blockers": list(dict.fromkeys(blockers))},
...
def build_plan_view(
    ...
    for section_number, section in enumerate(sections, 1):
        ...
        for unit_number, unit in enumerate(section.units, 1):
            unit_view, blockers = _unit_view(snapshot, unit, number=unit_number, index=index)
        ...
        section_views.append(
            {
                ...
                "units": unit_views,
                "resolvedRefs": list({item["ref"]: item for unit in unit_views for item in unit["resolvedRefs"]}.values()),
                "relations": [relation for unit in unit_views for relation in unit["relations"]],
                "readiness": {"ready": not section_blockers, "blockers": list(dict.fromkeys(section_blockers))},
    ...
    return {
        ...
        "aspectRatio": snapshot.text("settings/aspect-ratio.txt", required=True),
