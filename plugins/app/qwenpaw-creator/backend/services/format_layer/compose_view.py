...
from .inputs import ProjectionCatalogs, RevisionSelections, TextWorkspaceSnapshot
    ...
    *,
    ...
    for path in snapshot.paths(prefix):
        ...
        if not separator or not order_text.isdigit() or not source_part:
            raise ProjectionInputError(f"invalid compose sequence entry: {path}")
        source_kind = default_source_kind
        source_id = source_part
        encoded_kind, kind_separator, encoded_id = source_part.partition("--")
        if decode_source_kind and kind_separator and encoded_kind in source_artifact_kinds:
            source_kind = encoded_kind
            source_id = encoded_id
        if source_kind not in source_artifact_kinds or not source_id:
            raise ProjectionInputError(f"invalid compose sequence source identity: {path}")
...
def _transition_entries(snapshot: TextWorkspaceSnapshot, prefix: str) -> list[dict[str, str]]:
    return [
...
def build_section_compose_view(
    ...
    *,
    ...
    candidates = [
        _artifact_view(record, selected=selections.artifact_versions.get(record.slot_id) == record.id)
        for record in catalogs.artifacts
        if record.kind == "unit_video" and record.owner_ref in {f"unit:{unit.id}" for unit in section.units}
    ]
    ...
    if rendered_ref:
        ...
        if rendered is None:
            blockers.append(f"SECTION_RENDERED_VERSION_NOT_FOUND:{rendered_ref}")
        else:
    ...
    return {
        ...
        "sectionNumber": next(index + 1 for index, item in enumerate(sections) if item.id == section_id),
        ...
        "resolvedRefs": list({item["ref"]: item for item in resolved}.values()),
        "relations": [
            {"from": item["sourceRef"], "to": item["artifactRef"], "kind": "uses_version"}
...
def build_final_compose_view(
    ...
    *,
    ...
    candidates_with_order: list[tuple[tuple[int, int, str], dict[str, Any]]] = []
    for record in catalogs.artifacts:
        if (
    ...
    candidates = [item for _order, item in sorted(candidates_with_order, key=lambda pair: pair[0])]
    rendered_ref = snapshot.text(f"{root}/rendered-video.ref")
    rendered_video_url: str | None = None
    if rendered_ref:
        rendered = index.resolve(rendered_ref)
