...
def build_review_view(manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    required = ("id", "transactionId", "reviewRevisionId", "decisionGroups", "operations")
    missing = [field for field in required if field not in value]
    if missing:
        raise ProjectionInputError(f"review manifest fields are missing: {missing}")
    blockers = [
    ...
    return {
        ...
        "uiLocator": {"page": "review", "transactionId": str(value["transactionId"])},
    }
