...
def _find_unit(project: dict, unit_id: str) -> tuple[dict, dict] | tuple[None, None]:
...
def _selected_video_assets(project: dict, unit: dict, material_asset_ids: list[str] | None = None) -> list[dict]:
    ...
    if selected_version_ids:
        ...
        if selected:
            return selected
    if selected_ids:
        selected = [asset for asset in assets if asset.get("id") in selected_ids]
        if selected:
            return selected
    return [asset for asset in assets if asset.get("category") == "upload"] or assets
...
def _duration_target(unit: dict) -> int:
    raw = unit.get("duration") or sum(int(shot.get("duration", 0) or 0) for shot in unit.get("shots", []) or [])
    try:
        duration = int(raw)
...
def _validate_execution_overlay_copy(
    ...
    *,
    ...
    if appear_at >= clip_duration:
        return None
    duration = min(
        max(0.001, _safe_float(overlay.get("duration"), clip_duration - appear_at)),
...
def _selected_source_intelligence(
    ...
    selected_versions = {
        str(asset.get("versionId") or "") for asset in assets if asset.get("versionId")
    }
    selected_refs = {
    ...
    for analysis in analyses:
        ...
        for semantic in analysis.get("semanticEntries", []) or []:
            ...
            transcript_segments = [
                {
                    "start": round(
                        max(0.0, _safe_float(segment.get("startMs"), 0.0) / 1000),
                        3,
                    ),
                    "end": round(
                        max(0.0, _safe_float(segment.get("endMs"), 0.0) / 1000),
                        3,
                    ),
                ...
                and _safe_float(segment.get("endMs"), 0.0) > semantic_start * 1000
                and _safe_float(segment.get("startMs"), 0.0) < semantic_end * 1000
                and str(segment.get("text") or "").strip()
                and _safe_float(segment.get("endMs"), 0.0)
                > _safe_float(segment.get("startMs"), 0.0)
            ]
            transcript_segments.sort(key=lambda item: (item["start"], item["end"]))
            ...
            start = min(
                [semantic_start, *(item["start"] for item in transcript_segments)]
            )
            end = max([semantic_end, *(item["end"] for item in transcript_segments)])
            item = {
...
def _snap_to_asr_boundary(
    ...
    **<=** *timestamp* so the clip always includes complete speech.
    ...
    if prefer_end:
        ...
        if safe_best is not None:
            return safe_best
        return any_earlier_start if any_earlier_start is not None else timestamp
...
def _asr_segment_containing(
    ...
    containing = [
        ...
        if float(segment["start"]) + 0.001 < timestamp < float(segment["end"]) - 0.001
    ]
    if not containing:
        return None
    return min(
...
def _review_clip_asr_boundaries(
    ...
    *,
    ...
    transcript_segments = [
        ...
        if isinstance(item, dict)
        and _safe_float(item.get("end"), 0.0) > _safe_float(item.get("start"), 0.0)
    ]
    if not transcript_segments:
        return {
    ...
    *,
    ...
    for index, raw in enumerate(raw_clips, 1):
        ...
        if is_interview:
            ...
            start = _snap_to_asr_boundary(start, transcript_segments, tolerance=2.0, prefer_end=False)
            end = _snap_to_asr_boundary(end, transcript_segments, tolerance=2.0, prefer_end=True)
        ...
        if expected_overlay_kind is None:
            ...
            if not isinstance(raw_overlay, dict):
                raise ValueError(f"clip #{index} 必须由 AI Editing Director 生成 overlayCopy")
...
async def _semantic_agent_plan(
    ...
    prompt = _semantic_agent_prompt(project, section, unit, analyses, catalog, goal)
    ...
    for attempt in range(2):
        ...
        try:
            ...
            if project.get("contentType") == "interview":
                ...
                target = float(_duration_target(unit))
                if target >= 45 and float(plan["target_duration"]) < target * 0.9:
                    raise ValueError(
                        "语义选片总时长不足目标的 90%："
...
def _probe_duration_ffprobe(target: str) -> float | None:
    ...
    try:
        result = subprocess.run(
            [ffprobe, "-v", "error", "-show_entries", "format=duration", "-of", "json", target,
            ],
    ...
    try:
        return (
            float(json.loads(result.stdout).get("format", {}).get("duration") or 0)
            or None
        )
...
def _heuristic_plan(
    ...
    clip_count = (
        len(editorial_beats)
        if editorial_beats
        else max(1, min(10, len(assets) if target < 30 else max(len(assets), target // 8)))
    )
    ...
    for index in range(clip_count):
        ...
        if not source_duration:
            ...
            usable_end = max(default_clip_duration + 1, source_duration - default_clip_duration - 1)
            start = (
                0
                if clip_count == 1 else min(usable_end, (usable_end / max(1, clip_count - 1)) * index)
            )
        ...
        editorial_beat = editorial_beats[index] if index < len(editorial_beats) else ""
        selection_reason = (
            editorial_beat
...
def _prompt(
    ...
    for asset in assets:
        version_id = str(asset.get("versionId") or "")
        speech_segments = ((asr_segments_by_asset or {}).get(version_id) or [])[:30]
        inventory.append({
            "asset_id": asset.get("id"),
            "name": asset.get("name"),
            "url": asset.get("url"),
            "duration_seconds": round(_asset_duration_seconds(asset, project.get("id")), 2),
...
def _ensure_target_duration(plan: dict, project: dict, unit: dict, assets: list[dict], goal: str) -> dict:
    target = _duration_target(unit)
    if target < 45:
        return plan
    ...
    filler = _heuristic_plan(project, unit, assets, goal, "VLM timeline shorter than target; supplemented by duration balancer",
    )
    used = {
        (item.get("asset_id"), round(_safe_float(item.get("start"), 0.0), 1), round(_safe_float(item.get("end"), 0.0), 1),
        )
    ...
    for clip in filler.get("timeline", []):
        if current >= target * 0.92:
            break
        key = (clip.get("asset_id"), round(_safe_float(clip.get("start"), 0.0), 1), round(_safe_float(clip.get("end"), 0.0), 1),
        )
        if key in used:
            continue
        ...
        if duration <= 0:
            continue
        enriched = {**clip, "clip_id": f"clip-{next_order:02d}", "order": next_order}
        ...
        storyboard.append({
            ...
            "timestamp": round(_safe_float(clip.get("start"), 0.0) + duration / 2, 3),
    ...
    plan["target_duration"] = round(sum(_safe_float(item.get("duration"), 0.0) for item in timeline), 3)
    plan["summary"] = (
...
def _snap_and_cap_for_interview(
    ...
    *asr_segments_by_asset*) rather than running local Whisper analysis.
    ...
    for clip in timeline:
        ...
        asset_duration = _media_duration_seconds(str(clip.get("source_url") or ""), project_id)
        if asset_duration > 0:
    ...
    timeline = kept
    total_duration = sum(_safe_float(clip.get("duration"), 0.0) for clip in timeline)
    if total_duration < _INTERVIEW_MIN_DURATION and timeline:
        ...
        for clip in reversed(timeline):
            ...
            asset_dur = _media_duration_seconds(
                str(clip.get("source_url") or ""), project_id,
            )
            raw_max = end + deficit
    ...
    for index, clip in enumerate(timeline):
        original_id = original_clip_ids.get(id(clip), str(clip.get("clip_id") or ""))
        clip["order"] = index + 1
    ...
    for index, panel in enumerate(plan.get("storyboard") or []):
        ...
        if asr_segs:
            snapped_ts = find_nearest_sentence_boundary(
                timestamp, asr_segs, tolerance=2.0, prefer_end=False,
            )
        ...
        storyboard.append(
            {
                **panel,
                ...
                "timestamp": round(max(source_start, min(timestamp, source_end)), 3),
...
def _normalize_plan(payload: Any, project: dict, unit: dict, assets: list[dict], goal: str) -> dict:
    ...
    bound_active = bool(ep_asset_id) and (ep_start or ep_end) and ep_end > ep_start

    assets_by_id = _asset_index(assets)
    timeline = []
    for index, item in enumerate(payload.get("timeline", []) if isinstance(payload.get("timeline"), list) else []):
        ...
        if item.get("end") is not None:
            ...
            end = max(start + 1.0, start + _safe_float(item.get("duration"), 4.7))
        ...
        if bound_active and (end <= ep_start or start >= ep_end):
            continue
        clip = {
            "clip_id": str(item.get("clip_id") or f"clip-{len(timeline) + 1:02d}"),
            "asset_id": asset_id,
            "asset_name": asset.get("name", ""),
            "source_url": asset.get("url", ""),
            "start": round(start, 3),
    ...
    for clip in timeline:
        ...
        clip_output_window[clip["clip_id"]] = (round(cursor, 3), round(cursor + dur, 3))
        cursor += dur
    ...
    storyboard = []
    for index, item in enumerate(payload.get("storyboard", []) if isinstance(payload.get("storyboard"), list) else []):
        if not isinstance(item, dict):
            continue
        asset_id = str(item.get("source_asset_id") or item.get("asset_id") or ep_asset_id)
        if asset_id and asset_id not in assets_by_id:
            continue
        ts = _safe_float(item.get("timestamp"), 0.0)
        clip = clips_by_id.get(str(item.get("clip_id") or "")) or _find_clip_for_timestamp(ts)
        if not clip:
            continue
        out_start, out_end = clip_output_window[clip["clip_id"]]
        storyboard.append({
            "panel_id": str(item.get("panel_id") or f"panel-{index + 1:02d}"),
            "clip_id": clip["clip_id"],
            "order": len(storyboard) + 1,
            "title": str(item.get("title") or clip.get("asset_name") or f"关键帧 {index + 1}"),
            "description": str(item.get("description") or clip.get("reason") or ""),
    ...
    plan = {
        ...
        "target_duration": round(sum(_safe_float(item.get("duration"), 0.0) for item in timeline), 3),
        "timeline": timeline,
        "storyboard": sorted(storyboard, key=lambda item: item.get("order", 0)),
        "audio_plan": payload.get("audio_plan") if isinstance(payload.get("audio_plan"), dict) else fb["audio_plan"],
        "model": {
...
def _extract_keyframe(
    ...
    if not source_url.startswith(("/generated/", "file://", "http://", "https://")):
    ...
    clip_end = _safe_float(clip.get("end"), clip_start + _safe_float(clip.get("duration"), 1.0))
    timestamp = _safe_float(panel.get("timestamp"), clip_start)
    if timestamp <= 0 and clip_end > clip_start:
        timestamp = clip_start + min((clip_end - clip_start) / 2, max(0.5, clip_end - clip_start - 0.1))
    timestamp = max(clip_start, min(timestamp, max(clip_start, clip_end - 0.1)))

    frame_dir = ensure_preview_subdir("editing", project_id, f"{unit_id}-keyframes")
    frame_path = frame_dir / f"panel-{index + 1:02d}-{uuid.uuid4().hex[:8]}.jpg"
    cmd = [
        ffmpeg,
        "-y",
        "-ss",
    ...
    if result.returncode != 0 or not frame_path.exists():
        tail = (
            ...
            if result.stderr else "未知原因"
        )
        panel["keyframe_error"] = f"抽帧失败({tail[:80]})"
        logger.warning("Keyframe extraction failed for %s: %s", source_url, result.stderr[-300:])
        return ""
...
def _escape_lines(text: str, max_units: int = 36, max_lines: int = 4, limit: int = 150) -> list[str]:
...
def _write_storyboard_sheet(project_id: str, unit_id: str, plan: dict) -> str:
    ...
    summary_lines = _escape_lines(str(plan.get("summary", "AI 剪辑关键帧分镜")), max_units=92, max_lines=2, limit=180,
    )
    chunks = [
    ...
    for index, panel in enumerate(panels):
        ...
        title_line = _escape_lines(f"#{index + 1} {panel.get('title') or f'Panel {index + 1}'}", max_units=32, max_lines=1, limit=60,
        )[0]
        desc_lines = _escape_lines(str(panel.get("description") or ""), max_units=38, max_lines=3, limit=120)
        source = html.escape(str(panel.get("source_asset_id") or ""))
        source_line = _escape_lines(f"source: {panel.get('source_asset_id') or ''} · {panel.get('timestamp', 0)}s", max_units=38, max_lines=1, limit=72,
        )[0]
        timeline_line = _escape_lines(f"timeline {panel.get('timeline_start', 0)}s - {panel.get('timeline_end', 0)}s", max_units=38, max_lines=1, limit=72,
        )[0]
        image_href = _data_uri_for_media_image(str(panel.get("image_url") or ""))
        chunks.extend([
        ...
        if image_href:
            ...
            error_lines = _escape_lines(
                ...
                max_units=42, max_lines=2, limit=80,
            )
            chunks.append(
...
def _write_storyboard_sheet_with_retry(
    ...
    *,
    ...
    for attempt in range(1, max(1, attempts) + 1):
        ...
        except Exception as exc:  # noqa: BLE001 - bounded retry then caller policy
            last_error = exc
            logger.warning(
        ...
        if url:
            return url
        last_error = RuntimeError("AI Edit storyboard renderer returned an empty URL")
        logger.warning(
    ...
    raise RuntimeError(f"AI Edit storyboard generation failed after {attempts} attempts") from last_error
    ...
    *,
    ...
    if use_model and analyses:
        try:
            ...
            if project.get("contentType") == "interview":
                plan = _snap_and_cap_for_interview(plan, project, unit, asr_segments_by_asset)
            trace.append(
                {
        ...
        except Exception as exc:  # noqa: BLE001 - semantic planning is optional
            logger.warning("AI Edit semantic planning fallback unit=%s: %s", unit_id, exc)
...
def _execution_source_url(project: dict[str, Any], clip: dict[str, Any]) -> str:
...
async def execute_ai_edit_plan(
    ...
    logger.info(
        ...
        unit_id, len(plan.get("timeline") or []) if isinstance(plan.get("timeline"), list) else 0,
    )
    section, unit = _find_unit(project, unit_id)
    if not section or not unit:
        raise ValueError("AI 剪辑单元不存在")
    ...
    if expected_overlay_kind == "interview_summary":
        for index, clip in enumerate(ordered_timeline):
            ...
            clip_dur = max(
                ...
                float(
                    clip.get("duration")
                    or (float(clip.get("end", 5) or 5) - float(clip.get("start", 0) or 0))
                ),
            )
    ...
    if readiness["status"] != "ok":
        logger.warning("execute_ai_edit_plan ffmpeg 未就绪: %s", readiness.get("blockers"))
        trace_event(
            "creator.ai_edit.ffmpeg_failed",
            component="ai_edit_core",
            status="error",
            attributes={"stage": "readiness", "blockers": readiness["blockers"]},
            projectId=project.get("id"),
        )
        return {
            "success": False,
    ...
    for index, clip in enumerate(ordered_timeline):
        ...
        duration = max(0.5, float(clip.get("duration") or (float(clip.get("end", start + 4.7)) - start)),
        )
        clip_path = work_dir / f"clip-{index + 1:02d}.mp4"
        logger.info(
            "execute_ai_edit_plan clip#%d/%d source_url=%s source_path=%s start=%.2f duration=%.2f",
            index + 1, len(ordered_timeline), source_url[:80], source_path, start, duration,
        )
        ...
        trace_event(
            ...
            attributes={
                ...
                "overlayKind": validated_overlays[index].get("kind") if validated_overlays[index] else None,
            },
            projectId=project.get("id"),
        )
        cmd = [
        ...
        trace_event(
            ...
            attributes={
                ...
                "durationMs": round((time.perf_counter() - clip_started) * 1000, 3),
            },
            projectId=project.get("id"),
        )
        ...
        transitions.append("cut")  # always hard cut; xfade not used for manual timing
    ...
    if any_xfade and len(clip_paths) > 1:
        ...
        from services.media.ffmpeg import _has_audio_stream, build_xfade_command
        ...
        result = subprocess.run(xfade_cmd, capture_output=True, text=True, timeout=600, check=False)
        if result.returncode != 0:
            logger.warning(
    ...
    if not any_xfade or len(clip_paths) == 1:
        ...
        atomic_replace_bytes(
            concat_file,
            "\n".join(f"file '{path.as_posix()}'" for path in clip_paths).encode("utf-8"),
        )
        concat_cmd = [
            ffmpeg,
            "-y",
        ...
        result = subprocess.run(concat_cmd, capture_output=True, text=True, timeout=180, check=False)
        if result.returncode != 0:
            logger.warning(
    ...
    trace_event(
        ...
        attributes={
            ...
            "outputSizeBytes": output_path.stat().st_size if output_path.exists() else None,
            "durationMs": round((time.perf_counter() - started) * 1000, 3),
        },
        projectId=project.get("id"),
    )
