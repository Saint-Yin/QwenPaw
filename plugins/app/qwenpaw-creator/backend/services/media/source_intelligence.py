...
def _safe_id(value: str, *, label: str) -> str:
    if (
        not value
        or not _SAFE_ID.fullmatch(value) or value in {".", ".."} or "--" in value
    ):
...
def _record_line(kind: str, record: Mapping[str, Any]) -> str:
    ...
    if kind in {"shot", "transcript", "word", "ocr", "audio"}:
        ...
        fields.extend((_optional(record.get("startMs")), _optional(record.get("endMs"))))
    if kind == "shot":
        fields.extend(
            (
        ...
        fields.extend((_enc(record["label"]), _enc(record.get("description") or "")))
    elif kind == "entity":
        fields.extend(
            (
                _enc(record["kind"]),
        ...
        lines.append(
            "\t".join(
                (
                    ...
                    "-" if coverage.get("ratio") is None else _float(coverage["ratio"]),
                )
            )
        )
    media = payload["media"]
...
def render_source_intelligence_files(index: SourceIntelligenceIndex) -> dict[str, str]:
    ...
    for order, shot in enumerate(payload["shots"], 1):
        ...
        files[f"{root}/time-range.txt"] = f"[{shot['startMs']},{shot['endMs']})\n"
...
def _base_record(fields: Sequence[str], *, expected: int, kind: str) -> dict[str, Any]:
...
def _parse_record(fields: Sequence[str]) -> tuple[str, dict[str, Any]]:
    ...
    if expected is None:
        raise StorageIntegrityError(f"unknown Source Intelligence index row: {kind}")
    ...
    if kind == "shot":
        ...
        record.update({"label": _dec(fields[10]), "description": _dec(fields[11])})
    elif kind == "entity":
        record.update(
            {
                "kind": _dec(fields[10]),
        ...
        record.update({"text": _dec(fields[10]), "tags": _dec_values(fields[11])})
    return kind, record


def parse_source_intelligence_files(files: Mapping[str, str],
) -> SourceIntelligenceIndex:
    ...
    if not index_lines or index_lines[0] != _INDEX_HEADER:
        raise StorageIntegrityError("Source Intelligence index header/version is invalid")
    ...
    for line in index_lines[1:]:
        fields = line.split("\t")
        if fields[0] in {"id", "assetId", "assetVersionId", "sourceChecksum", "createdAt",
        }:
            if len(fields) != 2 or fields[0] in scalars:
                raise StorageIntegrityError(f"invalid or duplicate Source Intelligence scalar: {line}")
            scalars[fields[0]] = _dec(fields[1])
        elif fields[0] == "modelRun":
            if len(fields) != 4:
                raise StorageIntegrityError("invalid Source Intelligence modelRun row")
            model_runs.append(
                {"id": _dec(fields[1]), "provider": _dec(fields[2]), "model": _dec(fields[3]),
                }
            )
        elif fields[0] == "coverage":
            if len(fields) != 5 or fields[1] in coverage:
                raise StorageIntegrityError("invalid or duplicate Source Intelligence coverage row")
            coverage[fields[1]] = {
                "mode": fields[2],
            ...
            if len(fields) != 3 or fields[1] in media:
                raise StorageIntegrityError("invalid or duplicate Source Intelligence media row")
            value: Any = None if fields[2] == "-" else _dec(fields[2])
            if value is not None and fields[1] not in {"mediaKind", "mediaType"}:
                value = int(value)
            media[fields[1]] = value
        else:
            kind, record = _parse_record(fields)
            records[kind].append(record)
    expected_scalars = {"id", "assetId", "assetVersionId", "sourceChecksum", "createdAt",
    }
    if set(scalars) != expected_scalars:
        raise StorageIntegrityError("Source Intelligence index identity fields are incomplete")
    summary_prefix = "# Source Intelligence\n\n"
    summary_text = files["summary.md"]
    if not summary_text.startswith(summary_prefix) or not summary_text.endswith("\n"):
        raise StorageIntegrityError("Source Intelligence summary.md does not use the canonical template")
    payload = {
        **scalars,
    ...
    try:
        ...
        raise StorageIntegrityError(f"Source Intelligence index payload is invalid: {exc}") from exc
    ...
    mismatches = sorted(path for path, text in expected_files.items() if files[path] != text)
    if mismatches:
        raise StorageIntegrityError(
    ...
    *,
    ...
    if set(raw) != allowed:
        raise ValidationError(
            ...
            details={"missing": sorted(allowed.difference(raw)), "extra": sorted(set(raw).difference(allowed)),
            },
        )
    raw_coverage = raw.get("coverage")
    if not isinstance(raw_coverage, Mapping) or set(raw_coverage) != set(_MODALITIES):
        raise ValidationError("Source Understanding coverage must contain exactly visual/asr/ocr/audio")
    coverage: dict[str, SourceCoverage] = {}
    for modality in _MODALITIES:
        try:
            item = SourceCoverage.model_validate(raw_coverage[modality])
        except (TypeError, ValueError) as exc:
            raise ValidationError(f"Source Understanding {modality} coverage is invalid: {exc}") from exc
        policy = dict(coverage_policy.get(modality) or {})
        if item.mode != policy.get("mode") or item.producer != policy.get("producer"):
            raise ValidationError(
    ...
    def records(key: str, model: type[Any], allowed_fields: set[str]) -> list[Any]:
        values = raw.get(key)
        if not isinstance(values, list):
            raise ValidationError(f"Source Understanding provider {key} must be an array")
        result: list[Any] = []
        seen: set[str] = set()
        for number, value in enumerate(values, 1):
            if not isinstance(value, Mapping) or not set(value).issubset(allowed_fields):
                raise ValidationError(f"Source Understanding {key}[{number}] has invalid fields")
            evidence = value.get("evidenceFrameRefs")
            if (
                not isinstance(evidence, list)
                or not evidence or not set(map(str, evidence)).issubset(provenance)
            ):
                raise ValidationError(
            ...
            try:
                item = model.model_validate({**record, **default_base, "modelRunId": record_model_run_id}
                )
            except (TypeError, ValueError) as exc:
                raise ValidationError(f"Source Understanding {key}[{number}] is invalid: {exc}") from exc
            if item.id in seen:
                raise ValidationError(f"Source Understanding {key} contains duplicate id: {item.id}")
            seen.add(item.id)
            result.append(item)
        return result

    timed = {"id", "startMs", "endMs", "confidence", "evidenceFrameRefs", "modelRunId"}
    shots = records(
        "shots",
        SourceShot,
    ...
    audio = records("audioEvents", AudioEvent, timed | {"label", "description"})
    optional_range = {
        "id",
        "startMs",
        "endMs",
    ...
    if coverage["visual"].mode != "available" and shots:
        raise ValidationError("Source Understanding shots require available visual coverage")
    try:
        ...
        raise ValidationError(f"Source Understanding index is invalid: {exc}") from exc
