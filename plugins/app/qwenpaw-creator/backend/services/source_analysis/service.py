...
class SourceAnalyzerOutput:
    ...
    def __post_init__(self) -> None:
        if not self.model_runs:
            raise ValueError("Source analyzer output requires at least one real model run")
        if set(self.coverage_policy) != set(_MODALITIES):
            raise ValueError("coverage_policy must contain visual/asr/ocr/audio")
        if not self.provenance_refs:
            raise ValueError("Source analyzer output requires Runtime provenance")


class SourceMediaAnalyzer(Protocol):
    async def analyze(self, request: SourceMediaAnalysisInput) -> SourceAnalyzerOutput:
...
def _error_payload(error: BaseException, *, code: str | None = None) -> dict[str, Any]:
...
def _probe_media(path: Path, version: SourceAssetVersion) -> SourceMediaMetadata:
    try:
        ...
        raise RuntimeError(f"media probe failed for Source media: {error}") from error
...
class DefaultSourceMediaAnalyzer:
    ...
    async def analyze(self, request: SourceMediaAnalysisInput) -> SourceAnalyzerOutput:
        ...
        if kind in {"video", "audio"} and model_config.get_asr_api_key():
            if model_config.get_asr_provider() != "whisper" and kind == "audio":
                raise SourceAnalyzerConfigurationError(
        ...
        if kind in {"image", "video"} and not summary:
            raise RuntimeError("configured VLM returned an empty Source analysis")
...
class SourceMediaAnalysisService:
    ...
    def start(self, job: SourceAnalysisJob) -> asyncio.Task[TaskRecord] | None:
        ...
        if current is not None and not current.done():
            return current
        if self.executions.get_task(job.project_id, job.task_id).status is not TaskStatus.QUEUED:
            return None
        task = asyncio.create_task(self.execute(job), name=f"source-analysis:{job.task_id}")
    ...
    async def execute(self, job: SourceAnalysisJob) -> TaskRecord:
        ...
        try:
            run = await asyncio.to_thread(
                self.executions.get_run, job.project_id, job.run_id
            )
            task = await asyncio.to_thread(
                self.executions.get_task, job.project_id, job.task_id
            )
            ...
            temp_root, local_path = await _thread_boundary(
                self._materialize_verified_input_sync, job
            )
            ...
            if task.status is TaskStatus.CANCELLED:
                return await asyncio.to_thread(
                    self._quarantine_cancelled_sync, job, provider_result
                )
            try:
    ...
    def load(
        ...
        if source_version is None or source_version.logical_asset_id != logical_asset_id:
        ...
        if indexed is None or indexed.kind != "source_intelligence":
            raise StorageIntegrityError("Source Intelligence IndexedFile 不存在或类型错误")
        try:
        ...
        except (AssetFileError, UnicodeDecodeError, json.JSONDecodeError, ValueError) as error:
            raise StorageIntegrityError(f"Source Intelligence 索引损坏: {error}") from error
        if canonical_json_bytes(
            index.model_dump(mode="json", by_alias=True), pretty=True
        ) != payload:
            raise StorageIntegrityError("Source Intelligence 索引不是 canonical JSON")
        ...
        if (
            ...
            raise StorageIntegrityError("Source Intelligence 文件与 project.json 索引不一致")
        return index
    ...
    def query(
        ...
        for record in payload["transcript"]:
            add("transcript", record, record["text"], record.get("speaker") or "")
        ...
        for record in payload["entities"]:
            add("entity", record, record["kind"], record["label"], record["description"])
    ...
    def _prepare_sync(
        ...
        if version is None:
            raise StorageIntegrityError("ProjectSource selected AssetVersion 不存在")
        indexed = (
        ...
        if indexed is not None:
            if indexed.kind != "source_original":
                raise StorageIntegrityError("SourceAssetVersion IndexedFile 类型错误")
            if indexed.sha256 != version.checksum or indexed.media_type != version.media_type:
                raise StorageIntegrityError("SourceAssetVersion 与 IndexedFile 内容身份不一致")
        requested_version = arguments.get("assetVersionId") or arguments.get("versionId")
        if requested_version is not None and str(requested_version) != version.version_id:
            raise ValidationError("只能分析 ProjectSource 当前选中的 AssetVersion")
        if indexed is not None:
            file_store = AssetFileStore(self.services.projects.project_root(project_id))
        ...
        job = SourceAnalysisJob(
            ...
            intelligence_version_id=_stable_id(
                "source-intelligence", project_id, command_id
            ),
            intelligence_file_id=_stable_id(
                "source-intelligence-file", project_id, command_id
            ),
    ...
    @staticmethod
    def _source_for_logical_asset(project: Project, logical_asset_id: str) -> ProjectSource:
        matches = [
            item
    ...
    def _resolve_source(
        ...
        if target_ref.startswith("asset:"):
            return cls._source_for_logical_asset(
                project, target_ref.removeprefix("asset:")
            )
    ...
    def _materialize_verified_input_sync(
        ...
        with self.services.projects.lifecycle_lock(job.project_id):
            ...
            root = Path(tempfile.mkdtemp(prefix=f"{job.task_id}.", dir=temp_parent))
    ...
    def _recovery_job_from_task(self, task: TaskRecord) -> SourceAnalysisJob | None:
        ...
        if (
            ...
            return None
        expected_round_id = _stable_id("source-round", task.project_id, command_id)
        expected_run_id = _stable_id("source-run", task.project_id, command_id)
        expected_task_id = _stable_id("source-task", task.project_id, command_id)
        if (
        ...
        return SourceAnalysisJob(
            ...
            attempt_id=_stable_id("source-attempt", task.project_id, command_id),
            intelligence_version_id=_stable_id(
                "source-intelligence", task.project_id, command_id
            ),
            intelligence_file_id=_stable_id(
                "source-intelligence-file", task.project_id, command_id
            ),
    ...
    def _quarantine_stale_sync(
        ...
        self.executions.quarantine_task_result(
            job.project_id,
            job.task_id,
            quarantine_id=_stable_id("source-quarantine", job.project_id, job.command_id),
            reason=reason,
            result=result,
        ...
        if run.status is SpecialistRunStatus.RUNNING_MODEL:
            self.executions.transition_run(
                ...
                updates={"final_marker": "STALE", "final_summary_text": reason},
            )
        return self.executions.get_task(job.project_id, job.task_id)

    def _quarantine_cancelled_sync(
        self, job: SourceAnalysisJob, result: Mapping[str, Any]
    ) -> TaskRecord:
        ...
        if run.status is SpecialistRunStatus.RUNNING_MODEL:
            self.executions.transition_run(
                ...
                updates={"final_marker": "CANCELLED", "final_summary_text": reason},
            )
        return self.executions.get_task(job.project_id, job.task_id)

    def _fail_sync(self, job: SourceAnalysisJob, error: BaseException) -> TaskRecord:
        task = self.executions.get_task(job.project_id, job.task_id)
        if task.status is TaskStatus.CANCELLED:
    ...
    for project_id in services.projects.discover_project_ids():
        ...
        for run in executions.list_specialist_runs(project_id):
            ...
            if not isinstance(command_id, str) or not command_id:
                continue
            expected_task_id = _stable_id("source-task", project_id, command_id)
            try:
            ...
            except (ConflictError, NotFoundError, StorageIntegrityError, ValidationError) as repair_error:
        ...
        for task in executions.list_tasks(project_id):
            ...
            if recovery_job is not None:
                converged = convergence._converge_published_job_sync(recovery_job)
