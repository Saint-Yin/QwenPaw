...
Run from any directory while the standalone backend is available::
    ...
    python backend/scripts/seed_demo_projects.py \
        --base-url http://127.0.0.1:18112/api/creator
"""
...
def _stable_uuid(slug: str, operation: str) -> str:
    return str(uuid5(NAMESPACE_URL, f"qwenpaw-creator-demo:{slug}:{operation}"))
    ...
    *,
    ...
    while True:
        ...
        if status in {"FAILED", "CANCELLED"}:
            raise RuntimeError(f"asset ingest task {task_id} ended as {status}: {task.get('error')}")
        if time.monotonic() >= deadline:
            raise TimeoutError(f"asset ingest task {task_id} did not finish in time")
...
def seed_one(
    ...
    *,
    ...
    if version_id is None:
        raise RuntimeError("successful asset ingest did not return one immutable AssetVersion")
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--base-url",
        default=os.getenv("CREATOR_API_BASE_URL", "http://127.0.0.1:18112/api/creator"),
        help="Creator public API base URL",
    )
    parser.add_argument("--slug", choices=[demo.slug for demo in DEMOS], action="append")
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main() -> int:
    ...
    selected = [demo for demo in DEMOS if not args.slug or demo.slug in args.slug]
    if args.dry_run:
        print(json.dumps([asdict(demo) for demo in selected], ensure_ascii=False, indent=2))
        return 0
