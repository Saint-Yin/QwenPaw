...
class CreatorSessionView(StrictModel):
    ...
    active_transaction_id: str | None = Field(None, alias="activeTransactionId")
    active_goal_id: str | None = Field(None, alias="activeGoalId")
    last_message_seq: int = Field(alias="lastMessageSeq", ge=0)
    last_consumed_message_seq: int = Field(alias="lastConsumedMessageSeq", ge=0)
...
class CreatorMessageRequest(StrictModel):
    ...
    asset_version_refs: list[str] = Field(default_factory=list, alias="assetVersionRefs")
...
class CreatorMessageAccepted(StrictModel):
    ...
    append_state: Literal["appended", "queued_until_message_boundary"] = Field(alias="appendState")
    creator_session_id: str = Field(alias="creatorSessionId")
    conversation_id: str = Field(alias="conversationId")
