...
class TextOrUrlAssetRequest(StrictModel):
    ...
    name: str
    value: str
    post_ingest_action: Literal["NONE", "ATTACH_SOURCE"] = Field("NONE", alias="postIngestAction")
    ...
    def validate_availability(self) -> "SourceCoverage":
        if self.mode == "available":
            if self.producer is None or self.ratio is None or self.ratio <= 0:
                raise ValueError("available coverage requires producer and ratio in (0, 1]")
        elif self.producer is not None or self.ratio is not None:
            raise ValueError("unavailable/not_applicable coverage cannot declare producer or ratio")
        return self
...
class SourceMediaMetadata(StrictModel):
    media_kind: Literal["image", "video", "audio", "document", "other"] = Field(
        alias="mediaKind"
    )
    media_type: str = Field(alias="mediaType")
    duration_ms: int | None = Field(None, alias="durationMs", strict=True, ge=0)
    width: int | None = Field(None, strict=True, gt=0)
    height: int | None = Field(None, strict=True, gt=0)
    sample_rate_hz: int | None = Field(None, alias="sampleRateHz", strict=True, gt=0)
...
class SourceEvidenceRecord(StrictModel):
    ...
    evidence_frame_refs: list[str] = Field(alias="evidenceFrameRefs", min_length=1)
    ...
    def nonempty_unique_evidence_refs(cls, values: list[str]) -> list[str]:
        if any(not value.strip() for value in values) or len(values) != len(set(values)):
            raise ValueError("evidenceFrameRefs must contain unique non-empty refs")
        return values
...
class SourceTimedEvidenceRecord(SourceEvidenceRecord):
    ...
    def validate_half_open_range(self) -> "SourceTimedEvidenceRecord":
        if self.end_ms <= self.start_ms:
            raise ValueError("source interval must be a non-empty half-open [startMs, endMs) range")
        return self
...
class SourceEntity(SourceEvidenceRecord):
    ...
    def validate_optional_range(self) -> "SourceEntity":
        ...
        if self.start_ms is not None and self.end_ms <= self.start_ms:
            raise ValueError("entity interval must be a non-empty half-open range")
        return self
...
class SemanticIndexEntry(SourceEvidenceRecord):
    ...
    def validate_optional_range(self) -> "SemanticIndexEntry":
        if (self.start_ms is None) != (self.end_ms is None):
            raise ValueError("semantic entry startMs/endMs must both be present or absent")
        if self.start_ms is not None and self.end_ms <= self.start_ms:
            raise ValueError("semantic interval must be a non-empty half-open range")
        return self
...
class SourceIntelligenceIndex(StrictModel):
    ...
    def validate_complete_index(self) -> "SourceIntelligenceIndex":
        if set(self.coverage) != {"visual", "asr", "ocr", "audio"}:
            raise ValueError("coverage must contain exactly visual/asr/ocr/audio")
        if not self.id.strip() or not self.asset_id.strip() or not self.asset_version_id.strip():
            raise ValueError("source intelligence identities cannot be empty")
        if not self.source_checksum.strip() or not self.model_runs:
            raise ValueError("source intelligence checksum/modelRuns cannot be empty")
        ...
        for modality, records in records_by_modality.items():
            if self.coverage[modality].mode != "available" and records:
                raise ValueError(f"{modality} records require available coverage")
        ...
        for record in all_records:
            ...
            if record.model_run_id not in run_ids:
                raise ValueError("record modelRunId does not exist in modelRuns")
            ...
            if (
                ...
                raise ValueError("record interval exceeds authoritative media durationMs")
        return self
