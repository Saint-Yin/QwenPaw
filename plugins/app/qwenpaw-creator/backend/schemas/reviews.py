...
class ReviewOperationView(StrictModel):
    ...
    source: Literal["user_direct", "dependency_cascade", "system_derived_content"]
...
class ReviewManifestView(StrictModel):
    ...
    decision_groups: list[ReviewDecisionGroupView] = Field(alias="decisionGroups")
    operations: list[ReviewOperationView]
    created_artifact_version_refs: list[str] = Field(alias="createdArtifactVersionRefs")
    media_comparisons: list[MediaComparisonView] = Field(alias="mediaComparisons")
    integration_previews: list[IntegrationPreviewView] = Field(alias="integrationPreviews")
    created_at: str = Field(alias="createdAt")
