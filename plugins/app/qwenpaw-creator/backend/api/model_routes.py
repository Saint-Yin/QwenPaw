...
_ENV_MAPPING: dict[str, dict[str, tuple[str, ...]]] = {
    ...
    "image": {
        ...
        "api_key": ("DASHSCOPE_IMAGE_API_KEY", "OPENAI_IMAGE_API_KEY", "IMAGE_API_KEY"),
        "model_name": (
...
def _config_paths() -> Path:
    ...
    if not config_path.is_absolute() or not config_path.resolve(
        ...
        raise ValidationError("CREATOR_MODEL_CONFIG_PATH 必须位于 CREATOR_DATA_ROOT 内")
    return config_path
...
def load_model_config(*, include_environment: bool = True) -> ModelConfigData:
    ...
    for section in _SECTIONS:
        ...
        if isinstance(config_section, dict):
            ...
            explicit.update(
                key for key, value in config_section.items() if value not in {None, ""}
            )
            ...
            for field, env_names in _ENV_MAPPING[section].items():
                ...
                for name in env_names:
                    if os.environ.get(name):
                        ...
                        break
            if not isinstance(config_section, dict) and base[section].get("model_name"):
...
def save_model_config(data: ModelConfigData) -> None:
    ...
    with CrossProcessFileLock(config_path.parent / ".model-config.lock"):
        atomic_replace_bytes(config_path, canonical_json_bytes(payload) + b"\n")
    ...
    for section, tool_name in mapping.items():
        ...
        if section == "video":
            tool_config["_video_backend"] = (
                "seedance2"
                if "volcano" in item.protocol.casefold() or "火山" in item.protocol
                else "wan"
            )
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        for section in ("llm", "vlm", "asr", "image", "video"):
            ...
            if not getattr(item, "enabled", False) or not item.model_name:
                continue
            api_key = item.api_key
            if section == "asr" and getattr(item, "reuse_llm_key", False) and not api_key:
            ...
            try:
                ...
                if not resp.is_success:
                    try:
                        ...
                        msg = resp.text[:200]
                    failures.append(f"{section}: HTTP {resp.status_code}: {msg or '请求失败'}")
        ...
        oss = data.oss
        if oss.enabled and oss.endpoint and oss.access_key_id and oss.access_key_secret and oss.bucket:
            try:
                import oss2
...
async def test_model_connection(
    ...
    if not selected.base_url or not selected.api_key or not selected.model_name:
        return ConnectionTestResponse(
            ok=False, ms=0, error="配置不完整，请检查 Base URL、API Key 和模型名称"
        )
    ...
    try:
        ...
        if response.is_success:
            return ConnectionTestResponse(
                ok=True, ms=elapsed, detail="provider accepted the probe"
            )
        try:
            ...
            if isinstance(provider_body, dict):
                ...
                if isinstance(err_obj, dict):
                    provider_error = err_obj.get("message") or err_obj.get("type") or str(err_obj)
                else:
                    provider_error = provider_body.get("message") or provider_body.get("error") or str(provider_body)
            else:
                provider_error = str(provider_body)
        except ValueError:
            provider_error = response.text[:300]
        return ConnectionTestResponse(
            ok=False, ms=elapsed, error=f"HTTP {response.status_code}: {provider_error or '请求失败'}"
        )
    except httpx.ConnectError:
        return ConnectionTestResponse(
            ok=False,
...
async def test_oss_connection(
    ...
    if not body.endpoint or not body.access_key_id or not body.access_key_secret or not body.bucket:
        return ConnectionTestResponse(
            ok=False,
            ms=0,
    ...
    try:
        ...
        bucket.get_bucket_info()
        return ConnectionTestResponse(
            ok=True, detail="OSS bucket is reachable"
        )
    except Exception as exc:
        logger.warning('failed to test oss connection')
        exc_str = str(exc)
        if 'InvalidAccessKeyId' in exc_str or 'AccessDenied' in exc_str:
            return ConnectionTestResponse(ok=False, error="Access Key 无效或权限不足，请检查配置")
        if 'NoSuchBucket' in exc_str:
            return ConnectionTestResponse(ok=False, error="Bucket 不存在，请检查 Bucket 名称")
        if 'connect' in exc_str.lower() or 'timeout' in exc_str.lower():
            return ConnectionTestResponse(ok=False, error="无法连接到 OSS 服务，请检查 Endpoint 和网络")
