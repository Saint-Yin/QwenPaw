...
mounts the router at ``/creator`` and standalone development mounts it at
``/api/creator``.
"""
...
from services.runtime_files.runtime_dependencies import creator_runtime_dependency_health
...
from .file_source_intelligence_routes import router as file_source_intelligence_router
...
async def health(
    ...
    return {
        "status": (
            "ok"
            if project_errors + review_errors == 0 and dependencies["status"] == "ok"
            else "degraded"
        ),
