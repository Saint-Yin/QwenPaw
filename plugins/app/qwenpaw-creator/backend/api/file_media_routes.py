...
async def _indexed_version(
    ...
    *,
    ...
    for summary in summaries:
        snapshot = await asyncio.to_thread(services.projects.read, summary.project_id)
        if kind == "source":
            version: Any = snapshot.project.assets.source_versions_by_id.get(version_id)
        else:
            version = snapshot.project.assets.artifact_versions_by_id.get(version_id)
        if version is None:
            continue
        indexed = (
