...
async def _cancel_active_project_tasks(
    ...
    for task in tasks:
        ...
        if cancelled.kind is TaskKind.R2V_GENERATION:
            ...
            file_r2v_execution_service(services).notify_terminal_task(cancelled)


def _translate_runtime_error(error: BaseException) -> None:
    if isinstance(error, (RuntimeSessionNotFound, RuntimeConversationNotFound)):
        raise NotFoundError(str(error)) from error
    if isinstance(
        error,
        (MessagePayloadConflict, RequestAdmissionConflict, SessionStateConflict),
    ):
...
def _message_parts(request: CreatorMessageRequest) -> tuple[list[dict[str, Any]], str]:
    parts = [item.model_dump(mode="json", exclude_none=True) for item in request.content]
...
def _sse(event: SessionEventRecord) -> str:
    body = json.dumps(_event_payload(event), ensure_ascii=False, separators=(",", ":"))
...
async def get_session(
    ...
    try:
        session, tasks, runs = await asyncio.gather(
            asyncio.to_thread(session_store.get_project_session_snapshot, project_id),
    ...
    return {
        ...
        "agentStatusBar": build_agent_status_bar(session, tasks=tasks, runs=runs),
    }
...
async def list_conversations(
    ...
    try:
        session = await asyncio.to_thread(
            store.get_project_session_snapshot, project_id
        )
        items = await asyncio.to_thread(
            store.list_conversations, project_id, session.session_id
        )
    except BaseException as error:
        _translate_runtime_error(error)
...
async def create_conversation(
    ...
    conversation_id = "conversation-" + sha256(key.encode("utf-8")).hexdigest()[:24]
    store = _store(services)
    try:
        session = await asyncio.to_thread(store.get_project_session, project_id)
        try:
...
async def list_messages(
    ...
    try:
        ...
        await asyncio.to_thread(
            store.get_conversation, project_id, session.session_id, conversation_id
        )
        ...
        _translate_runtime_error(error)
    matching = [item for item in messages if item.conversation_id == conversation_id]
    page = matching[:limit]
    return {
        "items": [
            {
...
async def post_message(
    ...
    key = resolve_idempotency_key(
        idempotency_key, stable_client_id=request.client_message_id
    )
    parts, intent = _message_parts(request)
    store = _store(services)
    try:
        session = await asyncio.to_thread(store.get_project_session_snapshot, project_id)
    ...
    response.headers["X-Idempotent-Replay"] = "true" if admitted.replayed else "false"
    return {
        "messageSeq": admitted.message.message_seq,
    ...
    try:
        ...
        if session.status.value not in {"INTERRUPT_REQUESTED", "CANCELLED"}:
            ...
            await asyncio.to_thread(
                ...
                actor="user",
                payload={"status": "INTERRUPT_REQUESTED", "stopRequested": True},
            )
