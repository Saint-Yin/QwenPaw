...
async def create_project(
    ...
    client_request_id = resolve_idempotency_key(
        idempotency_key, stable_client_id=request.client_request_id
    )
    ...
    def operation() -> ProjectCreateResponse:
        ...
        def initialize(staged_project_root) -> None:
            holder.append(
                services.sessions.initialize_staged_project(
                    ...
                    goal_id=goal_id if request.initial_goal is not None else None,
                    initial_message_id=(
                        message_id if request.initial_goal is not None else None
                    ),
        ...
        try:
            snapshot = services.projects.create(
                project, initialize_staged_project=initialize
            )
...
async def delete_project(
    ...
    try:
        await asyncio.to_thread(services.poller.poll_once, project_id, force=True)
    except ProjectNotFound:
        pass
