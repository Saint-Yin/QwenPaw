...
from services.project_files.models import IndexedFile, ProjectSource, SourceAssetVersion
...
from services.runtime_files.models import ChangeOrigin, IdempotencyStatus, ReviewPolicy
...
def _stable_id(prefix: str, project_id: str, key: str, suffix: str = "") -> str:
    value = uuid5(
        NAMESPACE_URL,
    ...
    with records.operation_lock(
        ...
        with nullcontext():
            ...
            for index, item in enumerate(inputs):
                ...
                version_id = _stable_id("asset-version", project_id, key, suffix)
...
def _download_remote_to_staging(
    ...
    *,
    ...
    try:
            ...
            for redirect_index in range(_MAX_REMOTE_REDIRECTS + 1):
                with client.stream("GET", current) as response:
                    ...
                    if declared:
                        ...
                        if declared_size < 0:
                            raise ValidationError("Asset URL Content-Length 非法")
        ...
        if staged is None:  # pragma: no cover - successful response always stages
            raise ValidationError("Asset URL 未产生内容")
        name = Path(urlsplit(current).path or original.path).name or "remote-asset.bin"
    ...
    *,
    ...
    candidate = TaskRecord(
        ...
        metadata={
            ...
            "assetVersionId": _stable_id("asset-version", project_id, key, "0"),
        },
    )
    try:
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            return
        if current.status is TaskStatus.CANCELLED:
            _publish_remote_task_event(services, current, event_type="task.cancelled")
            return
        task = executions.transition_task(
            project_id,
            task_id,
...
async def ingest_asset(
    ...
    if content_type.startswith("multipart/form-data"):
        ...
        key = resolve_idempotency_key(
            idempotency_key, stable_client_id=body.client_request_id
        )
...
async def get_asset_import(
    ...
    return record.model_dump(
        mode="json", by_alias=True, exclude={"project_id", "created_at"}
    )
...
async def get_asset_content(
    ...
    try:
        stream = await asyncio.to_thread(
            AssetFileStore(services.projects.project_root(project_id)).open_verified,
            indexed,
        )
