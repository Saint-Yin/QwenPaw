...
from domain.enums import SpecialistRole, SpecialistRunStatus, TaskKind, TaskStatus
from domain.errors import ConflictError, NotFoundError, StorageIntegrityError
from schemas.common import StrictModel
from services.project_files.facade import CreatorFileServices
from services.runtime_files.errors import RecordNotFoundError
from services.runtime_files.execution_models import SpecialistRunRecord, TaskRecord
...
class ExecutionAuthorizationApprovalRequest(ExecutionAuthorizationDecisionRequest):
...
_ROLE_LABELS = {
    ...
    "unit_planning_routing_agent": "单元规划",
...
def _run_view(run: SpecialistRunRecord, tasks: list[TaskRecord]) -> dict[str, Any]:
    return {
        "id": run.run_id,
        "role": run.role.value,
        ...
        "taskRefs": [item.task_id for item in tasks if item.run_id == run.run_id],
...
def _authorization_view(record: ExecutionAuthorizationRecord) -> dict[str, Any]:
    status_value = (
        "DECLINED"
...
async def list_tasks(
    ...
    try:
        tasks = await asyncio.to_thread(_store(services).list_tasks, project_id)
    except BaseException as error:
        _translate(error)
...
async def get_task(
    ...
    try:
        task = await asyncio.to_thread(_store(services).get_task, project_id, task_id)
    except BaseException as error:
        _translate(error)
...
async def list_execution_authorizations(
    ...
    try:
        records = await asyncio.to_thread(
            _store(services).list_execution_authorizations, project_id
        )
    except BaseException as error:
        _translate(error)
    if authorization_status:
        requested = (
            "REJECTED" if authorization_status == "DECLINED" else authorization_status
        )
    ...
    *,
    ...
    try:
        current = await asyncio.to_thread(
            store.get_execution_authorization, project_id, authorization_id
        )
