...
def _target(context: _ViewContext, ref: str) -> str:
    ...
    return f"project:{context.snapshot.etag}:g{context.snapshot.generation}:{ref}"
...
def _source_ref_view(project: Project, version_id: str) -> dict[str, Any] | None:
...
def _artifact_ref_view(project: Project, version_id: str) -> dict[str, Any] | None:
...
def _visual_ref_view(project: Project, entity_id: str) -> dict[str, Any] | None:
    ...
    return {
        ...
        "mediaType": "image",
        "artifactVersionId": (selected.get("artifactVersionId") if selected else None),
        "uiLocator": {"page": "assets", "assetId": entity.entity_id},
    }


def _selected_source_ref(project: Project, source_id: str) -> dict[str, Any] | None:
...
def _resolve_exact_version(project: Project, version_id: str) -> dict[str, Any] | None:
...
def _unit_view(
    ...
    *,
    ...
    if isinstance(production, R2VProduction):
        ...
        storyboard_image_version_id = production.selected_storyboard_artifact_version_id
        ...
        if production.selected_video_artifact_version_id is not None:
            video_url = _artifact_url(production.selected_video_artifact_version_id)
    elif isinstance(production, EditProduction):
        if production.rendered_video_artifact_version_id is not None:
            video_url = _artifact_url(production.rendered_video_artifact_version_id)
    ...
    return {
        ...
        "sceneRef": (f"project://asset/{unit.scene_ref}" if unit.scene_ref else None),
        "characterRefs": [
            f"project://asset/{identifier}" for identifier in unit.character_refs
        ],
        "propRefs": [f"project://asset/{identifier}" for identifier in unit.prop_refs],
...
def _section_view(
    ...
    *,
    ...
    blockers = list(
        dict.fromkeys(blocker for unit in units for blocker in unit.get("blockers", []))
    )
...
def _plan_view(context: _ViewContext) -> dict[str, Any]:
    ...
    sections = [
        ...
        for section_number, section_id in enumerate(project.story.sections.order, 1)
    ]
    blockers = list(
        dict.fromkeys(
            blocker for section in sections for blocker in section.get("blockers", [])
        )
    )
    return {
        ...
        "resolvedRefs": _unique_refs(
            [item for section in sections for item in section.get("resolvedRefs", [])]
        ),
...
def _artifact_view(
    ...
    *,
    ...
    version = context.snapshot.project.assets.artifact_versions_by_id[version_id]
    result: dict[str, Any] = {
...
def _r2v_workbench(
    ...
    *,
    ...
    selected_ids = {item for item in (storyboard_id, video_id) if item is not None}
    ...
    for label, version_ids in (
        ...
        for version_id in version_ids:
            ...
            if resolved is None:
                continue
            key = (_reference_binding(project, version_id), str(resolved["ref"]))
    ...
    return {
        ...
        "storyboardInputRefs": [
            ...
            if (item := _resolve_exact_version(project, version_id)) is not None
        ],
        "videoInputRefs": [
            str(item["ref"])
...
def _edit_plan_view(
    project: Project, production: EditProduction
) -> dict[str, Any] | None:
    ...
    for order, clip_id in enumerate(plan.timeline.order, 1):
        ...
        source = project.assets.source_versions_by_id[clip.source_asset_version_id]
        timeline.append(
            {
                ...
                "reason": clip.reason,
                **({"overlay_copy": dict(clip.overlay)} if clip.overlay else {}),
            }
        )
    ...
    *,
    ...
    return {
        ...
        "relations": [
            {
                "from": f"project://unit/{unit.unit_id}",
                "to": _asset_ref(
                    project.assets.source_versions_by_id[version_id].logical_asset_id,
                    version_id,
                ),
                "kind": "uses_version",
            }
        ...
        "previousUnitLastClipEnd": _adjacent_edit_boundary(
            project, ordered_units, unit_index, previous=True
        ),
        "nextUnitFirstClipStart": _adjacent_edit_boundary(
            project, ordered_units, unit_index, previous=False
        ),
    }
...
def _workbench_view(context: _ViewContext, unit_id: str) -> dict[str, Any]:
    ...
    if unit.route.value == "r2v":
        return _r2v_workbench(
            ...
            unit,
            number=number,
            production=production if isinstance(production, R2VProduction) else None,
        )
    return _edit_workbench(
        context,
        unit,
        number=number,
        production=production if isinstance(production, EditProduction) else None,
    )
...
def _visual_detail(project: Project, entity_id: str) -> dict[str, Any]:
    ...
    selected = (
        project.assets.artifact_versions_by_id.get(entity.selected_artifact_version_id)
        if entity.selected_artifact_version_id is not None
        else None
    )
    images = []
    ...
    result: dict[str, Any] = {
        ...
        "prompts": [variant.prompt for variant in entity.variants.items.values()],
        "referenceImageRefs": [
            [
                _asset_ref(
                    project.assets.source_versions_by_id[version_id].logical_asset_id,
...
def _asset_library_view(context: _ViewContext) -> dict[str, Any]:
    ...
    for source_id in project.sources.sources.order:
        ...
        version = project.assets.source_versions_by_id[source.selected_asset_version_id]
        attached_sources.append(
            {
                "assetId": version.logical_asset_id,
                "assetVersionId": version.version_id,
                "sourceRef": _asset_ref(version.logical_asset_id, version.version_id),
        ...
        available_assets.append(
            {
                ...
                "objectVersion": _target(
                    context, f"asset-version:{version.version_id}"
                ),
                "createdAt": version.created_at.isoformat(),
                "referenceCount": _reference_count(project, version.version_id),
                "attached": attached,
                "uiLocator": {
                    "page": "assets",
                    "assetId": version.logical_asset_id,
        ...
        visual_assets.append(
            {
                ...
                "url": _artifact_url(selected.version_id) if selected else None,
                "thumbnailUrl": (
                ...
                "referenceCount": max(
                    0, _reference_count(project, entity.entity_id) - 1
                ),
                "uiLocator": {"page": "assets", "assetId": entity.entity_id},
            }
        )
        ...
        presentation_assets.append(
            {
                ...
                "artifactVersionId": selected.version_id if selected else None,
                "targetVersion": _target(context, f"visual:{entity.entity_id}"),
        ...
        presentation_assets.append(
            {
                ...
                "existence": "available",
                "presentationStatus": (
                    "stale" if artifact["freshnessStatus"] == "stale" else "accepted"
                ),
                ...
                "referenceCount": _reference_count(
                    project, artifact["artifactVersionId"]
                ),
    ...
    *,
    ...
    if sequence is not None:
        for order, selection_id in enumerate(sequence.order, 1):
            ...
            version = project.assets.artifact_versions_by_id[item.artifact_version_id]
            resolved.append(_artifact_ref_view(project, version.version_id))
            selections.append(
                {
                    "sourceRef": item.source_ref,
                    "sourceKind": (
                        "unit" if item.source_kind == "unit_video" else "section"
                    ),
                    "artifactRef": _artifact_ref(version.slot_id, version.version_id),
                    "artifactVersionId": version.version_id,
                    "slotId": version.slot_id,
                    "order": order,
                    "uiLocator": _artifact_locator(
                        version.owner_ref, version.version_id
                    ),
                }
            )
    ...
    blockers: list[str] = []
    if kind == "section" and section is not None:
        expected = {f"project://unit/{unit_id}" for unit_id in section.units.order}
    ...
    transitions = [
        ...
        for index, transition in enumerate(
            composition.transitions if composition is not None else [], 1
        )
    ]
    payload: dict[str, Any] = {
        ...
        "renderedVideoUrl": _artifact_url(rendered_id) if rendered_id else None,
        "resolvedRefs": _unique_refs(resolved),
        "relations": [
            {
                "from": item["sourceRef"],
        ...
        "targetVersion": _target(
            context,
            f"post:{section.section_id}" if section is not None else "post:final",
        ),
        "uiLocator": (
    ...
    if section is not None:
        payload.update(
            {
                ...
                "sectionNumber": project.story.sections.order.index(section.section_id)
                + 1,
        ...
        payload["sections"] = [
            ...
            for number, section_id in enumerate(project.story.sections.order, 1)
        ]
    return payload
...
def _envelope(context: _ViewContext, view: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "workingHead": None,
        "reviewRevisionId": (
            context.review.candidate_etag if context.review is not None else None
        ),
...
async def _context(
    ...
    try:
        session = await asyncio.to_thread(
            services.sessions.get_project_session_snapshot, project_id
        )
    ...
    try:
        review, tasks, runs = await asyncio.gather(
            ...
            asyncio.to_thread(execution_store.list_specialist_runs, project_id),
        )
    ...
    return _ViewContext(
        ...
        review=review,
        tasks=tuple(tasks),
        agent_status_bar=build_agent_status_bar(session, tasks=tasks, runs=runs),
    )
