...
from .dependencies import CreatorErrorRoute, project_file_services, resolve_idempotency_key
...
def _project_headers(*, etag: str, generation: int, sync_status: str) -> dict[str, str]:
    return {
...
def _as_creator_error(error: BaseException) -> CreatorError:
    ...
    if isinstance(error, (JsonCasConflict, ProjectConflict)):
        details = {"conflicts": error.conflicts} if isinstance(error, JsonCasConflict) else {}
    ...
    if isinstance(error, (JsonPointerError, ValueError)):
        return ValidationError(str(error))
    if isinstance(error, (ProjectIntegrityError, ProjectStoreError, RuntimeFileError)):
...
def _completed_idempotency_body(record: IdempotencyRecord) -> dict[str, Any]:
    if not isinstance(record.response, dict) or not isinstance(record.response_status, int):
...
def _recover_patch_response(
    ...
    *,
    ...
    journal_store = AtomicJsonRecordStore(
        transaction_root / "journal.json", ProjectCommitJournal
    )
    journal = journal_store.read_or_none()
    if journal is None:
        return None
    if journal.project_id != project_id or journal.transaction_id != transaction_id:
    ...
    project = AtomicJsonRecordStore(
        transaction_root / "final.json", Project
    ).read_or_none()
    if project is None:
    ...
    if project.project_id != project_id or project_etag(project) != journal.final_etag:
        raise StorageIntegrityError("Project Patch final snapshot 与 journal 不一致")
    ...
    if (
        ...
        raise StorageIntegrityError("Project Patch ChangeSet 与 transaction 不一致")
...
def _build_patch_candidate(
    ...
    if request.base_generation == current.generation and request.base_etag != current.etag:
        raise CasConflictError("相同 generation 的 Project ETag 不匹配")
    active_blocks = services.blocks(project_id).list_active(project_id=project_id)
    ...
    for operation in request.operations:
        ...
        for block in active_blocks:
            if pointers_overlap(operation.path, block.json_pointer) and request.block_token != block.token:
                raise ConflictError(
                    "目标字段正被其他写者编辑",
                    details={"pointer": operation.path, "blockId": block.block_id},
                )
        ...
        if operation.op == "add":
            if before is not MISSING:
                raise JsonPointerError(f"add target already exists: {operation.path}")
            change = JsonChange("create", operation.path, MISSING, operation.value)
        elif operation.op == "remove":
            if before is MISSING:
                raise JsonPointerError(f"remove target does not exist: {operation.path}")
            change = JsonChange("delete", operation.path, before, MISSING)
        else:
            if before is MISSING:
                raise JsonPointerError(f"replace target does not exist: {operation.path}")
...
async def patch_project(
    ...
    try:
        ...
        if record is None:
            raise StorageIntegrityError("Project Patch idempotency reservation 丢失")
    ...
    try:
        ...
        block = await asyncio.to_thread(
            ...
            current_field_hash=lambda: hash_json_value(
                value_at(
                    services.projects.read(project_id).project.model_dump(mode="json"),
...
@router.delete("/runtime/blocks/{block_id}", status_code=status.HTTP_204_NO_CONTENT)
...
async def active_project_review(
    ...
    return JSONResponse(
        ...
        headers={"ETag": _etag_header(review.decision_token), "Cache-Control": "no-cache"},
    )
...
async def decide_project_review(
    ...
    scope = f"POST /projects/{{projectId}}/runtime/reviews/{review_id}/decisions"
    payload = {
        "reviewId": review_id,
    ...
    try:
        ...
        if record is None:
            raise StorageIntegrityError("Review decision idempotency reservation 丢失")
        if record.status is IdempotencyStatus.COMPLETED:
            response.headers["X-Idempotent-Replay"] = "true"
            body = _completed_idempotency_body(record)
