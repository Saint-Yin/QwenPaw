...
class ComposerModal:
    ...
    def root(self) -> Locator:
        return self.page.locator(".ant-modal").filter(
            has=self.page.get_by_role(
                "heading", name="把目标、素材和限制交给 Agent", exact=True
            )
        ).last
    ...
    def add_files(self, paths: str | Path | Iterable[str | Path]):
        self.root.locator("input[type=file]:not([webkitdirectory])").set_input_files(paths)
        return self

    def add_folder_files(self, paths: Iterable[str | Path]):
        self.root.locator("input[webkitdirectory]").set_input_files(list(paths))
        return self
