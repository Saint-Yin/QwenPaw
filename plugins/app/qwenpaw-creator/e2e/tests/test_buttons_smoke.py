...
def test_open_composer_uses_current_contract(page):
    ...
    expect(composer.attachment_chip("https://example.com/ref.mp4")).to_be_visible()
    expect(
...
def test_model_config_single_file_values_are_visible_in_api_and_ui(page, api):
    ...
    for index, model_type in enumerate(("llm", "vlm", "asr", "image", "video")):
        tabs.nth(index).click()
        if model_type == "vlm" and config["vlm"]["use_llm"]:
            expect(modal.get_by_role("checkbox", name="复用 LLM 配置")).to_be_checked()
            expect(modal.get_by_placeholder("sk-...", exact=True)).to_have_count(0)
            continue
