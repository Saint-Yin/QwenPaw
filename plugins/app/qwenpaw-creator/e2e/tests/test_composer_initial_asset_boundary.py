...
def test_composer_real_file_ingest_precedes_atomic_initial_goal(page, api, tmp_path):
    ...
    def observe_without_mocking(route):
        match = re.search(r"/projects/([^/]+)/assets(?:\?|$)", route.request.url)
    ...
    page.route("**/api/creator/projects/*/assets", observe_without_mocking)
    ...
    try:
        with page.expect_response(
            ...
            and re.search(r"/api/creator/projects/[^/]+/messages$", response.url)
            is not None,
            timeout=30_000,
        ) as message_info:
            with page.expect_response(
                lambda response: response.request.method == "POST"
                and re.search(r"/api/creator/projects/[^/]+/assets$", response.url)
                is not None,
                timeout=30_000,
            ) as asset_info:
                with page.expect_response(
                    lambda response: response.request.method == "POST"
                    and response.url.endswith("/api/creator/projects"),
                    timeout=30_000,
                ) as project_info:
                    composer.launch()
        ...
        if accepted_asset.get("assetVersionId"):
            expected_refs = [f"asset-version:{accepted_asset['assetVersionId']}"]
        ...
        assert message_payload["creatorSessionId"] == project["creatorSessionId"]
        ...
        initial = next(item for item in messages if item["source"] == "initial_goal")
