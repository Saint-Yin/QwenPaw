...
def test_asset_library_origin_categories_grid_inspector_and_video_metadata(
    ...
    assert any(item["id"] == asset["assetId"] for item in view["presentationAssets"])

    page.goto(f"/#/project/{pid}/assets")
    expect(page.get_by_role("heading", name=re.compile(r"^资产库"))).to_be_visible()
    ...
    category_column.get_by_role(
        "button", name=f"用户上传 {upload_count}"
    ).click()
...
def test_origin_supplement_file_upload_sends_durable_attach_action(page, api, tmp_path):
    ...
    try:
        ...
        version_ref = next(
            ref for ref in task["resultRefs"] if ref.startswith("asset-version:")
        )
        content = api.get(
